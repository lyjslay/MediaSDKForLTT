...
hi_s32 wlan_sm_init(hi_void)
{
    ...
    if (pthread_mutex_init(&g_wlan_sm_mut, NULL) != 0) {
        dbg_print(("WiFi: 'g_wlan_sm_mut' pthread_mutex_init fail in %s!\n", __func__));
    }
...
hi_void wlan_sm_deinit(hi_void)
{
    ...
    if (pthread_mutex_destroy(&g_wlan_sm_mut) != 0) {
        dbg_print(("WiFi: 'g_wlan_sm_mut' pthread_mutex_destroy fail in %s\n", __func__));
    }
...
hi_s32 wlan_sm_send_message(hi_s32 msg, hi_void *data, hi_char *reply, hi_s32 *len)
{
    ...
    if (pthread_mutex_lock(&g_wlan_sm_mut) != 0) {
        dbg_print(("WiFi: 'g_wlan_sm_mut' pthread_mutex_lock fail in %s!\n", __func__));
    }
    ...
    if (pthread_mutex_unlock(&g_wlan_sm_mut) != 0) {
        dbg_print(("WiFi: 'g_wlan_sm_mut' pthread_mutex_unlock fail in %s!\n", __func__));
    }
    dbg_print(("WiFi: msg [0x%x] in state %d\n", msg, state));
...
hi_s32 wlan_sm_register_callback(hi_wlan_sta_event_callback event_cb)
{
    ...
    dbg_print(("WiFi:register callback: tmp_index=%d\n", tmp_index));
    return HI_SUCCESS;
}
...
hi_s32 wlan_sm_unregister_callback_given(hi_wlan_sta_event_callback event_cb)
{
    ...
    dbg_print(("WiFi:unregister callback given: tmp_index=%d\n", tmp_index));
    return HI_SUCCESS;
}
...
hi_s32 wlan_sm_unregister_callback(hi_void)
{
    ...
    dbg_print(("WiFi:unregister callback: tmp_index=%d\n", tmp_index));
    return HI_SUCCESS;
}
...
{
    dbg_print(("WiFi: Transfer state to %d\n", newstate));
    if (pthread_mutex_lock(&g_wlan_sm_mut) != 0) {
        dbg_print(("WiFi: g_wlan_sm_mut lock fail in '%s'!\n", __func__));
    }
    *state = newstate;
    if (pthread_mutex_unlock(&g_wlan_sm_mut) != 0) {
        dbg_print(("WiFi: g_wlan_sm_mut unlock fail in '%s'!\n", __func__));
    }
}

static hi_s32 wlan_sm_parse_p2p_device(const hi_char *buf, hi_u32 buf_size, hi_wlan_p2p_device_s *device)
{
    hi_unused(buf_size);
    hi_char *pos = NULL;
    hi_char *begin = NULL;
    ...
    if ((pos = strstr(buf, "wfd_dev_info="))) {
        ...
        ret = sscanf_s(tmp, "%x", &device->wfd_info.ctrl_port);
        if (ret == -1) {
            dbg_print(("WiFi: ret=%d file=%s,line=%d,func=%s\n", ret, __FILE__, __LINE__, __FUNCTION__));
        }
...
hi_s32 wlan_open(hi_wifi_mode_e mode, hi_char *ifname, hi_u32 ifname_size, hi_wlan_sta_config_s *sta_cfg)
{
    hi_s32 ret;
    ...
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: No driver rmmod!\n"));
    }
    ret = wlan_load_driver(sta_cfg);
    if (ret != HI_SUCCESS) {
        return ret;
    }
    dbg_print(("WiFi: Driver loaded successfully\n"));

    g_pmf_mode = sta_cfg->pmf_mode;

    /* For MT7601U, ifconfig wlan0 up to create p2p0 */
    if (wlan_ifc_up("wlan0")) {
        dbg_print(("WiFi: Failed to up wlan0\n"));
    }
    ...
    for (count = 0; count < 50; count++) {  /* loop 50th */
        ...
        if (ret == HI_SUCCESS) {
            dbg_print(("WiFi: Get interface '%s'\n", iface));
            if (strncpy_s(ifname, ifname_size, iface, strlen(iface) + 1) != 0) {
                dbg_print(("WiFi: file=%s, line=%d, func=%s\n", __FILE__, __LINE__, __FUNCTION__));
            }
    ...
    dbg_print(("WiFi: Failed to get interface, driver initialized fail!\n"));
...
hi_s32 wlan_common_close(const hi_char *ifname)
{
    ...
    dbg_print(("WiFi: Unloading driver\n"));
    /* unload WiFi driver */
    ret = wlan_unload_driver();
...
static hi_s32 wlan_update_supplicant_config_file(const hi_char *config_file)
{
    ...
    if (realpath(config_file, file_path) == NULL) {
        dbg_print(("WiFi: file path '%s' not exist and create it [%s]!\n", file_path, __func__));
    }
    hi_s32 fd = open(file_path, O_CREAT | O_TRUNC | O_WRONLY, 0666);   /* file_path mode 0666 */
    if (fd < 0) {
        dbg_print(("WiFi: Cann't open configure file '%s'\n", file_path));
        return HI_FAILURE;
    }
    ...
    if (strcmp(file_path, g_supp_config_file) == 0) {
        if (asprintf(&wbuf, "ctrl_interface=%s\n"
                     ...
                     g_ctrl_iface_dir,
                     g_pmf_mode) < 0) {
            dbg_print(("WiFi: asprintf g_supp_config_file failure\n"));
            close(fd);
            free(wbuf);
            return HI_FAILURE;
        }
        ...
        if (asprintf(&wbuf, "ctrl_interface=%s\n"
                     ...
                     g_ctrl_iface_dir) < 0) {
            dbg_print(("WiFi: asprintf g_p2p_config_file failure\n"));
            close(fd);
            free(wbuf);
            return HI_FAILURE;
        }
    ...
    if (write(fd, wbuf, strlen(wbuf)) < 0) {
        dbg_print(("WiFi: Cann't write configuration to '%s'\n", file_path));
        ret = HI_FAILURE;
    }
    close(fd);
    free(wbuf);
    ...
    if (chmod(file_path, 0666) < 0) {                       /* file_path mode 0666 */
        dbg_print(("WiFi: Failed to change '%s' to 0666\n", file_path));
        unlink(file_path);
        ret = HI_FAILURE;
    }
...
static hi_s32 wlan_sm_create_wifi_dir()
{
    DIR *dirptr = opendir(g_dev_wifi_dir);
    if (dirptr != NULL) {
        closedir(dirptr);
        return HI_SUCCESS;
    }

    if (mkdir(g_dev_wifi_dir, 0666) != 0) {
        dbg_print(("WiFi: Create '%s' fail\n", g_dev_wifi_dir));
        return HI_FAILURE;
    }
    if (chmod(g_dev_wifi_dir, 0666) != 0) {
        dbg_print(("WiFi: '%s' chmod fail in %s\n", g_dev_wifi_dir, __func__));
    }

    return HI_SUCCESS;
}

static hi_s32 wlan_sm_remove_wpa_supplicant()
{
    if (wlan_sm_create_wifi_dir() != HI_SUCCESS) {
        return HI_FAILURE;
    }
    ...
    if (asprintf(&wbuf, "rm -rf %s\n", g_ctrl_iface_dir) < 0) {
        dbg_print(("WiFi: asprintf rm g_ctrl_iface_dir failure\n"));
        free(wbuf);
        return HI_FAILURE;
    }
    if (system(wbuf) == -1) {
        dbg_print(("WiFi: system failure\n"));
    }
    free(wbuf);
...
static hi_s32 wlan_sm_sta_wpa_connect(const hi_char *sta_ifname)
{
    for (hi_s32 i = 0;; i++) {
        if (i == 50) {  /* 50th */
            dbg_print(("WiFi: Connect to wpa_supplicant timeout, stop wpa_supplicant and return fail\n"));
            wlan_stop_supplicant();
        ...
        if (g_mon_conn != NULL) {
            dbg_print(("WiFi: Connected to wpa_supplicant with '%s'\n", sta_ifname));
            break;
        }
        usleep(100000); /* sleep 100000us=100ms */
    }
    if (sprintf_s(g_wpa_message.ifname, sizeof(g_wpa_message.ifname), "%s", sta_ifname) < 0) {
        dbg_print(("WiFi: file=%s, line=%d, func=%s\n", __FILE__, __LINE__, __FUNCTION__));
    }
    ...
    if (g_wlan_ethread == 0) {
        if (pthread_create(&g_wlan_ethread, NULL, wpa_event_receiver_thread, &g_wpa_message) != HI_SUCCESS) {
            dbg_print(("WiFi: Cann't create monitor thread, stop wpa_supplicant and return fail\n"));
...
static hi_s32 wlan_sm_p2p_wpa_connect(const hi_char *p2p_ifname)
{
    for (hi_s32 i = 0;; i++) {
        if (i == 50) {  /* 50th */
            dbg_print(("WiFi: Connect to wpa_supplicant timeout, stop wpa_supplicant and return fail\n"));
    ...
    if (sprintf_s(g_wpa_message_p2p.ifname, sizeof(g_wpa_message_p2p.ifname), "%s", p2p_ifname) < 0) {
        dbg_print(("WiFi: file=%s, line=%d, func=%s\n", __FILE__, __LINE__, __FUNCTION__));
    }
    ...
    if (g_wlan_ethread_p2p == 0) {
        if (pthread_create(&g_wlan_ethread_p2p, NULL, wpa_event_receiver_thread, &g_wpa_message_p2p) != HI_SUCCESS) {
            dbg_print(("WiFi: Cann't create monitor thread, stop wpa_supplicant and return fail\n"));
...
hi_s32 wlan_common_start(hi_char *sta_ifname, hi_char *p2p_ifname, hi_wifi_mode_e mode)
{
    ...
    if (wlan_sm_remove_wpa_supplicant() != HI_SUCCESS) {
        return HI_FAILURE;
    }
    ...
    if (mode == WIFI_MODE_P2P || mode == WIFI_MODE_STA_P2P) {
        ret = wlan_sm_p2p_wpa_connect(p2p_ifname);
        if (ret != HI_SUCCESS) {
            return ret;
        }
    }

    dbg_print(("WiFi: started\n"));
    return HI_SUCCESS;
}
...
hi_s32 wlan_common_stop(const hi_char *ifname)
{
    ...
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: call wlan_ethread2_cancel fail!\n"));
    }
    ret = wlan_ethread_cancel();
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: call wlan_ethread_cancel fail!\n"));
    }
    ret = wlan_timer_thread_cancel();
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: call wlan_timer_thread_cancel fail!\n"));
    }
    ...
    dbg_print(("WiFi: Stop wpa_supplicant\n"));
    /* stop wpa_supplicant */
    if (wlan_stop_supplicant()) {
        dbg_print(("WiFi: Kill wpa_supplicant fail\n"));
        return HI_FAILURE;
    }

    dbg_print(("WiFi: Stopped\n"));
    return HI_SUCCESS;
}
...
hi_s32 wlan_sta_connect(hi_wlan_sta_config_s *config)
{
    ...
    if (config->security == HI_WLAN_SECURITY_OPEN) {
        dbg_print(("WiFi: wlan_sta_connect: security is OPEN\n"));
    }
    ...
    if (net_id > 0) {
        ...
        for (i = 0; i <= net_id; i++) {
            ...
            if (wlan_wpa_request(cmd, NULL, 0) == HI_FAILURE) {
                dbg_print(("WiFi: '%s' request fail in %s!\n", cmd, __func__));
            }
        }
        if (wlan_wpa_request("ADD_NETWORK", reply, &len)) {
            return HI_FAILURE;
     ...
     */
    ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd) - 1, "SET_NETWORK %d ssid P\"%s\"", net_id, config->ssid);
    if (ret < 0) {
        dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
    }
    if (wlan_wpa_request(cmd, NULL, 0)) {
        return HI_WLAN_SEND_COMMAND_FAIL;
    }

    ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d bssid %s", net_id, config->bssid);
    if (ret < 0) {
        dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
    }
    if (wlan_wpa_request(cmd, NULL, 0)) {
    ...
    if (config->security == HI_WLAN_SECURITY_OPEN || config->security == HI_WLAN_SECURITY_WEP) {
        ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d key_mgmt NONE", net_id);
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        if (wlan_wpa_request(cmd, NULL, 0)) {
    ...
    if (config->security == HI_WLAN_SECURITY_WEP) {
        hi_s32 pwd_len = (hi_s32)strlen(config->psswd);

        ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d auth_alg %s", net_id, "SHARED");
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        if (wlan_wpa_request(cmd, NULL, 0)) {
        ...
        if (pwd_len == 5 || pwd_len == 13) {            /* pwd len eual 5 or 13 */
            ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d wep_key0 \"%s\"", net_id, config->psswd);
            if (ret < 0) {
                dbg_print(("WiFi: snprintf_s return %d pwd_len=%d cmd=%s\n", ret, pwd_len, cmd));
            }
    ...
    if (config->security == HI_WLAN_SECURITY_WPA_WPA2_PSK) {
        ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d proto %s", net_id, "WPA RSN");
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        if (wlan_wpa_request(cmd, NULL, 0)) {
        ...
        if (g_pmf_mode >= HI_WLAN_PROTECTION_OPTIONAL) {
            ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d key_mgmt %s",
                             net_id, "WPA-PSK WPA-PSK-SHA256");
        } else {
            ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d key_mgmt %s", net_id, "WPA-PSK");
        }
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        ...
        ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d pairwise %s", net_id, "CCMP TKIP");
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        if (wlan_wpa_request(cmd, NULL, 0)) {
        ...
        ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d group %s", net_id, "CCMP TKIP");
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        if (wlan_wpa_request(cmd, NULL, 0)) {
        ...
        ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d psk \"%s\"", net_id, config->psswd);
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        if (wlan_wpa_request(cmd, NULL, 0)) {
    ...
    if (config->hidden_ssid == HI_TRUE) {
        ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SET_NETWORK %d scan_ssid 1", net_id);
        if (ret < 0) {
            dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
        }
        if (wlan_wpa_request(cmd, NULL, 0)) {
    ...
    ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd), "SELECT_NETWORK %d", net_id);
    if (ret < 0) {
        dbg_print(("WiFi: snprintf_s return %d cmd=%s\n", ret, cmd));
    }
    if (wlan_wpa_request(cmd, NULL, 0)) {
...
hi_s32 wlan_p2p_init(hi_void)
{
    ...
    dbg_print(("WiFi: P2P initialized successfully\n"));
    return ret;
}
...
hi_s32 wlan_sm_p2p_get_persistent_groupid(void)
{
    ...
    dbg_print(("WiFi: LIST_NETWORKS reply::\n\n"));
    dbg_print(("%s\n", reply));

    while (strncmp(reply, "network id", 10) != 0) { /* string <network id> len 10 */
        dbg_print(("WiFi: FAILED to receive LIST_NETWORKS reply, send again, left %d\n", i));
    ...
    while ((size = wlan_util_read_line(pos, line, sizeof(line)))) {
        ...
        dbg_print(("WiFi: line=%s\n", line));
        ret = sscanf_s(line, "%d\t%s\t%s\t%s", &network_id, ssid, sizeof(ssid),
    ...
    dbg_print(("WiFi: networkid = %d, ssid=%s,bssid=%s, flag=%s\n", network_id, ssid, bssid, flags));
    return network_id;
}
...
hi_s32 wlan_sm_p2p_delete_persistent_group(const hi_wlan_p2p_group_s *group)
{
    ...
    while ((size = wlan_util_read_line(pos, line, sizeof(line)))) {
        ...
        if (strcmp(ssid, group->network_name) == 0) {
            ret = snprintf_s(command, sizeof(command), sizeof(command) - 1, "REMOVE_NETWORK %d", network_id);
            if (ret < 0) {
                dbg_print(("WiFi: snprintf_s return %d command=%s\n", ret, command));
            }
...
hi_s32 wlan_p2p_find(const hi_s32 *timeout)
{
    ...
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: call wlan_p2p_remove_all_device fail!\n"));
    }
...
hi_s32 wlan_p2p_connect(hi_wlan_p2p_config_s *config)
{
    hi_s32 ret;
    hi_s32 results;
    hi_char command[256] = {0}; /* array command max len 256 */
    ...
    results = sprintf_s(tmp, sizeof(tmp), "P2P_CONNECT %s", config->bssid);
    if (results < EOK) {
        dbg_print(("WiFi: results=%d file=%s, line=%d, func=%s\n", results, __FILE__, __LINE__, __FUNCTION__));
    }
    switch (config->wps_method) {
        ...
        case WPS_PIN_KEYPAD:
            ...
            results = strcat_s(tmp, sizeof(tmp), config->pin);
            if (results != EOK) {
                dbg_print(("WiFi: results=%d file=%s, line=%d, func=%s\n", results, __FILE__, __LINE__, __FUNCTION__));
            }
    ...
    ret = snprintf_s(command, sizeof(command), sizeof(command) - 1, "%s", tmp);
    if (ret < 0) {
        dbg_print(("WiFi: ret=%d file=%s, line=%d, func=%s\n", ret, __FILE__, __LINE__, __FUNCTION__));
    }
    ...
    if (ret == HI_SUCCESS) {
        if (config->wps_method == WPS_PIN_DISPLAY) {
            len = len < PIN_CODE_LEN ? len : PIN_CODE_LEN;
            if (memcpy_s(config->pin, sizeof(config->pin), reply, len) < 0) {
                dbg_print(("WiFi: file=%s, line=%d, func=%s\n", __FILE__, __LINE__, __FUNCTION__));
            }
...
hi_s32 wlan_p2p_set_wfd_info(hi_wlan_wfd_info_s *config)
{
    hi_s32 ret;
    hi_char command[256];   /* array command max len 256 */
    hi_s32 results;
    ...
    results = sprintf_s(command, sizeof(command), "SET wifi_display %d", config->wfd_enabled);
    if (results < EOK) {
        dbg_print(("WiFi: results=%d file=%s,line=%d,func=%s\n", results, __FILE__, __LINE__, __FUNCTION__));
    }
    ret = wlan_wpa_request_p2p(command, NULL, 0);
    ...
    dbg_print(("WiFi: force to max_throughput to 30M\n"));
    config->max_throughput = 30;    /* max_throughput 30M */
    results = memset_s(command, sizeof(command), 0, sizeof(command));
    if (results < EOK) {
        dbg_print(("WiFi: results=%d file=%s,line=%d,func=%s\n", results, __FILE__, __LINE__, __FUNCTION__));
    }
    results = sprintf_s(command, sizeof(command), "WFD_SUBELEM_SET 0 %04x%04x%04x%04x", 6,  /* get first 6 char */
                        config->device_info, config->ctrl_port, config->max_throughput);
    if (results < EOK) {
        dbg_print(("WiFi: results=%d file=%s,line=%d,func=%s\n", results, __FILE__, __LINE__, __FUNCTION__));
    }
    ret = wlan_wpa_request_p2p(command, NULL, 0);
...
hi_s32 wlan_p2p_set_device_name(hi_char *name)
{
    ...
    if (name == NULL) {
        dbg_print(("WiFi:set device name is null\n"));
        return HI_WLAN_INVALID_PARAMETER;
    }
    dbg_print(("WiFi:set device name is %s\n", name));
...
hi_s32 wlan_p2p_set_device_info(hi_wlan_p2p_device_s *device)
{
    ...
    ret = snprintf_s(command, sizeof(command), sizeof(command) - 1, "SET device_name %s", device->name);
    if (ret < 0) {
        dbg_print(("WiFi: snprintf_s return %d command=%s\n", ret, command));
    }
    ret = wlan_wpa_request_p2p(command, NULL, 0);
    ...
    dbg_print(("WiFi: set config_methods to ONLY WPS_PBC(0x80)\n"));
    ...
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: call wlan_p2p_set_wfd_info fail!\n"));
    }
...
hi_s32 wlan_sm_start_wps_pbc(hi_char *bssid)
{
    ...
    if (strlen(bssid) == 0) {
        ...
        results = snprintf_s(command, sizeof(command), sizeof(command) - 1, "WPS_PBC %s", bssid);
        if (results == -1) {
            dbg_print(("WiFi: file=%s, line=%d, func=%s\n", __FILE__, __LINE__, __FUNCTION__));
        }
    }
...
hi_s32 wlan_sm_start_wps_pin_keypad(hi_char *bssid, hi_char *pin)
{
    ...
    ret = snprintf_s(command, sizeof(command), sizeof(command) - 1, "WPS_PIN %s", strlen(bssid) != 0 ? bssid : "any");
    if (ret <= 0) {
        dbg_print(("WiFi: file=%s, line=%d, func=%s\n", __FILE__, __LINE__, __FUNCTION__));
    }
    if (strlen(pin) != 0) {
    ...
    if (ret != HI_SUCCESS || size != PIN_CODE_LEN) {
        dbg_print(("WiFi: file=%s, line=%d, func=%s ret=%d size=%d\n", __FILE__, __LINE__, __FUNCTION__, ret, size));
        return HI_FAILURE;
    }
    printf("\n\tPIN_CODE:%s\n\n", reply);
...
static hi_void *wlan_go_negotiation_timer_thread(hi_void *args)
{
    ...
    sleep(20);  /* sleep 20s */
    dbg_print(("WiFi: P2P GO negotiate timeout\n"));
...
hi_s32 wlan_start_go_negotiation_timer(hi_void)
{
    ...
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: Cann't create timer thread\n"));
        g_wlan_timer_thread = 0;
    }
...
static hi_s32 wlan_p2p_process_group_started(hi_void *data)
{
    ...
    if (strcmp(g_p2p_group.iface, g_wlan_sm_ifname)) {
        ...
        if (g_mon_conn2 != NULL) {
            ...
            dbg_print(("WiFi: Connect to wpa_supplicant with '%s' failed\n", g_p2p_group.iface));
            return HI_FAILURE;
        }
        ...
        if (g_wlan_ethread2 == 0) {
            ...
            if (ret != HI_SUCCESS) {
                dbg_print(("WiFi: Cann't create monitor thread\n"));
...
hi_s32 wlan_state_closed(hi_s32 msg, hi_void *data, hi_char *reply, hi_s32 *len)
{
    ...
    hi_s32 results = 0;
    hi_wlan_sta_config_s *sta_cfg = (hi_wlan_sta_config_s *)data;
    ...
    switch (msg) {
        case CMD_STA_OPEN:
            ...
            ret = wlan_open(WIFI_MODE_STA, ifname, sizeof(ifname), sta_cfg);
            if (ret == HI_SUCCESS) {
        ...
        case CMD_P2P_OPEN:
            ...
            ret = wlan_open(WIFI_MODE_P2P, ifname, sizeof(ifname), sta_cfg);
            if (ret == HI_SUCCESS) {
                if (reply != NULL && len != NULL) {
                    results = strcpy_s(reply, sizeof(ifname), ifname);
                    if (results < EOK) {
...
hi_s32 wlan_state_p2p_started(hi_s32 msg, hi_void *data, hi_char *reply, hi_s32 *len)
{
    ...
    switch (msg) {
        ...
        case CMD_P2P_LISTEN:
            dbg_print(("WiFi: received cmd_p2p_listen, BUT ignore!\n"));
            break;

        case CMD_P2P_FLUSH:
            if (wlan_wpa_request_p2p("P2P_FLUSH", NULL, 0) == HI_FAILURE) {
                dbg_print(("WiFi: CMD_P2P_FLUSH-'P2P_FLUSH' request fail in '%s'!\n", __func__));
            }
            break;

        case CMD_P2P_GROUP_ADD:
            if (wlan_wpa_request_p2p("WFD_SUBELEM_SET 0 000600111c44001e", NULL, 0) == HI_FAILURE) {
                dbg_print(("WiFi: FAILED to send WFD_SUBELEM_SET 0 000600111c44001e\n"));
            }
            network_id = wlan_sm_p2p_get_persistent_groupid();
            dbg_print(("WiFi: CMD_P2P_GROUP_ADD:network_id=%d\n", network_id));
            if (network_id >= 0) {
                results = memset_s(command, sizeof(command), 0, sizeof(command));
                if (results != EOK) {
                    dbg_print(("WiFi: results=%d file=%s, line=%d, func=%s\n",
                ...
                results = sprintf_s(command, sizeof(command), "P2P_GROUP_ADD persistent=%d", network_id);
                if (results < 0) {
                    dbg_print(("WiFi: results=%d file=%s, line=%d, func=%s\n",
                        results, __FILE__, __LINE__, __FUNCTION__));
                }
                if (wlan_wpa_request_p2p(command, NULL, 0) == HI_FAILURE) {
                    dbg_print(("WiFi: P2P_GROUP_ADD persistent=x request fail in '%s'!\n", __func__));
                }
            } else {
                if (wlan_wpa_request_p2p("REMOVE_NETWORK all", NULL, 0) == HI_FAILURE) {
                    dbg_print(("WiFi: REMOVE_NETWORK all fail in '%s'!\n", __func__));
                }

                if (wlan_wpa_request_p2p("P2P_GROUP_ADD persistent", NULL, 0) == HI_FAILURE) {
                    dbg_print(("WiFi: P2P_GROUP_ADD request fail in '%s'!\n", __func__));
                }
            }
            break;
        ...
        case P2P_GO_NEGOTIATION_REQUEST_EVENT: {
            hi_wlan_p2p_config_s *config = (hi_wlan_p2p_config_s *)data;

            wlan_sm_transfer_state(&g_wlan_state_p2p, WLAN_STATE_P2P_USER_AUTHORIZING_INVITATION);

            if (config->wps_method == WPS_PIN_DISPLAY) {
                results = strncpy_s(config->pin, sizeof(config->pin),
            ...
            wlan_sm_broadcast_event(HI_WLAN_P2P_EVENT_CONNECTION_REQUESTED, config);
        }
        break;
        ...
        case P2P_GROUP_REMOVED_EVENT:
            if (wlan_wpa_request_p2p("P2P_FLUSH", NULL, 0)  == HI_FAILURE) {
                dbg_print(("WiFi: P2P_GROUP_REMOVED_EVENT-'P2P_FLUSH' request fail in '%s'!\n", __func__));
            }
...
hi_s32 wlan_state_p2p_user_authorizing_invitation(hi_s32 msg, hi_void *data, hi_char *reply, hi_s32 *len)
{
    ...
    switch (msg) {
        ...
        case CMD_P2P_CONNECT:
            ...
            dbg_print(("WiFi: User accept the connection request\n"));
            ret = wlan_p2p_connect((hi_wlan_p2p_config_s *)data);
            if (ret == HI_SUCCESS) {
                wlan_sm_transfer_state(&g_wlan_state_p2p, WLAN_STATE_P2P_GROUP_NEGOTIATION);
                wlan_start_go_negotiation_timer();
        ...
        case CMD_P2P_DISCONNECT:
            ...
            dbg_print(("WiFi: User reject the connection request\n"));
            wlan_sm_transfer_state(&g_wlan_state_p2p, WLAN_STATE_P2P_STARTED);
            break;
...
hi_s32 wlan_state_p2p_user_authorizing_join(hi_s32 msg, hi_void *data, hi_char *reply, hi_s32 *len)
{
    ...
    switch (msg) {
        ...
        case CMD_P2P_CONNECT: {
            ...
            dbg_print(("WiFi: User accept the connection request\n"));
            if (config->wps_method == WPS_PBC) {
                ret = wlan_sm_start_wps_pbc_p2p(NULL);
            } else if (config->wps_method == WPS_PIN_KEYPAD) {
                ret = wlan_sm_start_wps_pin_keypad_p2p(config->pin);
            } else {
                ret = HI_SUCCESS;
            }
            dbg_print(("WiFi: count 20s for WPS\n"));
            wlan_start_go_negotiation_timer();
            wlan_sm_transfer_state(&g_wlan_state_p2p, WLAN_STATE_P2P_GROUP_CREATED);
        }
        break;
        ...
        case P2P_DEVICE_LOST_EVENT:
            ...
            dbg_print(("WiFi: P2P_DEVICE_LOST, back to STATE_P2P_GROUP_CREATED\n"));
            wlan_sm_transfer_state(&g_wlan_state_p2p, WLAN_STATE_P2P_GROUP_CREATED);
            break;
...
hi_s32 wlan_state_p2p_group_negotiation(hi_s32 msg, hi_void *data, hi_char *reply, hi_s32 *len)
{
    ...
    switch (msg) {
        ...
        case P2P_GO_NEGOTIATION_SUCCESS_EVENT:
            dbg_print(("WiFi: P2P group owner negotiation success\n"));
            break;

        case P2P_GO_NEGOTIATION_FAILURE_EVENT:
        case P2P_GROUP_FORMATION_FAILURE_EVENT:
            dbg_print(("WiFi: P2P group owner negotiation fail\n"));
            ret = wlan_p2p_remove_group("p2p0");
            if (wlan_wpa_request_p2p("P2P_FLUSH", NULL, 0) == HI_FAILURE) {
                dbg_print(("WiFi: P2P_GO_NEGOTIATION_FAILURE_EVENT-'P2P_FLUSH' \
        ...
        case P2P_GROUP_FORMATION_SUCCESS_EVENT:
            dbg_print(("WiFi: P2P group formation success\n"));
            break;
...
hi_s32 wlan_state_p2p_group_created(hi_s32 msg, hi_void *data, hi_char *reply, hi_s32 *len)
{
    ...
    switch (msg) {
        ...
        case CMD_P2P_CONNECT: {
            ...
            ret = memcpy_s(&g_p2p_saved_config, sizeof(hi_wlan_p2p_config_s), config, sizeof(hi_wlan_p2p_config_s));
            if (ret != 0) {
                dbg_print(("WiFi: file=%s, line=%d, func=%s\n", __FILE__, __LINE__, __FUNCTION__));
            }
        ...
        case P2P_PROV_DISC_SHOW_PIN_EVENT: {
             ...
             * group negotiation comes through causes issues
             */
            dbg_print(("WiFi: set wfd session unavailable\n"));
            if (wlan_wpa_request_p2p("WFD_SUBELEM_SET 0 000600011c44001e", NULL, 0) == HI_FAILURE) {
                dbg_print(("WiFi: FAILED to send WFD_SUBELEM_SET 0 000600011c44001e\n"));
            }
            hi_wlan_p2p_provdisc_s *pd = (hi_wlan_p2p_provdisc_s *)data;
        ...
        case AP_STA_CONNECTED_EVENT:
            dbg_print(("WiFi: p2p_connected, cancel 20s timer, and set wfd session unavailable\n"));
            wlan_timer_thread_cancel();
            if (wlan_wpa_request_p2p("WFD_SUBELEM_SET 0 000600011c44001e", NULL, 0) == HI_FAILURE) {
                dbg_print(("WiFi: FAILED to send WFD_SUBELEM_SET 0 000600011c44001e\n"));
            }
            dbg_print(("WiFi: cancel already wps-active\n"));
            if (wlan_wpa_request_p2p("WPS_CANCEL", NULL, 0) == HI_FAILURE) {
                dbg_print(("WiFi: FAILED to send WPS_CANCEL\n"));
            }
            break;
        default:
            break;
