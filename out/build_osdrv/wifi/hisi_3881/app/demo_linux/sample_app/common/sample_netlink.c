...
/*****************************************************************************
  2 宏定义、全局变量
*****************************************************************************/
#define NETLINK_SOCKET_PORT_ID                  1100 /* port id 1100 */
#define NETLINK_CHANNEL_MODEID                  28 /* mode id 28 */
#undef NLMSG_ALIGNTO
#define NLMSG_ALIGNTO                           1 /* align 1 */
...
static hi_void *hi_channel_host_thread(hi_void *args)
{
    hi_s32 rev_len;
    hi_s32 payload_len;
    hi_char msg[MAX_SEND_DATA_SIZE + NLMSG_HDRLEN];
...
{
    if (g_channel_terminate == HI_TRUE) {
        return HI_SUCCESS;
    }
    if (g_channel_monitor == HI_NULL) {
        printf("hi_channel_register_rx_cb is fail\n");
        return HI_FAILURE;
    }
...
hi_s32 hi_channel_send_to_dev(unsigned char *buf, int len)
{
    ...
    if (memcpy_s(NLMSG_DATA(nlh), NLMSG_SPACE(len), buf, len) != EOK) {
        printf("memcpy nlh fail\n");
        free(nlh);
        return HI_FAILURE;
    }
...
hi_s32 hi_channel_init(hi_void)
{
    ...
    if (g_channel_monitor != HI_NULL) {
        printf("hi_channel_init is fail\n");
        return HI_FAILURE;
    }
...
hi_s32 hi_channel_deinit(hi_void)
{
    ...
    if (g_channel_monitor == HI_NULL) {
        printf("hi_channel_deinit is fail\n");
        return HI_FAILURE;
    }
    ...
    printf("hi_channel_deinit is success.\n");
    return HI_SUCCESS;
}
