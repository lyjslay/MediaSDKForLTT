...
static ssize_t wal_hipriv_sys_write(oal_device_stru *dev, oal_device_attribute_stru *attr,
    const char *buf, size_t count);
static ssize_t wal_hipriv_sys_read(oal_device_stru *dev, oal_device_attribute_stru *attr, char *buf);
...
static hi_u32  wal_hipriv_host_sleep_switch(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    if ((strcmp("0", ac_name) != 0) && (strcmp("1", ac_name) != 0)) {
...
static hi_u32 wal_hipriv_offload_parse_ipv4(hi_char *ip_str, hi_u32 *result)
{
    ...
    byte = oal_strtok(ip_str, delims, &pcmdstr);
    while (byte != HI_NULL) {
        byte_value = (hi_u32)oal_atoi(byte);
        byte_value <<= count * 8; /* 移位3次，每次8比特。 */
        count++;
        *result += byte_value;
        byte = oal_strtok(HI_NULL, delims, &pcmdstr);
    }
...
#ifdef _PRE_WLAN_FEATURE_P2P
    /* 创建P2P 相关VAP */
    } else if (strcmp("p2p_device", cmd_param->cmd_param2) == 0) {
        *mode = WLAN_VAP_MODE_BSS_STA;
        *p2p_mode = WLAN_P2P_DEV_MODE;
    } else if (strcmp("p2p_cl", cmd_param->cmd_param2) == 0) {
        *mode = WLAN_VAP_MODE_BSS_STA;
        *p2p_mode = WLAN_P2P_CL_MODE;
    } else if (strcmp("p2p_go", cmd_param->cmd_param2) == 0) {
        *mode = WLAN_VAP_MODE_BSS_AP;
...
static hi_u32 wal_hipriv_add_user(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    if (strcmp("0", cmd_param.cmd_param2) == 0) {
    ...
    const char *pc_buffer, size_t count)
{
    ...
    if (len > WAL_HIPRIV_CMD_MAX_LEN) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_sys_write::len>WAL_HIPRIV_CMD_MAX_LEN, len [%u]!}\r\n", len);
        return -OAL_EINVAL;
    }
...
static ssize_t wal_hipriv_sys_read(oal_device_stru *dev, oal_device_attribute_stru *attr, char *pc_buffer)
{
    hi_u32              cmd_idx;
    hi_u32              buff_index = 0;
