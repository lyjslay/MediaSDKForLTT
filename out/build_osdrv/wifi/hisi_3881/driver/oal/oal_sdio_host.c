...
#define SDIO_WAIT_TIME      500
#define SDIO_WAIT_CNT       10
...
static hi_pvoid g_sdio_priv_data;
hi_u32 g_sdio_card_mutex;
hi_s32 g_sdio_card_lock_cnt = 0;
#endif
...
#if ((!defined(_PRE_LOSCFG_KERNEL_SMP) && HW_LITEOS_OPEN_VERSION_NUM >= VERSION_NUM(3,3,3)) || \
    (defined(_PRE_LOSCFG_KERNEL_SMP) && HW_LITEOS_OPEN_VERSION_NUM >= VERSION_NUM(5,0,1)))
int sdio_reset_comm(struct mmc_softc *sc);
#else
...
static hi_s32 oal_sdio_transfer_cal_size(const oal_channel_stru *hi_sdio, hi_s32 rw, const oal_netbuf_head_stru *head,
    hi_u32 *buf_len)
{
    hi_u32 queue_len;
    hi_u32 sum_len = 0;
    ...
    skb_queue_walk_safe(head, netbuf, tmp) {
        if (!oal_is_aligned((uintptr_t)oal_netbuf_data(netbuf), SDIO_FLAT_MEMORY_BUFFLEN_ALIGN)) {
        ...
        if (sum_len + oal_netbuf_len(netbuf) > SDIO_MAX_FLAT_MEMORY_SIZE) {
            oam_error_log4(0, OAM_SF_ANY, "{length is invalid!} idx: %u, sum_len: %u, length: %u, max: %u",
                idx, sum_len, oal_netbuf_len(netbuf), SDIO_MAX_FLAT_MEMORY_SIZE);
            return -OAL_EFAIL;
        }

        if (rw == SDIO_WRITE) {
            hi_s32 err = memcpy_s((hi_s8 *)hi_sdio->tx_flat_buff + sum_len, SDIO_MAX_FLAT_MEMORY_SIZE - sum_len,
                oal_netbuf_data(netbuf), oal_netbuf_len(netbuf));
            if (err != EOK) {
                oam_error_log4(0, OAM_SF_ANY, "{copy failed!} err: %d, sum_len: %u, length: %u, max: %u",
                    err, sum_len, oal_netbuf_len(netbuf), SDIO_MAX_FLAT_MEMORY_SIZE);
                return -OAL_EFAIL;
            }
        }
    ...
    if (sum_len < HISDIO_BLOCK_SIZE) {
        sum_len = hi_byte_align(sum_len, SDIO_FLAT_MEMORY_BUFFLEN_ALIGN);
    } else {
...
static hi_s32 oal_sdio_transfer_copy(const oal_channel_stru *hi_sdio, const oal_netbuf_head_stru *head, hi_u8* buf,
    hi_u32 len)
{
    oal_netbuf_stru *netbuf = HI_NULL;
    oal_netbuf_stru *tmp = HI_NULL;
    ...
    hi_s32 index = 0;
    skb_queue_walk_safe(head, netbuf, tmp) {
        hi_s32 err = memcpy_s(oal_netbuf_data(netbuf), oal_netbuf_len(netbuf), buf + offset, oal_netbuf_len(netbuf));
        if (err != EOK) {
            oam_error_log3(0, OAM_SF_ANY, "{copy failed!} err: %d, sum_len: %u, length: %u",
                err, offset, oal_netbuf_len(netbuf));
            return -OAL_EFAIL;
        }
...
static hi_s32 oal_sdio_transfer_flat_memory(const oal_channel_stru *hi_sdio, const oal_netbuf_head_stru *head,
    hi_s32 rw)
{
    hi_s32 ret;
    if (hi_sdio == HI_NULL) {
    ...
    // get length, if SDIO_WRITE, copy data to buf
    hi_u32 buf_len = 0;
    ...
    // if SDIO_READ, copy result to buf
    if (rw == SDIO_READ) {
...
hi_s32 oal_sdio_transfer_rx_reserved_buff(const oal_channel_stru *hi_sdio)
{
    ...
    if (hi_sdio->sdio_extend == HI_NULL || hi_sdio->sdio_extend->xfer_count == 0) {
        oam_error_log0(0, OAM_SF_ANY, "{hi_sdio->sdio_extend invalid!}");
        return -OAL_EINVAL;
    }
...
#endif
    if (oal_unlikely(ret)) {
        printk("sdio trans revered mem failed! _oal_sdio_transfer_scatt, ret=%d\n", ret);
    }
    return ret;
}
...
#ifdef _PRE_LOS_VERSION_SDIO_NO_SCATTER
    sdio_mutex_init(&g_sdio_card_mutex);
#endif
...
static hi_s32 oal_sdio_probe(struct sdio_func *func, const struct sdio_device_id *ids)
{
    oal_channel_stru     *hi_sdio = HI_NULL;
    hi_s32           ret;
    printk("oal_sdio_probe \n");
#ifdef _PRE_LOS_VERSION_SDIO_NO_SCATTER
    if (func == HI_NULL || func->sc == HI_NULL || (!ids)) {
...
hi_void dump_sg_format(struct scatterlist *sg, hi_u32 sg_len)
{
    ...
    for_each_sg(sg, sg_t, sg_len, i) {
        ...
        printk("sg descr:%3d,addr:%p,len:%6u\n", i, sg_virt(sg_t), sg_t->length);
    }
}
...
#endif /* _PRE_SDIO_FEATURE_FLAT_MEMORY */
#endif
...
hi_s32 oal_sdio_transfer_netbuf_list(const oal_channel_stru *hi_sdio, const oal_netbuf_head_stru *head, hi_s32 rw)
{
    ...
    skb_queue_walk_safe(head, netbuf, tmp) {
        ...
        if (!oal_is_aligned((uintptr_t)oal_netbuf_data(netbuf), SDIO_FLAT_MEMORY_BUFFLEN_ALIGN)) { /* 4 �ֽڶ��� */
            oam_error_log0(0, OAM_SF_ANY, "{oal_sdio_transfer_netbuf_list netbuf 4 aligned fail!}");
            return -OAL_EINVAL;
        }
...
#ifdef _PRE_LOS_VERSION_SDIO_NO_SCATTER
    sdio_mutex_delete(g_sdio_card_mutex);
#endif
...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
hi_s32 sdio_card_wait_plugin(hi_s32 val, hi_u32 vendor, hi_u32 device)
{
    for (int i = 0; i < SDIO_WAIT_CNT; ++i) {
        struct sdio_func *func = sdio_get_func(val, vendor, device);
        if (func != HI_NULL) {
            printk("wait liteos card success. card:%d, vendor:0x%x, device:0x%x \n", val, vendor, device);
            return OAL_SUCC;
        }
        printk("wait liteos card plugin... card:%d, vendor:0x%x, device:0x%x \n", val, vendor, device);
        msleep(SDIO_WAIT_TIME);
    }
    printk("wait liteos card failed. card:%d, vendor:0x%x, device:0x%x \n", val, vendor, device);
    return -OAL_EFAIL;
}
#endif
...
hi_s32 oal_sdio_detectcard_to_core(hi_s32 val)
{
...
#elif defined(HISI_WIFI_PLATFORM_SD3403V100)
#else
    ...
    if (val == 1) {
#ifdef _PRE_LOS_VERSION_SDIO_NO_SCATTER
        if (sdio_card_wait_plugin(val, g_oal_sdio_ids[0].vendor, g_oal_sdio_ids[0].device) != OAL_SUCC) {
            oam_error_log0(0, 0, "wait liteos card failed.");
            return -OAL_EFAIL;
        }
#endif
        struct sdio_func *func = HI_NULL;
...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    hi_sdio->sdio_align_buff = memalign(CACHE_ALIGNED_SIZE, oal_skb_data_align(HISDIO_BLOCK_SIZE));
#else
...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    tx_scatt_buff_len = oal_skb_data_align(tx_scatt_buff_len);
...
#ifdef _PRE_LOS_VERSION_SDIO_NO_SCATTER
    if (hi_sdio == HI_NULL || hi_sdio->func == HI_NULL || hi_sdio->func->sc == HI_NULL) {
        printk("oal_sdio_func_max_req_size:hi_sdio /hi_sdio->func null!\n");
        return -OAL_EFAIL;
    }
#else
...
#if ((!defined(_PRE_LOSCFG_KERNEL_SMP) && HW_LITEOS_OPEN_VERSION_NUM >= VERSION_NUM(3,3,3)) || \
    (defined(_PRE_LOSCFG_KERNEL_SMP) && HW_LITEOS_OPEN_VERSION_NUM >= VERSION_NUM(5,0,1)))
    if ((pst_func == HI_NULL) || (pst_func->sc == HI_NULL)) {
...
#if ((!defined(_PRE_LOSCFG_KERNEL_SMP) && HW_LITEOS_OPEN_VERSION_NUM >= VERSION_NUM(3,3,3)) || \
    (defined(_PRE_LOSCFG_KERNEL_SMP) && HW_LITEOS_OPEN_VERSION_NUM >= VERSION_NUM(5,0,1)))
        if (sdio_reset_comm(device_get_softc(device_get_parent(pst_func->sc->dev))) == HI_SUCCESS) {
            break;
        }
#else
