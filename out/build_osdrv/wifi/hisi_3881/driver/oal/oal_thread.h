...
#if (HW_LITEOS_OPEN_VERSION_NUM >= KERNEL_VERSION(5,0,1))
    ...
    void *p_data, const oal_kthread_param_stru  *thread_param)
{
    ...
    ret = sched_setscheduler(kthread, thread_param->l_policy, &st_sched_param);
    if (oal_unlikely(ret)) {
        oal_io_print3("%s sched_setscheduler failed! ret =%d, prio=%d\n", \
            pc_thread_name, ret, thread_param->l_prio);
    }
