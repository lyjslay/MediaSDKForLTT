...
#define os_kmalloc_gfp(size)        memalign(32, oal_skb_data_align(size))
...
typedef struct firmware_globals_st {
    ...
    hi_u8             cfg_version[VERSION_LEN];   /* 存储cfg文件中配置的版本号信息 */
    hi_u8             dev_version[VERSION_LEN];   /* 存储加载时device侧上报的版本号信息 */
