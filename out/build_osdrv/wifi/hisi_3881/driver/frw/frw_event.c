...
hi_u32 frw_event_post_event(frw_event_mem_stru *event_mem)
{
    hi_u16                     us_qid;
    hi_u32                     ret;
    unsigned long              flag, irq_flag;

    /* 获取事件队列ID */
    if (frw_event_to_qid(event_mem, &us_qid) != HI_SUCCESS) {
        oam_warning_log0(0, OAM_SF_FRW, "{frw_event_post_event, frw_event_to_qid return != HI_SUCCESS!}");
        return HI_ERR_CODE_ARRAY_OVERFLOW;
    }
    /* 根据核号 + 队列ID，找到相应的事件队列 */
    if (g_ast_event_manager.event_queue == HI_NULL) {
    ...
    frw_event_queue_stru *event_queue = &(g_ast_event_manager.event_queue[us_qid]);
    /* 检查policy */
    ...
    /* 获取调度队列 */
    frw_event_sched_queue_stru *sched_queue = &(g_ast_event_manager.sched_queue[event_queue->policy]);
    ...
    if (oal_unlikely(ret != HI_SUCCESS)) {
        ...
        frw_event_hdr_stru *event_hdr = (frw_event_hdr_stru *)(event_mem->puc_data);
        oam_error_log4(0, OAM_SF_FRW,
