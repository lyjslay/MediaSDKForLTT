    ...
    hi_u32 remain_len)
{
    ...
    hi_u8 *buf = (hi_u8 *)oal_netbuf_data(descr_netbuf);
    hi_s32 len = (hi_s32)oal_netbuf_len(descr_netbuf);
    hi_u32 assemble_max_count = oal_max(1, g_hcc_assemble_count);
    hi_u32 queue_len = oal_netbuf_list_len(head);
    hi_u32 current_trans_len = oal_min(queue_len, assemble_max_count);
