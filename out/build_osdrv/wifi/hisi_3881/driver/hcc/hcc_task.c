...
static  hi_s32  hcc_thread_tx_wait_event_cond_check(hcc_handler_stru *hcc_handler)
{
     ...
     */
    hcc_trans_queue_stru queue;
    queue = hcc_handler->hcc_transer_info.hcc_queues[HCC_TX].queues[DATA_HI_QUEUE];
    if (oal_netbuf_list_len(&queue.data_queue)) {
        return HI_TRUE;
    }

    queue = hcc_handler->hcc_transer_info.hcc_queues[HCC_TX].queues[DATA_LO_QUEUE];
    ret = (oal_netbuf_list_len(&queue.data_queue) &&
