...
#define get_sin_addr(x)     (((struct sockaddr_in *) (&(x)))->sin_addr.s_addr)
...
static pthread_t wlan_timer_thread_power = 0;

static const hi_char g_dev_wifi_direct[] = "/dev/wifi";
static const hi_char g_ap_pid_file[] = "/dev/wifi/sample.pid";
static const hi_char g_ipaddr[] = "192.168.49.1";
static const hi_char g_udhcpd_config_file[] = "/etc/Wireless/udhcpd.conf";
...
hi_s32 wlan_ap_set_ip(const hi_char *ifname, const hi_char *ip)
{
    ...
    get_sin_addr(ifr.ifr_addr) = inet_addr(ip);
...
hi_s32 creat_udhcpd_config_file(hi_char *ifname, const hi_char *config_file)
{
    ...
    if (asprintf(&wbuf, "start           192.168.49.20\n"
                        "end             192.168.49.254\n"
                        "interface       %s\n"
...
static hi_void *wlan_power_timer(hi_void *args)
{
    ...
    if (g_ap_data->persist_on == HI_FALSE) {
        if (g_ap_data->ap_enabled) {
            wlan_ap_stop_udhcpd();
            hi_wlan_ap_stop(g_ap_data->ifname);
#ifdef DRIVER_EXCEPTION_REPORT
    ...
    if (hi_wlan_ap_open(g_ap_data->ifname, sizeof(g_ap_data->ifname),
        ...
        if (hi_wlan_ap_start(g_ap_data->ifname, &g_ap_data->config) == HI_SUCCESS) {
            ...
            if (wlan_ap_set_ip(g_ap_data->ifname, g_ipaddr) != 0) {
                printf("SAMPLE_AP: set local IP failed\n");
            }
            if (creat_udhcpd_config_file(g_ap_data->ifname, g_udhcpd_config_file) != 0) {
                printf("\nSAMPLE_AP: creat udhcpd.conf failure\n");
            }
            if (wlan_ap_start_udhcpd(g_udhcpd_config_file) != 0) {
...
hi_s32 wlan_timer_thread_cancel_power(hi_void)
{
    ...
    if (wlan_timer_thread_power) {
        pthread_cancel(wlan_timer_thread_power);
        pthread_join(wlan_timer_thread_power, (void*)&ret);
        wlan_timer_thread_power = 0;
    }
...
static hi_s32 wlan_start_power_timer(hi_void)
{
    ...
    ret = pthread_create(&wlan_timer_thread_power, NULL, wlan_power_timer, NULL);
    if (ret != HI_SUCCESS) {
...
hi_void wlan_create_pid_file(const hi_char *pid_file)
{
    ...
    if (access(g_dev_wifi_direct, F_OK) < 0) {
        if (mkdir(g_dev_wifi_direct, 0666) != 0) {                        /* g_dev_wifi_direct mode 0666 */
            printf("SAMPLE_AP: Create '%s' fail\n", g_dev_wifi_direct);
            return;
        }
        chmod(g_dev_wifi_direct, 0666);                                  /* g_dev_wifi_direct mode 0666 */
    }
...
hi_s32 main(hi_s32 argc, hi_char *argv[])
{
    ...
    wlan_create_pid_file(g_ap_pid_file);
    ...
    while (!terminate) {
        ...
        switch (message.what) {
            case AP_CMD_ENABLE:
                ...
                g_ap_data->ap_enabled = HI_TRUE;
                ret = wlan_ap_set_ip(g_ap_data->ifname, g_ipaddr);
                if (ret) {
                    printf("SAMPLE_AP: set local IP failed\n");
                }

                ret = creat_udhcpd_config_file(g_ap_data->ifname, g_udhcpd_config_file);
                if (ret != 0) {
                    printf("\nSAMPLE_AP: creat udhcpd.conf failure\n");
                }

                ret = wlan_ap_start_udhcpd(g_udhcpd_config_file);
                if (ret) {
                    printf("SAMPLE_AP: start udhcpd failed\n");
                }
