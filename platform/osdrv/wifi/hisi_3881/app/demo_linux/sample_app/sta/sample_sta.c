...
typedef enum hi_wlan_msg_s {
    ...
    STA_CMD_PROMIS_ENABLE,      /* start wifi promis */
    /* events */
...
static hi_void usage(hi_void)
{
    ...
    printf("\t\tpin:    pin code\n");
    printf("\tsample_cli  promis_enable,<enable>,<umngt_en>,<mmngt_en>,<udata_en>,<mdata_en>\
        set promis status\n");
...
void sdio_pin_mux_init(void)
{
    ...
    system_check("himm 0x112F0008 0x681");
    system_check("himm 0x112F0010 0x581");
    system_check("himm 0x112F0014 0x581");
    system_check("himm 0x112F0018 0x581");
    system_check("himm 0x112F000C 0x581");
    system_check("himm 0x112F001C 0x581");
#elif defined(HISI_WIFI_PLATFORM_HI3516) || defined(HISI_WIFI_PLATFORM_HI3516EV300)
    system_check("himm 0x100C0060 0x1D54");
    system_check("himm 0x100C006C 0x1174");
    system_check("himm 0x100C0068 0x1174");
    system_check("himm 0x100C0074 0x1174");
    system_check("himm 0x100C0064 0x1174");
    system_check("himm 0x100C0070 0x1174");
#else
    system_check("himm 0x112C0048 0x1D54");
    system_check("himm 0x112C004c 0x1134");
    system_check("himm 0x112C0060 0x1134");
    system_check("himm 0x112C0064 0x1134");
    system_check("himm 0x112C0058 0x1134");
    system_check("himm 0x112C005c 0x1134");
#endif
}
...
{
#if defined(HISI_WIFI_PLATFORM_HI3559V200) || defined(HISI_WIFI_PLATFORM_HI3556V200)
    system_check("himm 0x112F00B8 0x0601");
    system_check("himm 0x120DB400 0x08");
#elif defined(HISI_WIFI_PLATFORM_HI3516) || defined(HISI_WIFI_PLATFORM_HI3516EV300)
    system_check("himm 0x120F0000 0x00100164");
#else
    system_check("himm 0x112C0074 0x1000");
    system_check("himm 0x120B6400 0x10");
#endif
}
...
#if defined(HISI_WIFI_PLATFORM_HI3559V200) || defined(HISI_WIFI_PLATFORM_HI3556V200)
    if (level) {
        system_check("himm 0x120DB3FC 0x08");
    } else {
        system_check("himm 0x120DB3FC 0x00");
    }
#elif defined(HISI_WIFI_PLATFORM_HI3516) || defined(HISI_WIFI_PLATFORM_HI3516EV300)
    if (level) {
        system_check("himm 0x120F0000 0x00100164");
    } else {
        system_check("himm 0x120F0000 0x00100160");
    }
#else
    if (level) {
        system_check("himm 0x120B6040 0x10");
    } else {
        system_check("himm 0x120B6040 0x00");
    }
#endif
}
...
{
    hi_char *pstr = NULL;
    hi_char *pcmdstr = NULL;
    hi_u32  i = 0;
    hi_u32  current_chan_num = 0;
    hi_u32  chan_num = 0;

    pstr = strtok_r(strin, ",", &pcmdstr);
    while (pstr) {
...
{
    hi_channel_register_rx_cb(HI_NULL);
    hi_channel_deinit();
...
#define promis_return(tmp_arg) do { \
    if ((atoi(tmp_arg) != 0) && (atoi(tmp_arg) != 1)) { \
        printf("SAMPLE_STA: promis flag value is wrong!\n"); \
        return HI_FAILURE; \
    } \
} while (0)
static hi_u32 promis_count[3][15] = {0}; /* 3 帧的类型，15子帧的类型 */

void wlan_promis_data_func(unsigned char *data, int len)
{
    hi_u8 type;
    hi_u8 sub_type;
    hi_u8 to_ds;
    ...
    if ((data == HI_NULL) || (len <= 0)) {
        printf("SAMPLE_STA: wlan_promis_data_func data is fail!\n");
        return;
    }

    type = ((*data) >> 2) & 0x03; /* right shift 2, 0x03取2bit */
    sub_type = ((*data) >> 4) & 0x0F; /* right shift 4, 0x0F取4bit */
    to_ds = (*(data + 1)) & 0x01; /* 0x01取1bit */
    offset = (to_ds == 0) ? 4 : 16; /* 偏移4或者16字节 */
    is_multicast = (*(data + offset)) & 0x01; /* 0x01取1bit */

    if ((++promis_count[type][sub_type]) % 5 == 0) { // print every 5 count
        printf("Frame: type:%u, sub_type:%d, multi:%d, count:%d\n", type, sub_type,
            is_multicast, promis_count[type][sub_type]);
    }
}


hi_s32 wlan_promis_cmd_process(hi_void *wdata, hi_char *param, hi_void *pmsg)
{
    if (wdata == HI_NULL || pmsg == HI_NULL) {
        return HI_FAILURE;
    }
    ...
    printf(">SAMPLE_STA promis enable command\n");
    if (pdata->state == STA_STATE_DISABLED) {
        printf("SAMPLE_STA: file=%s,line=%d,func=%s\n", __FILE__, __LINE__, __FUNCTION__);
        return HI_FAILURE;
    }
    ...
    promis_return(tmp_arg);
    enable |= (atoi(tmp_arg) != 0) ? 0x10 : 0; /* 0x10 set value */
    param += off_set + 1;
    if (wlan_get_cmd_one_arg(param, tmp_arg, WLAN_CMD_MAX_LEN, &off_set) != HI_SUCCESS) {
        printf("SAMPLE_STA: file=%s,line=%d,func=%s\n", __FILE__, __LINE__, __FUNCTION__);
        return HI_FAILURE;
    ...
    promis_return(tmp_arg);
    enable |= (atoi(tmp_arg) != 0) ? 0x8 : 0; /* 0x8 set value */
    param += off_set + 1;
    if (wlan_get_cmd_one_arg(param, tmp_arg, WLAN_CMD_MAX_LEN, &off_set) != HI_SUCCESS) {
        printf("SAMPLE_STA: file=%s,line=%d,func=%s\n", __FILE__, __LINE__, __FUNCTION__);
        return HI_FAILURE;
    ...
    promis_return(tmp_arg);
    enable |= (atoi(tmp_arg) != 0) ? 0x4 : 0; /* 0x4 set value */
    param += off_set + 1;
    if (wlan_get_cmd_one_arg(param, tmp_arg, WLAN_CMD_MAX_LEN, &off_set) != HI_SUCCESS) {
        printf("SAMPLE_STA: file=%s,line=%d,func=%s\n", __FILE__, __LINE__, __FUNCTION__);
        return HI_FAILURE;
    ...
    promis_return(tmp_arg);
    enable |= (atoi(tmp_arg) != 0) ? 0x2 : 0; /* 0x2 set value */
    param += off_set + 1;
    if (wlan_get_cmd_one_arg(param, tmp_arg, WLAN_CMD_MAX_LEN, &off_set) != HI_SUCCESS) {
        printf("SAMPLE_STA: file=%s,line=%d,func=%s\n", __FILE__, __LINE__, __FUNCTION__);
        return HI_FAILURE;
    ...
    promis_return(tmp_arg);
    enable |= (atoi(tmp_arg) != 0) ? 0x1 : 0; /* 0x1 set value */
    msg->obj.config.promis_enable = enable;
    msg->what = STA_CMD_PROMIS_ENABLE;
    return HI_SUCCESS;
}
...
static const wlan_cmd_entry_stru  g_sta_cmd[] = {
    ...
    {"promis_enable",       wlan_promis_cmd_process}
};
...
hi_s32 main(hi_s32 argc, hi_char *argv[])
{
    ...
    while (!g_terminate) {
        ...
        switch (message.what) {
            case STA_CMD_ENABLE:
                ...
                if (ret == HI_SUCCESS) {
                    ...
                    if (ret == HI_SUCCESS) {
                        ...
                        wlan_wait_enable_ok();
                        hi_channel_init();
                        hi_channel_register_rx_cb(wlan_promis_data_func);
                    } else {
            ...
            case STA_CMD_PROMIS_ENABLE:
                memset_s(&mode, sizeof(mode), 0, sizeof(mode));
                memset_s(promis_count, sizeof(promis_count), 0, sizeof(promis_count));
                if (memcpy_s(mode.ifname, sizeof(mode.ifname), g_data->ifname,
                    sizeof(g_data->ifname)) != EOK) {
                    break;
                }

                mode.enable = message.obj.config.promis_enable;
                printf("SAMPLE_STA: STA_CMD_PROMIS_ENABLE(%d)(%s)\n", mode.enable, mode.ifname);
                hi_channel_send_to_dev((hi_u8 *)&mode, sizeof(mode));
                break;
