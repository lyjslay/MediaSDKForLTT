...
hi_s32 wlan_util_get_sdio_wifi_device(hi_void)
{
    ...
    while ((next = readdir(dir)) != NULL) {
         ...
         */
        results = snprintf_s(uevent_file, sizeof(uevent_file), sizeof(uevent_file) - 1,
        ...
        while (fgets(line, sizeof(line), fp)) {
            ...
            if (pos != NULL) {
                ...
                results = snprintf_s(device_id, sizeof(device_id), sizeof(device_id) - 1, "%04x:%04x",
...
hi_s32 wlan_util_get_wifi_device(hi_void)
{
    ...
    dbg_print(("No supported usb or pcie device found, set id to chosen HI3881\n"));
                               ...
                               hi_char *param, hi_char *bw_conf, hi_char *rate)
{
    ...
    ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd) - 1, "%s %s %s %s %s",
        spawn_args[0], spawn_args[1], spawn_args[2], spawn_args[3], spawn_args[4]); /* 2:module 3:bw 4:rate */
    if (ret == -1) {
        dbg_print(("WiFi: insmod cmd snprintf_s err!\n"));
    }
...
hi_s32 wlan_util_interface_exist(hi_wifi_mode_e mode)
{
    ...
    if (ret == HI_SUCCESS) {
        dbg_print(("WiFi: exist interface '%s'\n", ifname));
    }
    return ret;
}

hi_s32 wlan_util_get_interface(hi_wifi_mode_e mode, const hi_char *dev_file, hi_char *ifname, hi_u32 name_buf_size)
{
    ...
    if (mode == WIFI_MODE_P2P) {
        ...
        if (results != 0) {
            dbg_print(("WiFi: snprintf_s return %d\n", results));
        }
    ...
    if (realpath(dev_file, file) == NULL) {
        dbg_print(("WiFi: file path '%s' not exist!\n", file));
        return HI_FAILURE;
    }
    ...
    if (fh != NULL) {
        ...
        if (fgets(buff, sizeof(buff), fh) == NULL) {
            if (fclose(fh) != 0) {
                dbg_print(("WiFi: %s close fail in first fgets!\n", file));
            }
            fh = NULL;
            return ret;
        }
        if (fgets(buff, sizeof(buff), fh) == NULL) {
            if (fclose(fh) != 0) {
                dbg_print(("WiFi: %s close fail in second fgets!\n", file));
            }
            fh = NULL;
            return ret;
        }
        ...
        while (fgets(buff, sizeof(buff), fh)) {
            ...
            /* Copy */
            ret = memcpy_s(ifname, name_buf_size, begin, (end - begin));
            if (ret != 0) {
                dbg_print(("WiFi: memcpy_s return %d in '%s'\n", ret, __func__));
            }
            ifname[end - begin] = '\0';
            ret = HI_SUCCESS;
            break;
...
hi_s32 wlan_util_read_line(const hi_char *buf, hi_char *line, hi_u32 line_size)
{
    hi_s32 i = 0;
    hi_char *pos = line;
    ...
    while (*buf != '\0') {
        ...
        if ((hi_u32)(line - pos) < (line_size - 1)) {
            *line++ = *buf++;
        } else {
            break;
        }
