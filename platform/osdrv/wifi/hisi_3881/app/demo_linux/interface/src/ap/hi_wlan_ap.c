...
hi_s32 ap_update_firmware_path(hi_s32 device)
{
    ...
    if (fd < 0) {
        dbg_print(("WiFi: '/sys/module/bcmdhd/parameters/firmware_path' open fail in %s!\n", __func__));
        return -1;
    }
...
static hi_s32 ap_load_driver(hi_wlan_bandwith_e bw)
{
    ...
    if (g_device_id < 0) {
        dbg_print(("WiFi: Cann't find supported device\n"));
        return HI_WLAN_DEVICE_NOT_FOUND;
    }
    dbg_print(("WiFi: Find device %d\n", g_device_id));
    ...
    switch (g_device_id) {
        case WIFI_HISILICON_HI3881:
            ...
            if (results < EOK) {
                dbg_print(("WiFi: ap Narrow band config saprintf_s failure!\n"));
            }
            ...
            break;
        default:
            dbg_print(("WiFi: device %d is not supported, "
                      "cann't load driver\n", g_device_id));
    ...
    if (ret == HI_WLAN_LOAD_DRIVER_FAIL) {
        dbg_print(("WiFi: Load driver fail\n"));
    }
...
hi_s32 ap_config_with_iwpriv_cmd(hi_s32 s, const hi_char *ifname, const hi_wlan_ap_config_s *ap_cfg)
{
    ...
    if (results < EOK) {
        dbg_print(("WiFi: results=%d file=%s, func=%s, line=%d\n", results, __FILE__, __FUNCTION__, __LINE__));
    }
    ...
    if (cmd < SIOCDEVPRIVATE) {
        for (j = 0; j < i; j++) {
            if ((priv_ptr[j].set_args == priv_ptr[i].set_args) && (priv_ptr[j].get_args == priv_ptr[i].get_args) &&
                (priv_ptr[j].name[0] == '\0')) {
                break;
            }
        }
    ...
    /* configure SSID */
    if (snprintf_s(mbuf, sizeof(mbuf), sizeof(mbuf) - 1, "SSID=%s", ap_cfg->ssid) < 0) {
        dbg_print(("WiFi: file=%s, func=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__));
    }
    wrq.u.data.length = strlen(mbuf) + 1;
    if (ioctl(s, cmd, &wrq) < 0) {
        return HI_FAILURE;
...
static hi_s32 ap_start_hostapd(const hi_char *config_file, const hi_char *entropy_file)
{
    ...
    results = snprintf_s(param[0], sizeof(param[0]), sizeof(param[0]) - 1, "%s", config_file);
    if (results == -1) {
        dbg_print(("WiFi: results=%d file=%s,line=%d,func=%s\n", results, __FILE__, __LINE__, __FUNCTION__));
    }
    results = snprintf_s(param[1], sizeof(param[1]), sizeof(param[1]) - 1, "-e%s", entropy_file);
    if (results == -1) {
        dbg_print(("WiFi: results=%d file=%s,line=%d,func=%s\n", results, __FILE__, __LINE__, __FUNCTION__));
    }
...
static hi_s32 ap_kill_hostapd(hi_void)
{
    ...
    ret = snprintf_s(cmd, sizeof(cmd), sizeof(cmd) - 1, "%s %s", spawn_args[0], spawn_args[1]);
    if (ret < 0) {
        dbg_print(("WiFi: ret=%d file=%s, func=%s, line=%d\n", ret, __FILE__, __FUNCTION__, __LINE__));
    }
    dbg_print(("WiFi: system cmd = '%s'\n", cmd));
    ret = system(cmd);
    ...
    const hi_char *config_file)
{
    ...
    if (realpath(config_file, file_path) == NULL) {
        dbg_print(("WiFi: file path '%s' no exist and create it [%s]!\n", file_path, __func__));
    }
    ...
    if (ap_cfg->hidden_ssid == HI_TRUE) {
        dbg_print(("WiFi: Enable hidden SSID\n"));
    ...
    if (ap_cfg->security == HI_WLAN_SECURITY_WEP) {
        ...
        if (ap_cfg->hw_mode == 'n') {
            dbg_print(("WiFi: n mode does not support WEP encryption\n"));
            close(fd);
            return -1;
        }
        if (asprintf(&wbuf, "interface=%s\n"
            ...
            dbg_print(("WiFi: asprintf ap_cfg->security HI_WLAN_SECURITY_WEP failure\n"));
            close(fd);
            free(wbuf);
            return -1;
        }
        ...
        if (asprintf(&wbuf, "interface=%s\n"
            ...
            dbg_print(("WiFi: asprintf ap_cfg->security failure\n"));
            close(fd);
            free(wbuf);
            return -1;
        }
    ...
    if (ap_cfg->security == HI_WLAN_SECURITY_WEP) {
        ...
        if (pwd_len == WEP_PSSWD_LEN_5 || pwd_len == WEP_PSSWD_LEN_13) {
            if (asprintf(&fbuf, "%swep_default_key=0\n"
                         "wep_key0=\"%s\"\n", wbuf, ap_cfg->psswd) < 0) {
                dbg_print(("WiFi: asprintf HI_WLAN_SECURITY_WEP pwd_len failure\n"));
                close(fd);
                free(wbuf);
                free(fbuf);
                return -1;
            ...
            if (asprintf(&fbuf, "%swep_default_key=0\n"
                         "wep_key0=%s\n", wbuf, ap_cfg->psswd) < 0) {
                dbg_print(("WiFi: asprintf HI_WLAN_SECURITY_WEP pwd failure\n"));
                close(fd);
                free(wbuf);
                free(fbuf);
                return -1;
        ...
        if (asprintf(&fbuf, "%swpa=3\n"
                     ...
                     "wpa_passphrase=%s\n", wbuf, ap_cfg->psswd) < 0) {
            dbg_print(("WiFi: asprintf HI_WLAN_SECURITY_WPA_WPA2_PSK failure\n"));
            close(fd);
            free(wbuf);
            free(fbuf);
            return -1;
...
int ensure_entropy_file_exists(const hi_char *entropy_file)
{
    ...
    static unsigned char dummy_key[21] = { /* dummy_key arry len 21 */
        0x02, 0x11, 0xbe, 0x33, 0x43, 0x35, 0x68, 0x47, 0x84, 0x99, 0xa9, 0x2b,
        0x1c, 0xd3, 0xee, 0xff, 0xf1, 0xe2, 0xf3, 0xf4, 0xf5
    };

    if (realpath(entropy_file, file) != NULL) {
        dbg_print(("WiFi: entropy file already exists\n"));
        return HI_SUCCESS;
    }

    dbg_print(("WiFi: errno=%d errmsg=%s\n", errno, strerror(errno)));
    hi_s32 fd = open(file, O_CREAT | O_TRUNC | O_WRONLY, 0666);    /* file mode 0666 */
    if (fd < 0) {
        dbg_print(("WiFi: Cann't open entropy file '%s'\n", file));
        return HI_FAILURE;
    }
    if (asprintf(&fbuf, "%s", dummy_key) < 0) {
        dbg_print(("WiFi: asprintf dummy_key '%s' failure!\n", dummy_key));
        close(fd);
        free(fbuf);
        return HI_FAILURE;
    }
...
hi_s32 hi_wlan_ap_open(hi_char *ifname, hi_u32 name_buf_size, hi_wlan_bandwith_e bw)
{
    ...
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: ap unload driver fail!\n"));
    }
...
hi_s32 hi_wlan_ap_start(const hi_char *ifname, hi_wlan_ap_config_s *ap_cfg)
{
    ...
    if (-1 == ret) {
        dbg_print(("WiFi: update hostapd config file fail\n"));
        return ret;
    }

    ret = ensure_entropy_file_exists(g_entropy_file);
    if (-1 == ret) {
        dbg_print(("WiFi: creat entropy file fail\n"));
    }

    ret = ap_start_hostapd(g_hostapd_config_file, g_entropy_file);
    if (ret < 0) {
        dbg_print(("WiFi: start hostapd fail\n"));
        return HI_WLAN_START_HOSTAPD_FAIL;
    }

    dbg_print(("WiFi: SoftAP started\n"));
...
hi_s32 hi_wlan_ap_stop(const hi_char *ifname)
{
    ...
    if (ap_kill_hostapd() < 0) {
        dbg_print(("WiFi: Kill hostapd fail\n"));
        return HI_FAILURE;
    }
...
hi_s32 hi_wlan_ap_setsoftap(const hi_char *ifname, hi_wlan_ap_config_s *ap_cfg)
{
    ...
    dbg_print(("WiFi: Must stop SoftAP and close WiFi before setting it\n"));
    /* to set AP, we must restart it */
    ret = hi_wlan_ap_stop(ifname);
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: Stop SoftAP fail\n"));
        return HI_FAILURE;
    }

    ret = hi_wlan_ap_close(ifname);
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: Close WiFi fail\n"));
        return HI_FAILURE;
    }

    /* wait for driver deinitialization */
    sleep(1);
    dbg_print(("WiFi: Closed, then restart it\n"));
    ...
    if (ret != HI_SUCCESS) {
        dbg_print(("WiFi: Open SoftAP fail\n"));
        return HI_FAILURE;
    }
    if (strcmp(iface, ifname) != 0) {
        dbg_print(("WiFi: Fail, new interface is '%s', expect for '%s'\n",
                  ifname, iface));
        return HI_FAILURE;
    }
