...
hi_s32 wlan_hal_init(hi_void)
{
    ...
    if (pthread_mutex_init(&g_sta->ctrl_req_mut, NULL) != 0) {
        dbg_print(("WiFi: 'g_sta->ctrl_req_mut' pthread_mutex_init fail in %s!\n", __func__));
    }
    ...
    if (pthread_mutex_init(&g_p2p->ctrl_req_mut, NULL) != 0) {
        dbg_print(("WiFi: 'g_p2p->ctrl_req_mut' pthread_mutex_init fail in %s!\n", __func__));
    }
...
hi_void wlan_hal_deinit(hi_void)
{
    if (g_sta) {
        if (pthread_mutex_destroy(&g_sta->ctrl_req_mut) != 0) {
            dbg_print(("WiFi: 'g_sta->ctrl_req_mut' pthread_mutex_destroy fail in %s!\n", __func__));
        }
        free(g_sta);
        g_sta = NULL;
    }
    ...
    if (g_p2p) {
        if (pthread_mutex_destroy(&g_p2p->ctrl_req_mut) != 0) {
            dbg_print(("WiFi: 'g_p2p->ctrl_req_mut' pthread_mutex_destroy fail in %s!\n", __func__));
        }
        free(g_p2p);
        g_p2p = NULL;
    }
...
hi_s32 wlan_load_driver(const hi_wlan_sta_config_s *sta_cfg)
{
    ...
    switch (g_sta->device_id) {
        case WIFI_HISILICON_HI3881:
            if (sta_cfg->bw_sta_config.bw_enable) {
                /* configure BW Mode */
                results = sprintf_s(bw_cmd, sizeof(bw_cmd), "g_bw=%d", sta_cfg->bw_sta_config.bw_bandwidth);
                if (results < EOK) {
                    dbg_print(("WiFi: set Narrow band cmd sprintf_s failure!\n"));
                }
            } else {
                results = sprintf_s(bw_cmd, sizeof(bw_cmd), "g_bw=0");
                if (results < EOK) {
                    dbg_print(("WiFi: not set Narrow band cmd sprintf_s failure!\n"));
                }
            }
            results = sprintf_s(pro_cmd, sizeof(pro_cmd), "g_proto=%d", sta_cfg->hw_mode);
            if (results < EOK) {
                dbg_print(("WiFi: set Rate protocol mode sprintf_s failure!\n"));
            }
    ...
    const hi_char *driver, const hi_char *sta_config_file, const hi_char *p2p_config_file)
{
    ...
    if (environment_path == NULL) {
        dbg_print(("WiFi: getenv 'WPA_SUPPLICANT_DEBUG'no message!\n"));
        results = sprintf_s(param[num], sizeof(param[num]), "-B");
        if (results < EOK) {
            dbg_print(("WiFi: results=%d file=%s, func=%s, line=%d\n", results, __FILE__, __FUNCTION__, __LINE__));
        }
    } else {
        dbg_print(("WiFi: getenv WPA_SUPPLICANT_DEBUG=%s\n", environment_path));
...
hi_s32 wlan_wpa_request(const hi_char *cmd, hi_char *cbuf, hi_s32 *size)
{
    ...
    if (reply == NULL) {
        dbg_print(("WiFi: malloc reply mem fail in %s\n", __func__));
        return HI_FAILURE;
    }
    ...
    if (cbuf != NULL && size != NULL) {
        ...
        if (memcpy_s(cbuf, *size, reply, reply_len) != EOK) {
            free(reply);
            reply = NULL;
            return HI_FAILURE;
        }
...
hi_wpa_socket_s *wlan_wpa_open_connection(const hi_char *ifname, const hi_char *ctrl_iface_dir)
{
    ...
    ret = snprintf_s(cfile, flen, flen - 1, "%s/%s", ctrl_iface_dir, ifname);
    if (ret < 0 || ret >= flen) {
        free(cfile);
        return NULL;
    }
    ...
    ret = snprintf_s(mon_s->local.sun_path, sizeof(mon_s->local.sun_path), sizeof(mon_s->local.sun_path) - 1,
    ...
    if (ret == HI_FAILURE || strncmp(reply, "OK", 2) != 0) {    /* string OK len 2 */
        dbg_print(("WiFi: Attach to wpa_supplicant fail\n"));
        goto fail;
    }
...
hi_void wlan_wpa_close_connection(hi_s32 control, hi_wpa_socket_s *wpa_s)
{
    ...
    if (wpa_s) {
        if (wlan_wpa_send_command(wpa_s, "DETACH", NULL, NULL) == HI_FAILURE) {
            dbg_print(("WiFi: 'DETACH' wlan_wpa_send_command fail in %s!\n", __func__));
        }
