...
if [ "$BUILD_SELECT" == "linux_driver" ];then
	...
	make -f Makefile all 2>&1 |tee $CROOT/build/build_tmp/logs/build_linux.log; \
        ...
        if [ "$CFG_SDIO_FLAT_MEM" == "y" ];then
                sed -i 's/CFG_SDIO_FLAT_MEM = n/CFG_SDIO_FLAT_MEM = y/g' ${WIFI_DRIVER_DIR}/env_config.mk;
        fi
	...
	sed -i 's/CFG_LITEOS = y/CFG_LITEOS = n/g' ${WIFI_DRIVER_DIR}/env_config.mk; \
        ...
        if [ "$CFG_SDIO_FLAT_MEM" == "y" ];then
                sed -i 's/CFG_SDIO_FLAT_MEM = y/CFG_SDIO_FLAT_MEM = n/g' ${WIFI_DRIVER_DIR}/env_config.mk;
        fi
