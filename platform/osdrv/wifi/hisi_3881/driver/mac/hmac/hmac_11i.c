...
hi_u8 hmac_check_pcip_wpa_policy(const wlan_mib_ieee802dot11_stru *mib_info, hi_u8 policy)
{
    ...
    for (loop = 0; loop < MAC_PAIRWISE_CIPHER_SUITES_NUM; loop++) {
        /* 检测单播密钥是否使能和匹配 */
        if ((mib_info->ast_wlan_mib_rsna_cfg_wpa_pairwise_cipher[loop].
            dot11_rsna_config_pairwise_cipher_activated == HI_TRUE) &&
            (mib_info->ast_wlan_mib_rsna_cfg_wpa_pairwise_cipher[loop].
            dot11_rsna_config_pairwise_cipher_implemented == policy)) {
            return HI_TRUE;
        }
    }
    return HI_FALSE;
