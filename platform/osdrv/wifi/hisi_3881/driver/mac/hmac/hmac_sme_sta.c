...
hi_void hmac_handle_asoc_rsp_sta(hmac_vap_stru *hmac_vap, const hi_u8 *msg)
{
    ...
    oal_wireless_dev *wdev = oal_netdevice_wdev(net_device);
    hi_u8 *cur_bssid = wal_get_cur_connect_bssid();
    if (asoc_rsp->result_code == HMAC_MGMT_SUCCESS && wdev->ssid_len != 0 &&
    ...
    if (asoc_rsp->result_code == HMAC_MGMT_SUCCESS) {
        ...
        max_reassoc_count = (asoc_rsp->status_code == MAC_REJECT_TEMP) ? 1 : MAX_ASOC_CNT;
#endif
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
        if (hmac_vap->asoc_cnt >= max_reassoc_count || wdev->ssid_len == 0 ||
            oal_compare_mac_addr(asoc_rsp->auc_addr_ap, cur_bssid, WLAN_MAC_ADDR_LEN)) {
#else
        if (hmac_vap->asoc_cnt >= max_reassoc_count) {
            ...
            if ((asoc_rsp->result_code == HMAC_MGMT_SUCCESS) && (wdev->ssid_len == 0 ||
                oal_compare_mac_addr(asoc_rsp->auc_addr_ap, cur_bssid, WLAN_MAC_ADDR_LEN))) {
                oam_warning_log0(hmac_vap->base_vap->vap_id, OAM_SF_ASSOC,
                    "{hmac_handle_asoc_rsp_sta::different with connect ssid.}");
