                                   ...
                                   hi_u16 us_disasoc_reason_code, hi_u16 us_dmac_reason_code)
{
    ...
    /* 去关联的STA mac地址 */
    if (mac_addr != HI_NULL) {
        if (memcpy_s((hi_u8 *)data, sizeof(data), mac_addr, mac_addr_len) != EOK) {
    ...
    hi_u8 *puc_mac_hdr, hi_u8 *puc_payload, hi_u32 payload_len)
{
    ...
    hmac_ap_asoc_info_stru      asoc_info = {
        .sta_mac_addr = sta_mac_addr,
        .status_code = &status_code
    };
    hmac_ap_asoc_req_stru       asoc_req = {
        .hmac_vap = hmac_vap,
        .mgmt_frm_type = mgmt_frm_type,
        .puc_mac_hdr = puc_mac_hdr,
        .puc_payload = puc_payload,
