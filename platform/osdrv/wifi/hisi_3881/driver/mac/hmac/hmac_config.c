...
/*****************************************************************************
 功能描述  : hmac_config_normal_check_vap_num
 修改历史      :
  1.日    期   : 2014年5月13日
    作    者   : Hisilicon
    修改内容   : 新生成函数
...
hi_u32 hmac_config_def_chan(mac_vap_stru *mac_vap)
{
    ...
    if (((mac_vap->channel.band == WLAN_BAND_BUTT) ||
        ...
        (mac_vap->protocol == WLAN_PROTOCOL_BUTT)) && (!is_p2p_go(mac_vap))) {
...
#if defined (_PRE_WLAN_FEATURE_P2P) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
...
/*****************************************************************************
 功能描述  : hmac_config_set_monitor_switch
 修改历史      :
  1.日    期   : 2016年3月14日
    作    者   : Hisilicon
    修改内容   : 新生成函数
...
/*****************************************************************************
 功能描述  : hmac_config_set_forty_mhz_intolerant
 修改历史      :
  1.日    期   : 2014年6月5日
    作    者   : Hisilicon
    修改内容   : 新生成函数
...
oal_net_dev_tx_enum hmac_bridge_vap_xmit(oal_netbuf_stru *netbuf, oal_net_device_stru *netdev)
{
    ...
    if (oal_unlikely(!((mac_vap->vap_state == MAC_VAP_STATE_UP) || (mac_vap->vap_state == MAC_VAP_STATE_PAUSE)))) {
        oam_info_log1(0, 0, "{hmac_bridge_vap_xmit::vap state[%d] != MAC_VAP_STATE_{UP|PAUSE}}", mac_vap->vap_state);
        oal_netbuf_free(netbuf);
        return OAL_NETDEV_TX_OK;
    }

    if (oal_netbuf_headroom(netbuf) < NET_SKB_PAD) {
        if (oal_netbuf_expand_head(netbuf, (hi_s32)(NET_SKB_PAD - oal_netbuf_headroom(netbuf)), 0, GFP_ATOMIC)
            != HI_SUCCESS) {
            oam_error_log0(0, 0, "{hmac_bridge_vap_xmit:: oal_netbuf_expand_head failed.}");
            oal_netbuf_free(netbuf);
            return OAL_NETDEV_TX_OK;
        }
    }
