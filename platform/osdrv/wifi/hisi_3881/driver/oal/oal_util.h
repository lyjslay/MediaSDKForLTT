...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
#define oal_strtok    strtok_r
#elif (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
static inline hi_char *oal_strtok(hi_char *pc_token, const hi_char *pc_delemit, hi_char **pc_saveptr)
{
...
{
    if (cp[0] == '0') {
        if (util_tolower(cp[1]) == 'x' && _isxdigit(cp[2])) {
            return 16;  /* 16 hex */
        } else {
            return 8;   /* 8 oct */
        }
...
static inline unsigned long long oal_simple_strtoull(const hi_char *cp, hi_char **endp, hi_u32 base)
{
    ...
    if (base == 16 && cp[0] == '0' && util_tolower(cp[1]) == 'x') {          /* base equal 16 */
        cp += 2;    /* add 2 */
    }
    ...
    while (_isxdigit(*cp)) {
        ...
        value = isdigit(*cp) ? *cp - '0' : util_tolower(*cp) - 'a' + 10;     /* add 10 */
        if (value >= base) {
            break;
        }
