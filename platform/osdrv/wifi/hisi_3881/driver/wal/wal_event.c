...
static hi_u8                        g_query_flag = HI_FALSE;
/*****************************************************************************
  3 函数实现
*****************************************************************************/
/*****************************************************************************
...
hi_u32 wal_any_wait_query_result(hmac_vap_stru *hmac_vap, hi_wifi_any_peer_info *peer)
{
    ...
    if (g_query_flag == HI_FALSE) {
        ...
        ret = (hi_u32)hi_wait_event_timeout(hmac_vap->query_wait_q,
            (HI_TRUE == g_query_flag), (5 * HZ)); /* 5 频率,非wifi目录定义宏函数,误报告警,lin_t e26告警屏蔽 */
        if (ret == 0) {
                          ...
                          const hi_wifi_csi_entry *entry_list, hi_s32 entry_num)
{
    ...
    if (netdev == HI_NULL) {
        oam_warning_buf(0, OAM_SF_CSI, "{wal_csi_set_config::ifname [%s] len [%zu]is not found.}",
