...
/* support sta and ap concurrent */
static oal_ieee80211_iface_limit sta_ap_limits[] = {
    {
        .max = 1,
...
g_sta_ap_iface_combinations[] = {
    {
        .limits = sta_ap_limits,
        .n_limits = hi_array_size(sta_ap_limits),
...
static oal_ieee80211_iface_limit sta_p2p_limits[] = {
    {
        .max = 2,
        .types = bit(NL80211_IFTYPE_STATION),
    },
...
g_sta_p2p_iface_combinations[] = {
    {
        ...
        .n_limits = hi_array_size(sta_p2p_limits),
    },
};
    ...
    dmac_query_request_event *query_request, oal_station_info_stru *sta_info)
{
    ...
    hi_s32 i_leftime = hi_wait_event_interruptible_timeout(hmac_vap->query_wait_q,
        (hmac_vap->query_wait_q_flag == HI_TRUE), QUERY_STATION_INFO_TIME);
    if (i_leftime == 0) {
        /* 超时还没有上报扫描结束 */
...
/*****************************************************************************
 ...
 功能描述  : wal_cfg80211_start_sched_scan
 输入参数  : 无
 输出参数  : 无
 返 回 值  : hi_u32
...
#endif
{
    ...
    /* 通过net_device 找到对应的mac_device_stru 结构 */
    hmac_device_stru *hmac_dev = (hmac_device_stru *)hmac_get_device_stru();
    hmac_scan_stru *scan_mgmt = &(hmac_dev->scan_mgmt);
    ...
    if ((scan_mgmt->sched_scan_req != HI_NULL) && (scan_mgmt->sched_scan_complete != HI_TRUE)) {
        ...
        if (scan_mgmt->request == HI_NULL) {
            mac_device_stru *mac_device = mac_res_get_dev();
        ...
        hi_u32 ret = wal_send_cfg_event(netdev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(pedding_data),
            (hi_u8 *)&write_msg, HI_FALSE, HI_NULL);
        if (ret != HI_SUCCESS) {
...
#ifdef _PRE_WLAN_FEATURE_P2P
static hi_s32 wal_cfg80211_update_vap(oal_net_device_stru *net_dev, enum_nl80211_iftype type,
    wlan_p2p_mode_enum_uint8 p2p_mode, wlan_vap_mode_enum_uint8 vap_mode)
{
    ...
    */
    ...
    /* 删除VAP */
    mac_cfg_del_vap_param_stru del_vap_param;
    (hi_void)memset_s(&del_vap_param, sizeof(del_vap_param), 0, sizeof(del_vap_param));
    ...
    (hi_void)memset_s(&add_vap_param, sizeof(add_vap_param), 0, sizeof(add_vap_param));
...
#ifdef _PRE_WLAN_FEATURE_P2P
    if (wal_cfg80211_update_vap(net_dev, type, p2p_mode, vap_mode) != HI_SUCCESS) {
        return -HI_FAIL;
    }
#endif
...
#else
hi_u32 wal_cfg80211_del_send_event(oal_net_device_stru *netdev, hi_u8 *mac_addr, mac_vap_stru *mac_vap)
#endif
#endif
{
...
#else
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(3, 15, 0))
    wiphy->iface_combinations   = g_sta_ap_iface_combinations;
    wiphy->n_iface_combinations = hi_array_size(g_sta_ap_iface_combinations);
#endif
#endif
#ifdef _PRE_WLAN_FEATURE_P2P
