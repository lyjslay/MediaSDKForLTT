...
#ifdef _PRE_LINUX_BUILTIN
static ssize_t fw_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    if (buf == HI_NULL) {
        return -HI_FAIL;
    }
...
static ssize_t mfg_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    if (buf == HI_NULL) {
        return -HI_FAIL;
    }
