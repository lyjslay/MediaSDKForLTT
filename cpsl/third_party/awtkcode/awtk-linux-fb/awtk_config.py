...
if TOUCHPAD == 'y':
    ...
    TOUCHPAD_INCLUDE =  joinPath(SDK_ROOT, 'cpsl/hal/touchpad/include')
    print(TOUCHPAD_INCLUDE)

if RISCV == 'y':
    print("RISCV=======================================")
    print(SDK_ROOT)
    COMMON_CCFLAGS = COMMON_CCFLAGS + ' -DCONFIG_RISCV_ON '
    CSI2D_PATH = 'cpsl/third_party/csi2d/'
    CSI2D_INCLUDE =  joinPath(SDK_ROOT, CSI2D_PATH + 'include')
    CSI2D_LIBPATH = [joinPath(SDK_ROOT, CSI2D_PATH + CVILIB_DIR)]
    print(CSI2D_INCLUDE)
    print(CSI2D_LIBPATH)
else:
