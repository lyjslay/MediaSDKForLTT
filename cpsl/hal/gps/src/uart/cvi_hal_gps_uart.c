...
static int32_t GpsDeviceInit(void)
{
    ...
    ret = CVI_UART_Init(HAL_GPS_UART_DEVICE);
    if(0 > ret)
    {
    ...
    ret = CVI_UART_Set_Param(B9600, 0, 8, 1, 'n');
    if(0 != ret)
    {
...
static int32_t GpsGetRawData(CVI_GPSDATA_S *gpsData, int32_t timeout_ms)
{
    int32_t ret = 0;
    ...
    if (gpsData->wantReadLen > CVI_HAL_GPS_DATA_SIZE)
    {
    ...
    ret = CVI_UART_Receive(gpsData->rawData, gpsData->wantReadLen, timeout_ms);
    if ( ret > 0 ) {
