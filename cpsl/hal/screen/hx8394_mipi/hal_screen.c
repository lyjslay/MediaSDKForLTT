...
static CVI_HAL_SCREEN_STATE_E g_halScreenHx8394Ctx = CVI_HAL_SCREEN_STATE_ON;
...
CVI_HAL_SCREEN_PWM_S HAL_SCREEN_HX8394_GetLuma(void)
{
...
static int32_t HAL_SCREEN_HX8394_SetLuma(CVI_HAL_SCREEN_PWM_S pwmAttr)
{
	#if CONFIG_PWM_ON
...
static int32_t HAL_SCREEN_HX8394_SetBackLightState(CVI_HAL_SCREEN_STATE_E enBackLightState)
{
	...
	if (enBackLightState == CVI_HAL_SCREEN_STATE_ON) {
		...
		g_halScreenHx8394Ctx = CVI_HAL_SCREEN_STATE_ON;
		#endif
	} else {
		#if CONFIG_PWM_ON
		...
		g_halScreenHx8394Ctx = CVI_HAL_SCREEN_STATE_OFF;
		#endif
	}
    return 0;
}
