...
int32_t MIPIDSI_SetDeviceConfig(const struct combo_dev_cfg_s *devCfg)
{
    ...
    if (ret != 0) {
        printf("CVI_MIPI_TX_INIT failed with %#x\n", ret);
        return ret;
    }
    ret = CVI_MIPI_TX_Disable(g_MipiDsiFd);
    if (ret != 0) {
        printf("CVI_MIPI_TX_Disable failed with %#x\n", ret);
        return ret;
    }

    ret = CVI_MIPI_TX_Cfg(g_MipiDsiFd, &comboDevCfg);
    if (ret != 0) {
        printf("CVI_MIPI_TX_Cfg failed with %#x\n", ret);
        return ret;
    }
#endif
    return ret;
...
int32_t MIPIDSI_EnableVideoDataTransport(void)
{
    ...
    if (ret != 0) {
        printf("CVI_MIPI_TX_Enable failed\n");
        return -1;
    }
#endif
    return ret;
...
int32_t MIPIDSI_DisableVideoDataTransport(void)
{
	...
	ret = CVI_MIPI_TX_Disable(g_MipiDsiFd);
    if (ret != 0) {
        printf("CVI_MIPI_TX_Disable failed\n");
        return -1;
    }
#endif
    return ret;
...
int32_t MIPIDSI_SetHsSettle(const struct hs_settle_s *hs_timing_cfg)
{
    ...
    if (ret != 0) {
        printf("CVI_MIPI_TX_SetHsSettle failed with %#x\n", ret);
        return ret;
    }
#endif
	return ret;
