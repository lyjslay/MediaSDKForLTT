...
static CVI_HAL_SCREEN_STATE_E s_halScreenState = CVI_HAL_SCREEN_STATE_ON;
...
// #if READ_PANEL_ID_DBUG
// 	uint8_t data[128] = {0};
// 	for (uint32_t i = 0; i < ARRAY_SIZE(dsi_init_cmds); i++) {
// 		const struct dsc_instr *instr1 = &(dsi_init_cmds[i]);
// 		struct get_cmd_info_s get_cmd_info = { .devno = devno, .get_data_size = 1
// 			, .data_type = 0x6,  .data_param = instr1->data[0]};
// 		get_cmd_info.get_data = data;

// 		ret = MIPIDSI_ReadCmd(&get_cmd_info);
// 			if (ret) {
// 			printf("dsi read failed instr.\n");
// 			return ret;
// 			}

// 		printf("cmd = %d\t", i);
// 		printf("%x\t", get_cmd_info.get_data[0]);
// 		printf("\n");
// 	}
// #endif
...
static int32_t HAL_SCREEN_Init(void)
{
	...
	printf("[LY][ka26s4618]hactive= %d,vactive=%d,\n",dev_cfg.sync_info.vid_hline_pixels,dev_cfg.sync_info.vid_active_lines);
	...
	s_halScreenState = CVI_HAL_SCREEN_STATE_ON;
	return 0;
}
...
CVI_HAL_SCREEN_PWM_S HAL_SCREEN_GetLuma(void)
{
...
static int32_t HAL_SCREEN_SetLuma(CVI_HAL_SCREEN_PWM_S pwmAttr)
{
	#if CONFIG_PWM_ON
...
{
	printf("HAL_SCREEN_SetContrast ka26s4618 = %d\n", u32Contrast);
	return 0;
}

static int32_t HAL_SCREEN_SetBackLightState(CVI_HAL_SCREEN_STATE_E enBackLightState)
{
	printf("HAL_SCREEN_SetBackLightState ka26s4618 = %d\n", enBackLightState);
	//ctrl backlight pwm
	int32_t ret = 0;
	if (enBackLightState == CVI_HAL_SCREEN_STATE_ON) {
		...
		s_halScreenState = CVI_HAL_SCREEN_STATE_OFF;
		#endif
	}

	//ctrl backlight gpio
	//CVI_GPIO_Set_Value(screen_gpio[GPIO_TYPE_BACKLIGHT].gpio, CVI_GPIO_VALUE_H);
...
static int32_t HAL_SCREEN_GetBackLightState(CVI_HAL_SCREEN_STATE_E* penBackLightState)
{
	*penBackLightState = s_halScreenState;
	printf("HAL_SCREEN_GetBackLightState ka26s4618 = %d\n", *penBackLightState);
	return 0;
}
