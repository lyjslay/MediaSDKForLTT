...
/** \addtogroup 	SCREEN */
/** @{ */  /** <!-- [SCREEN] */
static CVI_HAL_SCREEN_STATE_E g_halScreenCtx = CVI_HAL_SCREEN_STATE_ON;
...
static int32_t HAL_SCREEN_Init(void)
{
	...
	for (i = GPIO_TYPE_RESET; i < GPIO_TYPE_BUTT; i++) {
		if (screen_gpio[i].gpio != 0) {
			CVI_GPIO_Export(screen_gpio[i].gpio);
			CVI_GPIO_Direction_Output(screen_gpio[i].gpio);
		}
	}
...
static int32_t HAL_SCREEN_SetBackLightState(CVI_HAL_SCREEN_STATE_E enBackLightState)
{
	...
	CVI_GPIO_Set_Value(screen_gpio[GPIO_TYPE_POWER].gpio, CVI_GPIO_VALUE_H);
