...
{
    CVI_GPIO_Set_Value(CVI_GPIOA_14, CVI_GPIO_VALUE_H);
...
int32_t CVI_WIFI_HAL_LoadAPDriver(void)
{
    int32_t s32Ret = 0;

    s32Ret = cvi_insmod("/mnt/system/ko/3rd/aic8800_bsp.ko","aic_fw_path=/mnt/system/bin/aic8800");
    //s32Ret |= cvi_insmod("/mnt/system/ko/aic8800_btlpm.ko",NULL);
    s32Ret |= cvi_insmod("/mnt/system/ko/3rd/aic8800_fdrv.ko", NULL);
    if(0 != s32Ret)
    {
...
int32_t CVI_WIFI_HAL_LoadSTADriver(void)
{
    printf("insmod sta mode driver not support,failed\n");
    return -1;
}

int32_t CVI_WIFI_HAL_RemoveDriver(void)
{
    int32_t s32Ret = 0;
    s32Ret |= cvi_rmmod("/mnt/system/ko/3rd/aic8800_fdrv.ko");
    s32Ret |= cvi_rmmod("/mnt/system/ko/3rd/aic8800_bsp.ko");
    if(0 != s32Ret)
    {
...
int32_t CVI_WIFI_HAL_UpdateApConfig(char *ifname,  const CVI_HAL_WIFI_APMODE_CFG_S *pstApCfg, const char *pszConfigFile)
{
    int32_t s32Ret = 0;
    int32_t s32fd;
    char* szWbuf = NULL;
