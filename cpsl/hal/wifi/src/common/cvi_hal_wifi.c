...
static int32_t HAL_WIFI_ApValidChck(const CVI_HAL_WIFI_APMODE_CFG_S* pstApCfg)
{
    /** cahnnel check */
...
static int32_t HAL_WIFI_StaValidChck(const CVI_HAL_WIFI_STAMODE_CFG_S* pstStaCfg)
{
    printf("[Errot]wifi STA mode parm validcheck not support %d", pstStaCfg->enStaMode);
    return -1;
}

static int32_t HAL_WIFI_InParmValidChck(const CVI_HAL_WIFI_CFG_S* pstCfg)
{
    int32_t s32Ret = 0;
