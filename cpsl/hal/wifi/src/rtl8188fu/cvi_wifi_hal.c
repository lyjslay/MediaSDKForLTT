...
void CVI_WIFI_HAL_SysPreInit(void)
{
    ...
    cvi_system("echo host > /proc/cviusb/otg_role");
    CVI_GPIO_Set_Value(CVI_GPIOE_18,  CVI_GPIO_VALUE_L);
    cvi_usleep(10 * 1000);
    CVI_GPIO_Set_Value(CVI_GPIOE_18,  CVI_GPIO_VALUE_H);
    if(true == bPinOutInit)
    {
...
int32_t CVI_WIFI_HAL_LoadAPDriver(void)
{
    ...
    s32Ret = cvi_insmod("/mnt/system/ko/3rd/8188fu.ko", NULL);
    if(0 != s32Ret)
    {
...
int32_t CVI_WIFI_HAL_RemoveDriver(void)
{
    ...
    s32Ret = cvi_rmmod("/mnt/system/ko/3rd/8188fu.ko");
    if(0 != s32Ret)
    {
        printf("insmod ap: failed, errno(%d)\n", errno);
        return -1;
