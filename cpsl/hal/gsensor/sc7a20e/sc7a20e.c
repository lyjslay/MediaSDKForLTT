...
int32_t gsensor_set_sensitity(CVI_HAL_GSENSOR_SENSITITY_E enSensitity)
{
	int32_t res = 0;
	unsigned char thrd = 0;
	switch (enSensitity) {
	...
	*/
	res |= sc7a20e_register_write(0x32, thrd);
...
/*return value: 0: is ok    other: is failed*/
int32_t gsensor_read_data(int32_t *x, int32_t *y, int32_t *z)
{
...
int32_t gsensor_read_int_status(unsigned char *flag)
{
	unsigned char data = 0;
	int32_t ret = 0;
	sc7a20e_register_read(0x31, &data);
	if((data & 0x40) == 0x40){
		*flag = 1;
	} else {
		*flag = 0;
