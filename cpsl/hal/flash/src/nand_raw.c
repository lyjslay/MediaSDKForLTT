...
CVI_S32 nand_raw_init(CVI_VOID)
{
	...
	if (flash_partition_info_init(CVI_FLASH_TYPE_NAND_0) != 0) {
	...
	for (ix = 0; ix < max_partition; ix++) {
		...
		printf("nandraw_ctrl->partition->start = %llx\n", ptn->start);
		printf("nandraw_ctrl->partition->end = %llx\n", ptn->end);
#endif
					 ...
					 CVI_S32 skip_badblock)
{
	...
	for (ix = 0; ix < nandraw_ctrl->num_partition && length; ix++) {
		...
		while ((ptn->start <= offset) && (offset < ptn->end) && length && (ptn->perm & ACCESS_RD) &&
			   ptn->fd != INVALID_FD) {
			//printf("dev: \"%s\", from: %llx, length: %llx\n",
			...
			// printf(">ptn->fd=%d, length=%llx, *startaddr=%llx, offset=%llx\n", ptn->fd, length,
			// 	*startaddr, offset);
		...
		while ((ptn->start <= offset) && (offset < ptn->end) && length && (ptn->perm & ACCESS_WR) &&
			...
			if (!rel) {
				...
				if (ioctl(ptn->fd, MEMERASE64, &eraseinfo)) {
					printf("Erase %llx failed!\n", offset);
					rel = nand_mark_badblock(offset, (CVI_U64)nandraw_ctrl->blocksize);
					if (rel != 0) {
						printf("\nMTD block_markbad at 0x%llx failed: %d, aborting\n",
								 offset, rel);
						return CVI_FAILURE;
					}
				}
		...
		while ((ptn->start <= offset) && (offset < ptn->end) && length && ptn->fd != INVALID_FD) {
		   ...
		   printf("dev: \"%s\", from: %llx, length: %llx\n",
			...
			if (ioctl(ptn->fd, MEMSETBADBLOCK, &blockoffset)) {
				printf("Mark bad block %llx failed!\n", offset);
			}
...
CVI_S32 nand_show_badblock(CVI_U64 offset, CVI_U64 length)
{
	...
	for (ix = 0; ix < nandraw_ctrl->num_partition && length; ix++) {
		...
		while ((ptn->start <= offset) && (offset < ptn->end) && length && ptn->fd != INVALID_FD) {
			...
			if (badblock == 1) {
				printf("Bad block at address: 0x%lX of \"%s\", absolute address: %llx\n",
								  ...
								  CVI_S32 blocksize)
{
	...
	do {
		...
		if (badblock == 1) {
			printf("Skip bad block at address: %llx of \"%s\", absolute address: %llx\n",
