	...
	if ((g_eDevStat[CVI_FLASH_TYPE_SPI_0] == CVI_FLASH_STAT_INSTALL)
		...
		if (fp) {
			...
			for (i = 0; fgets(line, sizeof(line), fp) != 0; i++) {
				...
				gPartInfo[i].FlashType = CVI_FLASH_TYPE_SPI_0;
			#endif
...
#ifdef FLASH_DEBUG
				printf("\n");
				printf("gPartInfo[%d].PartSize = %llx\n", i, gPartInfo[i].PartSize);
				...
				printf("gPartInfo[%d].StartAddr = %llx\n",  i, gPartInfo[i].StartAddr);
		...
		if (g_eDevStat[CVI_FLASH_TYPE_NAND_0] == CVI_FLASH_STAT_INSTALL) {
			...
			} else if (g_eDevStat[CVI_FLASH_TYPE_SPI_0] == CVI_FLASH_STAT_INSTALL) {
				g_stFlashOpt[CVI_FLASH_TYPE_SPI_0].raw_erase = spi_raw_erase;
				g_stFlashOpt[CVI_FLASH_TYPE_SPI_0].raw_read = spi_raw_read;
				g_stFlashOpt[CVI_FLASH_TYPE_SPI_0].raw_write = spi_raw_write;
			}
		if (fclose(fp)) {
			return CVI_FAILURE;
		}
...
#elif STORAGE_TYPE_NOR
        if (spi_raw_init() == CVI_SUCCESS) {
			g_eDevStat[CVI_FLASH_TYPE_SPI_0] = CVI_FLASH_STAT_INSTALL;
		}
#else
	if (0) {
...
//		emmc_raw_get_info(&TotalSize, &PageSize, &BlockSize, &OobSize, &BlockShift);
	...
	if ((CVI_S32)(u64Address % BlockSize) || (CVI_S32)(u64Len % BlockSize)) {
		printf("Open Address(%llx) and Len(%llx) should be align with BlockSize(0x%X)!\n",
					 u64Address, u64Len, BlockSize);
		return (CVI_HANDLE)INVALID_FD;
	}

	if ((u64Address >= TotalSize) || (u64Address + u64Len) > TotalSize) {
		printf("Open Address(%llx) and Len(%llx) should be smaller than TotalSize(%llx)!\n",
											  ...
											  CVI_CHAR *pPartitionName)
{
	...
	printf("hpart = %d\n", hPart);
		...
		if ((fd < 0) || (fd >= SPAN_PART_HANDLE)) {
			printf("Open %s flash partition failure(fd = %lld)!\n", DevName, (CVI_S64)fd);
			//(CVI_VOID)pthread_mutex_unlock(&gFlashMutex);
			if (fd >= 0) {
				close(fd);
			}
		...
		CVI_U64 u64Address, CVI_U64 u64Len)
{
	...
	// printf("FlashType=%d, PartitionName=%s, Address=%llx,
...
CVI_S32 CVI_Flash_Erase(CVI_HANDLE hFlash, CVI_U64 u64Address, CVI_U64 u64Len)
{
	...
	if (u64Len >= 0x80000000) {
		printf("u64Len is too big(%llx), should use CVI_Flash_Erase64() instead.\n", u64Len);
		return CVI_FAILURE;
	}
	...
	if (gFlashInfo[hFlash].fd >= (CVI_VOID *)SPAN_PART_HANDLE) {
		...
		printf("StartAddr = %llx, LimitLeng =%llx\n", StartAddr, LimitLeng);
	} else {
...
#ifdef FLASH_DEBUG
	printf("gFlashInfo[hFlash].OpenAddr = %llx\n", gFlashInfo[hFlash].OpenAddr);
#endif
	...
	printf("HANDLE=%d, Address=%llx, Len=%llx\n", hFlash, StartAddr, u64Len);
...
CVI_S32 CVI_Flash_Read(CVI_HANDLE hFlash, CVI_U64 u64Address, CVI_U8 *pBuf, CVI_U32 u32Len, CVI_U32 u32Flags)
{
	...
	printf("HANDLE=%d, StartAddr Address=%llx, Len=0x%x, Flag=%d\n", hFlash, StartAddr, u32Len, u32Flags);
...
#ifdef FLASH_DEBUG
	printf("HANDLE=%d, Address=%llx, Len=0x%x, Flag=%d\n", hFlash, StartAddr, u32Len, u32Flags);
#endif
...
CVI_S32 CVI_Flash_GetInfo(CVI_HANDLE hFlash, struct CVI_Flash_InterInfo_S *pFlashInfo)
{
...
#ifdef FLASH_DEBUG
	printf("pFlashInfo->TotalSize = %llx\n", pFlashInfo->TotalSize);
	printf("pFlashInfo->PartSize = %llx\n", pFlashInfo->PartSize);
	...
	printf("pFlashInfo->OpenAddr = %llx\n", pFlashInfo->OpenAddr);
	printf("pFlashInfo->OpenLeng = %llx\n", pFlashInfo->OpenLeng);
#endif
	return CVI_SUCCESS;
}
...
CVI_S32 CVI_Flash_GetStartAddr(CVI_S32 ix, CVI_U64 *startaddr, CVI_U64 *endaddr)
{
	...
	flash_partition_info_init(enFlashType);
	pstPartInfo = get_flash_partition_info(enFlashType, devname);
