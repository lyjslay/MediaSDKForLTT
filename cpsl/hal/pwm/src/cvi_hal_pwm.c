...
#define MAX_BUF 64
#define PWM_KO_PATH CVI_KOMOD_PATH "/" CHIP_TYPE  "_pwm.ko"
#define SYSFS_PWM_DIR "/sys/class/pwm/pwmchip"
static bool pwm_init = false;
static CVI_HAL_PWM_S s_PwmAttr = {0};
#else
#include <drv/pwm.h>
#endif
...
static int32_t CVI_PWM_Export(int32_t grp, int32_t chn)
{
	#ifndef CONFIG_KERNEL_RHINO
	int32_t fd;
	...
	snprintf(buf, sizeof(buf), SYSFS_PWM_DIR"%d/pwm%d", grp*4, chn);

	if ((access(buf, F_OK)) == -1) {
		snprintf(buf1, sizeof(buf1), SYSFS_PWM_DIR"%d/export", grp*4);
...
static int32_t CVI_PWM_UnExport(int32_t grp, int32_t chn)
{
	#ifndef CONFIG_KERNEL_RHINO
	int32_t fd;
	char buf[MAX_BUF]={0}, buf1[MAX_BUF]={0};
	...
	if ((access(buf, F_OK)) != -1) {
		snprintf(buf1, sizeof(buf1), SYSFS_PWM_DIR"%d/unexport", grp*4);
		fd = open(buf1, O_WRONLY);
		if (fd < 0) {
...
static int32_t CVI_PWM_Enable(int32_t grp, int32_t chn)
{
	#ifndef CONFIG_KERNEL_RHINO
	int32_t fd;
	char buf[MAX_BUF]={0};
	...
	snprintf(buf, sizeof(buf), SYSFS_PWM_DIR"%d/pwm%d/enable", grp*4, chn);
...
static int32_t CVI_PWM_Disable(int32_t grp, int32_t chn)
{
	#ifndef CONFIG_KERNEL_RHINO
	int32_t fd;
	char buf[MAX_BUF]={0};
...
static int32_t CVI_PWM_Insmod(void)
{
	#ifndef CONFIG_KERNEL_RHINO
...
static int32_t CVI_PWM_Rmmod(void)
{
	#ifndef CONFIG_KERNEL_RHINO
...
int32_t CVI_PWM_Set_Param(CVI_HAL_PWM_S pwmAttr)
{
	...
	snprintf(buf, sizeof(buf), SYSFS_PWM_DIR"%d/pwm%d/period", pwmAttr.group*4, pwmAttr.channel);
	fd = open(buf, O_WRONLY);
	if (fd < 0) {
		printf("open period error\n");
		return -1;
	...
	snprintf(buf, sizeof(buf), SYSFS_PWM_DIR"%d/pwm%d/duty_cycle", pwmAttr.group*4, pwmAttr.channel);
	fd = open(buf, O_WRONLY);
	if (fd < 0) {
...
int32_t CVI_PWM_Init(CVI_HAL_PWM_S pwmAttr)
{
	...
	if(pwm_init == false) {
		s_PwmAttr = pwmAttr;
		ret |= CVI_PWM_Insmod();
		ret |= CVI_PWM_Export(s_PwmAttr.group, s_PwmAttr.channel);
		//ret |= CVI_PWM_Get_Param(&s_PwmAttr.period, &s_PwmAttr.duty_cycle);
		ret |= CVI_PWM_Set_Param(s_PwmAttr);
		ret |= CVI_PWM_Enable(s_PwmAttr.group, s_PwmAttr.channel);
		pwm_init = true;
	}
	return ret;
	#else
...
int32_t CVI_PWM_Deinit(CVI_HAL_PWM_S pwmAttr)
{
	...
	ret |= CVI_PWM_Disable(pwmAttr.group, pwmAttr.channel);
	ret |= CVI_PWM_UnExport(pwmAttr.group, pwmAttr.channel);
	if (ret != 0) {
		printf("deinit pwm failed!\n");
		return ret;
	}
	pwm_init = false;
    return CVI_PWM_Rmmod();
	#else
	csi_pwm_t pwm;
