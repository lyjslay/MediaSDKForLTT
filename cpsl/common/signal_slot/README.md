...
CVI_SIGNAL_S and CVI_SLOT_S is a communicate mechanism between signal (event) and slot (handler).
...
Declare CVI_SIGNAL_S and call, the type is same as slot handler's type

```bash
CVI_SIGNAL_InitByType(CVI_SIGNAL_S *cvi_signal, CVI_SIGNAL_SLOT_TYPE_E type);
```
...
Declare CVI_SLOT_S and call

```bash
CVI_SLOT_Init(CVI_SLOT_S *cvi_slot, void *handle, CVI_SLOT_VOID_HANLDER handler);

int32_t handler(void *handle, AnyType *data);
```

CVI_SLOT_S can init with any type pointer (one arg), just cast to CVI_SLOT_VOID_HANLDER.
...
* Slot handler only support int32_t and void return type.
