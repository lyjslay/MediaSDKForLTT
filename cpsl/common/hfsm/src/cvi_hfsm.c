...
typedef struct CviHfsmInfo {
    ...
    CVI_HFSM_EVENT_CALLBACK  EventCallback;
    bool                        run;
    bool                        stop;
...
static void* Message_TaskQueue_Proc(void* pvParam)
{
    ...
    while (hfsm->run) {
        if (hfsm->stop == true) {
            ...
            while((0 != sem_wait(&hfsm->MessageSem)) && (errno == EINTR));
            if (0 == HFSM_Queue_GetLen(queue)) {
                continue;
            }
            CVI_MESSAGE_S msg = {0};
...
int32_t CVI_HFSM_Create(CVI_HFSM_ATTR_S *fsmAttr, CVI_HFSM_HANDLE *hfsm)
{
    int32_t s32Ret = 0;
    uint32_t i = 0;
    size_t stack_size = (1024 * 512); // 512kb
    pthread_attr_t attr;
    ...
    s32Ret = pthread_create(&t->taskid, &attr, Message_TaskQueue_Proc, (void*)t);
    if (s32Ret != 0) {
...
int32_t CVI_HFSM_Destroy(CVI_HFSM_HANDLE hfsm)
{
    ...
    LOCK(t->mutex);
    t->run = false;
    sem_post(&t->MessageSem);
    sem_destroy(&t->MessageSem);
