...
int32_t HFSM_Queue_GetLen(QueueHandle queueHdl)
{
    if (queueHdl == 0) {
        CVI_LOGE("queueHdl is NULL!\n");
        return -1;
...
int32_t HFSM_Queue_Push(QueueHandle queueHdl, const void *node)
{
    if (queueHdl == 0) {
        CVI_LOGE("queueHdl is NULL!\n");
        return -1;
...
int32_t HFSM_Queue_Pop(QueueHandle queueHdl, void *node)
{
    if (queueHdl == 0) {
        CVI_LOGE("queueHdl is NULL!\n");
        return -1;
