...
#define CVI_MOVIES_AHD_DIR     "Movie_ahd"
#define CVI_MOVIES_AHD_S_DIR   "Movie_ahd_s"
#define CVI_PARK_AHD_DIR       "Park_ahd"
#define CVI_PARK_AHD_S_DIR     "Park_ahd_s"
#define CVI_EMR_AHD_DIR        "EMR_ahd"
#define CVI_EMR_AHD_S_DIR      "EMR_ahd_s"
#define CVI_PHOTO_DIR        "Photo"
#define CVI_PHOTO_B_DIR      "Photo_b"
#define CVI_PHOTO_AHD_DIR      "Photo_ahd"
...
static int32_t DTCF_Addfile(const char *fileName, CVI_DTCF_DIR_E enDir)
{
    ...
    ret = snprintf(p->d_name, FILENAME_MAX, "%s", fileName);
...
int32_t CVI_DTCF_Scan(CVI_DTCF_DIR_E enDirs[DTCF_DIR_BUTT], uint32_t u32DirCount, uint32_t *pu32FileAmount)
{
    ...
    for (i = 0; i < u32DirCount; i++) {
        ...
        if (s32ScanNamelistTmpCount < 0) {
            CVI_LOGE("scandir %s return:%d %s\n", dirBuf, s32ScanNamelistTmpCount, strerror(errno));
