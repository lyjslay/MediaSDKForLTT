...
#ifdef __UCLIBC__
extern int32_t init_module(void *module_image, unsigned long len, const char *param_values);
extern int32_t delete_module(const char *name, int32_t flags);
#else
...
static const char *moderror(int32_t err)
{
    switch (err) {
...
static int32_t cvi_init_module(const char *filename, const char *options)
{
    size_t image_size;
    char *image;
    int32_t rc;
...
static int32_t cvi_delete_module(const char *module, uint32_t flags)
{
    errno = 0;
...
int32_t cvi_PathIsDirectory(const char *pszPath)
{
    ...
    int32_t s32Ret = stat(pszPath, &stStatBuf);
...
static char *last_char_is(const char *s, int32_t c)
{
    if (s && *s) {
