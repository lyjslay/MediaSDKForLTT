...
static pthread_mutex_t mDevProcLock;
static bool bStgInit = false;
				 ...
				 STG_DEVINFO_S *pstInfo)
{
	...
	case STG_DEVINFO_ERROR_COUNT_LINE_NO:
		s32FmtCount = snprintf(pstInfo->aszErrCnt, STG_DEVINFO_PROC_LINE_LEN, "%s", value);
		break;
	default:
		break;
	}
...
int32_t CVI_STG_ReadProc(STG_DEVINFO_S *pstDevInfo)
{
	uint8_t u8LineNo = 0;
	int32_t s32Ret = 0;
	FILE *pFDevProc = NULL;
	...
	if (strstr(aszProcLineStr, STG_DEVINFO_STATE_UNPLUGGED_CONNECTED)) {
		pstDevInfo->devState = CVI_STG_DEV_STATE_UNPLUGGED;
	} else if (strstr(aszProcLineStr, STG_DEVINFO_STATE_CONNECTED)) {
		pstDevInfo->devState = CVI_STG_DEV_STATE_CONNECTED;
		...
		pstDevInfo->devState = CVI_STG_DEV_STATE_CONNECTING;
	}
...
int32_t CVI_STG_DetectPartition(CVI_STG_HANDLE_T stg_hdl)
{
	...
	return Stg_Fstool_DetectPartition(stgdev->aszDevPath);
}
...
int32_t CVI_STG_TestPartition(CVI_STG_HANDLE_T stg_hdl)
{
	...
	stgdev = (STG_DEVINFO_S *)stg_hdl;
	return Stg_Fstool_TestPartition(stgdev->aszMntPath, stgdev->aszDevPath);
}

bool CVI_STG_CheckIsReadOnly(STG_DEVINFO_S *pstDevInfo)
{
	bool bRet = false;
	FILE *pFDevProc = NULL;
	char *pTmp = NULL;
	char aszProcLineStr[STG_MOUNTINFO_PROC_LINE_LEN] = {0};
	if (bStgInit == false) {
		printf("%s %d stg not init!\n", __FUNCTION__, __LINE__);
		return -1;
	}
	// STG_THREAD_MUTEX_LOCK(&gReadMciInfoLock);
	pthread_mutex_lock(&mDevProcLock);
	pFDevProc = fopen(PROC_MOUNT_PATH, "r");
	if (NULL == pFDevProc) {
		(void)pthread_mutex_unlock(&mDevProcLock);
		printf("%s  %d  open file failure, system errno(%d)!\n",
		       __FUNCTION__, __LINE__, errno);
	...
	while (NULL !=
	       fgets(aszProcLineStr, STG_MOUNTINFO_PROC_LINE_LEN - 1, pFDevProc)) {
		pTmp = strstr(aszProcLineStr, pstDevInfo->aszDevPath);
		if (pTmp) {
			break;
		}
	}
	...
	if (!pTmp) {
		//printf("%s  %d  dev path:%s is not found,!\n", __FUNCTION__, __LINE__, pstDevInfo->aszDevPath);
		goto CLOSE_FILE;
	}

	if (strstr(aszProcLineStr, STG_MOUNTINFO_STATE_READ_ONLY)) {
        bRet = true;
	} else if (strstr(aszProcLineStr, STG_MOUNTINFO_STATE_READ_WRITE_AVAILABLE)) {
	} else {
		printf("%s  %d  Unexpectation: %s \n", __FUNCTION__,
                __LINE__, aszProcLineStr);
	}
...
int32_t CVI_STG_Mount(CVI_STG_HANDLE_T stg_hdl)
{
	...
	//if (stgdev->stgState != CVI_STG_STATE_MOUNTED) {
		...
		if (ret == 0) {
			stgdev->stgState = CVI_STG_STATE_MOUNTED;
		}
	//}
	return ret;
}
...
int32_t CVI_STG_Umount(CVI_STG_HANDLE_T stg_hdl)
{
	...
	stgdev->stgState = CVI_STG_STATE_UNMOUNTED;

	return ret;
}

int32_t CVI_STG_GetDevState(CVI_STG_HANDLE_T stg_hdl, CVI_STG_DEV_STATE_E *state)
{
	STG_DEVINFO_S *stgdev;
	if (bStgInit == false) {
		printf("%s %d stg not init!\n", __FUNCTION__, __LINE__);
...
int32_t CVI_STG_RepairFAT32(CVI_STG_HANDLE_T stg_hdl)
{
	...
	stgdev = (STG_DEVINFO_S *)stg_hdl;
	if (stgdev->stgState != CVI_STG_STATE_MOUNTED) {
		snprintf(cmd, sizeof(cmd), "fsck.fat -a -V -v -w %s", stgdev->aszDevPath);
		ret = cvi_system(cmd);
		if (ret == -1) {
...
int32_t CVI_STG_GetFsType(CVI_STG_HANDLE_T stg_hdl)
{
	STG_DEVINFO_S *stgdev;
	if (bStgInit == false) {
		printf("%s %d stg not init!\n", __FUNCTION__, __LINE__);
	...
	ret = Stg_FsTool_GetFsType(stgdev->aszDevPath, stgdev->fsType,
				    STG_PATH_LEN_MAX);
	if(ret == 0) {
		stgdev->fsType_e = CVI_STG_FS_TYPE_FAT32;
	} else if(ret == 1) {
		stgdev->fsType_e = CVI_STG_FS_TYPE_EXFAT;
	} else {
		stgdev->fsType_e = CVI_STG_FS_TYPE_UNKNOWN;
	}
	return ret;
}
...
int32_t CVI_STG_GetSDInfo(CVI_STG_HANDLE_T stg_hdl)
{
	...
	stgdev = (STG_DEVINFO_S *)stg_hdl;
	return CVI_STG_ReadProc(stgdev);
}
...
int32_t CVI_STG_GetInfo(CVI_STG_HANDLE_T stg_hdl, CVI_STG_DEV_INFO_S *pInfo)
{
    ...
    s32Ret = CVI_STG_ReadProc(pstDevInfo);
    if (0 != s32Ret) {
    ...
    if (CVI_STG_DEV_STATE_CONNECTED != pstDevInfo->devState) {
	...
	pInfo->szErrCnt = atoi(pstDevInfo->aszErrCnt);
    return 0;
}
