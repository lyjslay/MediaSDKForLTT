...
int32_t Stg_Fstool_TestPartition(const char *pszMountPath, const char *pszDevPath)
{
    ...
    // (void)Stg_Fstool_Umount(pszMountPath);

    // s32Ret = Stg_Fstool_Mount(pszDevPath, pszMountPath);
    // if (0 != s32Ret) {
    ...
    snprintf(aszTestFile, CVI_STG_PATH_LEN_MAX + 16 - 1, "%s/%s",
    ...
    memset(aszTestData, 0x00, STG_FSTOOL_TEST_PARTITION_DATA_LEN);
...
static int32_t Stg_Fstool_ReadPartition(const char *pszPartition, off_t offset,
...
retry:
        for(i = 0; i < RETRY_TIMES; i ++) {
			...
			if (i == RETRY_TIMES/2 -1) {
				char cmd[128] = {0};
				sprintf(cmd, "mdev -s");
				printf("%s ,retry 5 times execute : %s\n", __FUNCTION__,cmd);
				cvi_system(cmd);
			}
		}
		if( i >= RETRY_TIMES)
	...
	if (NULL == pFile) {
		j++;
		if (j <= 2) {
			printf("%s,fopen fail goto retry (%d)\n", __FUNCTION__, j);
            char cmd[128] = {0};
			sprintf(cmd, "mdev -s");
			printf("%s,retry execute : %s\n", __FUNCTION__,cmd);
			cvi_system(cmd);
			usleep(200 * 1000);
			goto retry;
		}
...
static int32_t Stg_Fstool_IsExFat(const char *pszBootSector, uint32_t bootSecLen)
{
	if (bootSecLen <
...
static int32_t Stg_Fstool_IsFat32(const char *pszBootSector, uint32_t len)
{
...
int32_t Stg_Fstool_Mount(const char *pszPartition, const char *pszMountPath)
{
	...
	if (0 != mount(pszPartition, pszMountPath, aszFSType, MS_NOATIME|MS_NODIRATIME, "errors=continue")) {
...
static int32_t Stg_Fstool_CheckMMCBlkPath(const char *partionPath)
{
...
doublecheck:
	...
	if(i<1){
		char cmd[128] = {0};
		sprintf(cmd, "mdev -s");
		printf("%s retry 3 times execute : %s\n", __FUNCTION__ ,cmd);
		cvi_system(cmd);
		u32DetectCnt = 0;
		i++;
		goto doublecheck;
...
int32_t Stg_Fstool_Umount(const char *pszDevPath)
{
	if (0 != umount2(pszDevPath, MNT_DETACH)) {
...
int32_t Stg_Fstool_CheckFaile(const char *pszMntPath)
{
    int32_t ret = 0;
    char filename[128] = {0};
    snprintf(filename, sizeof(filename), "%s/cvi_test", pszMntPath);
    FILE *fp = NULL;
    fp = fopen(filename, "a+");
    if (NULL == fp)
    {
