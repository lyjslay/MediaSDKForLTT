...
typedef struct _UPGRADE_PKG_INFO_S {
	uint32_t   u32PkgLen;
	char szSoftVersion[CVI_COMM_STR_LEN]; /* Package version */
	char szModel[CVI_COMM_STR_LEN];   /* Package model */
	char szPkgFilePath[CVI_COMM_PATH_MAX_LEN];
} UPGRADE_PKG_INFO_S;
...
typedef struct cviUPGRADE_PARTITION_HEAD_S {
	uint32_t   u32Magic;
	uint32_t   u32Version;
	uint32_t   u32ChunkHeaderSize;
	uint32_t   u32TotalChunk;
	uint32_t   u32DataLen;
	char szExtraFlags[32];
	char reserved[12];
...
typedef struct cvi_MTDInfo_S {
	uint64_t StartAddr;                 /* Partiton start address */
	uint64_t PartSize;                  /* Partition size */
	uint32_t  BlockSize;                 /* The Block size of the flash where this partition at */
	char DevName[CVI_COMM_STR_LEN];  /* The device node name where this partition relate to */
	char PartName[CVI_COMM_STR_LEN]; /* The partition name of this partition */
} CVI_MTDInfo_S;
...
static int32_t UPGRADE_ReadPartitionHead(int32_t fd, CVI_UPGRADE_PARTITION_HEAD_S *partitionHead, uint32_t  *u32StartOffset, uint32_t  *RawDataLen);
static int32_t UPGRADE_CheckFlashSize(const CVI_UPGRADE_PKG_HEAD_S *pktHead)
{
	UNUSED(pktHead);
#ifdef _FOOLPROOF_
	// TODO
...
static int32_t UPGRADE_CheckALLPartitionsSize(const int32_t pktFd, const CVI_UPGRADE_PKG_HEAD_S *pktHead)
{
	int32_t s32Ret = CVI_SUCCESS;
	CVI_UPGRADE_PARTITION_HEAD_S partitionHead;
	uint32_t  u32StartOffset = 0;
	uint32_t  u32RawDataLen = 0;

	CVI_LOGD("PkgLen=0x%X, PartitionCnt=%d\n", pktHead->u32PkgLen, pktHead->s32PartitionCnt);

	int32_t  s32PartitionCnt = 0;
	if (1 == pktHead-> bMinusOne) {
		s32PartitionCnt = pktHead->s32PartitionCnt - 1;
	} else {
		s32PartitionCnt = pktHead->s32PartitionCnt;
	}

	for (int32_t i = 0; i < s32PartitionCnt; i++) {
		...
		s32Ret = CVI_FAILURE;
		for (int32_t j = 0; j < gUPGRADECtx.gMTDPartNum; j++)
		{
			if (strncmp(gUPGRADECtx.gPartInfo[j].PartName, partitionHead.szExtraFlags, 31) == 0) {
				if (gUPGRADECtx.gPartInfo[j].StartAddr == (uint64_t) u32StartOffset &&
					gUPGRADECtx.gPartInfo[j].PartSize >= (uint64_t) u32RawDataLen) {
...
static int32_t UPGRADE_CompareVersion(const char *pszVer1, const char *pszVer2)
{
	int32_t s32Ret = strncmp(pszVer1, pszVer2, CVI_COMM_STR_LEN);
...
static int32_t _Lookup_MTDdev_Index(const char *name)
{
	int32_t i;
...
static void _init_progress(uint32_t  totalSize)
{
...
static void UPGRADE_ReportProgress(uint32_t  offsetPos, uint32_t  writtenSize)
{
	uint32_t  offset = gUPGRADECtx.gProgress.offset;
	uint32_t  totalSize = gUPGRADECtx.gProgress.totalSize;
	uint32_t  percentage;
	...
	if (gUPGRADECtx.gProgress.offset == 0 || percentage > gUPGRADECtx.gProgress.curPercent) {
		if (gUPGRADECtx.event_cbFunc) {
			...
			event.argv = (void *) (intptr_t) percentage;
...
static int32_t UPGRADE_ReadPartitionHead(int32_t fd, CVI_UPGRADE_PARTITION_HEAD_S *partitionHead, uint32_t  *StartOffset, uint32_t  *RawDataLen)
{
	int32_t readLen = 0;
	uint32_t  i;
	...
	// if (strcmp(partitionHead->szExtraFlags, "SYSTEM") == 0) {
	// 	sleep(3);
	// }
	...
	CVI_SAFECHECK_POINTER(chunkHead, CVI_UPGRADE_EINTR);
...
static int32_t UPGRADE_WritePartitionFromChunk(int32_t fd, CVI_UPGRADE_PARTITION_HEAD_S *partitionHead
						, const char *partName, unsigned char *puReadBuf)
{
	int32_t readLen = 0;
	uint32_t  i;
	FILE *file = NULL;
	...
	if (strcmp(partName, "") != 0) {
		int32_t idx = _Lookup_MTDdev_Index(partName);

		//write partition (label "fip")
		if (strcmp(partName, "fip") == 0) {
			CVI_UPGRADE_CHUNK_HEAD_S *chunkHead = calloc(1, partitionHead->u32ChunkHeaderSize);
			CVI_SAFECHECK_POINTER(chunkHead, CVI_UPGRADE_EINTR);

			readLen = read(fd, chunkHead, partitionHead->u32ChunkHeaderSize);
			if (readLen < (int32_t) partitionHead->u32ChunkHeaderSize) {
				CVI_LOGE("read file failed\n");
				close(fd);
				CVI_SAFECHECK_FREE(chunkHead);
				return CVI_UPGRADE_EINTR;
...
#define MAX_BUFF_RW (4*1024*1024)
			...
			CVI_SAFECHECK_POINTER(FipBin, CVI_UPGRADE_EINTR);
			readLen = read(fd, FipBin, chunkHead->u32DataSize);
			if (readLen < (int32_t) chunkHead->u32DataSize) {
...
#if STORAGE_TYPE_NOR
			s32Ret = cvi_spi_flash_write_fip(FipBin, readLen);
#elif STORAGE_TYPE_NAND

			s32Ret = spi_nand_check_write_vector(FipBin, readLen);
#else
			s32Ret = CVI_SUCCESS;
#endif
			CVI_SAFECHECK_FREE(chunkHead);
		...
		//except partition fip, for other partition first flash open and erase
		if (idx >= 0) {
...
#else
			s32Ret = CVI_Flash_Open(gUPGRADECtx.FlashType, gUPGRADECtx.gPartInfo[idx].DevName
			...
			s32Ret = CVI_Flash_Erase(gUPGRADECtx.FlashType
					, gUPGRADECtx.gPartInfo[idx].StartAddr
					, gUPGRADECtx.gPartInfo[idx].PartSize);
			if (s32Ret >= 0) {
				CVI_LOGD(LIGHT_BLUE"CVI_Flash_Erase %s success %d\n"NONE
					, gUPGRADECtx.gPartInfo[idx].DevName, s32Ret);
			} else {
	...
	//then for other partition flash write
	CVI_UPGRADE_CHUNK_HEAD_S *chunkHead = calloc(1, partitionHead->u32ChunkHeaderSize);
	CVI_SAFECHECK_POINTER(chunkHead, CVI_UPGRADE_EINTR);
#ifndef _USE_NANDWRITE_
	uint32_t  offset = 0;
	uint32_t  u32Remain2Read = 0;
#endif
	for (i = 0; i < partitionHead->u32TotalChunk; i++) {
		readLen = read(fd, chunkHead, partitionHead->u32ChunkHeaderSize);
		if (readLen < (int32_t) partitionHead->u32ChunkHeaderSize) {
...
#ifdef _USE_NANDWRITE_
		if (file) {
			unsigned char *pBuf2Write;
			int32_t iLen2Wrtie;
			uint32_t  u32Remain2Read = chunkHead->u32DataSize;

			while (u32Remain2Read) {
				int32_t readSize = (u32Remain2Read > UPGRADE_IMGDATA_BUFF_MAX_SIZE) ?
				...
				do {
					int32_t ret = fwrite(pBuf2Write, 1, iLen2Wrtie, file);
...
#else
	   if (gUPGRADECtx.FlashType == CVI_FLASH_TYPE_NAND_0 || gUPGRADECtx.FlashType == CVI_FLASH_TYPE_SPI_0) {
			u32Remain2Read += chunkHead->u32DataSize;
			for (; offset < u32Remain2Read && eraseUnit > 0; offset += readLen) {
		...
		//after writing each partition, do sync
		system("sync");
	}
	// flash write finish

	if (file) {
		int32_t ret = pclose(file);
...
static int32_t UPGRADE_ReadPkgHead(int32_t s32Fd, CVI_UPGRADE_PKG_HEAD_S *pstPkgHead)
{
	int32_t s32Ret = CVI_SUCCESS;
...
static int32_t UPGRADE_CheckPkgCrc(const CVI_UPGRADE_PKG_HEAD_S *pstPkgHead,
						int32_t s32Fd, unsigned char *pu8Buf, uint32_t  u32BufSize)
{
	uint32_t  u32PkgCrc = 0;
	int32_t s32Ret = CVI_SUCCESS;
	UNUSED(u32BufSize);
	u32PkgCrc = update_crc(u32PkgCrc, (unsigned char *)&pstPkgHead->u32HeadVer, sizeof(CVI_UPGRADE_PKG_HEAD_S) - 8);
	...
	int32_t s32RemainReadLen = pstPkgHead->u32PkgLen - sizeof(CVI_UPGRADE_PKG_HEAD_S);
	int32_t s32BytesToRead = 0;
	int32_t s32ReadLen = 0;
...
static int32_t UPGRADE_CheckPkgValid(UPGRADE_PKG_INFO_S *pstPkgInfo)
{
	int32_t s32Ret = CVI_SUCCESS;
	unsigned char *pu8PkgBuf = (unsigned char *) malloc(UPGRADE_PKG_BUFF_SIZE);
...
{
	int32_t s32Ret = CVI_SUCCESS;
	unsigned char *pu8PkgBuf = (unsigned char *)calloc(1, UPGRADE_PKG_BUFF_SIZE);
	int32_t s32PkgFd = -1;
	int32_t i;
	...
	int32_t  s32PartitionCnt = 0;
	if (1 == stPkgHead.bMinusOne) {
		s32PartitionCnt = stPkgHead.s32PartitionCnt - 1;
	} else {
		s32PartitionCnt = stPkgHead.s32PartitionCnt;
	}

	for (i = 0; i < s32PartitionCnt; i++) {
		int32_t s32CurOffset;

		s32CurOffset = lseek(s32PkgFd, 0, SEEK_CUR);
		if (s32CurOffset <(int32_t)stPkgHead.au32PartitionOffSet[i]) {
...
static int32_t UPGRADE_WriteFile(int32_t s32PkgFd, FILE *Write_File, uint32_t  WriteSize)
{
	unsigned char *pBuf2Write;
    int32_t iLen2Wrtie;
	uint32_t  u32Remain2Read = WriteSize;
	unsigned char *puReadBuf = (unsigned char *)calloc(1, UPGRADE_PKG_BUFF_SIZE);
	CVI_SAFECHECK_POINTER(puReadBuf, CVI_UPGRADE_EINTR);

    while (u32Remain2Read) {
        int32_t readSize = (u32Remain2Read > UPGRADE_IMGDATA_BUFF_MAX_SIZE) ?
                             UPGRADE_IMGDATA_BUFF_MAX_SIZE : u32Remain2Read;

		//CVI_LOGE("u32Remain2Read: %d", u32Remain2Read);
        int32_t readLen = read(s32PkgFd, puReadBuf, readSize);
		if (readLen < 0) {
			CVI_LOGE("read file failed\n");
			fclose(Write_File);
			free(puReadBuf);
		...
		int32_t ret = fwrite(pBuf2Write, 1, iLen2Wrtie, Write_File);
        if (ret < 0) {
                CVI_LOGE("Write to file error: %s\n", strerror(errno));
				fclose(Write_File);
				free(puReadBuf);
                return CVI_UPGRADE_EINTR;
            }
...
int32_t CVI_UPGRADE_DoUpgradeViaSD(const char *pszPkgUrl, const char *path)
{
	...
	CVI_LOGE("PkgLen=0x%X, PartitionCnt=%d\n", stPkgHead.u32PkgLen, stPkgHead.s32PartitionCnt);
	...
	for (i = 0; i < stPkgHead.s32PartitionCnt; i++) {
		...
		char uppath[64] = {0};
		strcpy(uppath, path);
		strcat(uppath, stPkgHead.PartitionFileName[i]);

		if (0 == i) {
			s32CurOffset = lseek(s32PkgFd, stPkgHead.au32PartitionOffSet[i]+sizeof(CVI_UPGRADE_PARTITION_HEAD_S)+sizeof(CVI_UPGRADE_CHUNK_HEAD_S), SEEK_SET);
		}

		UPGRADE_ReportProgress(s32CurOffset, 0);

		FILE *ToFile = fopen(uppath, "wb+");
		if (ToFile == NULL) {
			CVI_LOGE("open file %s failed\n", uppath);
			return CVI_UPGRADE_EINTR;
		}

		uint32_t  u32Remain2Read = stPkgHead.au32PartitionOffSet[i+1] - s32CurOffset;
		s32Ret = UPGRADE_WriteFile(s32PkgFd, ToFile, u32Remain2Read);
		if (s32Ret != CVI_SUCCESS) {
			CVI_LOGE("UPGRADE_WriteFile failed\n");
			close(s32PkgFd);
			return CVI_UPGRADE_EINTR;
		}
		CVI_LOGE("Update %s file finish!!!\n", uppath);
	}
...
{
	int32_t minor;
	uint32_t size, erasesize;
	char name[65];
	char line[128];
	FILE *fp = fopen(PROC_MTD_PATH, "r");
	uint64_t u64StartAddr = 0;
	int32_t i;
	...
	for (i = 0; fgets(line, sizeof(line), fp) != 0; i++) {
		if (sscanf(line, "mtd%d: %x %x \"%64[^\"]\"", &minor, &size, &erasesize, name) == 4) {
			...
			gUPGRADECtx.gPartInfo[i].PartSize = (uint64_t) size;
			gUPGRADECtx.gPartInfo[i].BlockSize = (uint64_t) erasesize; // erase size
			...
			#ifdef STORAGE_TYPE_NAND
			gUPGRADECtx.FlashType = CVI_FLASH_TYPE_NAND_0;
			#elif STORAGE_TYPE_NOR
			gUPGRADECtx.FlashType = CVI_FLASH_TYPE_SPI_0;
			#endif
		}
	}
