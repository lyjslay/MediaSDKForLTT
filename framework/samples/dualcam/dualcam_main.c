...
static const char *rtsp_session_name[2] = {"cvi_cam_0", "cvi_cam_1"};
...
static int DispOpen(CVI_U32 width, CVI_U32 height, ROTATION_E rotate)
{
    ...
    dispCfg->videoLayerAttr.u32PixelFmt = PIXEL_FORMAT_RGB_888_PLANAR;
...
#define RTSP_HEIGHT   (720)
    ...
    for (int i = 0; i < vcap_attr.u8DevNum; i++) {
        CHECK_RET(CVI_MAPI_VCAP_InitSensor(&sns[i], i, &vcap_attr));
        CHECK_RET(CVI_MAPI_VCAP_InitISP(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartISP(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartDev(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartPipe(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartChn(sns[i]));
    }
    ...
    CVI_MAPI_VPROC_ATTR_T vproc_attr = CVI_MAPI_VPROC_DefaultAttr_TwoChn(
            ...
            DISP_WIDTH, DISP_HEIGHT, PIXEL_FORMAT_RGB_888_PLANAR,
    ...
    CVI_MAPI_VENC_HANDLE_T venc_rec[2];
    CVI_MAPI_VENC_HANDLE_T venc_rtsp[2];
    FILE *outfp[2] = {NULL};
    CVI_MUXER_HANDLE_T muxer[2];
    CVI_RTSP_SESSION_HANDLE_T rtsp[2];
    if (venc_flag) {
        for (int i = 0; i < 2; i++) {
            ...
            CHECK_RET(CVI_MAPI_VENC_InitChn(&venc_rec[i], &venc_attr));
            CHECK_RET(CVI_MAPI_VENC_StartRecvFrame(venc_rec[i], -1));

            if (outputfile) {
                char filename[128];
                snprintf(filename, 128, "sns%d_%s", i, outputfile);
                if (muxer_flag) {
                    ...
                    CHECK_RET(CVI_MUXER_Init(&muxer[i], &muxer_attr));
                    CHECK_RET(CVI_MUXER_StartMux(&muxer[i], filename));
                } else {
            ...
            if (rtsp_flag) {
                ...
                venc_attr.pixel_format = PIXEL_FORMAT_YUV_PLANAR_420;
                CHECK_RET(CVI_MAPI_VENC_InitChn(&venc_rtsp[i], &venc_attr));
                CHECK_RET(CVI_MAPI_VENC_StartRecvFrame(venc_rtsp[i], -1));

                CVI_LOGI("Start RTSP Streaming\n");
                CVI_MAPI_RTSP_SESSION_ATTR_T rtsp_attr;
                rtsp_attr.session_name = rtsp_session_name[i];
                CHECK_RET(CVI_RTSP_CreateSession(&rtsp[i], &rtsp_attr));
            }
        }
    }
    ...
    for (int i = 0; i < 2; i++) {
        char filename[128];
        snprintf(filename, 128, "sns%d_test.yuv", i);
        if (save_yuv_flag && !save_yuv_single) {
            save_yuv_open("test.yuv", &yuvfp[i]);
        }
    }
    ...
    while(count--) {
        ...
        CHECK_RET(CVI_MAPI_VCAP_GetFrame(sns[sns_id], &vcap_frame));

        cvi_osal_get_boot_time(&t2);

        if (save_yuv_flag) {
            if (save_yuv_single) {
                save_yuv_single_frame(&vcap_frame, count + 100000000 * sns_id);
            } else {
                save_yuv_frame(&vcap_frame, yuvfp[sns_id]);
            }
        }
        ...
        if (venc_flag) {
            CHECK_RET(CVI_MAPI_VENC_SendFrame(venc_rec[sns_id], &vcap_frame));
            VENC_STREAM_S stream = {0};
            CHECK_RET(CVI_MAPI_VENC_GetStream(venc_rec[sns_id], &stream));

            cvi_osal_get_boot_time(&t4);

            CHECK_RET(CVI_MAPI_VENC_GetStreamStatus(venc_rec[sns_id], &stream, &is_I_frame));
            ...
            if (muxer_flag) {
                ...
                CHECK_RET(CVI_MUXER_SendFrameStream(muxer[sns_id], &frame_stream));
            }
            ...
            for (uint32_t i = 0; i < stream.u32PackCount; i++) {
                ...
                //if (rtsp_flag) {
                //    CHECK_RET(CVI_RTSP_SendStream(rtsp[sns_id],
                //            ppack->pu8Addr + ppack->u32Offset,
                //            ppack->u32Len - ppack->u32Offset));
                //}
            }

            CHECK_RET(CVI_MAPI_VENC_ReleaseStream(venc_rec[sns_id], &stream));
        }
        ...
        if (venc_flag && rtsp_flag) {
            ...
            CHECK_RET(CVI_MAPI_VENC_SendFrame(venc_rtsp[sns_id], &vproc_frame_rtsp));

            CHECK_RET(CVI_MAPI_ReleaseFrame(&vproc_frame_rtsp));

            VENC_STREAM_S stream = {0};
            CHECK_RET(CVI_MAPI_VENC_GetStream(venc_rtsp[sns_id], &stream));

            cvi_osal_get_boot_time(&t9);

            CHECK_RET(CVI_MAPI_VENC_GetStreamStatus(venc_rtsp[sns_id], &stream, &is_I_frame));
            ...
            frame_stream.num = stream.u32PackCount;
            frame_stream.is_I_frame = is_I_frame;
            CHECK_RET(CVI_RTSP_SendStream(rtsp[sns_id], &frame_stream));

            cvi_osal_get_boot_time(&t10);

            CHECK_RET(CVI_MAPI_VENC_ReleaseStream(venc_rtsp[sns_id], &stream));
        }

        printf(".");
        if (count % 29 == 0 || is_I_frame) {
            printf("\n");
            if (perf_flag) {
                printf("Perf: sns %d, %s\n", sns_id, is_I_frame?"":"I Frame");
    ...
    for (int i = 0; i < 2; i++) {
        if (venc_flag) {
            if (rtsp_flag) {
                CHECK_RET(CVI_RTSP_DestroySession(rtsp[i]));
                CHECK_RET(CVI_MAPI_VENC_StopRecvFrame(venc_rtsp[i]));
                CHECK_RET(CVI_MAPI_VENC_DeinitChn(venc_rtsp[i]));
            }
            if (outputfile) {
                if (muxer_flag) {
                    CHECK_RET(CVI_MUXER_StopMux(muxer[i]));
                    CHECK_RET(CVI_MUXER_Deinit(muxer[i]));
                } else {
                    fclose(outfp[i]);
                }
            }

            CHECK_RET(CVI_MAPI_VENC_StopRecvFrame(venc_rec[i]));
            CHECK_RET(CVI_MAPI_VENC_DeinitChn(venc_rec[i]));
        }
    }
    ...
    for (int i = 0; i < 2; i++) {
        CHECK_RET(CVI_MAPI_VCAP_StopChn(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_StopDev(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_StopPipe(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_StopISP(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_DeInitISP(sns[i]));
        CHECK_RET(CVI_MAPI_VCAP_DeinitSensor(sns[i]));
    }
