...
CVI_MAPI_AUDIO_CODEC_E GetCodecIndex(const char *codecname)
{
    int ret,find;
    unsigned i;
    ...
    if(find) {
        return (CVI_MAPI_AUDIO_CODEC_E)i;
    }
    return -1;
}
...
#if 0
int  Ao_GetEncodeDataCB(CVI_MAPI_AO_HANDLE_T AoHdl,AUDIO_STREAM_S* pAStreamData, void *pPrivateData)
{
    CVI_AO_CB_INFO *pstAoCbInfo = (CVI_AO_CB_INFO *)pPrivateData;
    ...
    if (u32ReadLen ==  0) {
        ret = CVI_MAPI_ADEC_SendEndOfStream(et->AdecHdl);
        playing = 0;
    }
    printf("pstAoCbInfo->period_size = %d\n",pstAoCbInfo->u32PtNumPerFrm);
    pAStreamData->u32Len = u32ReadLen;
    ret = CVI_MAPI_ADEC_SendStream(et->AdecHdl,pAStreamData, CVI_TRUE);
    if (ret != CVI_SUCCESS) {
        CVI_LOGE("CVI_MAPI_ADEC_SendStream send failed,ret:%d.\n",ret);
        return -1;
    }
...
CVI_VOID* AudioAoOtherChnPlay(CVI_CHAR * argcs)
{
    ...
    if (pstChnInfo->ChnSample_rate != pstChnInfo->stAoAttr.enSampleRate) {
        s32Ret = CVI_MAPI_AO_EnableReSmp(pstChnInfo->AoHdl, pstChnInfo->AoChn,
    ...
    s32Ret = CVI_MAPI_AO_Start(pstChnInfo->AoHdl,pstChnInfo->AoChn);
    if(s32Ret){
        CVI_LOGE("CVI_MAPI_AO_Start() failed\n");
        goto ERR2;
    }
    if(CVI_MAPI_AO_SetVolume(pstChnInfo->AoHdl,10)){
    ...
    while(playing) {//
        ...
        if( fread(pstChnInfo->pBuffer,1,s32FrameBytes,pstChnInfo->pfile) > 0) {
            ...
            s32Ret =CVI_MAPI_AO_SendFrame(pstChnInfo->AoHdl,pstChnInfo->AoChn,&stFrame,1000);
            if(s32Ret){
                CVI_LOGE("[error] Thread 2 CVI_MAPI_AO_SendFrame err [%d]\n",__LINE__);
                goto ERR1;
            }
    ...
    if (pstChnInfo->ChnSample_rate != pstChnInfo->stAoAttr.enSampleRate)
        CVI_MAPI_AO_DisableReSmp(pstChnInfo->AoHdl,pstChnInfo->AoChn);

    audio_close_wavefile(pstChnInfo->pfile);
    CVI_MAPI_AO_Stop(pstChnInfo->AoHdl,pstChnInfo->AoChn);
ERR1:
...
int main(int argc,char *argv[])
{
    ...
    CVI_MAPI_ADEC_ATTR_S stAdecAttr;
    ...
    if(!bBind) {//no bind
       for(int i=0;i < filecount; i++) {
        if (_cvi_checkname_iswav(inputfile[i]))
    ...
    s32Ret = CVI_MAPI_AO_Init(&AoHdl,&stAoAttr);
    if(s32Ret){
        CVI_LOGE("CVI_MAPI_AO_Init() failed\n");
        return s32Ret;
    }
    pthread_t tid;
    char*p;
    if (!bBind && filecount > 1) {
        ...
        memcpy(&stAoChnInfo.stAoAttr,&stAoAttr,sizeof(CVI_MAPI_AO_ATTR_S));
        pthread_create(&tid,NULL,(void *)AudioAoOtherChnPlay,(void *)&stAoChnInfo);
    }

    s32Ret = CVI_MAPI_AO_Start(AoHdl,AoChn);
    if(s32Ret){
        CVI_LOGE("CVI_MAPI_AO_Start() failed\n");
        return s32Ret;
    }
    if(CVI_MAPI_AO_SetVolume(AoHdl,10)){
        CVI_LOGE("CVI_AO_SetVolume() ERR\n");
        playing = 0;
    }
    ...
    if (ChnSampleRate != (int)stAoAttr.enSampleRate) {
        s32Ret = CVI_MAPI_AO_EnableReSmp(AoHdl, AoChn, (AUDIO_SAMPLE_RATE_E)ChnSampleRate);
        if(s32Ret){
    ...
    if(bBind) {
        if (ChannelCnt != stAoAttr.channels) {
            CVI_LOGE("bind mode not support .please use get_use_mode\n");
            return -1;
        }
        memset(&stAdecAttr,0,sizeof(CVI_MAPI_ADEC_ATTR_S));
        ...
        CVI_MAPI_ADEC_Init(&AdecHdl,&stAdecAttr);
        CVI_MAPI_AO_BindAdec(0,0,0);
    ...
    while(playing && bBind) { //bind
        ...
        if (u32ReadLen ==  0) {
            ret = CVI_MAPI_ADEC_SendEndOfStream(AdecHdl);
            playing = 0;
        }

        stAdecStream.u32Len = u32ReadLen;
        ret = CVI_MAPI_ADEC_SendStream(AdecHdl,&stAdecStream, CVI_TRUE);
        if (ret != CVI_SUCCESS) {
            CVI_LOGE("CVI_MAPI_ADEC_SendStream send failed,ret:%d.\n",ret);
            return -1;
        }
    ...
    while(playing && (!bBind))
    {
        ...
        if(num_framebytes>0){
            ...
            s32Ret = CVI_MAPI_AO_SendFrame(AoHdl,AoChn,&stFrame,1000);
            if(s32Ret){
    ...
    if(bBind) {
        CVI_MAPI_AO_UnbindAdec(0,0,0);

        fclose(audio_file);
        CVI_MAPI_ADEC_Deinit(AdecHdl);

    }else {
        audio_close_wavefile(pfile[0]);
        if (ChnSampleRate != (int)stAoAttr.enSampleRate)
            CVI_MAPI_AO_DisableReSmp(AoHdl, AoChn);
        if(filecount > 1) {
        ...
        free(pBuffer);
        CVI_MAPI_AO_Stop(AoHdl,AoChn);
        CVI_MAPI_AO_Deinit(AoHdl);
    TEST_PASS;
    return 0;
}
