...
typedef struct _INPUT_CONFIG_T{
    ...
    char *inputfile;
    char *outputfile;
    CVI_MAPI_VDEC_HANDLE_T venc;
    CVI_MAPI_VDEC_CHN_ATTR_T venc_attr;
    VIDEO_FRAME_INFO_S *pstFrameInfo;
}INPUT_CONFIG_T;


typedef struct _MAPI_VDEC_THREAD_PARAM_S {
    CVI_MAPI_VDEC_HANDLE_T vdec_hdl;
    CVI_MAPI_VCODEC_E eVDecType;
...
//      CVI_LOGI("  s32Ret:%d    Vdec    %2.2f ms\n",s32Ret, (t2-t1)/1000.0);
        if (s32Ret == CVI_SUCCESS) {
            ...
            cvi_osal_get_boot_time(&t2);
//          CVI_LOGI("CVI_MAPI_VDEC_GetFrame %d suc,dumpyuv %2.2f ms\n",count++,(t2-t1)/1000.0);
            s32Ret = CVI_MAPI_VDEC_ReleaseFrame(pstVdecThreadParam->vdec_hdl, &stVFrame);
            if (s32Ret != CVI_SUCCESS) {
    ...
    while (1) {
        ...
        if (s32ReadLen == 0) {
            if (pstVdecThreadParam->bCircleSend == CVI_TRUE) {
                ...
                bEndOfStream = true;
                CVI_MAPI_VDEC_SendStream(pstVdecThreadParam->vdec_hdl,&stStream);
                break;
                //s32UsedBytes = 0;
        ...
        if (pstVdecThreadParam->eVDecType == CVI_MAPI_VCODEC_H264) {
        ...
        } else if (pstVdecThreadParam->eVDecType == CVI_MAPI_VCODEC_H265) {
            bool bNewPic = false;
        ...
        } else if (pstVdecThreadParam->eVDecType == CVI_MAPI_VCODEC_JPG || pstVdecThreadParam->eVDecType == CVI_MAPI_VCODEC_MJP) {
...
SendAgain:
        s32Ret = CVI_MAPI_VDEC_SendStream(pstVdecThreadParam->vdec_hdl,&stStream);
        if ((s32Ret != CVI_SUCCESS) && (timeoutcnt < 100)) {
            CVI_LOGE("CVI_MAPI_VDEC_SendStream failed,s32Ret:%d.s32UsedBytes:%d,timeoutcnt:%d\n",s32Ret,s32UsedBytes,timeoutcnt);
            usleep(10 * 1000);
            timeoutcnt++;
            goto SendAgain;
        } else {
            ...
            CVI_LOGI("CVI_MAPI_VDEC_SendStream over,ret:%d,send_suc_cnt:%d\n",s32Ret,send_suc_cnt++);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    CVI_MAPI_GetMaxSizeByEncodeType(enType, &vdec_attr.max_width, &vdec_attr.max_height);
    ...
    CHECK_RET(CVI_MAPI_VDEC_InitChn(&vdec_hdl, &vdec_attr));
