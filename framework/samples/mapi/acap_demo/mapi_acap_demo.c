...
int main(int argc, char *argv[])
{
    ...
    if (outSampleRate != stACapAttr.enSampleRate)
        CVI_MAPI_ACAP_EnableReSmp(AcapHdl, (AUDIO_SAMPLE_RATE_E)outSampleRate);
    ...
    {
        // read from ai module
        ret = CVI_MAPI_ACAP_GetFrame(AcapHdl, &stFrame, &stAecFrm);
        if (ret != CVI_SUCCESS) {
            CVI_LOGE("CVI_MAPI_ACAP_GetFrame failed,ret:%d.!!\n",ret);
        ...
        ret = CVI_MAPI_ACAP_ReleaseFrame(AcapHdl,&stFrame,&stAecFrm);
        if (ret != CVI_SUCCESS) {
            CVI_LOGE("CVI_MAPI_ACAP_ReleaseFrame, failed with %d!\n",ret);
        }
    ...
    if (outSampleRate != stACapAttr.enSampleRate)
        CVI_MAPI_ACAP_DisableReSmp(AcapHdl);
