...
CVI_MAPI_VCAP_ATTR_T g_vcap_attr;
CVI_MAPI_VCAP_SENSOR_HANDLE_T g_sns_hdl[VI_MAX_DEV_NUM];
...
static int vcap_demo_sys_init(void)
{
    ...
    memset(&g_vcap_attr, 0, sizeof(CVI_MAPI_VCAP_ATTR_T));
    CHECK_RET(CVI_MAPI_VCAP_GetGeneralVcapAttr(&g_vcap_attr));
...
static int vcap_demo_sns_init(void)
{
    ...
    for (i = 0; i < sns_num; i++) {
        CHECK_RET(CVI_MAPI_VCAP_InitSensor(&g_sns_hdl[i], i, &g_vcap_attr));
        CHECK_RET(CVI_MAPI_VCAP_InitISP(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartISP(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartDev(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartPipe(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_StartChn(g_sns_hdl[i]));
    }
...
static int vcap_demo_sns_deinit(void)
{
    ...
    sns_num = g_vcap_attr.u8DevNum;
    for (i = 0; i < sns_num; i++) {
        CHECK_RET(CVI_MAPI_VCAP_StopISP(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_DeInitISP(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_StopChn(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_StopPipe(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_StopDev(g_sns_hdl[i]));
        CHECK_RET(CVI_MAPI_VCAP_DeinitSensor(g_sns_hdl[i]));
        g_sns_hdl[i] = NULL;
    }
    return 0;
}
...
static int vcap_demo_dump_yuv(int sns_id)
{
    ...
    char filename[128];
    CHECK_RET(CVI_MAPI_VCAP_GetFrame(g_sns_hdl[sns_id], &vcap_frame));
    snprintf(filename, 128, "vcap_demo_out_%d", sns_id);
    CHECK_RET(CVI_MAPI_SaveFramePixelData(&vcap_frame, filename));
...
static int vcap_demo_set_crop(int sns_id, int32_t s32X, int32_t s32Y, uint32_t u32Width, uint32_t u32Height)
{
    ...
    CHECK_RET(CVI_MAPI_VCAP_SetChnCropAttr(g_sns_hdl[sns_id], &astCropInfo[0]));

    CHECK_RET(CVI_MAPI_VCAP_GetChnCropAttr(g_sns_hdl[sns_id], &astCropInfo[1]));
...
static int vcap_demo_set_rotation(int sns_id, ROTATION_E enRotation)
{
    ...
    CHECK_RET(CVI_MAPI_VCAP_SetAttrEx(g_sns_hdl[sns_id], CVI_MAPI_VCAP_CMD_Rotate,
                                      &aenRotation[0], sizeof(ROTATION_E)));

    CHECK_RET(CVI_MAPI_VCAP_GetAttrEx(g_sns_hdl[sns_id], CVI_MAPI_VCAP_CMD_Rotate,
...
{
    CVI_MAPI_VCAP_MIRRORFLIP_ATTR_S aenMirrorFlip[2] = {0};

    aenMirrorFlip[0].bMirror = mirror;
    aenMirrorFlip[0].bFlip = flip;

    CHECK_RET(CVI_MAPI_VCAP_SetAttrEx(g_sns_hdl[sns_id], CVI_MAPI_VCAP_CMD_MirrorFlip,
                                      &aenMirrorFlip[0], sizeof(CVI_MAPI_VCAP_MIRRORFLIP_ATTR_S)));

    CHECK_RET(CVI_MAPI_VCAP_GetAttrEx(g_sns_hdl[sns_id], CVI_MAPI_VCAP_CMD_MirrorFlip,
                                      &aenMirrorFlip[1], sizeof(CVI_MAPI_VCAP_MIRRORFLIP_ATTR_S)));
...
static int vcap_demo_set_fps(int sns_id, float fps)
{
    ...
    CHECK_RET(CVI_MAPI_VCAP_SetAttrEx(g_sns_hdl[sns_id], CVI_MAPI_VCAP_CMD_Fps,
                                      &af32FrameRate[0], sizeof(CVI_FLOAT)));

    CHECK_RET(CVI_MAPI_VCAP_GetAttrEx(g_sns_hdl[sns_id], CVI_MAPI_VCAP_CMD_Fps,
...
static int vcap_demo_set_dump_raw_attr(int sns_id)
{
    ...
    CHECK_RET(CVI_MAPI_VCAP_SetDumpRawAttr(g_sns_hdl[sns_id], &astDumpAttr[0]));

    CHECK_RET(CVI_MAPI_VCAP_GetDumpRawAttr(g_sns_hdl[sns_id], &astDumpAttr[1]));
                                    ...
                                    uint32_t u32DataNum, void* pPrivateData)
{
    ...
    for (i = 0; i < frm_num; i++) {
        ...
        printf("paddr(0x%lx) vaddr(0x%lx)\n",
                    (uint64_t)pstVideoFrame[i].stVFrame.u64PhyAddr[0],
                    (uint64_t)pstVideoFrame[i].stVFrame.pu8VirAddr[0]);
...
static int vcap_demo_dump_raw(const int sns_id)
{
    ...
    CHECK_RET(CVI_MAPI_VCAP_StartDumpRaw(g_sns_hdl[sns_id], 1, &stVCapRawData));

    CHECK_RET(CVI_MAPI_VCAP_StopDumpRaw(g_sns_hdl[sns_id]));
