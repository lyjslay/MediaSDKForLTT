...
typedef struct _INPUT_CONFIG_T{
    ...
    CVI_MAPI_VCODEC_E eVCodec;
    char *inputfile;
    char *outputfile;
    CVI_MAPI_VENC_HANDLE_T venc;
...
CVI_MAPI_VCODEC_E GetCodecIndex(const char *codecname)
{
    int ret,find;
    unsigned i;
    ...
    if(find) {
        return (CVI_MAPI_VCODEC_E)i;
    }

    return CVI_MAPI_VCODEC_MAX;
}
...
static int VencStreamProcThreadLoop(void *pArgs)
{
    ...
    uint32_t frame_size = getFrameSize(pstInputConfig->image_width, pstInputConfig->image_height, pstInputConfig->venc_attr.pixel_format);
    ...
    for (uint32_t i = 0; i < num_frames; i++) {
        ...
        CHECK_RET(CVI_MAPI_GetFrameFromMemory_YUV(&frame,
                pstInputConfig->venc_attr.width, pstInputConfig->venc_attr.height,
                pstInputConfig->venc_attr.pixel_format, yuv_data));

        VENC_STREAM_S stream = {0};
        cvi_osal_get_boot_time(&t1);
        CHECK_RET(CVI_MAPI_VENC_EncodeFrame(pstInputConfig->venc,&frame,&stream));
        ...
        CHECK_RET(CVI_MAPI_VENC_ReleaseStream(pstInputConfig->venc, &stream));
    }

    CHECK_RET(CVI_MAPI_VENC_StopRecvFrame(pstInputConfig->venc));
    CHECK_RET(CVI_MAPI_VENC_DeinitChn(pstInputConfig->venc));
    fclose(fpSrc);
    free(yuv_data);
...
int main(int argc, char *argv[])
{
    ...
    if(CVI_MAPI_VCODEC_MAX == eVCodeIndex) {
        CVI_LOGE("do not support %s.\n",codecname);
        print_usage(argv);
        return CVI_FAILURE;
    }
    if(CVI_MAPI_VCODEC_JPG == eVCodeIndex && ChannelCount> 1) {
        ChannelCount = 1;
    ...
    for(uint32_t i= 0; i < ChannelCount; i++) {
        ...
        astInputConfig[i]->venc_attr.pixel_format = PIXEL_FORMAT_YUV_PLANAR_420;
        astInputConfig[i]->venc_attr.codec = eVCodeIndex;
        astInputConfig[i]->venc_attr.width = width;
        astInputConfig[i]->venc_attr.height = height;
        astInputConfig[i]->venc_attr.rate_ctrl_mode = 4;
        astInputConfig[i]->venc_attr.bitrate_kbps = 8000;
        astInputConfig[i]->venc_attr.iqp = 46;
        astInputConfig[i]->venc_attr.pqp = 46;
        astInputConfig[i]->venc_attr.maxIqp = 60;
        astInputConfig[i]->venc_attr.minIqp = 30;
        astInputConfig[i]->venc_attr.maxQp = 60;
        astInputConfig[i]->venc_attr.minQp = 30;
    ...
    for(uint32_t idx = 0; idx < ChannelCount; idx++) {
        s32Ret = CVI_MAPI_VENC_InitChn(&astInputConfig[idx]->venc, &astInputConfig[idx]->venc_attr);
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_MAPI_VENC_InitChn, %d\n", s32Ret);
            return CVI_FAILURE;
        }
        s32Ret = CVI_MAPI_VENC_StartRecvFrame(astInputConfig[idx]->venc, -1);
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_MAPI_VENC_StartRecvFrame, %d\n", s32Ret);
            return CVI_FAILURE;
        }
    }
