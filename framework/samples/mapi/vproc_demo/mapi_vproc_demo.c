...
int test_vproc_basic(const char *inputfile, const char *outputfile_base)
{
    ...
    CHECK_RET(CVI_MAPI_VPROC_SendFrame(vproc, &frame));
    CHECK_RET(CVI_MAPI_ReleaseFrame(&frame));

    char filename[128];
    int vproc_chn_id_0 = 0;
    VIDEO_FRAME_INFO_S frame_chn_0;
    CHECK_RET(CVI_MAPI_VPROC_GetChnFrame(vproc, vproc_chn_id_0,
                                         &frame_chn_0));
    snprintf(filename, 128, "%s_chn_%d", outputfile_base, 0);
    CHECK_RET(CVI_MAPI_SaveFramePixelData(&frame_chn_0, filename));
    CHECK_RET(CVI_MAPI_ReleaseFrame(&frame_chn_0));

    int vproc_chn_id_1 = 1;
    VIDEO_FRAME_INFO_S frame_chn_1;
    CHECK_RET(CVI_MAPI_VPROC_GetChnFrame(vproc, vproc_chn_id_1,
                                         &frame_chn_1));
    snprintf(filename, 128, "%s_chn_%d", outputfile_base, 1);
    CHECK_RET(CVI_MAPI_SaveFramePixelData(&frame_chn_1, filename));
    CHECK_RET(CVI_MAPI_ReleaseFrame(&frame_chn_1));
...
int test_iproc_perf(const char *inputfile, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        CHECK_RET(CVI_MAPI_IPROC_Resize(&frame,
            &frame_resized,
            CHN_0_WIDTH, CHN_0_HEIGHT, PIXEL_FORMAT_YUV_PLANAR_420,
            false, NULL, NULL, NULL));
        CHECK_RET(CVI_MAPI_ReleaseFrame(&frame_resized));
    }
    cvi_osal_get_boot_time(&t1);
...
int test_rotate_perf(const char *inputfile, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        CHECK_RET(CVI_MAPI_IPROC_Rotate(&frame,
...
int save_frame_file(uint32_t Grp, uint32_t Chn, VIDEO_FRAME_INFO_S *pFrame, void *pPrivateData)
{
    ...
    CHECK_RET(CVI_MAPI_SaveFramePixelData(pFrame, filename));
    TEST_PASS;
    return 0;
}
...
int test_vproc_dump_frame()
{
    ...
    CVI_MAPI_VCAP_SENSOR_HANDLE_T sns_0;
    int sns_id = 0;
    CHECK_RET(CVI_MAPI_VCAP_InitSensor(&sns_0, sns_id, &vcap_attr));
    CHECK_RET(CVI_MAPI_VCAP_InitISP(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_StartISP(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_StartDev(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_StartPipe(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_StartChn(sns_0));

    CVI_MAPI_VCAP_SENSOR_HANDLE_T sns_1;
    int sns_id_1 = 1;
    CHECK_RET(CVI_MAPI_VCAP_InitSensor(&sns_1, sns_id_1, &vcap_attr));
    CHECK_RET(CVI_MAPI_VCAP_InitISP(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_StartISP(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_StartDev(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_StartPipe(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_StartChn(sns_1));
    ...
    CVI_MAPI_VPROC_ATTR_T vproc_attr = CVI_MAPI_VPROC_DefaultAttr_OneChn(
            IMAGE_WIDTH, IMAGE_HEIGHT, INAGE_FMT,
            CHN_0_WIDTH, CHN_0_HEIGHT, PIXEL_FORMAT_YUV_PLANAR_420);
    CHECK_RET(CVI_MAPI_VPROC_Init(&vproc_hdl, -1, &vproc_attr));
    CHECK_RET(CVI_MAPI_VPROC_BindVcap(vproc_hdl, sns_0));
    ...
    CHECK_RET(CVI_MAPI_VPROC_StartChnDump(vproc_hdl, chn_idx, 1, &stCallbackFun));
    CHECK_RET(CVI_MAPI_VPROC_StopChnDump(vproc_hdl, chn_idx));

    CHECK_RET(CVI_MAPI_VPROC_UnBindVcap(vproc_hdl, sns_0));
    CHECK_RET(CVI_MAPI_VPROC_Deinit(vproc_hdl));
    CHECK_RET(CVI_MAPI_VCAP_StopChn(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_StopDev(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_StopPipe(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_StopISP(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_DeInitISP(sns_0));
    CHECK_RET(CVI_MAPI_VCAP_DeinitSensor(sns_0));

    CHECK_RET(CVI_MAPI_VCAP_StopChn(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_StopDev(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_StopPipe(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_StopISP(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_DeInitISP(sns_1));
    CHECK_RET(CVI_MAPI_VCAP_DeinitSensor(sns_1));
...
int test_vproc_crop_rotate()
{
    ...
    CHECK_RET(CVI_MAPI_VPROC_SetChnAttrEx(vproc_hdl, chn_idx, CVI_VPROC_CMD_CHN_CROP,
        (CVI_VOID*)&stVpssCrop, sizeof(VPSS_CROP_INFO_S)));

    ROTATION_E enRotation = ROTATION_90;
    CHECK_RET(CVI_MAPI_VPROC_SetChnAttrEx(vproc_hdl, chn_idx, CVI_VPROC_CMD_CHN_ROTATE,
        (CVI_VOID*)&enRotation, sizeof(ROTATION_E)));

    CHECK_RET(CVI_MAPI_VPROC_BindVcap(vproc_hdl, sns_0));
    CHECK_RET(CVI_MAPI_DISP_BindVproc(videoOutCfg.dispCfg[0].dispHdl, vproc_hdl, chn_idx));
    sleep(10);

    CHECK_RET(CVI_MAPI_DISP_UnBindVproc(videoOutCfg.dispCfg[0].dispHdl, vproc_hdl, chn_idx));
    CHECK_RET(CVI_MAPI_VPROC_UnBindVcap(vproc_hdl, sns_0));
