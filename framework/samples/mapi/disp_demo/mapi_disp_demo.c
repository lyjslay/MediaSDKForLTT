...
static int test_disp_vo_rotate(const char *filename) {
    ...
    CHECK_RET(CVI_MAPI_GetFrameFromFile_YUV(&frame,
            DEMO_WIDTH, DEMO_HEIGHT, DEMO_FMT,
            filename, 0));
    CHECK_RET(CVI_MAPI_DISP_SendFrame(videoOutCfg.dispCfg[0].dispHdl, &frame));
    CHECK_RET(CVI_MAPI_ReleaseFrame(&frame));
...
static int test_disp_gdc_rotate(const char *filename) {
    ...
    uint64_t t0, t1;
    cvi_osal_get_boot_time(&t0);
    CHECK_RET(CVI_MAPI_IPROC_Rotate(&frame, &frame_rot, ROTATION_90));
    cvi_osal_get_boot_time(&t1);
    printf("GDC ROTATE use %.2f ms\n", (t1 - t0) / 1000.0f);
    CHECK_RET(CVI_MAPI_ReleaseFrame(&frame));

    CHECK_RET(CVI_MAPI_DISP_SendFrame(videoOutCfg.dispCfg[0].dispHdl, &frame_rot));
    CHECK_RET(CVI_MAPI_ReleaseFrame(&frame_rot));
...
{
    CVI_MAPI_VPROC_HANDLE_T vproc_hdl0 = NULL;
    CVI_MAPI_VPROC_HANDLE_T vproc_hdl1 = NULL;
    ...
    CHECK_RET(CVI_MAPI_VPROC_Init(&vproc_hdl0, -1, &vproc_attr));
    ...
    vproc_attr.attr_chn[0].stAspectRatio.stVideoRect.u32Width = DEMO_WIDTH/2;
    vproc_attr.attr_chn[0].stAspectRatio.stVideoRect.u32Height = DEMO_HEIGHT;
    CHECK_RET(CVI_MAPI_VPROC_Init(&vproc_hdl1, -1, &vproc_attr));

    CHECK_RET(DispOpen(1280, 720, ROTATION_90, false));

    VIDEO_FRAME_INFO_S frame0;
    VIDEO_FRAME_INFO_S frame1;
    CHECK_RET(CVI_MAPI_GetFrameFromFile_YUV(&frame0,
            MAX_WIDTH, MAX_HEIGHT, DEMO_FMT,
            filename1, 0));
    CHECK_RET(CVI_MAPI_GetFrameFromFile_YUV(&frame1,
            MAX_WIDTH, MAX_HEIGHT, DEMO_FMT,
            filename2, 0));

    CHECK_RET(CVI_MAPI_VPROC_SendFrame(vproc_hdl0, &frame0));
    int vproc_chn_id_0 = 0;
    VIDEO_FRAME_INFO_S frame_chn_0;
    CHECK_RET(CVI_MAPI_VPROC_GetChnFrame(vproc_hdl0, vproc_chn_id_0, &frame_chn_0));
    CHECK_RET(CVI_MAPI_VPROC_SendChnFrame(vproc_hdl1, vproc_chn_id_0, &frame_chn_0));
    CHECK_RET(CVI_MAPI_VPROC_ReleaseFrame(vproc_hdl0, vproc_chn_id_0, &frame_chn_0));

    VIDEO_FRAME_INFO_S frame_out;
    CHECK_RET(CVI_MAPI_VPROC_SendFrame(vproc_hdl1, &frame1));
    CHECK_RET(CVI_MAPI_VPROC_GetChnFrame(vproc_hdl1, vproc_chn_id_0, &frame_out));

    CHECK_RET(CVI_MAPI_DISP_SendFrame(videoOutCfg.dispCfg[0].dispHdl, &frame_out));
    CHECK_RET(CVI_MAPI_VPROC_ReleaseFrame(vproc_hdl1, vproc_chn_id_0, &frame_out));
    ...
    CHECK_RET(DispClose(videoOutCfg.dispCfg[0].dispHdl));
    CHECK_RET(CVI_MAPI_VPROC_Deinit(vproc_hdl1));
    CHECK_RET(CVI_MAPI_VPROC_Deinit(vproc_hdl0));
    TEST_PASS;
...
static void* disp_double_sensor(void* args)
{
    ...
    sys_attr.vb_pool[1].vb_blk_num = 4;
    sys_attr.vb_pool_num = 2;
    CHECK_RET_NULL(CVI_MAPI_Media_Init(&sys_attr));

    CVI_MAPI_VCAP_SENSOR_HANDLE_T sns[2];
    CVI_MAPI_VCAP_ATTR_T vcap_attr;
    CHECK_RET_NULL(CVI_MAPI_VCAP_GetGeneralVcapAttr(&vcap_attr));

    for (int i = 0; i < vcap_attr.u8DevNum; i++) {
        CHECK_RET_NULL(CVI_MAPI_VCAP_InitSensor(&sns[i], i, &vcap_attr));
        CHECK_RET_NULL(CVI_MAPI_VCAP_InitISP(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StartISP(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StartDev(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StartPipe(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StartChn(sns[i]));
    }
    ...
    vproc_attr.attr_chn[0].stAspectRatio.stVideoRect.u32Width = DEMO_WIDTH/2;
    vproc_attr.attr_chn[0].stAspectRatio.stVideoRect.u32Height = DEMO_HEIGHT;
    CHECK_RET_NULL(CVI_MAPI_VPROC_Init(&vproc_hdl0, -1, &vproc_attr));
    ...
    vproc_attr.attr_chn[0].stAspectRatio.stVideoRect.u32Width = DEMO_WIDTH/2;
    vproc_attr.attr_chn[0].stAspectRatio.stVideoRect.u32Height = DEMO_HEIGHT;
    CHECK_RET_NULL(CVI_MAPI_VPROC_Init(&vproc_hdl1, -1, &vproc_attr));
    ...
    while(!(*Stop))
    {
        CHECK_RET_NULL(CVI_MAPI_VCAP_GetFrame(sns[0], &frame0));
        CHECK_RET_NULL(CVI_MAPI_VPROC_SendFrame(vproc_hdl0, &frame0));
        CHECK_RET_NULL(CVI_MAPI_VCAP_ReleaseFrame(sns[0], &frame0));
        int vproc_chn_id_0 = 0;
        VIDEO_FRAME_INFO_S frame_chn_0;
        CHECK_RET_NULL(CVI_MAPI_VPROC_GetChnFrame(vproc_hdl0, vproc_chn_id_0, &frame_chn_0));
        CHECK_RET_NULL(CVI_MAPI_VPROC_SendChnFrame(vproc_hdl1, vproc_chn_id_0, &frame_chn_0));
        CHECK_RET_NULL(CVI_MAPI_VPROC_ReleaseFrame(vproc_hdl0, vproc_chn_id_0, &frame_chn_0));

        VIDEO_FRAME_INFO_S frame_out;
        CHECK_RET_NULL(CVI_MAPI_VCAP_GetFrame(sns[1], &frame1));
        CHECK_RET_NULL(CVI_MAPI_VPROC_SendFrame(vproc_hdl1, &frame1));
        CHECK_RET_NULL(CVI_MAPI_VCAP_ReleaseFrame(sns[1], &frame1));
        CHECK_RET_NULL(CVI_MAPI_VPROC_GetChnFrame(vproc_hdl1, vproc_chn_id_0, &frame_out));

        CHECK_RET_NULL(CVI_MAPI_DISP_SendFrame(videoOutCfg.dispCfg[0].dispHdl, &frame_out));
        CHECK_RET_NULL(CVI_MAPI_VPROC_ReleaseFrame(vproc_hdl1, vproc_chn_id_0, &frame_out));
    }

    CHECK_RET_NULL(DispClose(videoOutCfg.dispCfg[0].dispHdl));
    CHECK_RET_NULL(CVI_MAPI_VPROC_Deinit(vproc_hdl1));
    CHECK_RET_NULL(CVI_MAPI_VPROC_Deinit(vproc_hdl0));

    for (int i = 0; i < 2; i++) {
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopChn(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopDev(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopPipe(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopISP(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_DeInitISP(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_DeinitSensor(sns[i]));
    }
...
CVI_MAPI_WND_HANDLE_T wnd_hdl[2];
static void* disp_window_run(void* args)
{
    ...
    CVI_MAPI_VPROC_HANDLE_T vproc_hdl[2];

    CVI_MAPI_VPROC_ATTR_T vproc_attr = CVI_MAPI_VPROC_DefaultAttr_OneChn(
            MAX_WIDTH, MAX_HEIGHT, PIXEL_FORMAT_YUV_PLANAR_420,
            DEMO_WIDTH, DEMO_HEIGHT, PIXEL_FORMAT_YUV_PLANAR_420);
    CHECK_RET_NULL(CVI_MAPI_VPROC_Init(&vproc_hdl[0], -1, &vproc_attr));

    vproc_attr = CVI_MAPI_VPROC_DefaultAttr_OneChn(
            MAX_WIDTH, MAX_HEIGHT, PIXEL_FORMAT_YUV_PLANAR_420,
            MIN_WIDTH, MIN_HEIGHT, PIXEL_FORMAT_YUV_PLANAR_420);
    CHECK_RET_NULL(CVI_MAPI_VPROC_Init(&vproc_hdl[1], -1, &vproc_attr));

    for (int i = 0; i < 2; i++)
        CHECK_RET_NULL(CVI_MAPI_VPROC_BindVcap(vproc_hdl[i], sns[i]));

    CHECK_RET_NULL(DispOpen(1280, 720, ROTATION_90, true));

    CVI_MAPI_WND_ATTR_T wnd_attr0 = {0, 0, DEMO_WIDTH, DEMO_HEIGHT, 0};
    CHECK_RET_NULL(CVI_MAPI_DISP_CreateWindow(&wnd_hdl[0], 0, &wnd_attr0));

    CVI_MAPI_WND_ATTR_T wnd_attr1 = {0, 0, MIN_WIDTH, MIN_HEIGHT, 1};
    CHECK_RET_NULL(CVI_MAPI_DISP_CreateWindow(&wnd_hdl[1], 0, &wnd_attr1));
    ...
    while(!(*Stop))
    {
        ...
        for (int i = 0; i < 2; i++){
            CHECK_RET_NULL(CVI_MAPI_VPROC_GetChnFrame(vproc_hdl[i], vproc_chn_id_0, &frame_chn[i]));
            CHECK_RET_NULL(CVI_MAPI_DISP_SendWndFrame(wnd_hdl[i], &frame_chn[i]));
            //CHECK_RET_NULL(CVI_MAPI_VPROC_ReleaseFrame(vproc_hdl[i], vproc_chn_id_0, &frame_chn[i]));
        }
    }

    for (int i = 0; i < 2; i++)
        CHECK_RET_NULL(CVI_MAPI_DISP_DestroyWindow(wnd_hdl[i]));
    CHECK_RET_NULL(DispClose(videoOutCfg.dispCfg[0].dispHdl));
    for (int i = 0; i < 2; i++) {
        CHECK_RET_NULL(CVI_MAPI_VPROC_UnBindVcap(vproc_hdl[i], sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VPROC_Deinit(vproc_hdl[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopChn(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopDev(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopPipe(sns[i]));
        CHECK_RET_NULL(CVI_MAPI_VCAP_StopISP(sns[i]));
...
static void* change_window_pos_size(void* args)
{
    ...
    while(!(*Stop)){
        CVI_MAPI_WND_ATTR_T stWndAttr;
        CHECK_RET_NULL(CVI_MAPI_DISP_GetWndAttr(wnd_hdl[1], &stWndAttr));
        ...
        if(stWndAttr.wnd_y >= (DEMO_HEIGHT - stWndAttr.wnd_h))
            ...
            stWndAttr.wnd_y += 2;
        CHECK_RET_NULL(CVI_MAPI_DISP_SetWndAttr(wnd_hdl[1], &stWndAttr));
        sleep(1);
    }
    TEST_PASS;
    return NULL;
...
static void* switch_window(void* args)
{
    ...
    while(!(*Stop)){
        CVI_MAPI_WND_ATTR_T stWndAttr0, stWndAttr1;
        CHECK_RET_NULL(CVI_MAPI_DISP_GetWndAttr(wnd_hdl[0], &stWndAttr0));
        CHECK_RET_NULL(CVI_MAPI_DISP_GetWndAttr(wnd_hdl[1], &stWndAttr1));
        CHECK_RET_NULL(CVI_MAPI_DISP_SetWndAttr(wnd_hdl[1], &stWndAttr0));
        CHECK_RET_NULL(CVI_MAPI_DISP_SetWndAttr(wnd_hdl[0], &stWndAttr1));
        sleep(3);
    }
    TEST_PASS;
    return NULL;
