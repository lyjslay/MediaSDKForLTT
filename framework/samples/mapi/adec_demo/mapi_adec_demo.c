...
int main(int argc, char *argv[])
{
    ...
    CVI_MAPI_AUDIO_FRAME_INFO_S stAudFrmInfo = {0};
    ...
    ret = CVI_MAPI_AO_Init(&AoHdl,&stAoAttr);
    if(ret){
    ...
    if(stAdecAttr.enAdecFormat >= CVI_MAPI_AUDIO_CODEC_BUTT) {
    ...
    while(decoding) {
        ...
        ret = CVI_MAPI_ADEC_SendStream(AdecHdl, &stAudioStream, CVI_TRUE);
        if (ret != CVI_SUCCESS) {
            CVI_LOGE("CVI_MAPI_ADEC_SendStream send failed,ret:%d.\n",ret);
            break;
        }

        ret = CVI_MAPI_ADEC_GetFrame(AdecHdl, &stAudFrmInfo, CVI_FALSE);
        if (ret != CVI_SUCCESS) {
            CVI_LOGE("CVI_MAPI_ADEC_SendStream send failed,ret:%d.\n",ret);
            break;
        }
        ...
        CVI_MAPI_ADEC_ReleaseFrame(AdecHdl,&stAudFrmInfo);
    }
