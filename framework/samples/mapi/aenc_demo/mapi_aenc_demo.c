...
int  Aenc_GetEncodeDataCB(CVI_MAPI_AENC_HANDLE_T AencHdl,AUDIO_STREAM_S* pAStreamData, void *pPrivateData)
{
...
int main(int argc, char *argv[])
{
    ...
    CVI_MAPI_AENC_ATTR_S stAencAttr;
    ...
    CVI_MAPI_ACAP_ATTR_S stACapAttr;
    ...
    if(stAencAttr.enAencFormat >= CVI_MAPI_AUDIO_CODEC_BUTT) {
    ...
    stACapAttr.bVquOn = 1;
    stACapAttr.resample = 0;

    CVI_MAPI_ACAP_Init(&AcapHdl,&stACapAttr);

    CVI_MAPI_ACAP_Start(AcapHdl);
    CVI_MAPI_AENC_Init(&AencHdl,&stAencAttr);
    CVI_MAPI_AENC_Start(AencHdl);


    if(!audio_file) {
        CVI_MAPI_AENC_BindACap(0,0,0,0);
    }
    ...
    while(encoding && audio_file) { //not bind acap flow
        ...
        CVI_MAPI_AENC_SendFrame(AencHdl,&stFrame,&stAecFrm);
        CVI_MAPI_AENC_GetStream(AencHdl,&stStream,0);
    ...
    while(encoding && !audio_file) {
		...
		ret = CVI_MAPI_AENC_GetStream(AencHdl,&stStream,0);
		if (ret != CVI_SUCCESS) {
    ...
    if(!audio_file) {
        CVI_MAPI_AENC_UnbindACap(0,0,0,0);
    }
    ...
    fclose(audio_out);
    CVI_MAPI_AENC_Stop(AencHdl);
    CVI_MAPI_ACAP_Stop(AcapHdl);
    CVI_MAPI_AENC_Deinit(AencHdl);
    CVI_MAPI_ACAP_Deinit(AcapHdl);
    TEST_PASS;
    return 0;
}
