...
class MdatAtomParser final : public EventHandler<CviFileRecoverEvent>
{
public:
    ...
    int32_t parse(File& file);
    TrackPointer getTrack(TrackType type);
    void filetype(int32_t type);
...
private:
    ...
    bool loop_end(bool find_end, int32_t check_count);

private:
    AtomPointer mdat_atom;
    bool parsed_sps{false};
    double prev_dump_percentage{0};
    int32_t read_fail_count{0};
