...
void updateTrackHeader(const AtomPointer &track_atom, const TrackPointer& track, int32_t create_time);
...
int32_t  getFileCreateTime(const std::string &file_path);
void updateTrackSampleKeyframe(const AtomPointer &track_atom, const TrackPointer& track);

} // anonymous namespace

#define TYPE_MP4 (1)
#define TYPE_MOV (0)
#define DEFAULT_SPS_MAX_BUFFER_SIZE (22)
int32_t File_type = TYPE_MOV;
...
int32_t Mpeg4::parse()
{
    ...
    while (!file.eof()) {
        ...
        if (atom->isValid()) {
            ...
            CVI_LOGE("Atom %s is invalid", atom->getName().c_str());
            return -1;
        }
    }
...
int32_t Mpeg4::recover(const std::string &device_model, bool has_create_time, bool PreallocFlage, int32_t file_type)
{
    ...
    if (ret != 0) {
        CVI_LOGE("Can't open repair template file %s ret = %d", replair_template_path.c_str(), ret);
        return -1;
    }
    ...
    if (!repair_moov_atom || !repair_moov_atom->isValid()) {
        CVI_LOGE("Repair template is invalid");
        return -1;
    }
...
void Mpeg4::save(const string &file_path)
{
    ...
    if (file_path == this->file_path) {
        ...
        for (const string& atom_name : need_update_atoms_name) {
            ...
            if (atom->getName() == ATOM_MOOV_NAME) {
                AtomPointer mdatatom = getAtomByName(ATOM_MDAT_NAME);
                if (TYPE_MP4 != File_type) {
                    // mov：ftyp size:20 wide size:8
                    file.seek(mdatatom->getMdatLength() + 8 + 28);
                } else if (TYPE_MP4 == File_type) {
                    // mp4:ftyp size:32 free size:8
                    file.seek(mdatatom->getMdatLength() + 8 + 40);
                }
            } else {
...
void Mpeg4::saveAtomByName(const std::string &file_path, const std::string &atom_name)
{
    ...
    if (atom) {
        ...
        CVI_LOGE("Can't save atom %s, because not find this atom", atom_name.c_str());
    }
}
...
void Mpeg4::updateMvhd(const TrackPointer &track, int32_t create_time)
{
...
void Mpeg4::updateTrack(const TrackPointer& track, int32_t create_time)
{
    ...
    auto iterator = std::find_if(track_atoms.begin(), track_atoms.end(), [&track](const AtomPointer &atom) {
        ...
        if (tkhd_atom) {
            int32_t track_id = tkhd_atom->read<int32_t>(track_id_offset);
    ...
    if (iterator == track_atoms.end()) {
        CVI_LOGE("Can't find match track");
        return;
    }
    ...
    if (track->getType() == TrackType::Video) {
        updateTrackSampleKeyframe(track_atom, track);
    }
    // remove edts atom
...
void Mpeg4::updateUdta(const std::string &device_model)
{
    ...
    if (udta_atom) {
        ...
        udta_atom->resize(0 +  // entries
                          title_size +
                          25); // sample for chunk
        udta_atom->write(title_size, 0);
        udta_atom->write(static_cast<uint32_t>(0xA96E616D), 4);
        udta_atom->write(static_cast<uint16_t>(device_model.length()), 8);
        udta_atom->write(static_cast<uint16_t>(0x55C4), 10);
        udta_atom->write(const_cast<char*>(device_model.c_str()), device_model.length(), 12);
        udta_atom->write(static_cast<uint32_t>(0x00000019), 12 + device_model.length());
        udta_atom->write(static_cast<uint32_t>(0xA9737772), 12 + device_model.length() + 4);
        udta_atom->write(static_cast<uint32_t>(0x000D55C4), 12 + device_model.length() + 8);
        udta_atom->write(static_cast<uint32_t>(0x4C617666), 12 + device_model.length() + 12);
        udta_atom->write(static_cast<uint32_t>(0x35382E35), 12 + device_model.length() + 16);
        udta_atom->write(static_cast<uint32_t>(0x382E3130), 12 + device_model.length() + 20);
        udta_atom->write(static_cast<uint8_t>(0x30), 12 + device_model.length() + 24);
    } else {
        CVI_LOGE("Can't find %s atom in track", ATOM_UDTA_NAME);
    }
}
...
void updateTrackHeader(const AtomPointer &track_atom, const TrackPointer& track, int32_t create_time)
{
    ...
    if (tkhd_atom) {
        ...
        CVI_LOGE("Can't find %s atom in track", ATOM_TKHD_NAME);
    }
}
...
void updateTrackMediaHeader(const AtomPointer &track_atom, const TrackPointer& track)
{
    ...
    if (mdhd_atom) {
        ...
        CVI_LOGE("Can't find %s atom in track", ATOM_MDHD_NAME);
    }
}
...
void updateTrackSampleTimeTable(const AtomPointer &track_atom, const TrackPointer& track)
{
    ...
    if (stts_atom) {
        ...
        CVI_LOGE("Can't find %s atom in track", ATOM_STTS_NAME);
    }
}
...
void updateTrackSampleChunk(const AtomPointer &track_atom, const TrackPointer& track)
{
    ...
    if (stsc_atom) {
        ...
        CVI_LOGE("Can't find %s atom in track", ATOM_STSC_NAME);
    }
}

void updateTrackSampleKeyframe(const AtomPointer &track_atom, const TrackPointer& track)
{
    AtomPointer stss_atom = track_atom->getAtomByName(ATOM_STSS_NAME);
    if (stss_atom) {
        if (track->getType() == TrackType::Video) {
            constexpr uint64_t entries_offset = 4;
            uint32_t I_count = 0;
            ...
            stss_atom->resize(4 +
                                4 +
                                I_count * 4);
            stss_atom->write(static_cast<uint32_t>(I_count), entries_offset);

            uint64_t offset = 8;

            for (const TrackSample &sample : samples) {
                if (sample.naltype == 7) {
                    stss_atom->write(static_cast<uint32_t>(sample.g_count), offset);
                    offset += 4;
                }
            }
        }
    } else {
        CVI_LOGE("Can't find %s atom in track", ATOM_STSS_NAME);
    }
}
...
void updateTrackSampleSizeTable(const AtomPointer &track_atom, const TrackPointer& track)
{
    ...
    if (stsz_atom) {
        ...
        CVI_LOGE("Can't find %s atom in track", ATOM_STSZ_NAME);
    }
}
...
void updateTrackSampleOffsetTable(const AtomPointer &track_atom, const TrackPointer& track)
{
    ...
    if (stco_atom) {
        ...
        CVI_LOGE("Can't find %s atom in track", ATOM_STCO_NAME);
    }
}
    ...
    if (stsd_atom) {
        ...
        if (video_track->getCodec() == Codec::H264) {
            ...
            if (1280 == (frame_size.width) && 720 == (frame_size.height)) {
                ...
                if (1920 != (frame_size.width) && 1080 != (frame_size.height)) {
                    if (sps_buffer.size() > DEFAULT_SPS_MAX_BUFFER_SIZE) {
                        stsd_atom->write(static_cast<uint16_t>(sps_buffer.size() + 1), sps_offset - 3);
                        stsd_atom->write(static_cast<uint16_t>(50), sps_offset - 7);
                        stsd_atom->write(static_cast<uint8_t>(0), sps_offset + sps_buffer.size());
                    } else {
                        stsd_atom->write(static_cast<uint16_t>(23), sps_offset - 3);
                        stsd_atom->write(static_cast<uint16_t>(50), sps_offset - 7);
                    }
            ...
            if (sps_buffer.size() == 20) {
                for (uint32_t m = 0; m < sps_buffer.size(); m++) {
                    stsd_atom->write(sps_buffer[m], sps_offset + m);
                }
            }
