...
constexpr int32_t READ_DATA_LENGTH = 300;
constexpr int32_t MAX_READ_FAIL_COUNT = 10;
//constexpr int32_t MAX_AUDIO_CHECK_COUNT = 300;
constexpr uint16_t MAX_AUDIO_AMPLITUDE = 10000;
constexpr int32_t MAX_VIDEO_PARAMETER_SET_LENGTH = 1000;
constexpr array<uint32_t, 1> AUDIO_SAMPLE_RATES = {1280};
constexpr array<uint32_t, 1> AUDIO_SAMPLE_RATES_MP4 = {1};
constexpr int32_t SUBTITLE_HEADER_LENGTH = 2;
constexpr int32_t SUBTITLE_DATA_LENGTH = 200;
constexpr int32_t SUBTITLE_TOTAL_LENGTH = SUBTITLE_DATA_LENGTH + SUBTITLE_HEADER_LENGTH;
constexpr int32_t SKIP_BYTE_THRESHOLD = 2000;
constexpr int32_t SKIP_BYTE_HARD_THRESHOLD = 30000;
constexpr int32_t RECOVER_PERCENTAGE_THRESHOLD = 80;
constexpr int32_t THUMBNAIL_MAX_SEARCH_BYTES = 10000000;
constexpr int32_t RECOVER_PERCENTAGE_OVERTIME = 30;
#define CVI_CLOCKS_PER_SEC (10000000)
#define TYPE_MP4 (1)
#define TYPE_MOV (0)
static int32_t file_type = TYPE_MOV;

typedef struct VideoTrackSample
{
    uint64_t offset;
    uint32_t size;
    uint32_t naltype;
...
void MdatAtomParser::filetype(int32_t type)
{
    file_type = type;
    return;
}

int32_t MdatAtomParser::parse(File& file)
{
    reset();
...
/*     bool skipflage = false;
    ...
    clock_t start;
    start = clock();
    int32_t v_count = 0, a_count = 0, s_count = 0;
    uint32_t naltype = 1;
    uint64_t Mdat_offset = 0;

    VideoTrackSample video_track_info;
    memset(&video_track_info, 0, sizeof(VideoTrackSample));
    ...
    while (readDataFromFile(file, offset, READ_DATA_LENGTH, data)) {
        ...
        while (offset + 6 < read_end) {
            ...
            if (isThumbnailPacket(data, data_offset, file, offset, packet_length)) {
                ...
                offset += packet_length;
                s_count += 1;
            } else if (isVideoPacket(data, data_offset, file, offset, packet_length, naltype)) {
                if (v_count == 0) {
                    video_track_info.offset = offset + mdat_atom->getContentStartPos();
                    video_track_info.size = packet_length;
                    video_track_info.naltype = naltype;
                    video_track_info.g_count = uint32_t(v_count + 1);
                } else {
                    video_track->pushSample(TrackSample {
                        .offset = video_track_info.offset,
                        .size = video_track_info.size,
                        .naltype = video_track_info.naltype,
                        .g_count = video_track_info.g_count
                    });
                    Mdat_offset = video_track_info.offset;
...
*/
                ...
                if((clock() - start) >= CVI_CLOCKS_PER_SEC) {
...
END:
    ...
    if (g_preallocatestate) {
        ...
        mdat_atom->setMdatLength(offset);
    }
    dumpSummary(offset, total_skip_bytes);
    file.clear();
    ...
    CVI_LOGI("v_count = %d, a_count = %d, s_count = %d, offset = %"PRIu64", Mdat_offset == %"PRIu64", total_skip_bytes = %"PRIu64"",
              v_count, a_count, s_count, offset, Mdat_offset, total_skip_bytes);
    return 0;
}
...
void MdatAtomParser::dumpProgress(uint64_t current_pos, uint64_t total_skip_bytes)
{
    ...
    if (!mdat_atom->getpreallocatestate()) {
        ...
        if ((percentage - prev_dump_percentage) > dump_percentage_diff) {
            CVI_LOGI("Parse progress ... %"PRIu64"/%"PRIu64" (%.1lf%%), total_skip_bytes=[%"PRIu64"]",
                current_pos, total_data_length, percentage, total_skip_bytes);
            prev_dump_percentage = percentage;
        }

        handleEvent(CviFileRecoverEvent {
            .type = CVI_FILE_RECOVER_EVENT_RECOVER_PROGRESS,
            .value = percentage
        });
    } else {
        handleEvent(CviFileRecoverEvent {
...
void MdatAtomParser::dumpSummary(uint64_t parsed_pos, uint64_t total_skip_bytes)
{
    if (!mdat_atom->getpreallocatestate()) {
        ...
        CVI_LOGI("Parse finish at pos %"PRIu64"/%"PRIu64" (%.1lf%%)", parsed_pos, total_data_length, percentage);
    }
}
        ...
        File& file, uint64_t start_pos, uint32_t &packet_length)
{
    ...
    while (readDataFromFile(file, offset, READ_DATA_LENGTH, check_buffer)) {
        ...
        if (offset - start_pos > THUMBNAIL_MAX_SEARCH_BYTES) {
            CVI_LOGI("Thumbnail search over [%d] bytes", THUMBNAIL_MAX_SEARCH_BYTES);
            break;
        }
    }
...
bool MdatAtomParser::isVideoPacket(const DataBuffer &readed_data, uint64_t data_offset,
        File& file, uint64_t start_pos, uint32_t &packet_length, uint32_t &nal_type)
{
    ...
    if (isNaluStart(codec, &readed_data[data_offset])) {
        ...
        nal_type = getNalType(codec, readed_data[data_offset+NAL_HEADER_START_POS]);
...
bool MdatAtomParser::loop_end(bool find_end, int32_t check_count)
{
    int32_t MAX_AUDIO_CHECK_COUNT = 30;
    if (TYPE_MP4 == file_type) {
    ...
    while (!loop_end(find_end, check_count)) {
        if (TYPE_MP4 == file_type) {
            ...
            for (uint32_t sampe_rates : AUDIO_SAMPLE_RATES_MP4) {
                ...
                uint32_t naltype;
                if (isVideoPacket(check_buffer, 0, file, read_pos, dummy_length, naltype) ||
...
bool MdatAtomParser::parseSps(File& file, uint64_t offset)
{
    ...
    Codec codec = video_track->getCodec();
    int32_t sps_header_length = (codec == Codec::H265) ? H265_NAL_HEADER_LENGTH : H264_NAL_HEADER_LENGTH;
    int32_t sps_start_offset = NAL_HEADER_START_POS + sps_header_length;
