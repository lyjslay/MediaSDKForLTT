...
public:
    ...
    int32_t open(const std::string& file_path,
             std::ios_base::openmode mode = std::fstream::in | std::fstream::out | std::fstream::binary)
    {
        file.open(file_path, mode);
        if (!isOpened()) {
            CVI_LOGE("Open file %s failed", file_path.c_str());
            file.close();
            return -1;
        }
    ...
    std::vector<unsigned char> read(size_t size)
    {
        ...
        if (read_length != size) {
            CVI_LOGE("read() Could not read size %zd, real read %zd", size, read_length);
    ...
    std::string readString(size_t size)
    {
        ...
        size_t read_length = file.gcount();
        if (read_length != size) {
            CVI_LOGE("readString() Could not read size %zd, real read %zd", size, read_length);
        }
        value[size] = '\0';
