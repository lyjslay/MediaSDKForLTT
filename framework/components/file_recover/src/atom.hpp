...
public:
    ...
    void dump(int32_t depth = 0) const;
    void save(File& file);
    ...
    void Filetype(int32_t FileFlage);
    void setLength(uint64_t setlength);
    bool getpreallocatestate();
    void setMdatLength(uint64_t setlength);
    uint64_t getMdatLength() const;

private:
    int32_t parseHeader(File &file);
