...
bool SPSParser::parse(Codec codec, const vector<uint8_t>& buffer)
{
    try {
        ...
        CVI_LOGE("Parse exception %s", e.what());
        return false;
    }
...
{
    int32_t profile_idc = readBits(8);
    ...
    {
        int32_t chroma_format_idc = readExponentialGolombCode();
        ...
        readBit();
        int32_t seq_scaling_matrix_present_flag = readBit();
        if (seq_scaling_matrix_present_flag) {
            for (int32_t i = 0; i < ((chroma_format_idc != 3)?8:12); ++i) {
                int32_t seq_scaling_list_present_flag = readBit();
    ...
    readExponentialGolombCode();
    int32_t pic_order_cnt_type = readExponentialGolombCode();
    if (pic_order_cnt_type == 0) {
        ...
        readSE();
        int32_t num_ref_frames_in_pic_order_cnt_cycle = readExponentialGolombCode();
    ...
    readBit();
    int32_t pic_width_in_mbs_minus1 = readExponentialGolombCode();
    int32_t pic_height_in_map_units_minus1 = readExponentialGolombCode();
    int32_t frame_mbs_only_flag = readBit();
    ...
    int32_t frame_cropping_flag = readBit();
    ...
    int32_t vui_parameters_present_flag = readBit();
    if (vui_parameters_present_flag) {
        // aspect_ratio_info_present_flag
        int32_t aspect_ratio_info_present_flag = readBit();
        if (aspect_ratio_info_present_flag) {
            int32_t aspect_ratio_idc = readBits(8);
        ...
        int32_t overscan_info_present_flag = readBit();
        if (overscan_info_present_flag) {
            readBit();       //overscan_appropriate_flag
        }

        int32_t video_signal_type_present_flag = readBit();
        if (video_signal_type_present_flag) {
            readBits(3);      //video_format
            readBit();       //video_full_range_flag
            int32_t colour_description_present_flag = readBit();
        ...
        int32_t chroma_loc_info_present_flag = readBit();
        ...
        int32_t timing_info_present_flag = readBit();
        if (timing_info_present_flag) {
            int32_t num_units_in_tick = readBits(32);
            int32_t time_scale = readBits(32);
            int32_t fixed_frame_rate_flag = readBit();
            float fps;
...
void SPSParser::parseH265()
{
    ...
    int32_t max_sub_layers_minus1 = readBits(3);
    ...
    int32_t pic_width_in_luma_samples = readExponentialGolombCode();
    int32_t pic_height_in_luma_samples = readExponentialGolombCode();
    frame_info.width = pic_width_in_luma_samples;
    frame_info.height = pic_height_in_luma_samples;
}

void SPSParser::parseH265TierProfile(int32_t max_sub_layers_minus1)
{
    ...
    std::vector<int32_t> sub_layer_level_present_flag;
    for (int32_t i = 0; i < max_sub_layers_minus1; ++i) {
    ...
    if (max_sub_layers_minus1 > 0) {
        for (int32_t i = max_sub_layers_minus1; i < 8; ++i) {
            // reserved_zero_2bits
            readBits(2);
        }
    }
...
int32_t SPSParser::getBits(uint64_t bits_offset, uint32_t n)
{
    uint64_t original_bit_pos = current_bit;
    current_bit += bits_offset;
    int32_t value = readBits(n);
    current_bit = original_bit_pos;

    return value;
}

int32_t SPSParser::readBit()
{
...
int32_t SPSParser::readBits(uint32_t n)
{
    if (n > (sizeof(int32_t)*ONE_BYTE_BITS)) {
...
int32_t SPSParser::readExponentialGolombCode()
{
    int32_t value = 0;
    uint32_t count = 0;
...
int32_t SPSParser::readSE()
{
    int32_t value = readExponentialGolombCode();
    if (value & 0x01) {
        value = (value + 1)/2;
    } else {
        value = -(value/2);
