...
constexpr int32_t ATOM_NAME_SIZE = 4;
constexpr int32_t ATOM_HEADER_SIZE = 8;
constexpr int32_t ATOM_CONTENT_OFFSET = 8;
static bool PREALL_OCATE = true; // PREALL OCATE
static int32_t File_Type = 0;
...
void Atom::Filetype(int32_t FileFlage)
{
    File_Type = FileFlage;
}

int32_t Atom::parseHeader(File &file)
{
    start_pos = file.pos();
...
void Atom::parseContent(File &file)
{
    ...
    if (type == AtomType::UNKNOWN) {
        CVI_LOGD("Skip parse %s content", name.c_str());
...
void Atom::dump(int32_t depth) const
{
    if (!isRoot()) {
        string atom_indent = "";
        ...
        CVI_LOGI("%s %s (length: %"PRIu64")", atom_indent.c_str(), getName().c_str(), getLength());
    }
...
void Atom::save(File& file)
{
    if ((!isRoot() && isValid())) {
        if (utils::getAtomTypeFromName(name) == AtomType::UNKNOWN) {
            CVI_LOGE("unrecognized atom name: [%s]", name.c_str());
        } else {
            if (length > UINT32_MAX) {
                ...
                uint64_t atomlength = 8;
                if (name == ATOM_MDAT_NAME) {
                    atomlength = mdatlength + 8;
                } else {
                    atomlength = length;
                }
                file.write<uint32_t>(atomlength);
                file.writeString(name);
            }
...
void Atom::setMdatLength(uint64_t setlength)
{
    mdatlength = setlength;
}

uint64_t Atom::getMdatLength() const
{
    return mdatlength;
}
...
{
    if ((start_pos >= 0) && (!name.empty()) && (length > 0)) {
        if (utils::getAtomTypeFromName(name) == AtomType::UNKNOWN) {
