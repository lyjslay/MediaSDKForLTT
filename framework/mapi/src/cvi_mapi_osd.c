...
#include "cvi_sys.h"
#include "cvi_mapi_osd.h"
#include "cvi_log.h"
#include "cvi_mapi_fontmod.h"
...
typedef struct tagOSD_PARAM_S {
    CVI_MAPI_OSD_ATTR_S stAttr;
    SIZE_S stMaxSize;
    pthread_mutex_t mutexLock;
    bool bInit; /*   OSD Attribute Set or not, Canbe modified only CVI_MAPI_OSD_SetAttr   */
    bool bStart;   /*   OSD start/stop Flag, Canbe modified only by CVI_MAPI_OSD_Start/CVI_MAPI_OSD_Stop   */
    char szStr[CVI_MAPI_OSD_MAX_STR_LEN];
    RGN_TYPE_E enRgnType;
} OSD_PARAM_S;

/*  * OSD Time Update Runing Flag
  Canbe modified only by CVI_PDT_OSD_Init/CVI_PDT_OSD_DeInit   */
...
/*   OSD Fonts Lib, inited by CVI_PDT_OSD_Init   */
static CVI_MAPI_OSD_FONTS_S g_stOsdFonts;

/*   OSD Parameter Array   */
static OSD_PARAM_S g_astOSDParam[CVI_MAPI_OSD_MAX_CNT];

/*   Time OSD Update Task Thread ID, created by CVI_PDT_OSD_Init, destroyed by CVI_MAPI_OSD_DeInit   */
static pthread_t g_pOsdTimeTskId = 0;

static int CVI_OSD_GetFontMod(CVI_CHAR* Character,uint8_t** FontMod,int* FontModLen)
{
...
static void OSD_GetTimeStr(const struct tm* pstTime, CVI_MAPI_OSD_TIMEFMT_E enFmt, char* pazStr, int s32Len)
{
    /*   Get Time   */
    time_t nowTime;
    struct tm stTime = {
    ...
    switch (enFmt) {
        case CVI_MAPI_OSD_TIMEFMT_YMD24H:
        default:
    ...
    switch (stChn.enModId) {
        ...
        case CVI_ID_VPSS: {
            ...
            if (s32Ret != CVI_MAPI_SUCCESS) {
                CVI_LOGE("CVI_VPSS_GetChnAttr(%d, %d) fail,Error Code: [0x%08X]\n",
        ...
        case CVI_ID_VO: {
            ...
            if (s32Ret != CVI_MAPI_SUCCESS) {
                CVI_LOGE("CVI_VO_GetChnAttr(%d,%d) fail,Error Code: [0x%08X]\n",
                    stChn.s32DevId, stChn.s32ChnId, s32Ret);
                return s32Ret;
            }
...
static int OSD_UpdateDisplay(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_ATTR_S* pstAttr)
{
    int s32Ret;
    uint32_t u32DispIdx;
    ...
    for (u32DispIdx = 0; u32DispIdx < pstAttr->u32DispNum; ++u32DispIdx) {
        ...
        switch (pstAttr->astDispAttr[u32DispIdx].enBindedMod) {
            case CVI_MAPI_OSD_BINDMOD_VPROC:
                stChn.enModId = CVI_ID_VPSS;
                break;

            case CVI_MAPI_OSD_BINDMOD_DISP:
        ...
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_RGN_GetDisplayAttr fail,RgnHdl[%d] stChn[%d,%d,%d] Error Code: [0x%08X]\n",
        ...
        if (pstAttr->astDispAttr[u32DispIdx].enCoordinate == CVI_MAPI_OSD_COORDINATE_RATIO_COOR) {
        ...
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_RGN_SetDisplayAttr fail,RgnHdl[%d] stChn[%d,%d,%d] Error Code: [0x%08X]\n",
                RgnHdl, stChn.enModId, stChn.s32DevId, stChn.s32ChnId, s32Ret);
            return s32Ret;
        }
    }
...
static int OSD_UpdateTextBitmap(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_CONTENT_S* pstContent)
{
    int s32Ret;
    uint32_t u32CanvasWidth,u32CanvasHeight,u32BgColor;
    SIZE_S stFontSize;
    OSD_PARAM_S *pstOsdParam = &g_astOSDParam[RgnHdl];
    int s32StrLen = strnlen(pstOsdParam->szStr, CVI_MAPI_OSD_MAX_STR_LEN);
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_RGN_GetCanvasInfo fail,RgnHdl[%d] Error Code: [0x%08X]\n", RgnHdl, s32Ret);
        return s32Ret;
    }

    if (pstContent->enType == CVI_MAPI_OSD_TYPE_OBJECT){
        ...
        if (pstContent->enType == CVI_MAPI_OSD_TYPE_STRING) {
            u32CanvasWidth =
        ...
        for (u32BmRow = 0; u32BmRow < u32CanvasHeight; ++u32BmRow) {
            ...
            for (u32BmCol = 0; u32BmCol < u32CanvasWidth; ++u32BmCol) {
                ...
                int FontModLen = 0;
                if (CVI_MAPI_SUCCESS == CVI_OSD_GetFontMod(&pstOsdParam->szStr[s32StringIdx], &FontMod, &FontModLen)) {
...
static void* OSD_TimeUpdateThread(void* pvParam)
{
    ...
    while (g_bOsdTimeRun) {
        ...
        if (nowTime == lastTime) {
            ...
            localtime_r(&nowTime, &stTime);
            for (s32OsdIdx = 0; s32OsdIdx < CVI_MAPI_OSD_MAX_CNT; ++s32OsdIdx) {
                pthread_mutex_lock(&g_astOSDParam[s32OsdIdx].mutexLock);
                if (g_astOSDParam[s32OsdIdx].stAttr.stContent.enType == CVI_MAPI_OSD_TYPE_TIME &&
                    ...
                    OSD_GetTimeStr(&stTime, g_astOSDParam[s32OsdIdx].stAttr.stContent.stTimeContent.enTimeFmt,
                                    g_astOSDParam[s32OsdIdx].szStr, CVI_MAPI_OSD_MAX_STR_LEN);
...
static int OSD_CheckAttr(int s32OsdIdx, CVI_MAPI_OSD_ATTR_S* pstAttr)
{
    if(s32OsdIdx < 0 || s32OsdIdx > CVI_MAPI_OSD_MAX_CNT
            || pstAttr->u32DispNum > CVI_MAPI_OSD_MAX_DISP_CNT
            || pstAttr->stContent.enType >= CVI_MAPI_OSD_TYPE_BUTT) {
        CVI_LOGE("OSD attr param is error ! \n");
        return CVI_MAPI_ERR_FAILURE;
    }
    uint32_t i;
    for (i = 0; i < pstAttr->u32DispNum; i++) {
        if(pstAttr->astDispAttr[i].enBindedMod >= CVI_MAPI_OSD_BINDMOD_BUTT || \
            pstAttr->astDispAttr[i].enCoordinate > CVI_MAPI_OSD_COORDINATE_ABS_COOR) {
            CVI_LOGE("OSD astDispAttr is error ! \n");
            return CVI_MAPI_ERR_FAILURE;
        }
    }

    if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_TIME) {
        if(pstAttr->stContent.stTimeContent.enTimeFmt >= CVI_MAPI_OSD_TIMEFMT_BUTT) {
           CVI_LOGE("TimeFmt is error ! \n");
           return CVI_MAPI_ERR_FAILURE;
        }
    } else if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_BITMAP) {
...
static void OSD_GetMaxSize(CVI_MAPI_OSD_ATTR_S* pstAttr, SIZE_S* pstOsdSize)
{
    uint32_t u32StrLen = strnlen(pstAttr->stContent.stStrContent.szStr, CVI_MAPI_OSD_MAX_STR_LEN);
    uint32_t u32Width;
    uint32_t u32Height;

    if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_BITMAP) {
        u32Width = pstAttr->stContent.stBitmapContent.u32Width;
        u32Height = pstAttr->stContent.stBitmapContent.u32Height;
    } else if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_STRING) {
...
static bool OSD_CheckIfRebuild(int s32OsdIdx, CVI_MAPI_OSD_ATTR_S* pstAttr, SIZE_S* pstOsdMaxSize)
{
    OSD_PARAM_S *pstOsdParam = &g_astOSDParam[s32OsdIdx];
    /* return TRUE means go rebuild process */
    if ((pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_BITMAP) ||
        (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_STRING) ||
        (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_TIME)) {
...
static int OSD_DetachRgn(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_DISP_ATTR_S* pstDispAttr)
{
    int s32Ret;
    MMF_CHN_S stChn;
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_RGN_DetachFromChn fail,RgnHdl[%d] stChn[%d,%d,%d] Error Code: [0x%08X]\n",
            RgnHdl, stChn.enModId, stChn.s32DevId, stChn.s32ChnId, s32Ret);
        return s32Ret;
    }
...
static int OSD_DestroyRgn(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_ATTR_S* pstAttr)
{
    int s32Ret;
    uint32_t u32DispIdx;
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_RGN_Destroy fail,RgnHdl[%d] Error Code: [0x%08X]\n", RgnHdl, s32Ret);
        return s32Ret;
    }
...
static void OSD_GetRgnAttr(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_ATTR_S* pstAttr, RGN_ATTR_S* pstRgnAttr)
{

    SIZE_S stRgnSize = {0};
    OSD_PARAM_S *pstOsdParam = &g_astOSDParam[RgnHdl];
    uint32_t u32StrLen = strnlen(pstOsdParam->szStr, CVI_MAPI_OSD_MAX_STR_LEN);
    ...
    if(pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_BITMAP) {
    ...
    } else if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_TIME) {
        ...
        pstRgnAttr->enType = OVERLAY_RGN;
    } else if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_OBJECT) { // new
    ...
    if (pstRgnAttr->enType == OVERLAY_RGN) {
        ...
        pstRgnAttr->unAttr.stOverlay.u32CanvasNum = (CVI_MAPI_OSD_TYPE_BITMAP == pstAttr->stContent.enType) ? 1 : 2;
        if(pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_OBJECT){
        ...
        pstRgnAttr->unAttr.stOverlayEx.bFlip = pstAttr->bFlip;
        pstRgnAttr->unAttr.stOverlayEx.bMirror = pstAttr->bMirror;
    }
    pstOsdParam->enRgnType = pstRgnAttr->enType;
    return;
}

static int OSD_UpdateRgnContent(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_ATTR_S* pstAttr)
{
    int s32Ret = CVI_MAPI_SUCCESS;
    if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_BITMAP) {
        BITMAP_S stBitmap;
        ...
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_RGN_SetBitMap fail,RgnHdl[%d] Error Code: [0x%08X]\n", RgnHdl, s32Ret);
            return s32Ret;
        }
    } else if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_TIME ||
        pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_STRING ||
        pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_OBJECT) {
...
static int OSD_AttachRgn(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_DISP_ATTR_S* pstDispAttr,
    const CVI_MAPI_OSD_CONTENT_S* pstOsdContent)
{
    int s32Ret = CVI_MAPI_SUCCESS;
    RGN_CHN_ATTR_S stRgnChnAttr;
    MMF_CHN_S stChn;
    ...
    if (pstDispAttr->enCoordinate == CVI_MAPI_OSD_COORDINATE_RATIO_COOR) {
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_RGN_AttachToChn fail,RgnHdl[%d] stChn[%d,%d,%d] Error Code: [0x%08X]\n",
                       RgnHdl, stChn.enModId, stChn.s32DevId, stChn.s32ChnId, s32Ret);
        return s32Ret;
    }
    return CVI_MAPI_SUCCESS;
...
static int OSD_CreateRgn(RGN_HANDLE RgnHdl, const CVI_MAPI_OSD_ATTR_S* pstAttr)
{
    int s32Ret;
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_RGN_Create fail,RgnHdl[%d] Error Code: [0x%08X]\n", RgnHdl, s32Ret);
        return s32Ret;
    }
...
static int OSD_Start(int s32OsdIdx)
{
    ...
    if (pstOsdParam->stAttr.stContent.enType == CVI_MAPI_OSD_TYPE_TIME) {
...
int CVI_MAPI_OSD_Init(const CVI_MAPI_OSD_FONTS_S* pstFonts)
{
    ...
    if (pstFonts != NULL) {
        ...
        memcpy(&g_stOsdFonts, pstFonts, sizeof(CVI_MAPI_OSD_FONTS_S));
    } else {
        memset(&g_stOsdFonts, 0, sizeof(CVI_MAPI_OSD_FONTS_S));
    }

    /*   Init OSD Param   */
    for (s32Idx = 0; s32Idx < CVI_MAPI_OSD_MAX_CNT; ++s32Idx) {
        pthread_mutex_init(&g_astOSDParam[s32Idx].mutexLock, NULL);
        pthread_mutex_lock(&g_astOSDParam[s32Idx].mutexLock);
        memset(&g_astOSDParam[s32Idx].stAttr, 0, sizeof(CVI_MAPI_OSD_ATTR_S));
        ...
        memset(&g_astOSDParam[s32Idx].szStr, 0, sizeof(char) * CVI_MAPI_OSD_MAX_STR_LEN);
...
int CVI_MAPI_OSD_SetAttr(int s32OsdIdx, CVI_MAPI_OSD_ATTR_S* pstAttr)
{
    ...
    if (pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_STRING) {
        memset(pstOsdParam->szStr, 0, CVI_MAPI_OSD_MAX_STR_LEN);
        snprintf(pstOsdParam->szStr, CVI_MAPI_OSD_MAX_STR_LEN, "%s", pstAttr->stContent.stStrContent.szStr);
    } else if(pstAttr->stContent.enType == CVI_MAPI_OSD_TYPE_TIME) {
    ...
    memcpy(&pstOsdParam->stAttr, pstAttr, sizeof(CVI_MAPI_OSD_ATTR_S));
...
{
    if(g_bOsdInitFlg != true || s32OsdIdx < 0 || s32OsdIdx > CVI_MAPI_OSD_MAX_CNT || pstAttr == NULL) {
        CVI_LOGE("CVI_MAPI_OSD_ATTR_S is error!\n");
        return CVI_MAPI_ERR_FAILURE;
    }
    ...
    memcpy(pstAttr, &pstOsdParam->stAttr, sizeof(CVI_MAPI_OSD_ATTR_S));
    pthread_mutex_unlock(&pstOsdParam->mutexLock);
    return CVI_MAPI_SUCCESS;
}
...
{
    if(g_bOsdInitFlg != true || s32OsdIdx < 0 || s32OsdIdx > CVI_MAPI_OSD_MAX_CNT) {
        CVI_LOGE("CVI_MAPI_OSD_Start is error!\n");
        return CVI_MAPI_ERR_FAILURE;
    }
    int s32Ret;
    OSD_PARAM_S *pstOsdParam = &g_astOSDParam[s32OsdIdx];
...
{
    if(g_bOsdInitFlg != true || s32OsdIdx < 0 || s32OsdIdx > CVI_MAPI_OSD_MAX_CNT) {
        CVI_LOGE("CVI_MAPI_OSD_Stop is error!\n");
        return CVI_MAPI_ERR_FAILURE;
    }
...
{
    if(g_bOsdInitFlg != true) {
        CVI_LOGE("CVI_MAPI_OSD_Batch is error!\n");
        return CVI_MAPI_ERR_FAILURE;
    }
    int s32OsdIdx;
    uint32_t u32DispIdx;
