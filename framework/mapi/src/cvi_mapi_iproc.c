        ...
        CVI_MAPI_PREPROCESS_ATTR_T *preprocess)
{
    ...
    bool fbOnVpss[VPSS_MAX_PHY_CHN_NUM] = {0};
    ...
    /*start vpss*/
    if (!iproc_initialized) {
        rc = Vproc_Init(VpssGrp, abChnEnable, &attr_inp, attr_chn, NULL, fbOnVpss);
        if (rc != CVI_SUCCESS) {
        ...
        CVI_MAPI_IPROC_RECT_T *rect_dst)
{
    ...
    if (frame_dst_has_content) {
        ...
        if (rc != CVI_SUCCESS) {
            CVI_LOGE("CVI_VPSS_SendChnFrame failed. rc: 0x%x !\n", rc);
            goto err;
        }
        // CAUTION: need to release here
        // and will getChnFrame for this frame again
        CVI_MAPI_ReleaseFrame(frame_dst);
    }
        ...
        ROTATION_E rotation)
{
    ...
    int rc = CVI_MAPI_AllocateFrame(frame_dst, out_w, out_h, out_fmt);
    if (rc != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_MAPI_AllocateFrame failed, w %d, h %d, fmt %d, rc %d\n",
    ...
    if (ret != 0) {
        CVI_LOGE("CVI_GDC_AddRotationTask failed, w %d, h %d, fmt %d, ret %d\n",
