...
extern CVI_S32 CVI_MPI_ADEC_AacInit(void);

extern CVI_S32 CVI_MPI_ADEC_AacDeInit(void);
...
{
    CVI_MAPI_ADEC_CTX_T *pstAdecCtx;
    ADEC_CHN_ATTR_S stAdecAttr;
    ADEC_ATTR_G711_S stAdecG711;
    ADEC_ATTR_AAC_S stAdecAac;
    int ret = CVI_MAPI_SUCCESS;

    pstAdecCtx = (CVI_MAPI_ADEC_CTX_T *)malloc(sizeof(CVI_MAPI_ADEC_CTX_T));
    if (!pstAdecCtx) {
        CVI_LOGE("malloc failed\n");
        return CVI_MAPI_ERR_NOMEM;
    }
    memset(&stAdecAttr, 0, sizeof(stAdecAttr));
    memset(pstAdecCtx, 0, sizeof(CVI_MAPI_ADEC_CTX_T));
    memcpy(&pstAdecCtx->attr,pstAdecAttr,sizeof(CVI_MAPI_ADEC_ATTR_S));
    ...
    if(pstAdecAttr->enAdecFormat == CVI_MAPI_AUDIO_CODEC_G711U) {
        stAdecAttr.enType = PT_G711U;
        stAdecAttr.pValue       = &stAdecG711;
    }else if(pstAdecAttr->enAdecFormat == CVI_MAPI_AUDIO_CODEC_G711A) {
        stAdecAttr.enType = PT_G711A;
        stAdecAttr.pValue       = &stAdecG711;

    }else if(pstAdecAttr->enAdecFormat == CVI_MAPI_AUDIO_CODEC_AAC) {
    ...
    *AdecHdl = (CVI_MAPI_ADEC_HANDLE_T)pstAdecCtx;
...
{
    CVI_MAPI_ADEC_CTX_T *pstAdecCtx = (CVI_MAPI_ADEC_CTX_T *)AdecHdl;
    if(!pstAdecCtx) {
...
int CVI_MAPI_ADEC_GetFrame(CVI_MAPI_ADEC_HANDLE_T AdecHdl, CVI_MAPI_AUDIO_FRAME_INFO_S* pstAudioFrameInfo, int bBlock)
{
    ...
    if(ret != CVI_SUCCESS) {
        CVI_LOGE("CVI_ADEC_GetFrame failed,ret:%d.\n",ret);
        return ret;
    }
...
int CVI_MAPI_ADEC_ReleaseFrame(CVI_MAPI_ADEC_HANDLE_T AdecHdl, const CVI_MAPI_AUDIO_FRAME_INFO_S* pstAudioFrameInfo)
{
    ...
    if(ret != CVI_SUCCESS) {
        CVI_LOGE("CVI_ADEC_ReleaseFrame failed,ret:%d.\n",ret);
        return ret;
    }
