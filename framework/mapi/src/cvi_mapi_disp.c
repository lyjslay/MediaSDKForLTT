...
#include "cvi_mapi.h"
#include "cvi_mapi_disp.h"
#include "cvi_mapi_internal.h"

typedef struct _CVI_MAPI_WND_CTX_S {
    CVI_MAPI_DISP_HANDLE_T disp;
    CVI_MAPI_WND_ATTR_T attr;
...
typedef struct CVI_MAPI_DISP_CTX_S {
    ...
    bool window_mode;
    int window_num;
    CVI_MAPI_WND_HANDLE_T windows[CVI_DISP_MAX_WND_NUM];
    CVI_MAPI_VPROC_HANDLE_T vproc; // is dt->use_iproc is false
} CVI_MAPI_DISP_CTX_S;

static int MAPI_DISP_SetRotation(CVI_MAPI_DISP_HANDLE_T disp_hdl,
        ROTATION_E rotate);
static int MAPI_DISP_SendFrame(CVI_MAPI_DISP_HANDLE_T disp_hdl,
        VIDEO_FRAME_INFO_S *frame);
...
static int MAPI_DISP_SetUserIntfSyncInfo(CVI_MAPI_DISP_HANDLE_T disp_hdl,
        CVI_MAPI_DISP_ATTR_T *attr);
static int MAPI_DISP_StopVideoLayer(VO_LAYER VoLayer, MAPI_DISP_MODE_E enMode);
static bool gDumpVoYuv = false;

void sort_wnd_list(CVI_MAPI_WND_HANDLE_T* wnd_list, int wnd_num)
{
    CVI_MAPI_WND_HANDLE_T cur_wnd = wnd_list[wnd_num-1];
    CVI_MAPI_WND_CTX_T *wt = (CVI_MAPI_WND_CTX_T *)cur_wnd;
    int index = -1;
    for(int i = 0; i <= wnd_num - 2; i++){
        CVI_MAPI_WND_CTX_T *wt_tmp = (CVI_MAPI_WND_CTX_T *)wnd_list[i];
...
static int MAPI_DISP_StartChn(VO_LAYER VoLayer, MAPI_DISP_MODE_E enMode)
{
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_GetVideoLayerAttr failed with %#x!\n", s32Ret);
        return s32Ret;
    }
    ...
    for (i = 0; i < u32WndNum; i++) {
        ...
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_VO_SetChnAttr failed with %#x!\n", s32Ret);
            return s32Ret;
        }

        s32Ret = CVI_VO_EnableChn(VoLayer, i);
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_VO_EnableChn failed with %#x!\n", s32Ret);
            return s32Ret;
        }
    }
...
static int MAPI_DISP_StopChn(VO_LAYER VoLayer, MAPI_DISP_MODE_E enMode)
{
    ...
    for (unsigned i = 0; i < u32WndNum; i++) {
        ...
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_VO_DisableChn failed with %#x!\n", s32Ret);
            return CVI_MAPI_ERR_FAILURE;
        }
    }
        ...
        ROTATION_E rotate)
{
    ...
    CVI_MAPI_DISP_CTX_S *dt = (CVI_MAPI_DISP_CTX_S *)disp_hdl;
    if (CVI_VO_SetChnRotation(dt->dev_id, dt->chn_id, rotate) != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_SetChnRotation failed\n");
        return CVI_MAPI_ERR_FAILURE;
    }
    return CVI_MAPI_SUCCESS;
}
        ...
        VIDEO_FRAME_INFO_S *frame)
{
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_SendFrame failed, ret=%x\n", s32Ret);
        return CVI_MAPI_ERR_FAILURE;
    }
...
static int MAPI_DISP_StopVideoLayer(VO_LAYER VoLayer, MAPI_DISP_MODE_E enMode)
{
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_DisableVideoLayer failed with %#x!\n", s32Ret);
        return s32Ret;
    }
...
#define CVI_DISP_MAX_NUM  (1)
static CVI_MAPI_DISP_HANDLE_T g_disp_list[CVI_DISP_MAX_NUM] = {NULL};
            ...
            CVI_MAPI_DISP_ATTR_T *attr)
{
    ...
    CVI_MAPI_DISP_CTX_S *dt;
    unsigned int VoDev = disp_id;
    int s32Ret;

    dt = (CVI_MAPI_DISP_CTX_S *)malloc(sizeof(CVI_MAPI_DISP_CTX_S));
    if (!dt) {
        CVI_LOGE("malloc failed\n");
        return CVI_MAPI_ERR_NOMEM;
    }
    memset(dt, 0, sizeof(CVI_MAPI_DISP_CTX_S));
    dt->attr = *attr;
    // assuming chn 0 only
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_SetPubAttr failed with %#x!\n", s32Ret);
        free(dt);
        dt = NULL;
        return s32Ret;
    }
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_Enable failed with %#x!\n", s32Ret);
        free(dt);
        dt = NULL;
        return s32Ret;
    }
    ...
    *disp_hdl = (CVI_MAPI_DISP_HANDLE_T)dt;
...
int CVI_MAPI_DISP_Deinit(CVI_MAPI_DISP_HANDLE_T disp_hdl)
{
    ...
    // if (dt->vproc) {
    //     CVI_MAPI_VPROC_Deinit(dt->vproc);
    // }
    VO_DEV VoDev = dt->dev_id;

    s32Ret = CVI_VO_Disable(VoDev);
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_Disable failed with %#x!\n", s32Ret);
        free(dt);
        dt = NULL;
        return s32Ret;
    }
            ...
            CVI_MAPI_DISP_VIDEOLAYER_ATTR_S *pstVideoLayerAttr)
{
    ...
    if (pstVideoLayerAttr->u32BufLen) {
        ...
        if (s32Ret != CVI_MAPI_SUCCESS) {
            CVI_LOGE("CVI_VO_SetDisplayBufLen failed with %#x!\n", s32Ret);
            CVI_MAPI_DISP_Deinit(disp_hdl);
            return s32Ret;
        }
    }
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_SetVideoLayerAttr failed with %#x!\n", s32Ret);
        CVI_MAPI_DISP_Deinit(disp_hdl);
        return s32Ret;
    }
    ...
    if (dt->window_mode) {
        ...
        ma.name = "DISP Lock";
        s32Ret = cvi_osal_mutex_create(&ma, &dt->mutex);
        if (s32Ret != CVI_OSAL_SUCCESS) {
            CVI_LOGE("cvi_osal_mutex_create %s failed\n", ma.name);
            MAPI_DISP_StopVideoLayer(VoLayer, enVoMode);
            CVI_MAPI_DISP_Deinit(disp_hdl);
            return s32Ret;
        }
...
int CVI_MAPI_DISP_Stop(CVI_MAPI_DISP_HANDLE_T disp_hdl)
{
    ...
    cvi_osal_mutex_destroy(dt->mutex);
    MAPI_DISP_StopVideoLayer(VoLayer, enVoMode);
        ...
        CVI_MAPI_VPROC_HANDLE_T vproc_hdl, int vproc_chn_idx)
{
	...
	stSrcChn.s32DevId = CVI_MAPI_VPROC_GetGrp(vproc_hdl);
    ...
    if (CVI_SYS_UnBind(&stSrcChn, &stDestChn) != CVI_MAPI_SUCCESS) {
        ...
        CVI_MAPI_VPROC_HANDLE_T vproc_hdl, int vproc_chn_idx)
{
    ...
    if (CVI_SYS_Bind(&stSrcChn, &stDestChn) != CVI_MAPI_SUCCESS) {
        CVI_LOGE("SAMPLE_COMM_VPSS_Bind_VO failed\n");
        return CVI_MAPI_ERR_FAILURE;
    }
    return CVI_MAPI_SUCCESS;
        ...
        VIDEO_FRAME_INFO_S *frame)
{
    ...
    #endif
    return MAPI_DISP_SendFrame(disp_hdl, frame);
}
...
int CVI_MAPI_DISP_ClearBuf(CVI_MAPI_DISP_HANDLE_T disp_hdl)
{
    ...
    if (s32Ret != CVI_MAPI_SUCCESS) {
        CVI_LOGE("CVI_VO_ClearChnBuf failed ! ret: 0x%x \n", s32Ret);
        return s32Ret;
    }
...
int CVI_MAPI_DISP_WndBindVproc(CVI_MAPI_WND_HANDLE_T wnd_hdl,
        CVI_MAPI_VPROC_HANDLE_T vproc_hdl, int vproc_chn_idx)
{
    CVI_MAPI_WND_CTX_T *wt = (CVI_MAPI_WND_CTX_T *)wnd_hdl;
    cvi_osal_mutex_lock(wt->mutex, CVI_OSAL_WAIT_FOREVER);
    wt->vproc_hdl = vproc_hdl;
    wt->chn_idx = vproc_chn_idx;
    cvi_osal_mutex_unlock(wt->mutex);
    return CVI_MAPI_SUCCESS;
}
...
int CVI_MAPI_DISP_CreateWindow(int disp_id, CVI_STITCH_ATTR_S *pstStitchAttr)
{
    ...
    CVI_MAPI_DISP_CTX_S *dt = (CVI_MAPI_DISP_CTX_S *)g_disp_list[disp_id];
...
int CVI_MAPI_DISP_DestroyWindow(int disp_id, CVI_STITCH_ATTR_S *pstStitchAttr)
{
    ...
    CVI_MAPI_DISP_ClearBuf(g_disp_list[disp_id]);
        ...
        VIDEO_FRAME_INFO_S *frame)
{
    ...
    CVI_MAPI_WND_CTX_T *wt = (CVI_MAPI_WND_CTX_T *)wnd_hdl;
    //CVI_MAPI_DISP_CTX_S *dt = wt->disp;

    cvi_osal_mutex_lock(wt->mutex, CVI_OSAL_WAIT_FOREVER);

    if (wt->cur_frame) {
      // decrease the ref count of the old frame
      CVI_MAPI_ReleaseFrame(wt->cur_frame);
      free(wt->cur_frame);
      wt->cur_frame = NULL;
    }
