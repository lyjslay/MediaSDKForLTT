        ...
        int grp_id, CVI_MAPI_VPROC_ATTR_T *attr)
{
    ...
    // since dev 1 has 3 chn, and dev 0 has only 1
    ...
    /*start vpss*/
    rc = Vproc_Init(VpssGrp, abChnEnable, &attr->attr_inp, attr->attr_chn, attr->chn_vbcnt, attr->bFbOnVpss);
    if (rc != CVI_SUCCESS) {
...
err2:
    Vproc_Deinit(VpssGrp, abChnEnable, pt->attr.bFbOnVpss);
...
int CVI_MAPI_VPROC_Deinit(CVI_MAPI_VPROC_HANDLE_T vproc_hdl)
{
	...
	for(i = 0; i < VPSS_MAX_PHY_CHN_NUM; i++) {
        if (pt->ExtChn[i].ExtChnEnable == true) {
            ...
            Vproc_Deinit(pt->ExtChn[i].ExtChnGrp, ChnEnable, pt->attr.bFbOnVpss);
        }
    }

    Vproc_Deinit(pt->VpssGrp, pt->abChnEnable, pt->attr.bFbOnVpss);
    free(pt);
...
int CVI_MAPI_VPROC_SetGrpAttr(CVI_MAPI_VPROC_HANDLE_T vproc_hdl, VPSS_GRP_ATTR_S *stGrpAttr)
{
    ...
    if (s32Ret != CVI_SUCCESS) {
        CVI_LOGE("CVI_VPSS_SetGrpAttr failed with %#x\n", s32Ret);
        return CVI_MAPI_ERR_FAILURE;
    }
        ...
        CVI_MAPI_VCAP_SENSOR_HANDLE_T sns_hdl, uint32_t vichn_idx)
{
    ...
    stSrcChn.s32DevId = 0;
    stSrcChn.s32ChnId = vichn_idx;//CVI_MAPI_VCAP_GetSensorChn(sns_hdl);
        ...
        CVI_MAPI_VCAP_SENSOR_HANDLE_T sns_hdl, uint32_t vichn_idx)
{
    ...
    stSrcChn.enModId = CVI_ID_VI;
    stSrcChn.s32DevId = CVI_MAPI_VCAP_GetSensorPipe(sns_hdl);
    stSrcChn.s32ChnId = vichn_idx;//CVI_MAPI_VCAP_GetSensorChn(sns_hdl);
		...
		uint32_t chn_idx, CVI_MAPI_VPROC_HANDLE_T vproc_dest_hdl)
{
	...
	CVI_MAPI_VPROC_CTX_T *pt_src = (CVI_MAPI_VPROC_CTX_T *)vproc_src_hdl;
	CVI_MAPI_VPROC_CTX_T *pt_dest = (CVI_MAPI_VPROC_CTX_T *)vproc_dest_hdl;
        ...
        VIDEO_FRAME_INFO_S *frame)
{
    ...
    CVI_S32 rc = CVI_VPSS_SendFrame(pt->VpssGrp, frame, CVI_MAPI_VPROC_TIMEOUT_MS);
    if (rc != CVI_SUCCESS) {
        CVI_LOGE("CVI_VPSS_SendFrame failed, rc: 0x%x ! GrpId :%d \n", rc, pt->VpssGrp);
        return CVI_MAPI_ERR_FAILURE;
    }
            ...
            CVI_MAPI_VPROC_CMD_T enCMD, void *pAttr, uint32_t u32Len)
{
    ...
    switch(enCMD){
        case CVI_VPROC_CMD_CHN_ROTATE:
            ...
            if(s32Ret != CVI_SUCCESS){
                CVI_LOGE("CVI_VPSS_GetChnRotation failed\n");
                return CVI_MAPI_ERR_FAILURE;
            }
            break;
        case CVI_VPROC_CMD_CHN_CROP:
            ...
            if(s32Ret != CVI_SUCCESS){
                CVI_LOGE("CVI_VPSS_GetChnCrop failed\n");
                return CVI_MAPI_ERR_FAILURE;
            }
            break;
        default:
            ...
            CVI_MAPI_VPROC_CMD_T enCMD, void *pAttr, uint32_t u32Len)
{
    ...
    switch(enCMD){
        case CVI_VPROC_CMD_CHN_ROTATE:
            ...
            if(s32Ret != CVI_SUCCESS){
                CVI_LOGE("CVI_VPSS_SetChnRotation failed\n");
                return CVI_MAPI_ERR_FAILURE;
            }
            break;
        case CVI_VPROC_CMD_CHN_CROP:
            ...
            if(s32Ret != CVI_SUCCESS){
                CVI_LOGE("CVI_VPSS_SetChnCrop failed\n");
                return CVI_MAPI_ERR_FAILURE;
            }
            break;
        default:
        ...
        uint32_t chn_idx, VIDEO_FRAME_INFO_S *frame)
{
    ...
    if (ExtChnEn == true) {
        if (CVI_VPSS_GetChnFrame(pt->ExtChn[i].ExtChnGrp, 0, frame, CVI_MAPI_VPROC_TIMEOUT_MS) != CVI_SUCCESS) {
            CVI_LOGE("CVI_VPSS_GetChnFrame %u %u failed\n", pt->VpssGrp, chn_idx);
            return CVI_MAPI_ERR_FAILURE;
        }
    } else {
        CHECK_VPROC_CHN(chn_idx);
        if (CVI_VPSS_GetChnFrame(pt->VpssGrp, chn_idx, frame, CVI_MAPI_VPROC_TIMEOUT_MS) != CVI_SUCCESS) {
            CVI_LOGE("CVI_VPSS_GetChnFrame %u %u failed\n", pt->VpssGrp, chn_idx);
            return CVI_MAPI_ERR_FAILURE;
        }
    }
        ...
        uint32_t chn_idx, VIDEO_FRAME_INFO_S *frame)
{
    ...
    if (ExtChnEn == true) {
        if (CVI_VPSS_ReleaseChnFrame(pt->ExtChn[i].ExtChnGrp, 0, frame) != CVI_SUCCESS) {
            CVI_LOGE("CVI_VPSS_ReleaseChnFrame failed\n");
            return CVI_MAPI_ERR_FAILURE;
        }
    } else {
        CHECK_VPROC_CHN(chn_idx);
...
int VpssDumpFrame(CVI_U32 Grp, CVI_S32 Chn, VIDEO_FRAME_INFO_S* pstFrameData,
        CVI_DUMP_FRAME_CALLBACK_FUNC_T* pstCallBack)
{
    if (CVI_VPSS_GetChnFrame(Grp, Chn, pstFrameData, CVI_MAPI_VPROC_TIMEOUT_MS) != CVI_SUCCESS) {
...
int CVI_MAPI_VPROC_SetExtChnAttr(CVI_MAPI_VPROC_HANDLE_T vproc_hdl, CVI_MAPI_EXTCHN_ATTR_T *pstExtChnAttr)
{
    VPROC_CHECK_NULL_PTR(vproc_hdl);
    CVI_MAPI_VPROC_CTX_T *pt = (CVI_MAPI_VPROC_CTX_T *)vproc_hdl;
    CHECK_VPROC_GRP(pt->VpssGrp);
    ...
    memcpy(&pt->ExtChn[i].ExtChnAttr, pstExtChnAttr, sizeof(CVI_MAPI_EXTCHN_ATTR_T));
...
int CVI_MAPI_VPROC_GetExtChnAttr(CVI_MAPI_VPROC_HANDLE_T vproc_hdl, uint32_t chn_idx, CVI_MAPI_EXTCHN_ATTR_T *pstExtChnAttr)
{
    VPROC_CHECK_NULL_PTR(vproc_hdl);
    CVI_MAPI_VPROC_CTX_T *pt = (CVI_MAPI_VPROC_CTX_T *)vproc_hdl;
    CHECK_VPROC_GRP(pt->VpssGrp);
    ...
    if (ExtChnEn == true) {
        memcpy(pstExtChnAttr, &pt->ExtChn[i].ExtChnAttr, sizeof(CVI_MAPI_EXTCHN_ATTR_T));
    } else {
        CHECK_VPROC_CHN(chn_idx);
...
int CVI_MAPI_VPROC_EnableTileMode(void)
{
    ...
    if (s32Ret != CVI_SUCCESS) {
        CVI_LOGE("CVI_VPSS_EnableTileMode failed with %#x\n", s32Ret);
        return CVI_MAPI_ERR_FAILURE;
    }
...
int CVI_MAPI_VPROC_DisableTileMode(void)
{
    ...
    if (s32Ret != CVI_SUCCESS) {
        CVI_LOGE("CVI_VPSS_DisableTileMode failed with %#x\n", s32Ret);
        return CVI_MAPI_ERR_FAILURE;
    }
