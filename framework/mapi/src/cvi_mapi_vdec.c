...
typedef struct CVI_MAPI_VDEC_CTX_S {
...
static pthread_mutex_t gVdecMutex[CVI_MAPI_VCODEC_MAX] = {
...
void CVI_MAPI_GetMaxSizeByEncodeType(PAYLOAD_TYPE_E enType, CVI_U32 *max_width, CVI_U32 *max_height)
{
    if (enType == PT_JPEG) {
...
static CVI_S32 _CVI_MAPI_VDEC_GetValidChan(VDEC_CHN *vdecChn, CVI_MAPI_VCODEC_E codec)
{
	CVI_U32 i;
...
static void _CVI_MAPI_VDEC_ReleaseChan(VDEC_CHN vdecChn, CVI_MAPI_VCODEC_E codec)
{
	pthread_mutex_lock(&gVdecMutex[codec]);
...
int CVI_MAPI_VDEC_InitChn(CVI_MAPI_VDEC_HANDLE_T *vdec_hdl, CVI_MAPI_VDEC_CHN_ATTR_T *attr)
{
	VDEC_CHN vdecChn;
	...
	CVI_MAPI_VDEC_CTX_T *vdecCtx = NULL;

	if (attr->codec >= CVI_MAPI_VCODEC_MAX) {
	...
	encodeType = CVI_VCODEC_TO_PAYLOAD_TYPE[attr->codec];
	...
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_CreateChn Fail: 0x%x\n", ret);
		goto err;
	}
	...
	if (gVbMode == VB_SOURCE_USER) {
		...
		if (ret != CVI_SUCCESS) {
			CVI_LOGE("CVI_VDEC_AttachVbPool Fail: 0x%x\n", ret);
			goto err;
		}
	}
	...
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_GetChnParam Fail: 0x%x\n", ret);
		goto err;
	}
	...
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_SetChnParam Fail: 0x%x\n", ret);
		goto err;
	}

	ret = CVI_VDEC_StartRecvStream(vdecChn);
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_StartRecvStream Fail: 0x%x\n", ret);
		goto err;
	}

	CVI_LOGI("VDEC chn %d created\n", vdecChn);

	*vdec_hdl = (CVI_MAPI_VDEC_HANDLE_T)vdecCtx;
	return CVI_MAPI_SUCCESS;

err:
	_CVI_MAPI_VDEC_ReleaseChan(vdecChn, attr->codec);
	if (vdecCtx != NULL) {
		free(vdecCtx);
	}
...
{
	CVI_MAPI_VDEC_CTX_T *vdecCtx = (CVI_MAPI_VDEC_CTX_T *)vdec_hdl;
	int ret = CVI_MAPI_SUCCESS;

	ret = CVI_VDEC_StopRecvStream(vdecCtx->vdecChn);
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_StopRecvStream Fail: 0x%x\n", ret);
		return ret;
	}
	...
	if (gVbMode == VB_SOURCE_USER) {
		...
		if (ret != CVI_SUCCESS) {
			CVI_LOGE("CVI_VDEC_DetachVbPool Fail: 0x%x\n", ret);
			return ret;
		}
		ret = CVI_VB_DestroyPool(gPicVbPool[vdecCtx->vdecChn]);
		if (ret != CVI_SUCCESS) {
			CVI_LOGE("CVI_VB_DestroyPool Fail: 0x%x\n", ret);
			return ret;
		}
	...
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_DestroyChn Fail: 0x%x\n", ret);
		return ret;
	}

	_CVI_MAPI_VDEC_ReleaseChan(vdecCtx->vdecChn, vdecCtx->codec);
...
int CVI_MAPI_VDEC_SendStream(CVI_MAPI_VDEC_HANDLE_T vdec_hdl, VDEC_STREAM_S *stream)
{
	CVI_MAPI_VDEC_CTX_T *vdecCtx = (CVI_MAPI_VDEC_CTX_T *)vdec_hdl;
	int ret;
	...
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_SendStream failed, %d\n", ret);
		return ret;
	}
...
int CVI_MAPI_VDEC_GetFrame(CVI_MAPI_VDEC_HANDLE_T vdec_hdl, VIDEO_FRAME_INFO_S *frame)
{
	CVI_MAPI_VDEC_CTX_T *vdecCtx = (CVI_MAPI_VDEC_CTX_T *)vdec_hdl;
	int ret;
	...
	if (ret != CVI_SUCCESS) {
		//CVI_LOGE("CVI_VDEC_GetFrame failed, %d\n", ret);
		return ret;
	}
...
int CVI_MAPI_VDEC_ReleaseFrame(CVI_MAPI_VDEC_HANDLE_T vdec_hdl, VIDEO_FRAME_INFO_S *frame)
{
	CVI_MAPI_VDEC_CTX_T *vdecCtx = (CVI_MAPI_VDEC_CTX_T *)vdec_hdl;
	int ret;
	...
	if (ret != CVI_SUCCESS) {
		CVI_LOGE("CVI_VDEC_ReleaseFrame failed, %d\n", ret);
		return ret;
    }
