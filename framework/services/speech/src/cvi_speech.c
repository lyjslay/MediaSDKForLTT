...
static FUNC_SPEECH_HANDLE CVI_SPEECH_SERVICE_pFunc = NULL;
...
{
    CVI_SPEECH_SERVICE_pFunc = pfunction;
}

int32_t CVI_SPEECH_Audio_Buffer_Get(uint8_t* pAudioBuffer, uint32_t u32BufferLen, SPEECH_CONTEXT_HANDLE_S speech)
{
...
static void speech_task_entry(void *arg)
{
    ...
    while (!speech->shutdown)
    {
        while (!speech->shutdown && speech->enable == 0) {
            cvi_osal_task_sleep(SR_NO_ORDER_PAUSE_TIME_MS);
        }
        pack_idx = g_pack_idx;
        CVI_SPEECH_Audio_Buffer_Get(buffer, u32BufferSize, speech);

        CVI_TDL_SoundClassification_V2_Pack(speech->SpeechHandle, &Frame, pack_idx, pack_len, &index);
        if (index > 0){
            CVI_LOGI("esc class: %s, index = %d\n", g_stEnumOrderStr[index], index);
            if (NULL != CVI_SPEECH_SERVICE_pFunc){
                CVI_SPEECH_SERVICE_pFunc(index);
            }
            cvi_osal_task_sleep(SR_ORDER_PAUSE_TIME_MS);
        }else{
            cvi_osal_task_sleep(SR_NO_ORDER_PAUSE_TIME_MS);
        }
    }
...
static int32_t speech_start_task(SPEECH_CONTEXT_HANDLE_S speech)
{
    ...
    ta.detached = false;
    int32_t rc = cvi_osal_task_create(&ta, &speech->speech_task);
...
static int32_t speech_stop_task(SPEECH_CONTEXT_HANDLE_S speech) {
    ...
    int32_t rc = cvi_osal_task_join(speech->speech_task);
    if (rc != CVI_OSAL_SUCCESS) {
        CVI_LOGE("speech task join failed, %d\n", rc);
        return -1;
    }
    cvi_osal_task_destroy(&speech->speech_task);
...
static int32_t CVI_Speech_Proc_Init(SPEECH_CONTEXT_HANDLE_S speech)
{
    CVI_LOGI("CVI_Speech_Proc_Init\n");
    int32_t rc = CVI_SUCCESS;
    ...
    rc = CVI_TDL_SetPerfEvalInterval(speech->SpeechHandle,  CVI_TDL_SUPPORTED_MODEL_SOUNDCLASSIFICATION_V2, 10);
    if (rc != CVI_SUCCESS)
    {
    ...
    rc = CVI_TDL_OpenModel(speech->SpeechHandle, CVI_TDL_SUPPORTED_MODEL_SOUNDCLASSIFICATION_V2, speech->params.ModelPath);
    if (rc != CVI_SUCCESS)
    {
...
{
    CVI_LOGI("enter CVI_SPEECH_SERVICE_Create\n");
    int32_t rc = CVI_SUCCESS;
    g_pack_idx = 0;
    SPEECH_CONTEXT_HANDLE_S speech = (SPEECH_CONTEXT_HANDLE_S)malloc(sizeof(SPEECH_CONTEXT_S));
    memcpy(&speech->params, params, sizeof(CVI_SPEECH_SERVICE_PARAM_S));
    uint32_t SampleRate = speech->params.SampleRate;
    uint32_t BitWidth = speech->params.BitWidth;
    uint32_t buffer_len = SampleRate * (BitWidth + 1) * SPEECH_BUFFER_SECOND;
    ...
    //ai init todo
    rc = CVI_Speech_Proc_Init(speech);
    CVI_AUDIO_SERVICR_ACAP_CallbackSet("Speech_Recognition", process_speech_audio_frame, speech);
    // start speech task
    rc = speech_start_task(speech);
    *hdl = (CVI_SPEECH_HANDLE_S *)speech;
    return rc;
}
...
int32_t CVI_SPEECH_SERVICE_Destroy(CVI_SPEECH_HANDLE_S hdl)
{
    ...
    rc = CVI_AUDIO_SERVICR_ACAP_CallbackUnset("Speech_Recognition");
    if (rc != 0) {
        CVI_LOGI("CVI_AUDIO_SERVICR_ACAP_CallbackUnset fail\n");
    }
    //ai deinit todo
