...
#define DEBUG_VENC_STREAM_STATS_COUNT (3000)
...
#define RS_PERF_MARK_ADD(mark)            \
    do {                                  \
        if (RS_STATE_ENABLED(rs, PERF)) { \
            uint64_t tt;                  \
            cvi_osal_get_boot_time(&tt);  \
            CVI_PERF_MarkAdd((mark), tt); \
        }                                 \
    } while (0)

typedef int32_t (*CVI_GENERATE_FILENAME_CB_FN_PTR)(CVI_DTCF_FILE_TYPE_E enType, CVI_DTCF_DIR_E enDir, char *pstFileName);

typedef struct recorder_get_filename_param_t {
    rs_context_handle_t rs;
    CVI_DTCF_FILE_TYPE_E file_type;
    CVI_DTCF_DIR_E dir_type;
...
static int32_t rs_get_venc_stream(CVI_MAPI_VENC_HANDLE_T vhdl, VENC_STREAM_S *stream){
...
static int32_t process_venc_rec_stream(rs_context_handle_t rs, CVI_MAPI_VENC_HANDLE_T rec_venc_hdl,CVI_MUXER_FRAME_TYPE_E type, VENC_STREAM_S *stream) {
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    ...
    switch (p->recorder_file_type) {
        case CVI_RECORD_SERVICE_FILE_TYPE_ES:
        ...
        case CVI_RECORD_SERVICE_FILE_TYPE_MP4:
        case CVI_RECORD_SERVICE_FILE_TYPE_MOV:
        case CVI_RECORD_SERVICE_FILE_TYPE_TS: {
            CVI_RECORDER_FRAME_STREAM_S frame_stream;
            memset(&frame_stream, 0x0, sizeof(CVI_RECORDER_FRAME_STREAM_S));
            bool iskey = 0;
            for (unsigned i = 0; i < stream->u32PackCount; i++) {
                ...
                CVI_MAPI_VENC_GetStreamStatus(rec_venc_hdl, ppack, &iskey);
            ...
            if (type == CVI_MUXER_FRAME_TYPE_VIDEO) {
                frame_stream.type = CVI_MUXER_FRAME_TYPE_VIDEO;
                 ...
                 frame_stream.type = CVI_MUXER_FRAME_TYPE_SUB_VIDEO;
            }


            if (CVI_RECORDER_SendFrame(rs->recorder[0], &frame_stream) == 0 && frame_stream.thumbnail_len > 0) {
                *thumbnail_len = 0;
            }
        } break;

        case CVI_RECORD_SERVICE_FILE_TYPE_NONE:
            // do nothing
            break;
    ...
    if (rs->venc_rec_stream_count >= DEBUG_VENC_STREAM_STATS_COUNT) {
        CVI_LOGD("RS[%d]: [STREAM_SIZE][REC] cnt %"PRIu64", total %"PRIu64", bitrate %2.2f bps", rs->id,
                 rs->venc_rec_stream_count, rs->venc_rec_stream_total_len,
                 (float)rs->venc_rec_stream_total_len / rs->venc_rec_stream_count * p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_VIDEO].unTrackSourceAttr.stVideoInfo.fFrameRate * 8);
...
static int32_t recorder_request_idr_cb(void *param, CVI_MUXER_FRAME_TYPE_E type) {
    rs_context_handle_t rs = (rs_context_handle_t)param;
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    CVI_LOGD("RS[%d] recorder_request_idr_cb request idr for new video\n",rs->id);
    if(type == CVI_MUXER_FRAME_TYPE_VIDEO){
        CVI_MAPI_VENC_RequestIDR(p->handles.rec_venc_hdl);
    } else if(type == CVI_MUXER_FRAME_TYPE_SUB_VIDEO){
        CVI_MAPI_VENC_RequestIDR(p->handles.sub_rec_venc_hdl);
    } else {
...
static int32_t recorder_get_filename_cb(void *param, char *filename, int32_t filename_len) {
    ...
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rec_param->rs->attr;
    char azFilePath[CVI_FILE_PATH_LEN_MAX] = {0};
    memset(azFilePath, 0x0, sizeof(char) * CVI_FILE_PATH_LEN_MAX);

    int32_t ret = ((CVI_GENERATE_FILENAME_CB_FN_PTR)p->stCallback.pfnRequestFileNames)(rec_param->file_type, rec_param->dir_type, azFilePath);
    if (ret != 0) {
...
static int32_t reset_vproc_bind_state(rs_context_handle_t rs, int32_t bind){
    ...
    int32_t sub_vpss_grp = (p->enable_subvideo == 1 ? CVI_MAPI_VPROC_GetGrp(p->handles.sub_rec_vproc) : 0);
#endif
    int32_t vpss_grp = CVI_MAPI_VPROC_GetGrp(p->handles.rec_vproc);
    if (bind == 0 && p->handles.venc_bind_mode == CVI_RECORD_SERVICE_VENC_BIND_MODE_VPSS) {
        CVI_MAPI_VENC_UnBindVproc(p->handles.rec_venc_hdl, vpss_grp, p->handles.vproc_chn_id_venc);
#ifndef SERVICES_SUBVIDEO_ON
        if(p->enable_subvideo == 1){
            CVI_MAPI_VENC_UnBindVproc(p->handles.sub_rec_venc_hdl, sub_vpss_grp, p->handles.sub_vproc_chn_id_venc);
        }
#endif
    }
    ...
    if (p->handles.rec_venc_hdl != NULL) {
        if (p->handles.venc_rec_start == 1) {
            ...
            CVI_MAPI_VENC_StopRecvFrame(p->handles.rec_venc_hdl);
#ifndef SERVICES_SUBVIDEO_ON
            if(p->enable_subvideo == 1){
                CVI_MAPI_VENC_StopRecvFrame(p->handles.sub_rec_venc_hdl);
            }
#endif
        }
    }
    ...
    if (bind == 1) {
        CVI_MAPI_VENC_BindVproc(p->handles.rec_venc_hdl, vpss_grp, p->handles.vproc_chn_id_venc);
#ifndef SERVICES_SUBVIDEO_ON
        if(p->enable_subvideo == 1){
            CVI_MAPI_VENC_BindVproc(p->handles.sub_rec_venc_hdl, sub_vpss_grp, p->handles.sub_vproc_chn_id_venc); /*new*/
        }
#endif
    }
...
static CVI_DTCF_FILE_TYPE_E get_dtcf_type(CVI_RECORD_SERVICE_FILE_TYPE_E file_type) {
    CVI_DTCF_FILE_TYPE_E dtcf_type = CVI_DTCF_FILE_TYPE_BUTT;

    if (CVI_RECORD_SERVICE_FILE_TYPE_MP4 == file_type) {
        dtcf_type = CVI_DTCF_FILE_TYPE_MP4;
    } else if (CVI_RECORD_SERVICE_FILE_TYPE_MOV == file_type) {
        dtcf_type = CVI_DTCF_FILE_TYPE_MOV;
    } else if (CVI_RECORD_SERVICE_FILE_TYPE_TS == file_type) {
        dtcf_type = CVI_DTCF_FILE_TYPE_TS;
    }

    return dtcf_type;
}

static bool is_ts_file(CVI_RECORD_SERVICE_FILE_TYPE_E file_type) {
    return (CVI_RECORD_SERVICE_FILE_TYPE_TS == file_type);
}
...
static void process_recorder_audio_frame(const AUDIO_FRAME_S *audio_frame, const AEC_FRAME_S *aec_frame,
                                         void *arg) {
    UNUSED(aec_frame);
    CVI_RECORDER_HANDLE_T recorder = (CVI_RECORDER_HANDLE_T)arg;

    if (!recorder) {
        return;
    }

    CVI_RECORDER_FRAME_STREAM_S frame;
    memset(&frame, 0x0, sizeof(CVI_RECORDER_FRAME_STREAM_S));
    ...
    CVI_RECORDER_SendFrame(recorder, &frame);
}
...
static void process_recorder_subvideo_stream(const VENC_STREAM_S *stream, void *arg) {
    ...
    uint8_t *thumbnail_data = (uint8_t *)g_rec_thumbnail_buf[rs->id].buf;
    uint32_t *thumbnail_len = &g_rec_thumbnail_buf[rs->id].actsize;
    CVI_MAPI_VENC_HANDLE_T rec_venc_hdl = p->handles.sub_rec_venc_hdl;
...
#define PRINT_INTERVAL (1000)
    ...
    CVI_PERF_MarkInit(&rs->perf_mark_frame, name, PRINT_INTERVAL, 10);  // skip 10 frames
    snprintf(name, 32, "  VCAP      %d", rs->id);
    CVI_PERF_StatInit(&rs->perf_stat_vcap, name, PRINT_INTERVAL);
    snprintf(name, 32, "  VENC_REC  %d", rs->id);
    CVI_PERF_StatInit(&rs->perf_stat_venc_rec, name, PRINT_INTERVAL);
    snprintf(name, 32, "  REC_SAVE  %d", rs->id);
    CVI_PERF_StatInit(&rs->perf_stat_rec_save, name, PRINT_INTERVAL);
    snprintf(name, 32, "  VPROC     %d", rs->id);
    CVI_PERF_StatInit(&rs->perf_stat_vproc, name, PRINT_INTERVAL);

    return RS_SUCCESS;
}

static int32_t stop_perf(rs_context_handle_t rs) {
    CVI_PERF_MarkDeinit(rs->perf_mark_frame);
    CVI_PERF_StatDeinit(rs->perf_stat_vcap);
    CVI_PERF_StatDeinit(rs->perf_stat_venc_rec);
    CVI_PERF_StatDeinit(rs->perf_stat_rec_save);
    CVI_PERF_StatDeinit(rs->perf_stat_vproc);
    return RS_SUCCESS;
}
...
static void rs_thumb_task_entry(void *arg){
    ...
    while (!rs->shutdown) {
        ...
        *thmbsize = stream.pstPack[0].u32Len;
        LOG_RET(CVI_MAPI_VENC_ReleaseStream(p->handles.thumbnail_venc_hdl, &stream));
END:
        LOG_RET(CVI_MAPI_ReleaseFrame(&frame));
END1:
        ...
        CVI_LOGD("RS[%d] catch thumbnail %s", rs->id, (*thmbsize > 0)?("success"):("failed"));
...
static void  rs_snap_task_entry(void *arg) {
    ...
    while (!rs->shutdown) {
        ...
        if (p->stCallback.pfnNormalRecCb) {
            ((CVI_RECORDER_EVENT_CALLBACK)p->stCallback.pfnNormalRecCb)(CVI_RECORDER_EVENT_PIV_START, rs->piv_filename, (void*)(&rs->id));
        }
        ...
        CVI_MAPI_VENC_StartRecvFrame(p->handles.piv_venc_hdl, -1);

        if (0 != CVI_MAPI_VPROC_GetChnFrame(p->handles.rec_vproc, p->handles.vproc_chn_id_venc, &piv_frame)) {
            CVI_LOGE("RS[%d]: CVI_MAPI_VPROC_GetChnFrame failed", rs->id);
            CVI_MAPI_VENC_StopRecvFrame(p->handles.piv_venc_hdl);
            rs->piv_finish = 1;
            pthread_mutex_unlock(&rs->piv_mutex);
            continue;
        }

        if (0 > CVI_MAPI_VENC_SendFrame(p->handles.piv_venc_hdl, &piv_frame)) {
            CVI_LOGE("RS[%d]: snapshot venc send frame fail", rs->id);
            CVI_MAPI_ReleaseFrame(&piv_frame);
            CVI_MAPI_VENC_StopRecvFrame(p->handles.piv_venc_hdl);
            rs->piv_finish = 1;
            pthread_mutex_unlock(&rs->piv_mutex);
            continue;
        ...
        if (len > g_snap_buf[rs->id].size) {
            ...
            CVI_MAPI_VENC_ReleaseStream(p->handles.piv_venc_hdl, &stream);
            CVI_MAPI_VENC_StopRecvFrame(p->handles.piv_venc_hdl);
            rs->piv_finish = 1;
            pthread_mutex_unlock(&rs->piv_mutex);
            continue;
        ...
        if (p->stCallback.pfnNormalRecCb) {
            ((CVI_RECORDER_EVENT_CALLBACK)p->stCallback.pfnNormalRecCb)(CVI_RECORDER_EVENT_PIV_END, rs->piv_filename, (void*)(&rs->id));
        }

        // zbar_processor_t *proc = zbar_processor_create(0);
        CVI_LOGD("RS[%d] snap success", rs->id);
        CVI_FILESYNC_Push(rs->piv_filename, NULL);
        rs->piv_finish = 1;
        *thumbnail_len = 0;
        pthread_mutex_unlock(&rs->piv_mutex);
    }
...
static int32_t process_one_frame(rs_context_handle_t rs) {
    ...
    LOG_RET(process_venc_rec_stream(rs,p->handles.rec_venc_hdl,CVI_MUXER_FRAME_TYPE_VIDEO, &stream));
    LOG_RET(CVI_MAPI_VENC_ReleaseStream(p->handles.rec_venc_hdl,&stream));
    pthread_mutex_unlock(&rs->rec_mutex);
...
static int32_t process_one_subframe(rs_context_handle_t rs) {
    ...
    LOG_RET(process_venc_rec_stream(rs, p->handles.sub_rec_venc_hdl,CVI_MUXER_FRAME_TYPE_SUB_VIDEO,&sub_stream));
    LOG_RET(CVI_MAPI_VENC_ReleaseStream(p->handles.sub_rec_venc_hdl, &sub_stream));
    pthread_mutex_unlock(&rs->rec_mutex);
...
static int32_t process_one_subframe_timelapse(rs_context_handle_t rs)
{
    ...
    if(CVI_RECORDER_Timelapse_Is_SendVenc(rs->recorder[0],CVI_MUXER_FRAME_TYPE_SUB_VIDEO) != 1){
        pthread_mutex_unlock(&rs->rec_mutex);
        return 0;
    }

    ret = CVI_MAPI_VPROC_GetChnFrame(p->handles.sub_rec_vproc,p->handles.sub_vproc_chn_id_venc, &vproc_frame);
    if (ret != 0) {
        CVI_LOGE("RS[%d]: CVI_MAPI_VPROC_GetChnFrame failed", rs->id);
        pthread_mutex_unlock(&rs->rec_mutex);
        return -1;
    ...
    ret = CVI_MAPI_VENC_SendFrame(p->handles.sub_rec_venc_hdl, &vproc_frame);
    if (ret < 0) {
        CVI_LOGE("RS[%d]: CVI_MAPI_VENC_SendFrame failed", rs->id);
        LOG_RET(CVI_MAPI_ReleaseFrame(&vproc_frame));
        pthread_mutex_unlock(&rs->rec_mutex);
        return -1;
    }
    ...
    if (p->handles.venc_bind_mode == CVI_RECORD_SERVICE_VENC_BIND_MODE_NONE) {
        LOG_RET(CVI_MAPI_ReleaseFrame(&vproc_frame));
    }
    LOG_RET(process_venc_rec_stream(rs, p->handles.sub_rec_venc_hdl, CVI_MUXER_FRAME_TYPE_SUB_VIDEO, &stream));
    LOG_RET(CVI_MAPI_VENC_ReleaseStream(p->handles.sub_rec_venc_hdl, &stream));
...
static int32_t process_one_frame_timelapse(rs_context_handle_t rs)
{
    ...
    if(CVI_RECORDER_Timelapse_Is_SendVenc(rs->recorder[0],CVI_MUXER_FRAME_TYPE_VIDEO) != 1){
        pthread_mutex_unlock(&rs->rec_mutex);
        return 0;
    }

    ret = CVI_MAPI_VPROC_GetChnFrame(p->handles.rec_vproc, p->handles.vproc_chn_id_venc, &vproc_frame);
    if (ret != 0) {
        CVI_LOGE("RS[%d]: CVI_MAPI_VPROC_GetChnFrame failed", rs->id);
        pthread_mutex_unlock(&rs->rec_mutex);
        return -1;
    ...
    ret = CVI_MAPI_VENC_SendFrame(p->handles.rec_venc_hdl, &vproc_frame);
    if (ret < 0) {
        CVI_LOGE("RS[%d]: CVI_MAPI_VENC_SendFrame failed", rs->id);
        LOG_RET(CVI_MAPI_ReleaseFrame(&vproc_frame));
        pthread_mutex_unlock(&rs->rec_mutex);
...
static int32_t add_audio_mute_callback_to_recorder(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0]) {
        ...
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackSet(name, process_recorder_mute_audio_frame, rs->recorder[0]));
    }

    if (rs->recorder[1]) {
        fill_rec_audio_callback_name(name, sizeof(name), rs->id, 1);
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackSet(name, process_recorder_mute_audio_frame, rs->recorder[1]));
    }
    return RS_SUCCESS;
}
...
static int32_t add_audio_callback_to_recorder(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0]) {
        ...
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackSet(name, process_recorder_audio_frame, rs->recorder[0]));
    }

    if (rs->recorder[1]) {
        fill_rec_audio_callback_name(name, sizeof(name), rs->id, 1);
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_CallbackSet(name, process_recorder_audio_frame, rs->recorder[1]));
    }
    return RS_SUCCESS;
}
...
static int32_t add_subvideo_callback_to_recorder(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0]) {
        ...
        LOG_RET(CVI_VIDEO_SERVICR_CallbackUnSet(rs->id, name));
        LOG_RET(CVI_VIDEO_SERVICR_CallbackSet(rs->id, name, process_recorder_subvideo_stream, rs));
        CVI_MAPI_VENC_RequestIDR(p->handles.sub_rec_venc_hdl);/*请求I帧*/
    }
...
static int32_t add_audio_aac_mute_callback_to_recorder(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0]) {
        fill_rec_audio_callback_name(name, sizeof(name), rs->id, 0);
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackSet(name, process_recorder_mute_aac_frame, rs->recorder[0]));
    }

    if (rs->recorder[1]) {
        fill_rec_audio_callback_name(name, sizeof(name), rs->id, 1);
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackSet(name, process_recorder_mute_aac_frame, rs->recorder[1]));
    }
    return RS_SUCCESS;
}
...
static int32_t add_audio_aac_callback_to_recorder(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0]) {
        fill_rec_audio_callback_name(name, sizeof(name), rs->id, 0);
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackSet(name, process_recorder_aac_frame, rs->recorder[0]));
    }

    if (rs->recorder[1]) {
        fill_rec_audio_callback_name(name, sizeof(name), rs->id, 1);
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackUnset(name));
        LOG_RET(CVI_AUDIO_SERVICR_ACAP_AacCallbackSet(name, process_recorder_aac_frame, rs->recorder[1]));
    }
    return RS_SUCCESS;
}


static void cvi_master_update_audio_callback(rs_context_handle_t rs, bool is_mute) {
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    CVI_RECORDER_TRACK_SOURCE_S *handle = &p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_AUDIO];
    CVI_LOGD("RS[%d]: Start audio %d enable %d", rs->id, is_mute, handle->enable);
    if (p->enRecType == CVI_RECORDER_TYPE_LAPSE) {
        return;
    }
    if (1 || CVI_MUXER_TRACK_AUDIO_CODEC_ADPCM == handle->unTrackSourceAttr.stAudioInfo.enCodecType){
        if (is_mute) {
            add_audio_mute_callback_to_recorder(rs);
        } else {
            add_audio_callback_to_recorder(rs);
        }
    } else if (CVI_MUXER_TRACK_AUDIO_CODEC_AAC == handle->unTrackSourceAttr.stAudioInfo.enCodecType) {
        if (is_mute) {
...
static int32_t start_mute(rs_context_handle_t rs) {
    ...
    cvi_master_update_audio_callback(rs, true);
...
static int32_t stop_mute(rs_context_handle_t rs) {
    ...
    cvi_master_update_audio_callback(rs, false);

    return RS_SUCCESS;
}


static int32_t cvi_master_destroy_rec(rs_context_handle_t rs) {
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    ...
    switch (p->recorder_file_type) {
        ...
        case CVI_RECORD_SERVICE_FILE_TYPE_TS:
        {
            ...
            CVI_RECORDER_Destroy(&rs->recorder[0]);
            CVI_RECORDER_Destroy(&rs->recorder[1]);
            rs->recorder[0] = NULL;
            rs->recorder[1] = NULL;
            break;
        }
...
static int32_t cvi_master_create_rec(rs_context_handle_t rs) {
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    ...
    switch (p->recorder_file_type) {
        ...
        case CVI_RECORD_SERVICE_FILE_TYPE_MOV:
        case CVI_RECORD_SERVICE_FILE_TYPE_TS: {
            CVI_RECORDER_ATTR_S rec_attr;
            memset(&rec_attr, 0x00, sizeof(CVI_RECORDER_ATTR_S));
            memcpy(&rec_attr.astStreamAttr, &p->astStreamAttr[0], sizeof(CVI_RECORDER_STREAM_ATTR_S));

            rec_attr.enRecType = p->enRecType;
            rec_attr.fncallback.pfn_rec_malloc_mem = CVI_MEM_Allocate;
            rec_attr.fncallback.pfn_rec_free_mem = CVI_MEM_Free;
            CVI_RECORDER_TRACK_SOURCE_S *handle = &p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_AUDIO];
            {
                CVI_RECORDER_TRACK_SOURCE_S *thandle = &rec_attr.astStreamAttr.aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_AUDIO];
                if (rec_attr.enRecType == CVI_RECORDER_TYPE_NORMAL) {
                    thandle->enable = 1;
                }
                ...
                if (CVI_MUXER_TRACK_AUDIO_CODEC_ADPCM == handle->unTrackSourceAttr.stAudioInfo.enCodecType) {
            ...
            rec_attr.fncallback.pfn_request_idr = (CVI_RECORDER_REQUEST_IDR_CALLBACK)recorder_request_idr_cb;
            ...
            if (rec_attr.enable_subtitle) {
                ...
                rec_attr.fncallback.pfn_get_subtitle_cb = (CVI_RECORDER_GET_SUBTITLE_CALLBACK)p->stCallback.pfnGetSubtitleCb;
        ...
        #endif
            rec_attr.fncallback.pfn_get_filename = (CVI_RECORDER_GET_FILENAME_CALLBACK)recorder_get_filename_cb;
            recorder_get_filename_param_t *fparam;
            if (rec_attr.enRecType == CVI_RECORDER_TYPE_NORMAL) {
                rec_attr.fncallback.pfn_event_cb[CVI_RECORDER_TYPE_NORMAL_INDEX] = p->stCallback.pfnNormalRecCb;
                ...
                fparam->rs = rs;
                fparam->dir_type = ((CVI_RECORDER_GET_DIR_TYPE_CALLBACK)(p->stCallback.pfnGetDirType))(rs->id, DTCF_DIR_NORM_FRONT);
                fparam->file_type = get_dtcf_type(p->recorder_file_type);
                rec_attr.fncallback.pfn_get_filename_param[CVI_RECORDER_CALLBACK_TYPE_NORMAL] = (void *)fparam;

                rec_attr.fncallback.pfn_event_cb[CVI_RECORDER_TYPE_EVENT_INDEX] = p->stCallback.pfnEventRecCb;
                rec_attr.u32PostRecTimeSec = p->u32PostRecTimeSec;
                rec_attr.u32PreRecTimeSec = p->u32PreRecTimeSec;
                fparam = (recorder_get_filename_param_t *)malloc(sizeof(recorder_get_filename_param_t));
                fparam->rs = rs;
                fparam->dir_type = ((CVI_RECORDER_GET_DIR_TYPE_CALLBACK)(p->stCallback.pfnGetDirType))(rs->id, DTCF_DIR_EMR_FRONT);
                fparam->file_type = get_dtcf_type(p->recorder_file_type);
                rec_attr.fncallback.pfn_get_filename_param[CVI_RECORDER_CALLBACK_TYPE_EVENT] = (void *)fparam;
            } else if (rec_attr.enRecType == CVI_RECORDER_TYPE_LAPSE) {
                rec_attr.fncallback.pfn_event_cb[CVI_RECORDER_TYPE_LAPSE_INDEX] = p->stCallback.pfnLapseRecCb;
                rec_attr.fncallback.pfn_event_cb_param = (void *)&rs->id;
                fparam = (recorder_get_filename_param_t *)malloc(sizeof(recorder_get_filename_param_t));
                fparam->rs = rs;
                fparam->dir_type = ((CVI_RECORDER_GET_DIR_TYPE_CALLBACK)(p->stCallback.pfnGetDirType))(rs->id, DTCF_DIR_NORM_FRONT);
                fparam->file_type = get_dtcf_type(p->recorder_file_type);
                rec_attr.fncallback.pfn_get_filename_param[CVI_RECORDER_CALLBACK_TYPE_LAPSE] = (void *)fparam;
            ...
            CVI_S32 presec = ((rec_attr.u32PreRecTimeSec >= (CVI_U32)rec_attr.s32MemRecPreSec) ? rec_attr.u32PreRecTimeSec : (CVI_U32)rec_attr.s32MemRecPreSec);
            if (presec == 0 || rec_attr.enRecType == CVI_RECORDER_TYPE_LAPSE) {
                presec = 1;
            } else {
                presec += 1;
            }
            CVI_U32 bitrate = p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_VIDEO].unTrackSourceAttr.stVideoInfo.u32BitRate;
            CVI_U32 sub_bitrate = p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_SUB_VIDEO].unTrackSourceAttr.stVideoInfo.u32BitRate;
            CVI_U32 sampleRate = p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_AUDIO].unTrackSourceAttr.stAudioInfo.u32SampleRate;
            CVI_U32 chns = p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_AUDIO].unTrackSourceAttr.stAudioInfo.u32ChnCnt;
            CVI_LOGD("video bitrate %u audio sampleRate %u chns %u presec %d", bitrate, sampleRate, chns, presec);
            rec_attr.stRbufAttr[CVI_RECORDER_RBUF_VIDEO].size = get_video_buffer_size(bitrate, presec) * 1024;
            rec_attr.stRbufAttr[CVI_RECORDER_RBUF_VIDEO].name = (const CVI_CHAR *)"rs_v";
            if(p->enable_subvideo){
                rec_attr.enable_subvideo = p->enable_subvideo;
                rec_attr.stRbufAttr[CVI_RECORDER_RBUF_SUB_VIDEO].size = get_video_buffer_size(sub_bitrate, presec) * 1024;
                rec_attr.stRbufAttr[CVI_RECORDER_RBUF_SUB_VIDEO].name = (const CVI_CHAR *)"rs_sub_v";
            }
            rec_attr.stRbufAttr[CVI_RECORDER_RBUF_AUDIO].size =  get_audio_pcm_buffer_size(sampleRate, chns, 2, presec);
            rec_attr.stRbufAttr[CVI_RECORDER_RBUF_AUDIO].name = (const CVI_CHAR *)"rs_a";
            rec_attr.stRbufAttr[CVI_RECORDER_RBUF_SUBTITLE].size = 50 * 1024;
            rec_attr.stRbufAttr[CVI_RECORDER_RBUF_SUBTITLE].name = (const CVI_CHAR *)"rs_s";

            rec_attr.id = rs->id;
            CHECK_RET(CVI_RECORDER_Create(&rs->recorder[0], &rec_attr));

            if (rec_attr.enRecType == CVI_RECORDER_TYPE_NORMAL) {
                if (p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_AUDIO].enable == 1) {
                    stop_mute(rs);
                } else {
                    start_mute(rs);
                }
...
typedef int32_t (*CVI_GET_GPSINFO_CB_FN_PTR)(CVI_RECORD_SERVICE_GPS_INFO_S *info);
static void cvi_master_subtitle_task_entry(void *arg) {
    rs_context_handle_t rs = (rs_context_handle_t)arg;
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    CVI_RECORDER_FRAME_STREAM_S frame;
    frame.type = CVI_MUXER_FRAME_TYPE_SUBTITLE;
    frame.num = 1;
    frame.thumbnail_data = NULL;
    frame.thumbnail_len = 0;
    CVI_GET_GPSINFO_CB_FN_PTR cb = (CVI_GET_GPSINFO_CB_FN_PTR)(p->stCallback.pfnGetGPSInfoCb);
    CVI_RECORD_SERVICE_GPS_INFO_S info;

#define CVI_MUXER_SUBTITLE_MAX_LEN (200)
#define SUBTITLE_GPS_HEAD_LEN (5) /*0'0x0,1'dataLen,2''G',3''P',4''S'*/
#define SUBTITLE_HEAD_LEN (2) /*0'0x0,1'dataLen*/

    if(cb){
        frame.len[0] = sizeof(CVI_RECORD_SERVICE_GPS_RMCINFO_S) + SUBTITLE_GPS_HEAD_LEN;
    }else{
        frame.len[0] = CVI_MUXER_SUBTITLE_MAX_LEN + SUBTITLE_HEAD_LEN;
    }
    frame.data[0] = (uint8_t *)malloc(frame.len[0]);

    uint64_t subtitle_duration_ms = 200; /*1000 / framerate*/
    uint64_t start = CVI_RECORDER_GetUs();
    uint64_t sub_cnt = 0;
    while (!rs->shutdown) {
        if(rs->stop_flag != 0 || !p->enable_subtitle || p->enRecType != CVI_RECORDER_TYPE_NORMAL){
            cvi_osal_task_sleep(50 * 1000);
            sub_cnt = 0;
            continue;
        }
        uint64_t end = CVI_RECORDER_GetUs();
        if(sub_cnt == 0 || (end - start) >= subtitle_duration_ms * 1000 * sub_cnt){
            if(sub_cnt == 0){
                start = CVI_RECORDER_GetUs();
            }
            memset(frame.data[0], 0, frame.len[0]);
            if(cb){
                cb(&info);
                frame.data[0][0] = 0x0;
                frame.data[0][1] = sizeof(CVI_RECORD_SERVICE_GPS_RMCINFO_S);
                frame.data[0][2] = 'G';
                frame.data[0][3] = 'P';
                frame.data[0][4] = 'S';
                ...
                frame.data[0][0] = 0x0;
                frame.data[0][1] = CVI_MUXER_SUBTITLE_MAX_LEN;
                ((CVI_RECORDER_GET_SUBTITLE_CALLBACK)(p->stCallback.pfnGetSubtitleCb))(NULL, rs->id, (char *)frame.data[0] + SUBTITLE_HEAD_LEN, CVI_MUXER_SUBTITLE_MAX_LEN);
            }
            frame.vi_pts[0] = end;
            LOG_RET(CVI_RECORDER_SendFrame(rs->recorder[0], &frame));
            sub_cnt++;
        }
...
static void cvi_master_video_task_entry(void *arg) {
    rs_context_handle_t rs = (rs_context_handle_t)arg;
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
...
#ifndef SERVICES_SUBVIDEO_ON
static void cvi_master_sub_video_task_entry(void *arg) {
    rs_context_handle_t rs = (rs_context_handle_t)arg;
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
...
static int32_t start_mem_rec(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0] && (p->s32MemRecPreSec > 0)) {
        CHECK_RET(CVI_RECORDER_Start_MemRec(rs->recorder[0]));
    } else {
...
static int32_t stop_mem_rec(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0] && (p->s32MemRecPreSec > 0)) {
        CHECK_RET(CVI_RECORDER_Stop_MemRec(rs->recorder[0]));
    } else {
        return RS_ERR_FAILURE;
    }
...
static int32_t start_normal_rec(rs_context_handle_t rs) {
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    p->enRecType = CVI_RECORDER_TYPE_NORMAL;
    ...
    switch (p->recorder_file_type) {
        ...
        case CVI_RECORD_SERVICE_FILE_TYPE_MOV:
        case CVI_RECORD_SERVICE_FILE_TYPE_TS: {
            if (CVI_RECORDER_Start_NormalRec(rs->recorder[0]) != 0) {
                return RS_ERR_FAILURE;
            }
        } break;
        ...
        case CVI_RECORD_SERVICE_FILE_TYPE_ES: {
            static int32_t i = 0;
            char outputfile[64];
            if (p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_VIDEO].unTrackSourceAttr.stVideoInfo.enCodecType
                    == CVI_MUXER_TRACK_VIDEO_CODEC_H264) {
                sprintf(outputfile, "%svid_%04d.h264", p->mntpath, i++);
            } else if (p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_VIDEO].unTrackSourceAttr.stVideoInfo.enCodecType
                    == CVI_MUXER_TRACK_VIDEO_CODEC_H265) {
                sprintf(outputfile, "%svid_%04d.h265", p->mntpath, i++);
            } else {
                CVI_LOG_ASSERT(0, "invalide video codec %d", p->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_VIDEO].unTrackSourceAttr.stVideoInfo.enCodecType);
            }
    ...
    if (p->handles.venc_rec_start == 0) {
        CVI_MAPI_VENC_StartRecvFrame(p->handles.rec_venc_hdl, -1);
#ifndef SERVICES_SUBVIDEO_ON
        if(p->enable_subvideo){
          CVI_MAPI_VENC_StartRecvFrame(p->handles.sub_rec_venc_hdl, -1);
        }
#endif
...
static int32_t stop_normal_rec(rs_context_handle_t rs) {
    ...
    switch (p->recorder_file_type) {
        ...
        case CVI_RECORD_SERVICE_FILE_TYPE_TS:
        {
            ...
            LOG_RET(CVI_RECORDER_Stop_NormalRec(rs->recorder[0]));
#ifdef SERVICES_SUBVIDEO_ON
...
static int32_t start_event_rec(rs_context_handle_t rs) {
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    if (p->handles.venc_rec_start == 0 || p->enRecType == CVI_RECORDER_TYPE_LAPSE) {
        return RS_SUCCESS;
    }
    if (0 != CVI_RECORDER_Start_EventRec(rs->recorder[0])) {
...
static int32_t stop_event_rec(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0]) {
        LOG_RET(CVI_RECORDER_Stop_EventRec(rs->recorder[0]));
    } else {
...
static int32_t stop_event_rec_post(rs_context_handle_t rs) {
    ...
    if (rs->recorder[0]) {
        LOG_RET(CVI_RECORDER_Stop_EventRecPost(rs->recorder[0]));
    } else {
        CVI_LOGE("RS[%d]: Event recorder is NULL", rs->id);
    }
    return RS_SUCCESS;
...
#ifndef SERVICES_SUBVIDEO_ON
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    p->enRecType = CVI_RECORDER_TYPE_LAPSE;

    CVI_RECORDER_Start_LapseRec(rs->recorder[0]);
...
#else
    UNUSED(rs);
    CVI_LOGE("If you enable the VCAP service, Timelapse is not supported");
#endif
    return RS_SUCCESS;
}
...
#ifndef SERVICES_SUBVIDEO_ON
    CVI_RECORDER_Stop_LapseRec(rs->recorder[0]);
#else
    UNUSED(rs);
    CVI_LOGE("If you enable the VCAP service, Timelapse is not supported");
#endif
...
static int32_t handle_state_transition(rs_context_handle_t rs) {
    ...
    if (RS_STATE_UP(rs, REC_CREATE)) {
        CHECK_RET(cvi_master_create_rec(rs));
    }
    ...
    if (RS_STATE_DOWN(rs, REC_CREATE)) {
        LOG_RET(cvi_master_destroy_rec(rs));
    }

    return RS_SUCCESS;
}

static void cvi_master_state_task_entry(void *arg)
{
    rs_context_handle_t rs = (rs_context_handle_t)arg;
    while (!rs->shutdown) {
        // handle transition
...
static int32_t cvi_master_start_task(rs_context_handle_t rs) {
    static char state_name[16] = {0};
    snprintf(state_name, sizeof(state_name), "rs_state_%d", rs->id);
    cvi_osal_task_attr_t ta;
    ta.name = state_name;
    ta.entry = cvi_master_state_task_entry;
    ta.param = (void *)rs;
    ta.priority = CVI_OSAL_PRI_RT_HIGH;
    ta.detached = false;
    int32_t rc = cvi_osal_task_create(&ta, &rs->state_task);
    if (rc != CVI_OSAL_SUCCESS) {
        CVI_LOGE("rs_state task create failed, %d", rc);
        return RS_ERR_FAILURE;
    }

    cvi_osal_task_attr_t va;
    static char rs_name[16] = {0};
    snprintf(rs_name, sizeof(rs_name), "rs_video_%d", rs->id);
    va.name = rs_name;
    va.entry = cvi_master_video_task_entry;
    va.param = (void *)rs;
    va.priority = CVI_OSAL_PRI_RT_MID + 20;
    va.detached = false;
    rc = cvi_osal_task_create(&va, &rs->video_task);
    if (rc != CVI_OSAL_SUCCESS) {
    ...
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)rs->attr;
    if(p->enable_subvideo){
        cvi_osal_task_attr_t sub_va;
        static char subv_name[16] = {0};
        snprintf(subv_name, sizeof(subv_name), "rs_sub_video_%d", rs->id);
        sub_va.name = subv_name;
        sub_va.entry = cvi_master_sub_video_task_entry;
        sub_va.param = (void *)rs;
        sub_va.priority = CVI_OSAL_PRI_RT_MID + 20;
        sub_va.detached = false;
        rc = cvi_osal_task_create(&sub_va, &rs->sub_video_task);
        if (rc != CVI_OSAL_SUCCESS) {
...
#endif
    cvi_osal_task_attr_t sa;
    static char sa_name[16] = {0};
    snprintf(sa_name, sizeof(sa_name), "rs_subtitle_%d", rs->id);
    sa.name = sa_name;
    sa.entry = cvi_master_subtitle_task_entry;
    sa.param = (void *)rs;
    sa.priority = CVI_OSAL_PRI_RT_MID;
    sa.detached = false;
    rc = cvi_osal_task_create(&sa, &rs->subtitle_task);
    if (rc != CVI_OSAL_SUCCESS) {
    ...
    piv_ta.priority = CVI_OSAL_PRI_RT_LOW;
    piv_ta.detached = false;
    rc = cvi_osal_task_create(&piv_ta, &rs->piv_task);
    if (rc != CVI_OSAL_SUCCESS) {
    ...
    rc = cvi_osal_task_create(&thumb_ta, &rs->thumb_task);
    if (rc != CVI_OSAL_SUCCESS) {
...
void *cvi_master_create(int32_t id, CVI_REC_ATTR_T *attr) {
    ...
    // int32_t rc = cvi_osal_mutex_create(&ma, &handle->state_mutex);
    // if (rc != CVI_OSAL_SUCCESS) {
    ...
    cvi_master_start_task(handle);

    if (attr->enRecType == CVI_RECORDER_TYPE_NORMAL) {
        rs_enable_state(handle, RS_STATE_MUTE_EN);
        if (attr->astStreamAttr[0].aHTrackSrcHandle[CVI_RECORDER_TRACK_SOURCE_TYPE_AUDIO].enable == 1) {
...
int32_t cvi_master_destroy(int32_t id) {
    ...
    rs_context_handle_t handle = &gstRecMasterCtx[id];
    CVI_REC_ATTR_T *p = (CVI_REC_ATTR_T *)handle->attr;

    CVI_LOGD("RS[%d] destroy start", id);
    rs_change_state(handle, RS_STATE_IDLE);
    handle->shutdown = 1;
    int32_t rc = cvi_osal_task_join(handle->state_task);
    if (rc != CVI_OSAL_SUCCESS) {
    ...
    rc = cvi_osal_task_join(handle->video_task);
    if (rc != CVI_OSAL_SUCCESS) {
        CVI_LOGE("rs_video task join failed, %d", rc);
        return RS_ERR_FAILURE;
    }
    cvi_osal_task_destroy(&handle->video_task);

    if(p->enable_subvideo){
        rc = cvi_osal_task_join(handle->sub_video_task);
        if (rc != CVI_OSAL_SUCCESS) {
            CVI_LOGE("rs_sub_video task join failed, %d", rc);
            return RS_ERR_FAILURE;
        }
        cvi_osal_task_destroy(&handle->sub_video_task);
    }

    rc = cvi_osal_task_join(handle->subtitle_task);
    if (rc != CVI_OSAL_SUCCESS) {
        CVI_LOGE("subtitle task join failed, %d", rc);
        return RS_ERR_FAILURE;
    }
    cvi_osal_task_destroy(&handle->subtitle_task);

    rc = cvi_osal_task_join(handle->thumb_task);
    if (rc != CVI_OSAL_SUCCESS) {
    ...
    CVI_RECORDER_Destroy(&handle->recorder[0]);
    CVI_RECORDER_Destroy(&handle->recorder[1]);

#ifdef ENABLE_SNAP_ON
    pthread_mutex_lock(&handle->piv_mutex);
    pthread_cond_signal(&handle->piv_cond);
    pthread_mutex_unlock(&handle->piv_mutex);
    rc = cvi_osal_task_join(handle->piv_task);
    if (rc != CVI_OSAL_SUCCESS) {
...
int32_t cvi_master_start_mem_rec(int32_t id) {
    ...
    rs_context_handle_t handle = &gstRecMasterCtx[id];
    if (((CVI_REC_ATTR_T *)handle->attr)->s32MemRecPreSec <= 0) {
        return 0;
    }
