...
void ImageViewer::setDecodeHandle(CVI_MAPI_VDEC_HANDLE_T handle)
{
    vdec_handle = handle;
}

void ImageViewer::setDisplayHandle(CVI_MAPI_DISP_HANDLE_T handle)
{
...
{
    CVI_THUMBNAIL_PACKET_S thumbnail_packet = {};
    VIDEO_FRAME_INFO_S yuv_frame = {};

    if (getThumbnailPacket(input, thumbnail_packet) != 0) {
        CVI_THUMBNAIL_EXTRACTOR_ClearPacket(&thumbnail_packet);
        return -1;
    }
    ...
    if (output_handler) {
        ...
        CVI_MAPI_ReleaseFrame(&yuv_frame);
    } else {
...
int32_t ImageViewer::getThumbnailPacket(const std::string &input, CVI_THUMBNAIL_PACKET_S &packet)
{
    if (extractor_handle == nullptr) {
        CVI_THUMBNAIL_EXTRACTOR_Create(&extractor_handle);
    }
    // get thumbnail packet from input file
    if (CVI_THUMBNAIL_EXTRACTOR_GetThumbnail(extractor_handle, input.c_str(),
            &packet) != 0) {
...
int32_t ImageViewer::decodePacketToYUVFrame(CviDemuxerPacket &src_packet, VIDEO_FRAME_INFO_S &dst_frame)
{
