...
typedef ret_t (*CVI_UISet_CallBack)(void* ctx, event_t* e);

typedef struct tagUI_SELOPTION_CALLBACK_S {
    CVI_UISet_CallBack ui_seloption_EVT;
...
typedef struct tagUI_MEU_CALLBACK_S {
    CVI_UISet_CallBack ui_widget_on;
    bool ui_set_video;
	bool ui_set_label;
} UI_MEU_CALLBACK_S;
...
typedef struct tagUI_CLICK_CALLBACK_S {
    uint32_t type;
    CVI_UISet_CallBack ui_seloption;
} UI_CLICK_CALLBACK_S;
...
{
    if (viedosize == CVI_MEDIA_VIDEO_SIZE_2560X1440P25){
        *itemcnt = 0;
    } else if (viedosize == CVI_MEDIA_VIDEO_SIZE_1920X1080P25){
        *itemcnt = 1;
    } else {
...
static void set_option_usbmodeitemcnt(uint32_t mode, uint32_t *itemcnt)
{
    if (mode == CVI_WORK_MODE_UVC){
        *itemcnt = 0;
    } else if (mode == CVI_WORK_MODE_STORAGE){
        *itemcnt = 1;
    } else {
                ...
                if (tk_str_eq(name, ui_optionwidget[id])) {
                    if (s_astOptionUpdateWinCallBack[id].ui_set_update_video) {
                        ...
                        if (CVI_WORK_MODE_MOVIE == s32CurMode) {
                            ...
                            CVI_PARAM_MEDIA_COMM_S menMediaComm;
                            CVI_PARAM_GetMediaPhotoSize(&menMediaComm);
...
static ret_t on_optionuvc_click(void* ctx, event_t* e)
{
    ...
    widget_t* win = widget_get_window(list_item);
    int32_t  index = widget_index_of(list_item);
    int32_t  a_usbmode = CVI_WORK_MODE_BUTT;
    ...
    if (index != selOpt) {
        ...
        if (0 == index) {
            ...
            if (USB_POWER_SOURCE_PC == enPowerState) {
                if ((CVI_WORK_MODE_UVC != CVI_MODEMNG_GetCurWorkMode()) && (CVI_WORK_MODE_STORAGE != CVI_MODEMNG_GetCurWorkMode())) {
                    if (1 == index) {
                        a_usbmode = CVI_WORK_MODE_UVC;
                    } else {
                        a_usbmode = CVI_WORK_MODE_STORAGE;
                    }
...
static ret_t on_option_pwm_bri_click(void* ctx, event_t* e)
{
    ...
    if (index != selOpt) {
        CVI_MESSAGE_S Msg = {0};
        Msg.topic = CVI_EVENT_MODEMNG_SETTING;
        Msg.arg1 = CVI_PARAM_MENU_PWM_BRI_STATUS;
        Msg.arg2 = index;
        CVI_MODEMNG_SendMessage(&Msg);
        selOpt = index;
        update_optionssel(win);
...
static ret_t on_option_view_win_click(void* ctx, event_t* e)
{
    ...
    if(index < sizeof(ui_optionviewwin) / sizeof(ui_optionviewwin[0]) - 1){
        enWind = (0x1 << index);
    }else{
        enWind = enSns;
    }
    ...
    if((enWind & enSns) != 0){
        if (index != selOpt) {
            CVI_MESSAGE_S Msg = {0};
            Msg.topic = CVI_EVENT_MODEMNG_SETTING;
            Msg.arg1 = CVI_PARAM_MENU_VIEW_WIN_STATUS;
...
static ret_t on_option_screendormant_click(void* ctx, event_t* e)
{
    ...
    if (index != selOpt) {
        CVI_MESSAGE_S Msg = {0};
        Msg.topic = CVI_EVENT_MODEMNG_SETTING;
        Msg.arg1 = CVI_PARAM_MENU_SCREENDORMANT;
        Msg.arg2 = index;
        CVI_MODEMNG_SendMessage(&Msg);
        selOpt = index;
        update_optionssel(win);
...
static ret_t on_optioncodec_click(void* ctx, event_t* e)
{
    ...
    if (index != selOpt) {
        ...
        CVI_MESSAGE_S Msg = {0};
        Msg.topic = CVI_EVENT_MODEMNG_SETTING;
        Msg.arg1 = CVI_PARAM_MENU_VIDEO_CODEC;
        Msg.arg2 = index;
        CVI_UICOMM_SendAsyncMsg(&Msg, ui_winmng_receivemsgresult);
        selOpt = index;
        update_optionssel(win);
