...
static ret_t on_setup_click(void* ctx, event_t* e)
{
    ...
    Msg.arg1 = CVI_EVENT_MODEMNG_PHOTO_SET;
...
static ret_t on_photo_click(void* ctx, event_t* e)
{
    ...
    if(ui_home_photo_cardstatus()) {
        ...
        s32Ret = CVI_UICOMM_SendAsyncMsg(&Msg, UI_Home_OnReceiveMsgResult);
        if(s32Ret != 0) {
            return RET_OK;
        }
...
static ret_t progress_bar_on_timer(const timer_info_t* timer)
{
    ...
    progress_bar_set_value(progress_bar, CVI_USB_STATE_INSERT == penState ? 100 : CVI_Vbat_Get_Percentage());
    #endif
    return RET_REPEAT;
}
...
int32_t  ui_home_photo_page_eventcb(void *argv, CVI_EVENT_S *msg)
{
    CVI_MESSAGE_S Msg = {0};
