...
#define DATETIME_MAX_YEAR       2030
#define DATETIME_DEFAULT_YEAR   2021
...
static cvi_ui_time ui_time = { DATETIME_DEFAULT_YEAR, DATETIME_DEFAULT_MONTH, DATETIME_DEFAULT_DAY, \
...
static void on_settime_page_add_daytime(uint32_t curbutton, void* ctx)
{
    switch(curbutton) {
        case UI_DATETIME_Y:
            if(ui_time.year==DATETIME_MAX_YEAR)
                ui_time.year = DATETIME_DEFAULT_YEAR;
            else
                ui_time.year++;
            break;
        case UI_DATETIME_M:
            ...
            break;
        case UI_DATETIME_D:
            if(ui_time.day==DayOfMonth[Year_IsLeapYear(ui_time.year)][ui_time.mouth-1])
                ui_time.day=1;
            else
                ui_time.day++;
            break;
...
static void on_settime_page_sub_daytime(uint32_t curbutton, void* ctx)
{
    switch(curbutton) {
        case UI_DATETIME_Y:
            if(ui_time.year==DATETIME_DEFAULT_YEAR)
                ui_time.year = DATETIME_MAX_YEAR;
            else
                ui_time.year--;
            break;
        case UI_DATETIME_M:
        ...
        case UI_DATETIME_D:
            if(ui_time.day==1)
                ui_time.day=DayOfMonth[Year_IsLeapYear(ui_time.year)][ui_time.mouth-1];
            else
                ui_time.day--;
            break;
        case UI_DATETIME_HR:
...
static ret_t return_time_event(void* ctx, event_t* e)
{
    widget_t* win = WIDGET(ctx);
    (void)e;
    CVI_SYSTEM_TM_S pstDateTime;
    memcpy(&pstDateTime, &ui_time, sizeof(CVI_SYSTEM_TM_S));
...
ret_t open_settime_window(void* ctx, event_t* e)
{
    ...
    widget_t* win = WIDGET(ctx);
    if (win) {
        CVI_SYSTEM_GetRTCDateTime(&pstDateTime);
        memcpy(&ui_time, &pstDateTime, sizeof(CVI_SYSTEM_TM_S));
...
ret_t close_settime_window(void* ctx, event_t* e)
{
    ...
    widget_t* win = WIDGET(ctx);
    if (win) {
        CVI_SYSTEM_SetDateTime(&ui_time);
    }
    return RET_FAIL;
}
