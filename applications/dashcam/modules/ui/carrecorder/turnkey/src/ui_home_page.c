...
static ret_t on_home_page_key_down(void* ctx, event_t* e)
{
    ...
    uint32_t u32ModeState = 0;
    CVI_MESSAGE_S Msg = {0};
    CVI_HAL_SCREEN_STATE_E ScreenState = CVI_HAL_SCREEN_STATE_OFF;
    CVI_HAL_SCREEN_GetBackLightState(CVI_HAL_SCREEN_IDX_0, &ScreenState);
    ...
    if (ScreenState == CVI_HAL_SCREEN_STATE_OFF) {
    ...
    if (evt->key == UI_KEY_POWER) {
        ...
        if(evt->alt == false) {
            ...
            if(ui_home_cardstatus() && (u32ModeState != CVI_MEDIA_MOVIE_STATE_REC)) {
        ...
        if (ui_home_cardstatus() && CVI_MODEMNG_GetEmrState() == false) {
