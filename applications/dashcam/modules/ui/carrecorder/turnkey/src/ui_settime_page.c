...
static CVI_SYSTEM_TM_S ui_time =    {CVI_DATETIME_DEFAULT_YEAR,
                                    CVI_DATETIME_DEFAULT_MONTH,
                                    CVI_DATETIME_DEFAULT_DAY,
                                    CVI_DATETIME_DEFAULT_HOUR,
                                    CVI_DATETIME_DEFAULT_MINUTE,
                                    CVI_DATETIME_DEFAULT_SECOND};
...
bool_t ui_check_time_format_type(void)
{
    ...
    if (menu_param.TimeFormat.Current == CVI_MENU_TIME_FORMAT_12) {
        return true;
    }
...
{
    if (ui_time.s32year == CVI_DATETIME_MAX_YEAR) {
        ui_time.s32year = CVI_DATETIME_DEFAULT_YEAR;
    } else {
        ui_time.s32year++;
    }
}
...
{
    if (ui_time.s32year == CVI_DATETIME_DEFAULT_YEAR) {
        ui_time.s32year = CVI_DATETIME_MAX_YEAR;
    } else {
        ui_time.s32year--;
    }
}
...
ret_t ui_datetime_page_open(void* ctx, event_t* e)
{
    ...
    if (win) {
        // 获取RTC时间
        CVI_SYSTEM_GetRTCDateTime(&ui_time);
