...
static void show_wrnwin(void)
{
    ...
    CVI_MODEMNG_SetModeState(CVI_MEDIA_PLAYBACK_STATE_ABNORMAL);
}
...
static ret_t ui_playback_page_updatefile(void)
{
    ...
    s32Ret =  CVI_FILEMNG_GetFileByIndex(playback_info.curfilenum, playback_info.curfilename, MAX_PATH);
    if (s32Ret != 0) {
    ...
    s32Ret = CVI_PLAYER_SERVICE_SetInput(ps_handle, playback_info.curfilename);
    if (s32Ret != 0) {
...
static ret_t on_stopplay_click(void* ctx, event_t* e) 
{
    ...
    CVI_PLAYER_SERVICE_HANDLE_T ps_handle = CVI_MEDIA_GetCtx()->SysServices.PsHdl;
    CVI_PLAYER_SERVICE_Stop(ps_handle);
    CVI_MODEMNG_SetModeState(CVI_MEDIA_PLAYBACK_STATE_VIEW);
    return RET_OK;
}
...
static int32_t  playback_delete_curfile(void)
{
    ...
    if (1 < playback_info.totalfile) {
        s32Ret = CVI_FILEMNG_RemoveFile(playback_info.curfilename);
        CVI_APPCOMM_CHECK_RETURN(s32Ret, s32Ret);
        ...
        CVI_APPCOMM_CHECK_RETURN(s32Ret, s32Ret);
        CVI_FILEMNG_GetFileByIndex(playback_info.curfilenum, playback_info.curfilename, MAX_PATH);
    } else {
...
static ret_t on_playback_page_key_down(void* ctx, event_t* e)
{
    ...
    CVI_MODEMNG_GetModeState(&CurModeState);
    ...
    if (evt->key == UI_KEY_POWER) {
        if(evt->alt == false) {
            if (CVI_MEDIA_PLAYBACK_STATE_ABNORMAL != CurModeState) {
                if (playback_info.backforward == 0 || playback_info.backforward == 1) {
                    ...
                    speeds = 1;
                    CVI_PLAYER_SERVICE_PlayerSeep(ps_handle, speeds);
                    playback_info.backforward = 2;
                    widget_set_text_utf8(label, "");
                ...
                widget_t* image = widget_lookup(win, "play_pause_image", TRUE);
                if (CVI_MEDIA_PLAYBACK_STATE_VIEW == CurModeState || CVI_MEDIA_PLAYBACK_STATE_PAUSE == CurModeState) {
        ...
        if(evt->alt == false) {
            if (CVI_MEDIA_PLAYBACK_STATE_PLAY != CurModeState) {
        ...
        if(evt->alt == false) {
            if (CVI_MEDIA_PLAYBACK_STATE_VIEW == CurModeState || CVI_MEDIA_PLAYBACK_STATE_PAUSE == CurModeState || 
                CVI_MEDIA_PLAYBACK_STATE_ABNORMAL == CurModeState || playback_info.curdirtype == DTCF_DIR_PHOTO_FRONT || 
                ...
                if (playback_info.backforward == 2) {
                    ...
                    CVI_PLAYER_SERVICE_PlayerSeepBack(ps_handle, speeds);
...
static ret_t ui_playback_page_init(widget_t* widget)
{
    ...
    if (0 == playback_info.init_flag) {
        CVI_FILEMNG_SetSearchScope(&playback_info.curdirtype, 1, &playback_info.pu32FileObjCnt);
