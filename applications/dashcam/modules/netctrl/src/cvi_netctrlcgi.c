...
unsigned long CVI_CGI_Snprintf(char *cgibuff, char *buf, int32_t size, const char *fmt, ...)
{
    unsigned long len = 0;
    va_list marker;
    va_start(marker, fmt);
...
void CVI_CGI_Currency(httpd_conn *hc, char *cmd, char *str, int32_t ret, char *log)
{
    char *buf = NULL;
    ...
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, ret, status);

    if (ret != 0 && log != NULL) {
        CVI_CGI_Snprintf(cgibuff, buf, strlen(log), log);
    } else if (cmd != NULL && str != NULL) {
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, cmd, str);
    }

    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, ret, status);
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
int32_t CVI_CGI_SetMovieRecord(httpd_conn *hc, char *str)
{
    ...
    if (strcmp(str, "720P25")==0) {pstr = 0;}
        ...
        CVI_CGI_Currency(hc, NULL, NULL, result, "error input\r\n");
        return result;
    }
    ...
    if (pstr < 0 || pstr > 11) {
        ...
        if (0 != sendresult || 0 != s32Result) {
            ...
            result = CVI_WIFIAPP_FALSE;
            CVI_CGI_Currency(hc, NULL, NULL, result, "send message fail\r\n");
            return result;
        } else {
            CVI_NETCTRLINNER_UiUpdate();
        }
    }
    CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
...
int32_t CVI_CGI_SetCyclicRecordValue(httpd_conn *hc, char *str)
{
    ...
    if (0 != sendresult || 0 != s32Result) {
        CVI_LOGE("send message fail");
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "start rec: send message fail\r\n");
        return result;
    }
...
int32_t CVI_CGI_SetGSensorSensitivity(httpd_conn *hc, char *str)
{
    ...
    if ((pstr < 0) || (pstr > 3)) {
        ...
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "EDisablemovieGSensorsensitivity parm error\r\n");
    } else {
        Msg.topic = CVI_EVENT_MODEMNG_SETTING;
...
int32_t CVI_CGI_SetMovieEvValue(httpd_conn *hc, char *str)
{
    ...
    if (strcmp(str,"EVN200") == 0)      value = EV_value / 3.98;
        ...
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "input false\r\n");
    }
...
int32_t CVI_CGI_CommandWouldFormatStorage(httpd_conn *hc, char *str)
{
    ...
    if (0 != s32Result || 0 != sendresult) {
        ...
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "CVI_MODE_Format failed\r\n");
    } else {
...
static int32_t CVI_CGI_SetSSID(httpd_conn *hc, char *str)
{
    int32_t s32Ret = 0;
    int32_t result = 0;
    ...
    if (0 != s32Ret) {
        ...
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "Api passphrase cmd CVI_PARAM_GetWifiParam faile\r\n");
        return result;
    }
    ...
    if (0 != s32Ret) {
        ...
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "SetWiFipassphrase CVI_PARAM_SetWifiParam fail\r\n");
        return result;
    }
    CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
...
static int32_t CVI_CGI_GetSSID(httpd_conn *hc, char *str)
{
    int32_t s32Ret = 0;
    int32_t result = 0;
    CVI_PARAM_WIFI_S wifipar;
    ...
    if (0 != s32Ret) {
        ...
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "Api GetSSIDandpassphrase cmd CVI_PARAM_GetWifiParam faile\r\n");
        return result;
    }

    memset(cgibuff, 0, sizeof(cgibuff));
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, result, "OK");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Net.WIFI_AP.SSID", wifipar.WifiCfg.unCfg.stApCfg.stCfg.szWiFiSSID);
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
/*cgi 对wifi名称进行操作*/
int32_t CVI_CGI_SetGetSSID(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    int32_t s32Result = 0;
    int32_t sendresult = 0;
    ...
    //连接APP，需从录像模式进入
    if (CVI_MODEMNG_GetCurWorkMode() != CVI_WORK_MODE_MOVIE) {
        CVI_MESSAGE_S Msg = {0};
    ...
    if (str != NULL) {
        result = CVI_CGI_SetSSID(hc, str);
    } else {
        result = CVI_CGI_GetSSID(hc, str);
    }
...
static int32_t CVI_CGI_SetCryptoKey(httpd_conn *hc, char *str)
{
    ...
    s32Ret = CVI_PARAM_GetWifiParam(&wifipar);
    if (0 != s32Ret) {
        CVI_LOGI("Api passphrase cmd CVI_PARAM_GetWifiParam faile\n");
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "Api passphrase cmd CVI_PARAM_GetWifiParam faile\r\n");
        return result;
    }
    ...
    s32Ret = CVI_PARAM_SetWifiParam(&wifipar);
    if (0 != s32Ret) {
        CVI_LOGI("SetWiFipassphrase CVI_PARAM_SetWifiParam fail\n");
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "SetWiFipassphrase CVI_PARAM_SetWifiParam fail\r\n");
        return result;
    }
...
static int32_t CVI_CGI_GetCryptoKey(httpd_conn *hc, char *str)
{
    int32_t s32Ret = 0;
    int32_t result = 0;
    CVI_PARAM_WIFI_S wifipar;
    ...
    memset(cgibuff, 0, sizeof(cgibuff));
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, result, "OK");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Net.WIFI_AP.CryptoKey", wifipar.WifiCfg.unCfg.stApCfg.stCfg.szWiFiPassWord);
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
/*cgi 对wifi密码进行操作*/
int32_t CVI_CGI_SetGetCryptoKey(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    CVI_LOGE("enter CVI_CGI_SetGetCryptoKey, str = %s\n", str);
    if (str != NULL) {
        result = CVI_CGI_SetCryptoKey(hc, str);
    } else {
        result = CVI_CGI_GetCryptoKey(hc, str);
    }
...
int32_t CVI_CGI_ResetAllSettingTodeFault(httpd_conn *hc, char *str)
{
    ...
    if (0 != sendresult || 0 != s32Result) {
        ...
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "netctrl CVI_PARAM_SetDefaultParam failed\r\n");
    } else {
        CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
    }
    return result;
...
{
    int32_t result = 0;
    CVI_CGI_Currency(hc, "Camera.Menu.FWversion", CVI_VERSION_NUM, result, NULL);
    return result;
}
...
int32_t CVI_CGI_GetMenuUUID(httpd_conn *hc, char *str)
{
    ...
    CVI_CGI_Currency(hc, "Camera.menu.UUID", szMacAddrResult, result, NULL);
...
int32_t CVI_CGI_GetDevid(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
...
int32_t CVI_CGI_ListAllFileCreatTime(httpd_conn *hc, char *str)
{
    ...
    CVI_CGI_Snprintf(cgibuff_file, buf, sizeof(DEF_XML_HEAD), DEF_XML_HEAD);
    CVI_CGI_Snprintf(cgibuff_file, buf, sizeof(dir), dir);
    CVI_FILEMNG_SetSearchScope(&aenDirs, 1, &pu32FileObjCnt);
    ...
    for(uint32_t i = 0; i < pu32FileObjCnt; i++) {
        ...
        if (CVI_WIFIAPP_OK != result) {
            ...
            CVI_CGI_Snprintf(cgibuff_file, buf, BUFFSIZE, DEF_CGI_LISTAL_FILE, fpathname, format, filebuf.st_size, "RW", nowtime);
        }
    }
    ...
    CVI_CGI_Snprintf(cgibuff_file, buf, BUFFSIZE, DEF_CGI_CNT_FILE, pu32FileObjCnt);
    CVI_CGI_Snprintf(cgibuff_file, buf, sizeof(dir_l), dir_l);
    CVI_NET_AddResponse(hc, cgibuff_file, strlen(cgibuff_file));
...
int32_t CVI_CGI_GetScreennail(httpd_conn *hc, char *str)
{
    int32_t ret = 0;
    ...
    if (size == 0) {
        ...
        CVI_CGI_Currency(hc, NULL, NULL, CVI_WIFIAPP_FALSE, NULL);
...
int32_t CVI_CGI_DeviceCapability(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
...
int32_t CVI_CGI_BatteryLevel(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
...
int32_t CVI_CGI_GetMJPEGStatus(httpd_conn *hc, char *str)
{
    ...
    if ((u32ModeState != CVI_MEDIA_MOVIE_STATE_REC) &&
        (u32ModeState != CVI_MEDIA_MOVIE_STATE_LAPSE_REC)) {
        CVI_CGI_Currency(hc, "Camera.Preview.MJPEG.status.record", "Standy", result, NULL);
    } else if ((u32ModeState == CVI_MEDIA_MOVIE_STATE_REC) ||
            (u32ModeState == CVI_MEDIA_MOVIE_STATE_LAPSE_REC)) {
        CVI_CGI_Currency(hc, "Camera.Preview.MJPEG.status.record", "Recording", result, NULL);
    }
...
int32_t CVI_CGI_GetPreviewStatus(httpd_conn *hc, char *str)
{
    ...
    memset(cgibuff, 0, sizeof(cgibuff));
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, 0, "OK");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Preview.RTSP.av", way);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Preview.Source.1.Camid", FoRStat);
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
int32_t CVI_CGI_Startmovierecord(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    uint32_t u32ModeState = 0;
    int32_t s32Result = 0;
    ...
    int32_t CardStatus = CVI_MODEMNG_GetCardState();
    if (CardStatus != CVI_CARD_STATE_AVAILABLE) {
...
int32_t CVI_CGI_Stopmovierecord(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    uint32_t u32ModeState = 0;
    int32_t s32Result = 0;
...
int32_t CVI_CGI_getpictureend(httpd_conn *hc, char *str)
{
    int32_t result = 0;

    if (CVI_MODEMNG_GetCardState() == 0) {
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, " No SD\r\n");
        return result;
    }
...
int32_t CVI_CGI_MovieRecord(httpd_conn *hc, char *str)
{
    ...
    if (strcmp(str, "recordon") == 0) {
        CVI_CGI_Startmovierecord(hc, str);
    } else if (strcmp(str, "recordoff") == 0) {
        CVI_CGI_Stopmovierecord(hc, str);
    } else if (strcmp(str, "capture") == 0) {
        CVI_CGI_getpictureend(hc, str);
    } else {
        result = CVI_WIFIAPP_FALSE;
    }
...
int32_t CVI_CGI_OtaVersion(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
...
int32_t CVI_CGI_SDInfo(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    char *CarStat = NULL;
    char *buf = NULL;
    ...
    if (CVI_MODEMNG_GetCardState() == CVI_CARD_STATE_AVAILABLE) {
        CarStat = "ON";
    } else {
        CarStat = "OFF";
    }
    ...
    memset(cgibuff, 0, sizeof(cgibuff));
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, 0, "OK");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, "Get SD Status\n");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "SD0INFO", "1");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.SDInfo", CarStat);
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
int32_t CVI_CGI_CardInfo(httpd_conn *hc, char *str)
{
    ...
    if (FreePicNum == CVI_CARD_STATE_AVAILABLE) {
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.CardStatus", "NORMAL");
        ...
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.total", total);
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.remain", remain);
    } else if (FreePicNum > 0) {
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.CardStatus", "NEED_FORMAT");
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.total", "0");
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.remain", "0");
    } else {
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.CardStatus", "NONE");
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.total", "0");
        CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.CardInfo.remain", "0");
    }
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
int32_t CVI_CGI_EmmcInfo(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    // CVI_CGI_Currency(hc, NULL, NULL, result, NULL);
...
int32_t CVI_CGI_VideoresCapability(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    CVI_CGI_Currency(hc, "Camera.Menu.VideoresCapability", "ON", result, NULL);
...
int32_t CVI_CGI_GetCamMenu(httpd_conn *hc, char *str)
{
    ...
    CVI_PARAM_GetVideoSizeEnum(menuparam.VideoSize.Current, &videosize);
    ...
    memset(cgibuff, 0, sizeof(cgibuff));
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, 0, "OK");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.VideoRes", resolution);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.LoopingVideo", LoopingVideo);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.EV", EV);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.GSensor", gsensor);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.FWversion", CVI_VERSION_NUM);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.SoundRecord", SoundRecord);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.menuXML", "ON");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.RearStarus", enSns);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.SDInfo", CarStat);
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, 0, "OK");
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
int32_t CVI_CGI_SetEV(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    int32_t value = 0;
    if (strcmp(str,"EVN200") == 0)      value = EV_value / 3.98;
...
int32_t CVI_CGI_GetEdisableMovieAudio(httpd_conn *hc, char *str)
{
    ...
    int32_t result = 0;
    CVI_PARAM_MENU_S menuparam;
    s32Ret = CVI_PARAM_GetMenuParam(&menuparam);

    if (0 != s32Ret) {
        CVI_LOGE(" %s ,fail  \n", __func__);
        CVI_LOGE("Api CVI_PARAM_GetMenuParam faile\n");
        result = CVI_WIFIAPP_FALSE;
        CVI_CGI_Currency(hc, NULL, NULL, result, "Api CVI_PARAM_GetMenuParam faile\r\n");
        return result;
    }
    ...
    CVI_CGI_Currency(hc, "Camera.Menu.SoundRecord", ret, result, NULL);
...
int32_t CVI_CGI_Cammenu(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    char *buf = NULL;
...
int32_t CVI_CGI_Setting(httpd_conn *hc, char *str)
{
    CVI_CGI_Currency(hc, NULL, NULL, 0, NULL);
    return 0;
}

/*cgi 进入或退出回放界面，url = /cgi-bin/Config.cgi?action=set&property=Playback&value=*/
int32_t CVI_CGI_Playback(httpd_conn *hc, char *str)
{
    uint32_t pu32FileObjCnt = 0;
...
int32_t CVI_CGI_PreviewRTSP(httpd_conn *hc, char *str)
{
    int32_t result = 0;
...
int32_t CVI_CGI_RearStarus(httpd_conn *hc, char *str)
{
    int32_t result = 0;
    char *buf = NULL;
    char *RearStarus = "OFF";
    ...
    memset(cgibuff, 0, sizeof(cgibuff));
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_STS_TEST, 0, "OK");
    CVI_CGI_Snprintf(cgibuff, buf, LBUFFSIZE, DEF_CGI_LOG_TEST, "Camera.Menu.RearStarus", RearStarus);
    CVI_NET_AddCgiResponse(strlen(cgibuff), hc, "%s", cgibuff);
...
int32_t CVI_CGI_SourceCamid(httpd_conn *hc, char *str)
{
    FoRStat = str;
    CVI_CGI_Currency(hc, NULL, NULL, 0, NULL);
    return 0;
...
int32_t CVI_CGI_StreamStatus(httpd_conn *hc, char *str)
{
    CVI_CGI_Currency(hc, NULL, NULL, 0, NULL);
    CVI_EVENT_S stEvent;
    memset(&stEvent, 0, sizeof(CVI_EVENT_S));
...
static int32_t cvi_custom_register_cgi(const char *cmd, void *cb)
{
    int32_t ret = 0;
    CVI_NET_WIFIAPPMAPTO_CGI_S stCgiCmd;
    ...
    CVI_NET_RegisterCgiCmd_CGI(&stCgiCmd);
...
{
    CVI_LOGE("enter: %s \n", __func__);
    cvi_custom_register_cgi("Videores", (void *)CVI_CGI_SetMovieRecord);                           /*cgi 视频分辨率的设置和获取*/
    cvi_custom_register_cgi("LoopingVideo", (void *)CVI_CGI_SetCyclicRecordValue);                 /*cgi 录影分段时间设置和获取*/
    cvi_custom_register_cgi("GSensor", (void *)CVI_CGI_SetGSensorSensitivity);                     /*cgi 碰撞锁定的灵敏度的设置和获取*/
    cvi_custom_register_cgi("VideoTimeLapse", (void *)CVI_CGI_SetVideoTimeLapse);                  /*cgi 缩时录音的设置和获取*/
    cvi_custom_register_cgi("Exposure", (void *)CVI_CGI_SetMovieEvValue);                          /*cgi 曝光设定的设置和获取*/
    cvi_custom_register_cgi("SD0", (void *)CVI_CGI_CommandWouldFormatStorage);                     /*cgi 格式化SD卡*/
    cvi_custom_register_cgi("DEL", (void *)CVI_CGI_DeleteOneFile);                                 /*cgi 删除文件*/
    cvi_custom_register_cgi("Net.WIFI_AP.SSID", (void *)CVI_CGI_SetGetSSID);                       /*cgi 对wifi名称进行操作*/
    cvi_custom_register_cgi("Net.WIFI_AP.CryptoKey", (void *)CVI_CGI_SetGetCryptoKey);             /*cgi 对wifi密码进行操作*/
    cvi_custom_register_cgi("FactoryReset", (void *)CVI_CGI_ResetAllSettingTodeFault);             /*cgi 恢复出厂设置*/
    cvi_custom_register_cgi("Camera.Menu.FWVersion", (void *)CVI_CGI_GetProjectVersion);           /*cgi 版本值获取*/
    cvi_custom_register_cgi("Camera.menu.UUID", (void *)CVI_CGI_GetMenuUUID);                      /*cgi 设备的唯一标志*/
    cvi_custom_register_cgi("Preview.MJPEG.status.*", (void *)CVI_CGI_GetMJPEGStatus);             /*cgi 获取当前设备的状态*/
    cvi_custom_register_cgi("Camera.Preview.*", (void *)CVI_CGI_GetPreviewStatus);                 /*cgi 获取当前设备的状态*/
    cvi_custom_register_cgi("Camera.Menu.*", (void *)CVI_CGI_GetCamMenu);                          /*cgi 获取相机目录*/
    cvi_custom_register_cgi("SoundRecord", (void *)CVI_CGI_SetEdisableMovieAudio);                 /*cgi 设置录音开关*/
    cvi_custom_register_cgi("Camera.Menu.SoundRecord", (void *)CVI_CGI_GetEdisableMovieAudio);     /*cgi 获取录音开关*/
    cvi_custom_register_cgi("Heartbeat", (void *)CVI_CGI_Heartbeat);                               /*cgi 心跳包*/
    cvi_custom_register_cgi("Video", (void *)CVI_CGI_MovieRecord);                                 /*cgi 更改录像*/
    cvi_custom_register_cgi("Camera.Menu.otaversion", (void *)CVI_CGI_OtaVersion);                 /*cgi ota版本号*/
    cvi_custom_register_cgi("Camera.Menu.SDInfo", (void *)CVI_CGI_SDInfo);                         /*cgi SDInfo*/
    cvi_custom_register_cgi("Camera.Menu.CardInfo.*", (void *)CVI_CGI_CardInfo);                   /*cgi CardInfo*/
    cvi_custom_register_cgi("Camera.Menu.EmmcInfo.*", (void *)CVI_CGI_EmmcInfo);                   /*cgi EmmcInfo*/
    cvi_custom_register_cgi("Camera.Menu.VideoresCapability", (void *)CVI_CGI_VideoresCapability); /*cgi 是否可以录像*/
    cvi_custom_register_cgi("Camera.Menu.devid", (void *)CVI_CGI_GetDevid);                        /*cgi 获取设备信息*/
    cvi_custom_register_cgi("dir", (void *)CVI_CGI_ListAllFileCreatTime);                          /*cgi 获取文件清单*/
    cvi_custom_register_cgi("thumb", (void *)CVI_CGI_GetScreennail);                               /*cgi 获取文件缩略图*/
    cvi_custom_register_cgi("Camera.Menu.DeviceCapability", (void *)CVI_CGI_DeviceCapability);     /*cgi 获取可运行设备编号*/
    cvi_custom_register_cgi("Camera.Battery.Level", (void *)CVI_CGI_BatteryLevel);                 /*cgi 电量信息*/
    cvi_custom_register_cgi("TimeSettings", (void *)CVI_CGI_SetSystemTime);                        /*cgi 设置当前时间*/
    cvi_custom_register_cgi("NULL", (void *)CVI_CGI_Heartbeat);                                    /*cgi 空函数，不回复*/
    cvi_custom_register_cgi("EV", (void *)CVI_CGI_SetEV);                                          /*cgi 设置曝光量*/
    cvi_custom_register_cgi("cammenu", (void *)CVI_CGI_Cammenu);                                   /*cgi 设置参数界面*/
    cvi_custom_register_cgi("Setting", (void *)CVI_CGI_Setting);                                   /*cgi 进入或退出设置界面*/
    cvi_custom_register_cgi("Playback", (void *)CVI_CGI_Playback);                                 /*cgi 进入或退出回放界面*/
    cvi_custom_register_cgi("Camera.Preview.RTSP.av", (void *)CVI_CGI_PreviewRTSP);                /*cgi 查看有几路RTSP*/
    cvi_custom_register_cgi("Camera.Menu.RearStarus", (void *)CVI_CGI_RearStarus);                 /*cgi 查看后路状态*/
    cvi_custom_register_cgi("Camera.Preview.Source.1.Camid", (void *)CVI_CGI_SourceCamid);         /*cgi 设置前后路*/
    cvi_custom_register_cgi("StreamStatus", (void *)CVI_CGI_StreamStatus);                         /*cgi 开启或关闭串口流*/
    cvi_custom_register_cgi("Camera.Menu.SD0", (void *)CVI_CGI_SDInfo);
}
