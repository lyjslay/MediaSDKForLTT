...
static void cJson_common(httpd_conn *hc, int32_t state, char *info)
{
    ...
    CVI_NET_AddResponse(hc, res, strlen(res));
    if (res != NULL) {
        free(res);
    }
...
int32_t CVI_APP_GetProductInfo(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
...
int32_t CVI_APP_GetDeviceAttr(httpd_conn *hc, char *fun ,char *str)
{
    int32_t s32Result = 0;
    ...
    if (CVI_MODEMNG_GetCurWorkMode() != CVI_WORK_MODE_MOVIE) {
        ...
        CVI_NETCTRLINNER_SendSyncMsg(&Msg, &s32Result);
        ...
        //等待切换录像模式
        while (CVI_MODEMNG_GetCurWorkMode() != CVI_WORK_MODE_MOVIE) {
            usleep(100 * 1000);
            if (count++ > 40) {
    ...
    memset(&stEvent, 0, sizeof(CVI_EVENT_S));
    stEvent.topic = CVI_EVENT_NETCTRL_APPCONNECT_SETTING;
    ...
    CVI_PARAM_GetWifiParam(&wifipar);
...
int32_t CVI_APP_GetMediaInfo(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    ...
    strcat(RtspName_F, HAL_WIFI_IP);strcat(RtspName_F, "/");
    strcat(RtspName_R, HAL_WIFI_IP);strcat(RtspName_R, "/");
    CVI_PARAM_MENU_S param;
    CVI_PARAM_GetMenuParam(&param);
    CVI_PARAM_MEDIA_COMM_S meida_param;
    CVI_PARAM_GetMediaComm(&meida_param);
    ...
    CVI_NET_AddResponse(hc, appbuff, strlen(appbuff));
...
int32_t CVI_APP_GetSDInfo(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    cJSON* cjson_info = NULL;
    ...
    } else if (CVI_MODEMNG_GetCardState() == CVI_CARD_STATE_UNAVAILABLE) {
...
int32_t CVI_APP_SetSysTime(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    if (fun != NULL) {
        fun = strrchr(fun, '=') + 1;
    ...
    CVI_SYSTEM_SetDateTime(&pstDateTime);
    cJson_common(hc, result, "set success.");
    return result;
}

//6.1.7 设置时区 http://(IP)/app/settimezone
int32_t CVI_APP_SetTimeZone(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    cJson_common(hc, result, "set success.");
    return result;
...
int32_t CVI_APP_GetRecDuration(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    // cJSON* cjson_info;
    // uint32_t time = 0;
...
int32_t CVI_APP_Setting(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    CVI_EVENT_S stEvent;
    memset(&stEvent, 0, sizeof(CVI_EVENT_S));
...
int32_t CVI_APP_Playback(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    CVI_EVENT_S stEvent;
    memset(&stEvent, 0, sizeof(CVI_EVENT_S));
...
int32_t CVI_APP_SetWiFissid(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t s32Ret = 0;
    CVI_PARAM_WIFI_S wifipar;
    ...
    if(fun != NULL) {
        fun = strrchr(fun, '=') + 1;
        memcpy(wifipar.WifiCfg.unCfg.stApCfg.stCfg.szWiFiSSID, fun, CVI_HAL_WIFI_SSID_LEN);
    }
...
int32_t CVI_APP_SetWiFipwd(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t s32Ret = 0;
    CVI_PARAM_WIFI_S wifipar;
    ...
    if(fun != NULL) {
        fun = strrchr(fun, '=') + 1;
        memcpy(wifipar.WifiCfg.unCfg.stApCfg.stCfg.szWiFiPassWord, fun, CVI_HAL_WIFI_PASSWORD_LEN);
    }
...
int32_t CVI_APP_SetWiFi(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    if (fun != NULL) {
        if (strstr(fun, "wifissid") != NULL) {
            result = CVI_APP_SetWiFissid(hc, fun, str);
        } else if (strstr(fun, "wifipwd") != NULL) {
            result = CVI_APP_SetWiFipwd(hc, fun, str);
        }
    } else {
        result = CVI_WIFIAPP_FALSE;
    }
...
int32_t CVI_APP_SDFormat(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t s32Result = 0;
    int32_t sendresult = 0;
...
int32_t CVI_APP_Reset(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t s32Result = 0;
    int32_t sendresult = 0;
...
int32_t CVI_APP_GetCameraInfo(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    char *appbuff = NULL;
    cJSON* cjson_app = NULL;
...
int32_t CVI_APP_ParamSwitchCam(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    char *cam_id = NULL;
...
int32_t CVI_APP_ParamRec(httpd_conn *hc, char *fun ,char *str)
{
    int32_t pstr = 0;
    int32_t result = 0;
    int32_t sendresult = 0;
...
int32_t CVI_APP_ParamRes(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t pstr = 0;
    int32_t s32Result = 0;
    ...
    if (pstr == 1) pstr = CVI_MEDIA_VIDEO_SIZE_1920X1080P25;
    else if (pstr == 0) pstr = CVI_MEDIA_VIDEO_SIZE_2560X1440P25;
    else {
        result = CVI_WIFIAPP_FALSE;
        return result;
    }
...
int32_t CVI_APP_ParamSpl(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t pstr = 0;
    int32_t s32Result = 0;
...
int32_t CVI_APP_ParamMic(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t pstr = 0;
    int32_t s32Result = 0;
...
int32_t CVI_APP_ParamEv(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    int32_t pstr = 0;
    int32_t value = 0;
    ...
    else if (pstr == 12) value = EV_value * 3.98;
    else {
        result = CVI_WIFIAPP_FALSE;
        cJson_common(hc, result, "set fail.");
        return result;
...
int32_t CVI_APP_SetParamValue(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    if (fun != NULL) {
        fun = strrchr(fun, '=') + 1;
        if (strcmp(fun, "switchcam") == 0) {
            result = CVI_APP_ParamSwitchCam(hc, fun, str);
        } else if (strcmp(fun, "rec") == 0) {
            result = CVI_APP_ParamRec(hc, fun, str);
        } else if (strcmp(fun, "rec_resolution") == 0) {
            result = CVI_APP_ParamRes(hc, fun, str);
        } else if (strcmp(fun, "rec_split_duration") == 0) {
            result = CVI_APP_ParamSpl(hc, fun, str);
        } else if (strcmp(fun, "mic") == 0) {
            result = CVI_APP_ParamMic(hc, fun, str);
        } else if (strcmp(fun, "ev") == 0) {
            result = CVI_APP_ParamEv(hc, fun, str);
        }
    } else {
        result = CVI_WIFIAPP_FALSE;
    }
...
int32_t CVI_APP_SnapShot(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    CVI_MESSAGE_S Msg = {0};
    int32_t s32Result = 0;
...
int32_t CVI_APP_GetLockVideoStatus(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    cJSON* INFO = cJSON_CreateArray();
    // if (CVI_MODEMNG_GetEmrState() == true) {
...
int32_t CVI_APP_WiFiReboot(httpd_conn *hc, char *fun ,char *str)
{
    int32_t s32Result = 0;
    int32_t sendresult = 0;
    int32_t result = 0;
...
int32_t CVI_APP_GetFileListAll(httpd_conn *hc, char *fun ,char *str)
{
    ...
    CVI_DTCF_DIR_E aenDirs[DTCF_DIR_BUTT][MAX_CAMERA_INSTANCES] = {{DTCF_DIR_EMR_FRONT},
                                                                   {DTCF_DIR_NORM_FRONT},
                                                                   {DTCF_DIR_PHOTO_FRONT}};
    if (MAX_CAMERA_INSTANCES > 1) {
        aenDirs[0][1] = DTCF_DIR_EMR_REAR;
        aenDirs[1][1] = DTCF_DIR_NORM_REAR;
        aenDirs[2][1] = DTCF_DIR_PHOTO_REAR;
    }
    ...
    for (; i < 3; i++){
        ...
        CVI_FILEMNG_SetSearchScope(aenDirs[i], MAX_CAMERA_INSTANCES, &pu32FileObjCnt);
        bufflen += pu32FileObjCnt;
        cJSON_AddStringToObject(cjson_dirs, "folder", stCfg.FileMngDtcf.aszDirNames[aenDirs[i][0]]);
        cJSON_AddNumberToObject(cjson_dirs, "count", pu32FileObjCnt);
        if(aenDirs[i][0] != DTCF_DIR_PHOTO_FRONT || (MAX_CAMERA_INSTANCES > 1 && aenDirs[i][1] != DTCF_DIR_PHOTO_REAR)) {
            for (; j<pu32FileObjCnt; j++) {
                cJSON* cjson_file = cJSON_CreateObject();
                CVI_FILEMNG_GetFileByIndex(j, fpathname, 255);
                if (NULL == fpathname) {
                    CVI_LOGE("pathname is null");
                    result = CVI_WIFIAPP_FALSE;
                }
                ...
                // CVI_PLAYER_SetDataSource(player, fpathname);
                // CVI_PLAYER_MEDIA_INFO_S info = {};
                // result = CVI_PLAYER_GetMediaInfo(player, &info);
                // cJSON_AddNumberToObject(cjson_file, "duration",info.duration_sec);
                // CVI_PLAYER_Destroy(&player);
                cJSON_AddNumberToObject(cjson_file, "size", filebuf.st_size / 1024);
                cJSON_AddNumberToObject(cjson_file, "createtime",filebuf.st_ctime);
...
//6.3.1 获取文件列表 http://(IP)/app/getfilelist?folder=loop&start=0&end=99
int32_t CVI_APP_GetFileListPage(httpd_conn *hc, char *fun ,char *str)
{
    flag_file = 1;  //设置标志位，如果异常断联，需要重新扫描文件
    int32_t result = 0;
    if (fun == NULL || str == NULL) {
        result = CVI_WIFIAPP_FALSE;
        return result;
    }
    fun = strrchr(fun, '=') + 1;
    ...
    struct stat filebuf;
    char *file_buff = NULL;
    // CVI_PLAYER_MEDIA_INFO_S info = {0};
    CVI_DTCF_DIR_E aenDirs[DTCF_DIR_BUTT][MAX_CAMERA_INSTANCES] = {{DTCF_DIR_EMR_FRONT},
                                                                   {DTCF_DIR_NORM_FRONT},
                                                                   {DTCF_DIR_PHOTO_FRONT}};
    if (MAX_CAMERA_INSTANCES > 1) {
    ...
    cJSON* cjson_files = cJSON_CreateArray();
    if (strcmp(fun, "emr") == 0) {
        cJSON_AddStringToObject(cjson_dirs, "folder", "emr");
        CVI_FILEMNG_SetSearchScope(aenDirs[0], MAX_CAMERA_INSTANCES, &pu32FileObjCnt);
    } else if (strcmp(fun, "loop") == 0) {
        CVI_FILEMNG_SetSearchScope(aenDirs[1], MAX_CAMERA_INSTANCES, &pu32FileObjCnt);
        cJSON_AddStringToObject(cjson_dirs, "folder", "loop");
    } else if (strcmp(fun, "event") == 0) {
        CVI_FILEMNG_SetSearchScope(aenDirs[2], MAX_CAMERA_INSTANCES, &pu32FileObjCnt);
        cJSON_AddStringToObject(cjson_dirs, "folder", "event");
    } else {
        result = CVI_WIFIAPP_FALSE;
        return result;
    }
    uint32_t i =  (uint32_t)pstr -100 +1;
    if (pu32FileObjCnt >  (uint32_t)pstr) {
        pu32FileObjCnt =  (uint32_t)pstr + 1;
    } else if (pu32FileObjCnt < i) {
        result = CVI_WIFIAPP_FALSE;
        return result;
    }
    CVI_NETCTRLINNER_StopTimer();
    cJSON_AddNumberToObject(cjson_dirs, "count", pu32FileObjCnt);
    for (; i < pu32FileObjCnt; i++) {
        cJSON* cjson_file = cJSON_CreateObject();
    ...
    CVI_NET_AddResponse(hc, file_buff, strlen(file_buff));
    cJSON_Delete(cJson_app);
    if (file_buff != NULL) {
        free(file_buff);
    }
...
//6.3.1 获取文件列表 http://(IP)/app/getfilelist
int32_t CVI_APP_GetFileList(httpd_conn *hc, char *fun ,char *str)
{
    int result = 0;
    if (fun == NULL) {
        result = CVI_APP_GetFileListAll(hc, fun, str);
    } else {
        result = CVI_APP_GetFileListPage(hc, fun, str);
    }
    return result;
}

//6.3.3 获取缩略图 http://192.168.169.1/app/getthumbnail?file=emr/video/202012121859.mp4
int32_t CVI_APP_GetThumbNail(httpd_conn *hc, char *fun ,char *str)
{
    int32_t ret = 0;
    ...
    ret = CVI_THUMBNAIL_EXTRACTOR_GetThumbnail(viewer_handle, fun, &packet);
...
int32_t CVI_APP_DeleteFile(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    fun = strrchr(fun, '=') + 1;
    CVI_FILEMNG_RemoveFile(fun);
    cJson_common(hc, result, "delete success.");
    return 0;
}

//6.4.1 查询当前所有设置项的参数列表 http://(IP)/app/getparamitems
int32_t CVI_APP_GetParamItems(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
...
int32_t CVI_APP_GetParamValue(httpd_conn *hc, char *fun ,char *str)
{
    CVI_EVENT_S stEvent;
    memset(&stEvent, 0, sizeof(CVI_EVENT_S));
    stEvent.topic = CVI_EVENT_NETCTRL_CONNECT;
    ...
    //分辨率
    int32_t resolution;
    if (menuparam.VideoSize.Current == 1) resolution = 1;
    else resolution = 0;

    //循环播放时间
    int32_t LoopingVideo;
    if(menuparam.VideoLoop.Current == 0) LoopingVideo = 0;
    else if(menuparam.VideoLoop.Current == 1) LoopingVideo = 1;
    else LoopingVideo = 2;

    //声音录制开关
    int32_t SoundRecord = 0;
    if (menuparam.AudioEnable.Current == 0) SoundRecord = 0;
    else SoundRecord = 1;
    ...
    CVI_MAPI_VCAP_GetSensorPipeAttr(Syshdl->sns[0], &yuv_status);
    if (0 == yuv_status) {
    ...
    if (EV_Manual == 0) {
        ...
        if (scale > 3.5) evflage = 0;
        else if (scale >= 2.8 && scale < 3.5) evflage = 1;
        else if (scale >= 2.2 && scale < 2.8) evflage = 2;
        else if (scale >= 1.8 && scale < 2.2) evflage = 3;
        else if (scale >= 1.4 && scale < 1.8) evflage = 4;
        else if (scale >= 1.1 && scale < 1.4) evflage = 5;
        else if (scale >= 0.9 && scale < 1.1) evflage = 6;
        else if (scale >= 0.7 && scale < 0.9) evflage = 7;
        else if (scale >= 0.55 && scale < 0.7) evflage = 8;
        else if (scale >= 0.45 && scale < 0.55) evflage = 9;
        else if (scale >= 0.36 && scale < 0.45) evflage = 10;
        else if (scale >= 0.28 && scale < 0.36) evflage = 11;
        else evflage = 12;
    }
...
int32_t CVI_APP_GetAdasItems(httpd_conn *hc, char *fun ,char *str)
{
    int32_t result = 0;
    cJSON* INFO = cJSON_CreateArray();
    cJson_itemcommon(hc, result, INFO);
...
static int32_t app_socket_write(int32_t new_fd)
{
    ...
    CVI_PARAM_GetMenuParam(&param);
    int32_t CardStatus = CVI_MODEMNG_GetCardState();;
    int32_t card_status = CVI_MODEMNG_GetCardState();;
    ...
    while (flag_socket == WIFI_APP_CONNECTTED) {
        ...
        if (card_status != CardStatus) {
            ...
            if (card_status == CVI_CARD_STATE_AVAILABLE) {
                ...
                sd_appbuff = cJSON_Print(cjson_sd);
            } else if (card_status == CVI_CARD_STATE_UNAVAILABLE) {
                tick = time(NULL);
...
int32_t cvi_custom_register_app(const char *cmd, void *cb)
{
    int ret = 0;
    CVI_NET_WIFIAPPMAPTO_APP_S stCgiCmd;
    ...
    CVI_RegisterCgiCmd_App(&stCgiCmd);
...
{
    CVI_LOGE("enter: %s \n", __func__);
    cvi_custom_register_app("getproductinfo", (void *)CVI_APP_GetProductInfo);           //6.1.1 获取产品信息
    cvi_custom_register_app("getdeviceattr", (void *)CVI_APP_GetDeviceAttr);             //6.1.2 获取设备信息
    cvi_custom_register_app("getmediainfo", (void *)CVI_APP_GetMediaInfo);               //6.1.3 获取媒体信息
    cvi_custom_register_app("getsdinfo", (void *)CVI_APP_GetSDInfo);                     //6.1.4 获取SD卡信息
    cvi_custom_register_app("setsystime", (void *)CVI_APP_SetSysTime);                   //6.1.6 设置系统时间
    cvi_custom_register_app("settimezone", (void *)CVI_APP_SetTimeZone);                 //6.1.7 设置时区
    cvi_custom_register_app("getrecduration", (void *)CVI_APP_GetRecDuration);           //6.1.8 获取当前录像时长
    cvi_custom_register_app("setting", (void *)CVI_APP_Setting);                         //6.1.9 设置界面
    cvi_custom_register_app("playback", (void *)CVI_APP_Playback);                       //6.1.10 回放界面
    cvi_custom_register_app("setparamvalue", (void *)CVI_APP_SetParamValue);             //6.2   设备参数的设置
    cvi_custom_register_app("setwifi", (void *)CVI_APP_SetWiFi);                         //6.2.1-2 修改wifi
    cvi_custom_register_app("sdformat", (void *)CVI_APP_SDFormat);                       //6.2.3 格式化SD卡
    cvi_custom_register_app("reset", (void *)CVI_APP_Reset);                             //6.2.4 恢复出厂设置
    cvi_custom_register_app("getcamerainfo", (void *)CVI_APP_GetCameraInfo);             //6.2.5 获取摄像头信息
    cvi_custom_register_app("snapshot", (void *)CVI_APP_SnapShot);                       //6.2.8 拍照
    cvi_custom_register_app("getlockvideostatus", (void *)CVI_APP_GetLockVideoStatus);   //6.2.10 查看锁挡状态
    cvi_custom_register_app("wifireboot", (void *)CVI_APP_WiFiReboot);                   //6.2.11 重启WiFi
    cvi_custom_register_app("getfilelist", (void *)CVI_APP_GetFileList);                 //6.3.1 获取文件列表
    cvi_custom_register_app("getthumbnail", (void *)CVI_APP_GetThumbNail);               //6.3.4 获取缩略图
    cvi_custom_register_app("deletefile", (void *)CVI_APP_DeleteFile);                   //6.3.5 删除文件
    cvi_custom_register_app("getparamvalue", (void *)CVI_APP_GetParamValue);             //6.4.2 查询所有设置项的当前值
    cvi_custom_register_app("getparamitems", (void *)CVI_APP_GetParamItems);             //6.4.1 查询当前所有设置项的参数列表
    cvi_custom_register_app("getadasitems", (void *)CVI_APP_GetAdasItems);
}
