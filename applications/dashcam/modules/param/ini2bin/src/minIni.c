...
int32_t strnicmp(const TCHAR *s1, const TCHAR *s2, size_t n)
{
  while (n-- != 0 && (*s1 || *s2)) {
...
static int32_t getkeystring(INI_FILETYPE *fp, const TCHAR *Section, const TCHAR *Key,
                       int32_t idxSection,int32_t idxKey, TCHAR *Buffer,int32_t BufferSize,
                        INI_FILEPOS *mark)
{
  TCHAR *sp, *ep;
 int32_t len, idx;
   ...
   */
  len = (Section != NULL) ? (int32_t)_tcslen(Section) : 0;
  if (len > 0 || idxSection >= 0) {
    ...
    do {
      ...
      ep = _tcsrchr(sp, ']');
             ...
             (((int32_t)(ep-sp-1) != len || Section == NULL || _tcsnicmp(sp+1,Section,len) != 0) && ++idx != idxSection));
    if (idxSection >= 0) {
      if (idx == idxSection) {
        assert(ep != NULL);
        assert(*ep == ']');
  ...
  len = (Key != NULL) ? (int32_t)_tcslen(Key) : 0;
  idx = -1;
  do {
    if (mark != NULL)
    ...
    if (ep == NULL)
      ep = _tcschr(sp, ':');
           ...
           || ((len == 0 || (int32_t)(skiptrailing(ep,sp)-sp) != len || _tcsnicmp(sp,Key,len) != 0) && ++idx != idxKey));
  if (idxKey >= 0) {
    if (idx == idxKey) {
      assert(ep != NULL);
      assert(*ep == '=' || *ep == ':');
 ...
 */
int32_t ini_gets(const TCHAR *Section, const TCHAR *Key, const TCHAR *DefValue,
             TCHAR *Buffer,int32_t BufferSize, const TCHAR *Filename)
{
  INI_FILETYPE fp;
 int32_t ok = 0;
...
long ini_getl(const TCHAR *Section, const TCHAR *Key, long DefValue, const TCHAR *Filename)
{
 ...
 int32_t len = ini_gets(Section, Key, __T(""), LocalBuffer, sizearray(LocalBuffer), Filename);
  return (len == 0) ? DefValue
                    : ((len >= 2 && _totupper((int32_t)LocalBuffer[1]) == 'X') ? _tcstol(LocalBuffer, NULL, 16)
 ...
 */
int32_t ini_getbool(const TCHAR *Section, const TCHAR *Key,int32_t DefValue, const TCHAR *Filename)
{
  TCHAR LocalBuffer[2] = __T("");
 int32_t ret;

  ini_gets(Section, Key, __T(""), LocalBuffer, sizearray(LocalBuffer), Filename);
  LocalBuffer[0] = (TCHAR)_totupper((int32_t)LocalBuffer[0]);
 ...
 * \return            the number of characters copied into the supplied buffer
 */
int32_t  ini_getsection(int32_t idx, TCHAR *Buffer,int32_t BufferSize, const TCHAR *Filename)
{
  INI_FILETYPE fp;
 int32_t ok = 0;
 ...
 * \return            the number of characters copied into the supplied buffer
 */
int32_t  ini_getkey(const TCHAR *Section,int32_t idx, TCHAR *Buffer,int32_t BufferSize, const TCHAR *Filename)
{
  INI_FILETYPE fp;
 int32_t ok = 0;
 ...
 *                    (for success).
 */
int32_t ini_browse(INI_CALLBACK Callback, void *UserData, const TCHAR *Filename)
{
  TCHAR LocalBuffer[INI_BUFFERSIZE];
 int32_t lenSec, lenKey;
  enum quote_option quotes;
  INI_FILETYPE fp;
  ...
  lenSec = (int32_t)_tcslen(LocalBuffer) + 1;
  for ( ;; ) {
    TCHAR *sp, *ep;
    ...
    *ep++ = '\0';             /* split the key from the value */
    ...
    lenKey = (int32_t)_tcslen(LocalBuffer + lenSec) + 1;
    /* clean up the value */
    sp = skipleading(ep);
    sp = cleanstring(sp, &quotes);  /* Remove a trailing comment */
...
static void ini_tempname(TCHAR *dest, const TCHAR *source,int32_t maxlength)
{
  TCHAR *p;
...
static int32_t cache_accum(const TCHAR *string,int32_t *size,int32_t max)
{
 int32_t len = (int32_t)_tcslen(string);
  if (*size + len >= max)
    return 0;
  *size += len;
  return 1;
...
static int32_t cache_flush(TCHAR *buffer,int32_t *size,
                      INI_FILETYPE *rfp, INI_FILETYPE *wfp, INI_FILEPOS *mark)
{
 int32_t terminator_len = (int32_t)_tcslen(INI_LINETERM);
 int32_t pos = 0;
...
static int32_t close_rename(INI_FILETYPE *rfp, INI_FILETYPE *wfp, const TCHAR *filename, TCHAR *buffer)
{
  (void)ini_close(rfp);
  (void)ini_close(wfp);
 ...
 */
int32_t ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, const TCHAR *Filename)
{
  INI_FILETYPE rfp;
  INI_FILETYPE wfp;
  INI_FILEPOS mark;
  ...
  TCHAR *sp, *ep;
  TCHAR LocalBuffer[INI_BUFFERSIZE];
 int32_t len, match, flag, cachelen;
  ...
  if (len > 0) {
    do {
      ...
      sp = skipleading(LocalBuffer);
      ep = _tcsrchr(sp, ']');
      match = (*sp == '[' && ep != NULL && (int32_t)(ep-sp-1) == len && _tcsnicmp(sp + 1,Section,len) == 0);
      if (!match || Key != NULL) {
  ...
  for( ;; ) {
    ...
    if (ep == NULL)
      ep = _tcschr(sp, ':');
    match = (ep != NULL && len > 0 && (int32_t)(skiptrailing(ep,sp)-sp) == len && _tcsnicmp(sp,Key,len) == 0);
...
{
 int32_t i, j;
  for (i = 0, j = (int32_t)_tcslen(str) - 1; i < j; i++, j--) {
    TCHAR t = str[i];
    str[i] = str[j];
    str[j] = t;
  } /* for */
...
static void long2str(long value, TCHAR *str)
{
 int32_t i = 0;
  ...
  do {
   int32_t n = (int32_t)(value % 10);          /* get next lowest digit */
 ...
 * \return            1 if successful, otherwise 0
 */
int32_t ini_putl(const TCHAR *Section, const TCHAR *Key, long Value, const TCHAR *Filename)
{
 ...
 * \return            1 if successful, otherwise 0
 */
int32_t ini_putf(const TCHAR *Section, const TCHAR *Key, INI_REAL Value, const TCHAR *Filename)
{
  TCHAR LocalBuffer[64];
