...
#include "cvi_ae.h"
#include "cvi_awb.h"
extern CVI_S32 SAMPLE_COMM_VI_OPEN(void);
...
int32_t CVI_MEDIA_DUMP_SetDumpRawAttr(int32_t sns_id)
{
    ...
    CVI_MAPI_VCAP_SENSOR_HANDLE_T g_sns_hdl = CVI_MEDIA_GetCtx()->SysHandle.sns[sns_id];
    ...
    s32Ret = CVI_MAPI_VCAP_SetDumpRawAttr(g_sns_hdl, &astDumpAttr[0]);
    MEDIA_CHECK_RET(s32Ret, CVI_MEDIA_EINVAL, "CVI_MAPI_VCAP_SetDumpRawAttr fail");

    s32Ret = CVI_MAPI_VCAP_GetDumpRawAttr(g_sns_hdl, &astDumpAttr[1]);
    MEDIA_CHECK_RET(s32Ret, CVI_MEDIA_EINVAL, "CVI_MAPI_VCAP_GetDumpRawAttr fail");
...
static char* CVI_MEDIA_DUMP_GetSdConfig(void)
{
...
static void get_file_name_prefix(int32_t sns_id, char* prefix, CVI_U32 len)
{
	struct tm *t;
	time_t tt;
    ...
    int32_t status = 0;
    CVI_MAPI_VCAP_GetSensorPipeAttr(Syshdl->sns[sns_id], &status);
    if (0 == status) {
...
// 	CVI_U8 *buf = NULL;
//     snprintf(name, 128, "%s%s.json", CVI_MEDIA_DUMP_GetSdConfig(), prefix);
// 	fp = fopen(name, "w");
// 	if (fp == NULL) {
// 		printf("open %s fail!!!\n", name);
// 		return;
...
// 	snprintf(name, 128, "%s%s.txt", CVI_MEDIA_DUMP_GetSdConfig(), prefix);
// 	fp = fopen(name, "w");
// 	if (fp == NULL) {
// 		printf("open %s fail!!!\n", name);
// 		return;
...
// 	CVI_U32 size = CVI_ISP_GetAWBDbgBinSize();
...
// 	snprintf(name, 128, "%s%s-awb.bin", CVI_MEDIA_DUMP_GetSdConfig(), prefix);
// 	fp = fopen(name, "w");
// 	if (fp == NULL) {
// 		printf("open %s fail!!!\n", name);
// 		return;
...
// 	snprintf(name, 128, "%s%s-awblog.txt", CVI_MEDIA_DUMP_GetSdConfig(), prefix);
// 	fp = fopen(name, "w");
// 	if (fp == NULL) {
// 		printf("open %s fail!!!\n", name);
// 		return;
...
// 	snprintf(name, 128, "%s%s-aelog.txt", CVI_MEDIA_DUMP_GetSdConfig(), prefix);
// 	fp = fopen(name, "w");
// 	if (fp == NULL) {
// 		printf("open %s fail!!!\n", name);
// 		return;
...
static int32_t CVI_MEDIA_RAWDATA_DumpCallback(uint32_t ViPipe, VIDEO_FRAME_INFO_S* pstVideoFrame,
                                    uint32_t u32DataNum, void* pPrivateData)
{
	...
	snprintf(name, 128, "%s%s.raw", CVI_MEDIA_DUMP_GetSdConfig(), prefix);
...
int32_t CVI_MEDIA_DUMP_DumpRaw(const int32_t sns_id)
{
    int32_t s32Ret = 0;
    CVI_MAPI_VCAP_SENSOR_HANDLE_T g_sns_hdl = CVI_MEDIA_GetCtx()->SysHandle.sns[sns_id];
    CVI_MAPI_VCAP_RAW_DATA_T stVCapRawData;
    stVCapRawData.pPrivateData = (CVI_VOID*)__FUNCTION__;
    stVCapRawData.pfn_VCAP_RawDataProc = (void *)CVI_MEDIA_RAWDATA_DumpCallback;

    s32Ret = CVI_MAPI_VCAP_StartDumpRaw(g_sns_hdl, 1, &stVCapRawData);
    MEDIA_CHECK_RET(s32Ret, CVI_MEDIA_EINVAL, "CVI_MAPI_VCAP_StartDumpRaw fail");

    s32Ret = CVI_MAPI_VCAP_StopDumpRaw(g_sns_hdl);
    MEDIA_CHECK_RET(s32Ret, CVI_MEDIA_EINVAL, "CVI_MAPI_VCAP_StopDumpRaw fail");
...
int32_t save_frame_file(uint32_t Grp, uint32_t Chn, VIDEO_FRAME_INFO_S *pFrame, void *pPrivateData)
{
    ...
    (CVI_MAPI_SaveFramePixelData(pFrame, filename));
    return 0;
}
...
int32_t CVI_MEDIA_DUMP_DumpYuv(int32_t sns_id)
{
    ...
    char prefix[128] = {0};
    CVI_MAPI_VCAP_SENSOR_HANDLE_T g_sns_hdl = CVI_MEDIA_GetCtx()->SysHandle.sns[sns_id];
    CVI_MAPI_VPROC_HANDLE_T vproc_hdl = CVI_MEDIA_GetCtx()->SysHandle.vproc[sns_id];
    ...
    CVI_SYSTEM_TM_S stDateTime;
    CVI_SYSTEM_GetRTCDateTime(&stDateTime);
	char *LogPath = CVI_MEDIA_DUMP_GetSdConfig();
    ...
    if (Vpssmode.stVIVPSSMode.aenMode[sns_id] == VI_OFFLINE_VPSS_OFFLINE ||
        ...
        (CVI_MAPI_VCAP_GetFrame(g_sns_hdl, &vcap_frame));
        snprintf(filename, 128, "%s%04d%02d%02d-%02d%02d%02d-media_dump_%d", CVI_MEDIA_DUMP_GetSdConfig(), stDateTime.s32year, stDateTime.s32mon, stDateTime.s32mday, stDateTime.s32hour, stDateTime.s32min, stDateTime.s32sec, sns_id);
        (CVI_MAPI_SaveFramePixelData(&vcap_frame, filename));
        (CVI_MAPI_ReleaseFrame(&vcap_frame));
        ...
        snprintf(filename, 128, "%s%s", CVI_MEDIA_DUMP_GetSdConfig(), prefix);
		// snprintf(filename, 128, "%s%04d%02d%02d-%02d%02d%02d-media_dump_%d", CVI_MEDIA_DUMP_GetSdConfig(), stDateTime.s32year, stDateTime.s32mon, stDateTime.s32mday, stDateTime.s32hour, stDateTime.s32min, stDateTime.s32sec, sns_id);
		stCallbackFun.pPrivateData = (void*)filename;
		stCallbackFun.pfunFrameProc = save_frame_file;
		(CVI_MAPI_VPROC_StartChnDump(vproc_hdl, 0, 1, &stCallbackFun));
		(CVI_MAPI_VPROC_StopChnDump(vproc_hdl, 0));
    }
...
void CVI_MEDIA_DUMP_VIISPReset(uint8_t bInit)
{
	return;
	if (bInit == 1) {
		...
		CVI_MEDIA_DispInit(true);

		CVI_MEDIA_LiveViewSerInit();
		...
		// CVI_MEDIA_RtspSerInit();
	} else if(bInit == 0) {
		VI_VPSS_MODE_S stVIVPSSMode;
		CVI_SYS_GetVIVPSSMode(&stVIVPSSMode);
		CVI_MEDIA_LiveViewSerDeInit();
#ifdef ENABLE_ISP_PQ_TOOL
		bool en = false;
		CVI_MEDIA_DUMP_GetSizeStatus(&en);
		if (en == false) {
		...
		// CVI_MEDIA_RtspSerDeInit();

		CVI_MEDIA_StopOsd();

		CVI_MEDIA_VideoDeInit();

		CVI_MEDIA_DispDeInit();
...
void CVI_MEDIA_DUMP_ReplayInit(void)
{
	...
	// raw_replay_register_callback_fun(CVI_MEDIA_DUMP_VIISPReset);
}
