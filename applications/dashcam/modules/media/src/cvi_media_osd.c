...
#include "cvi_bitmap.h"
#ifdef CONFIG_GPS_ON
#include "cvi_gpsmng.h"
static CVI_GPSMNG_CALLBACK GpsCallback = {0};
#endif

#define FONTMOD_WIDTH 24
#define FONTMOD_HEIGHT 30
#define CVI_MEDIA_MAX_INFO_OSD_LEN        (120)
#define CVI_MEDIA_MAX_GPS_OSD_CNT         (2)
...
{
    bool init;
    CVI_PARAM_MEDIA_OSD_ATTR_S osdCfg;
    ...
    CVI_MAPI_OSD_ATTR_S infoOsdAttr[MAX_CAMERA_INSTANCES][ISP_INFO_LINES_CNT];
#endif
} CVI_MEDIA_OSDCtx;
static CVI_MEDIA_OSDCtx g_mediaOsdCtx;

#if defined(ISP_INFO_OSD)
static void CVI_MEDIA_OSD_GetInfoStr(int32_t viPipe, char* str, uint32_t strLen, int32_t line)
{
...
static void* CVI_MEDIA_OSD_UpdateInfoTsk(void* param)
{
    prctl(PR_SET_NAME, "OSDInfoTsk", 0, 0, 0);
    int32_t ret = 0;
    CVI_PARAM_MEDIA_OSD_ATTR_S* osdCfg = &g_mediaOsdCtx.osdCfg;
    ...
    for(i = 0;i < ISP_INFO_LINES_CNT; i++) {
        CVI_MAPI_OSD_ATTR_S * infoOsdAttr = &g_mediaOsdCtx.infoOsdAttr[devIndex][i];

        CVI_MEDIA_OSD_GetInfoStr(viPipe, infoOsdAttr->stContent.stStrContent.szStr, CVI_MEDIA_MAX_INFO_OSD_LEN, i);
        ret = CVI_MAPI_OSD_SetAttr(osdIdx, infoOsdAttr);
        if (ret == 0) {
            ret = CVI_MAPI_OSD_Start(osdIdx);
            CVI_APPCOMM_CHECK_RETURN(ret, NULL);
        } else {
            return NULL;
        }
        tempid[i] = osdIdx;
    ...
    while (g_mediaOsdCtx.infoTskRun[devIndex]) {
        for(i = 0;i < ISP_INFO_LINES_CNT; i++) {
            ...
            ret = CVI_MAPI_OSD_SetAttr(osdIdx, infoOsdAttr);
            if (ret != 0) {
                CVI_LOGE("CVI_MAPI_OSD_SetAttr[%d] failed %x\n", osdIdx,ret);
                break;
            }
        }
        cvi_usleep(1*100*1000);
    }

    for(i = 0;i < ISP_INFO_LINES_CNT; i++){
        CVI_MAPI_OSD_Stop(tempid[i]);
    }
    return NULL;
}
#endif
...
#if defined(ISP_INFO_OSD)
static int32_t CVI_MEDIA_StartInfoOsd(void)
{
    int32_t ret = 0;
    CVI_PARAM_MEDIA_OSD_ATTR_S* osdCfg = &g_mediaOsdCtx.osdCfg;
    int32_t i = 0, j = 0;
    for (i = 0; i < MAX_CAMERA_INSTANCES - 1; ++i) {
        ...
        for (j = 0; j < ISP_INFO_LINES_CNT; ++j) {
            ...
            CVI_MAPI_OSD_ATTR_S * infoOsdAttr = &g_mediaOsdCtx.infoOsdAttr[i][j];
            ...
            infoOsdAttr->stContent.enType = CVI_MAPI_OSD_TYPE_STRING;
            infoOsdAttr->astDispAttr[0].enCoordinate = CVI_MAPI_OSD_COORDINATE_RATIO_COOR;
            infoOsdAttr->astDispAttr[0].stStartPos.s32X = OSD_INFO_START_X;
            infoOsdAttr->astDispAttr[0].stStartPos.s32Y = OSD_INFO_START_Y + 3 * j;
        }
        CVI_MAPI_OSD_ATTR_S * infoOsdAttr = &g_mediaOsdCtx.infoOsdAttr[i][0];
        if (infoOsdAttr->astDispAttr[0].bShow && g_mediaOsdCtx.infoTskId[i] == 0) {
            g_mediaOsdCtx.infoTskRun[i] = true;
            ret = pthread_create(&g_mediaOsdCtx.infoTskId[i], NULL, CVI_MEDIA_OSD_UpdateInfoTsk, &g_mediaOsdCtx.vcapDevIdx[i]);
            if (ret != 0) {
...
int32_t CVI_MEDIA_StopInfoOsd(void)
{
#if defined(ISP_INFO_OSD)
...
#ifdef CONFIG_GPS_ON
static int32_t CVI_MEDIA_GPSMsgHandler(CVI_GPSMNG_MSG_PACKET *msgPacket, void* privateData)
{
    int32_t i = 0;
    int32_t s32GpsOsdId[CVI_MEDIA_MAX_GPS_OSD_CNT] = {4,5};
    CVI_PARAM_MENU_S MenuParam = {0};
    CVI_PARAM_GetMenuParam(&MenuParam);
    for (; i < CVI_MEDIA_MAX_GPS_OSD_CNT; ++i) {
        if ((msgPacket->gpsRMC.status == 'A') && ((MenuParam.GPSStamp.Current == CVI_MENU_SPEEDSTAMP_ON) ||
                (MenuParam.SpeedStamp.Current == CVI_MENU_GPSSTAMP_ON))) {
        // if ((MenuParam.GPSStamp.Current == CVI_MENU_SPEEDSTAMP_ON) || (MenuParam.SpeedStamp.Current == CVI_MENU_GPSSTAMP_ON)) {
            CVI_MAPI_OSD_ATTR_S OsdAttr = {0};
            char latitude, longitude;
            latitude = msgPacket->gpsRMC.ns;
            longitude = msgPacket->gpsRMC.ew;
            CVI_MAPI_OSD_GetAttr(s32GpsOsdId[i], &OsdAttr);
            if ((MenuParam.GPSStamp.Current == CVI_MENU_SPEEDSTAMP_ON) &&
                (MenuParam.SpeedStamp.Current == CVI_MENU_GPSSTAMP_ON)) {
                snprintf(OsdAttr.stContent.stStrContent.szStr, CVI_MEDIA_MAX_INFO_OSD_LEN, "%03fKM/H %C:%f %C:%f",
                msgPacket->gpsRMC.speed, latitude, msgPacket->gpsRMC.lat, longitude, msgPacket->gpsRMC.lon);
                OsdAttr.astDispAttr[0].bShow = true;
            } else if((MenuParam.GPSStamp.Current == CVI_MENU_SPEEDSTAMP_OFF) &&
                (MenuParam.SpeedStamp.Current == CVI_MENU_GPSSTAMP_ON)) {
                snprintf(OsdAttr.stContent.stStrContent.szStr, CVI_MEDIA_MAX_INFO_OSD_LEN, "%C:%f %C:%f",
                latitude, msgPacket->gpsRMC.lat, longitude, msgPacket->gpsRMC.lon);
                OsdAttr.astDispAttr[0].bShow = true;
            } else if((MenuParam.GPSStamp.Current == CVI_MENU_SPEEDSTAMP_ON) &&
                (MenuParam.SpeedStamp.Current == CVI_MENU_GPSSTAMP_OFF)) {
                snprintf(OsdAttr.stContent.stStrContent.szStr, CVI_MEDIA_MAX_INFO_OSD_LEN, "%03fKM/H",
                msgPacket->gpsRMC.speed);
                OsdAttr.astDispAttr[0].bShow = true;
            }

            CVI_MAPI_OSD_SetAttr(s32GpsOsdId[i], &OsdAttr);
        } else {
            CVI_MAPI_OSD_Show(s32GpsOsdId[i], 0, false);
        }
...
int32_t CVI_MEDIA_StartOsd(void)
{
    int32_t ret = 0;
    int32_t osdIdx = 0;
    ...
    //CVI_MAPI_OSD_ATTR_S osdAttr = {0};
    CVI_MAPI_OSD_FONTS_S pstFonts;
    pstFonts.u32FontWidth = FONTMOD_WIDTH;
    pstFonts.u32FontHeight = FONTMOD_HEIGHT;
    CVI_MAPI_OSD_Init(&pstFonts);
    CVI_PARAM_MEDIA_OSD_ATTR_S pstOsdCfg;
    CVI_PARAM_GetOsdParam(&pstOsdCfg);
    ...
    for (osdIdx = 0; osdIdx < pstOsdCfg.OsdCnt; ++osdIdx) {
        ...
        for (uint32_t j = 0; j < pstOsdCfg.OsdAttrs[osdIdx].u32DispNum; j++) {
            ...
            CVI_MAPI_VPROC_HANDLE_T vproc_hdl = CVI_MEDIA_GetCtx()->SysHandle.vproc[vproc_id];
            if (CVI_MAPI_VPROC_IsExtChn(vproc_hdl, vproc_chn)) {
                vproc_id = CVI_MAPI_VPROC_GetExtChnGrp(vproc_hdl, vproc_chn);
                vproc_chn = 0;
            }
        ...
        if (pstOsdCfg.OsdAttrs[osdIdx].stContent.enType == CVI_MAPI_OSD_TYPE_BITMAP) {
        ...
        ret = CVI_MAPI_OSD_SetAttr(osdIdx, &pstOsdCfg.OsdAttrs[osdIdx]);
        if (ret == 0) {
            ret = CVI_MAPI_OSD_Start(osdIdx);
        } else {
    ...
    memcpy(&g_mediaOsdCtx.osdCfg, &pstOsdCfg, sizeof(CVI_PARAM_MEDIA_OSD_ATTR_S));
#if defined(ISP_INFO_OSD)
    CVI_MEDIA_StartInfoOsd();
#endif

#ifdef CONFIG_GPS_ON
    GpsCallback.fnGpsDataCB = CVI_MEDIA_GPSMsgHandler;
    GpsCallback.privateData = NULL;
    CVI_GPSMNG_Register(&GpsCallback);
#endif
...
int32_t CVI_MEDIA_UpdateCarNumOsd(void)
{
        ...
        for(j = 0; j < OsdParam.OsdCnt; j++){
            for(z = 0; z < OsdParam.OsdAttrs[j].u32DispNum; z++){
                if (OsdParam.OsdAttrs[j].astDispAttr[z].u32Batch == i &&
                    OsdParam.OsdAttrs[j].stContent.enType == CVI_MAPI_OSD_TYPE_STRING){
                    // printf("update carname = %s\n", OsdParam.OsdAttrs[j].stContent.stStrContent.szStr);
                    s32Ret = CVI_MAPI_OSD_SetAttr(j, &OsdParam.OsdAttrs[j]);
                    if(s32Ret == 0){
...
int32_t CVI_MEDIA_StopOsd(void)
{
...
#ifdef CONFIG_GPS_ON
    CVI_GPSMNG_UnRegister(&GpsCallback);
#endif
    ...
    for (osdIdx = 0; osdIdx < osdCfg->OsdCnt ; ++osdIdx) {
        ...
        ret = CVI_MAPI_OSD_Stop(osdIdx);
        CVI_APPCOMM_CHECK_RETURN(ret, ret);
    }

#if defined(ISP_INFO_OSD)
    CVI_MEDIA_StopInfoOsd();
#endif
    CVI_MAPI_OSD_Deinit();
