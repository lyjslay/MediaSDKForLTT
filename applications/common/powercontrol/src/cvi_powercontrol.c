...
#define CVI_MUTEX_TRYLOCK(mutex) (pthread_mutex_trylock(&mutex))
...
typedef struct tagPWRCTRL_MGR_S
{
    ...
    PWRCTRL_TASK_CONTEXT_S astTaskContext[CVI_PWRCTRL_TASK_BUIT];
} PWRCTRL_MGR_S;

static PWRCTRL_MGR_S s_stPWRCTRLMgr = {.Mutex = PTHREAD_MUTEX_INITIALIZER,.bInitState = false,\
    .astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT].szPwrCtrlTaskName="screendormant",\
    .astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT].bCtrlEventHandleOnDormant=true,\
    .astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT].szPwrCtrlTaskName="sysdormant",\
    .astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT].bCtrlEventHandleOnDormant=true};
...
static void POWERCTRL_DormantProc(void *privData, struct timespec *now)
{
    ...
    s32Ret = CVI_MUTEX_TRYLOCK(s_stPWRCTRLMgr.Mutex);/**CVI_MUTEX_LOCK(s_stPWRCTRLMgr.Mutex)*/
    if (s32Ret != 0) {
        /** <mutex fail */
    ...
    if (true != s_stPWRCTRLMgr.bInitState) {
        CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
        ...
        if (0 == pstTaskContext->s32PauseCount) {
            ...
            if (0 != s32Ret) {
                CVI_LOGW("pfnScreenDormantProc fail\n");
                 ...
                 s32Ret = CVI_TIMEDTASK_SetAttr(pstTaskContext->TimedTaskId,&stAttr);
                 if (0 != s32Ret) {
...
static int32_t POWERCTRL_WakeupEventTaskProc(void* pvPrivData)
{
    ...
    if (NULL == pstTaskContext->stCfg.pfnWakeupProc) {
        ...
        return CVI_PWRCTRL_EFATA;
    }
    s32Ret = pstTaskContext->stCfg.pfnWakeupProc(pstTaskContext->stCfg.pvWakeupPrivData);
    if (0 != s32Ret) {
        CVI_LOGE("pfnWakeupProc (%s) fail\n",pstTaskContext->szPwrCtrlTaskName);
        s32Ret = CVI_PWRCTRL_EWAKEUPCB;
    }
    return s32Ret;
}

static int32_t POWERCTRL_WakeupEventProc(CVI_PWRCTRL_WAKEUP_TACTICS_E enWakeupTacticsType,bool* pbEventContinueHandle)
{
    int32_t s32Ret = 0;
    PWRCTRL_TASK_CONTEXT_S* pstTaskContext = NULL;
    int32_t s32Index = CVI_PWRCTRL_TASK_BUIT;
    *pbEventContinueHandle = true;
    switch (enWakeupTacticsType) {
        case CVI_PWRCTRL_WAKEUP_TACTICS_DISCARD: /** pause screen dormant check proc */
        case CVI_PWRCTRL_WAKEUP_TACTICS_CONTINUE:
            break;
        default:
            return CVI_PWRCTRL_EINVAL;
    }
    for (s32Index = CVI_PWRCTRL_TASK_SYSTEMDORMANT ;s32Index >= CVI_PWRCTRL_TASK_SCREENDORMANT;s32Index--) {
        ...
        if (true == pstTaskContext->bDormantState) {
            ...
            if (0 == pstTaskContext->s32PauseCount) {
               s32Ret = CVI_TIMEDTASK_SetAttr(pstTaskContext->TimedTaskId,&pstTaskContext->stCfg.stAttr);
               if (0 != s32Ret) {
                    CVI_LOGE("reset dormant (%s) attr fail\n",pstTaskContext->szPwrCtrlTaskName);
                    return CVI_PWRCTRL_ETIMEDTASK;
               }
            } else {
                ...
                if (pstTaskContext->bCtrlEventHandleOnDormant == false) {
                    CVI_LOGE("task(%s) wakeup,but s32PauseCount(%d),never run here,fail\n", pstTaskContext->szPwrCtrlTaskName, \
                        pstTaskContext->s32PauseCount);
                    return CVI_PWRCTRL_EINTER;
                }
            }
            /** when dormant,the event just use for wakeup ,no need proc the event */
            if (CVI_PWRCTRL_WAKEUP_TACTICS_DISCARD == enWakeupTacticsType) {
...
static int32_t POWERCTRL_PauseEventProc(CVI_PWRCTRL_EVENT_SCOPE_E enScopeType,bool* pbEventContinueHandle)
{
    int32_t s32Ret = 0;
    bool bContinueHandle = true;
    switch(enScopeType)
    {
        case CVI_PWRCTRL_EVENT_SCOPE_SYSTEM_SCREEN: /** pause screen dormant check proc */
            s32Ret = POWERCTRL_PauseEventTaskProc(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT],&bContinueHandle);
            CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
            s32Ret = POWERCTRL_PauseEventTaskProc(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT],pbEventContinueHandle);
            CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
            break;
        case CVI_PWRCTRL_EVENT_SCOPE_SYSTEM:
            s32Ret = POWERCTRL_PauseEventTaskProc(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT],pbEventContinueHandle);
            CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
            break;
        default:
            s32Ret = CVI_PWRCTRL_EINVAL;
            break;
    }
...
static int32_t POWERCTRL_ResumeEventProc(CVI_PWRCTRL_EVENT_SCOPE_E enScopeType,bool* pbEventContinueHandle)
{
    int32_t s32Ret = 0;
    bool bContinueHandle = true;
    switch(enScopeType)
    {
        case CVI_PWRCTRL_EVENT_SCOPE_SYSTEM_SCREEN: /** pause screen dormant check proc */
            s32Ret = POWERCTRL_ResumeEventTaskProc(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT],&bContinueHandle);
            CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
            s32Ret = POWERCTRL_ResumeEventTaskProc(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT],pbEventContinueHandle);
            CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
            break;
        case CVI_PWRCTRL_EVENT_SCOPE_SYSTEM:
            s32Ret = POWERCTRL_ResumeEventTaskProc(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT],pbEventContinueHandle);
...
static int32_t POWERCTRL_CommonEventProc(CVI_PWRCTRL_EVENT_SCOPE_E enScopeType,bool* pbEventContinueHandle)
{
    int32_t s32Ret = 0;
    *pbEventContinueHandle = true;
    switch(enScopeType)
    {
        case CVI_PWRCTRL_EVENT_SCOPE_SYSTEM_SCREEN: /** touch action discard on screen dormant*/
            s32Ret = POWERCTRL_CommonEventTaskProc(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT],pbEventContinueHandle);
            CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
...
static int32_t POWERCTRL_ResetTaskCheckTimer(void* pvPrivData)
{
    ...
    if (true == pstTaskContext->stCfg.stAttr.bEnable) {
        s32Ret=CVI_TIMEDTASK_ResetTime(pstTaskContext->TimedTaskId);
        if (0 != s32Ret) {
...
{
    int32_t s32Ret = 0;
    s32Ret = POWERCTRL_ResetTaskCheckTimer(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT]);
    CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
    s32Ret = POWERCTRL_ResetTaskCheckTimer(&s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT]);
    CVI_APPCOMM_CHECK_RETURN(s32Ret,s32Ret);
    return s32Ret;
}
...
int32_t CVI_POWERCTRL_Init(const CVI_PWRCTRL_CFG_S* pstCfg)
{
    ...
    CVI_PWRCTRL_TASK_PROC_CALLBACK pfnDormantProc = NULL;
    CVI_PWRCTRL_TASK_PROC_CALLBACK pfnWakeupProc = NULL;
    if (true == s_stPWRCTRLMgr.bInitState) {
        CVI_LOGE("power control already init\n");
        return CVI_PWRCTRL_EINITIALIZED;
    }
    /** parm invalid check */
    CVI_APPCOMM_CHECK_POINTER(pstCfg,CVI_PWRCTRL_EINVAL);
    for (s32Index = CVI_PWRCTRL_TASK_SCREENDORMANT ;s32Index < CVI_PWRCTRL_TASK_BUIT;s32Index++) {
        pfnDormantProc = pstCfg->astTaskCfg[s32Index].pfnDormantProc;
        pfnWakeupProc = pstCfg->astTaskCfg[s32Index].pfnWakeupProc;
        CVI_APPCOMM_CHECK_EXPR(((pfnDormantProc != NULL && pfnWakeupProc != NULL) \
            || (pfnDormantProc == NULL && pfnWakeupProc == NULL)),CVI_PWRCTRL_EINVAL);

        CVI_APPCOMM_CHECK_EXPR((pstCfg->astTaskCfg[s32Index].stAttr.bEnable ==false) || \
    ...
    if (pstCfg->astTaskCfg[CVI_PWRCTRL_TASK_SCREENDORMANT].stAttr.bEnable && pstCfg->astTaskCfg[CVI_PWRCTRL_TASK_SYSTEMDORMANT].stAttr.bEnable) {
        CVI_APPCOMM_CHECK_EXPR((pstCfg->astTaskCfg[CVI_PWRCTRL_TASK_SCREENDORMANT].stAttr.u32Time_sec <= \
            pstCfg->astTaskCfg[CVI_PWRCTRL_TASK_SYSTEMDORMANT].stAttr.u32Time_sec),CVI_PWRCTRL_EINVAL);
    }
    ...
    for (s32Index = CVI_PWRCTRL_TASK_SCREENDORMANT; s32Index < CVI_PWRCTRL_TASK_BUIT; s32Index++) {
        ...
        s32Ret = CVI_TIMEDTASK_Create(&stTimedTaskCfg,&TimedTaskId);
        if (0 != s32Ret) {
...
int32_t CVI_POWERCTRL_DeInit(void)
{
    int32_t s32Ret = 0;
    int32_t s32Index = CVI_PWRCTRL_TASK_BUIT;
    CVI_MUTEX_LOCK(s_stPWRCTRLMgr.Mutex);
    if (true != s_stPWRCTRLMgr.bInitState) {
        CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
        CVI_LOGE("power control not init\n");
        return CVI_PWRCTRL_ENOINIT;
    }
    for (s32Index = CVI_PWRCTRL_TASK_SCREENDORMANT ;s32Index < CVI_PWRCTRL_TASK_BUIT;s32Index++) {
        s32Ret = CVI_TIMEDTASK_Destroy(s_stPWRCTRLMgr.astTaskContext[s32Index].TimedTaskId);
        if (0 != s32Ret) {
            CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
...
{
    CVI_APPCOMM_CHECK_POINTER(pstTaskAttr,CVI_PWRCTRL_EINVAL);
    CVI_MUTEX_LOCK(s_stPWRCTRLMgr.Mutex);
    if (true != s_stPWRCTRLMgr.bInitState) {
        CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
        CVI_LOGE("power control not init\n");
    ...
    switch(enType)
    {
        case CVI_PWRCTRL_TASK_SCREENDORMANT:
            break;
        case CVI_PWRCTRL_TASK_SYSTEMDORMANT:
            break;
        default:
            CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
...
int32_t CVI_POWERCTRL_SetTaskAttr(CVI_PWRCTRL_TASK_E enType,const CVI_TIMEDTASK_ATTR_S* pstTaskAttr)
{
    int32_t s32Ret = 0;
    uint32_t s32Index = 0;
    CVI_PWRCTRL_TASK_E enAutoTaskType = CVI_PWRCTRL_TASK_BUIT;
    CVI_APPCOMM_CHECK_POINTER(pstTaskAttr,CVI_PWRCTRL_EINVAL);
    CVI_APPCOMM_CHECK_EXPR((pstTaskAttr->bEnable == false) || ((pstTaskAttr->bEnable == true)&&(pstTaskAttr->u32Time_sec != 0)), \
        CVI_PWRCTRL_EINVAL);
    ...
    switch(enType)
    {
        case CVI_PWRCTRL_TASK_SCREENDORMANT:
            if ((true == pstTaskAttr->bEnable) && \
                (true == s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT].stCfg.stAttr.bEnable) && \
                (pstTaskAttr->u32Time_sec  > s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SYSTEMDORMANT].stCfg.stAttr.u32Time_sec )) {
                enAutoTaskType = CVI_PWRCTRL_TASK_SYSTEMDORMANT;
            }
            break;
        case CVI_PWRCTRL_TASK_SYSTEMDORMANT:
            if ((true == pstTaskAttr->bEnable) && \
                (true == s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT].stCfg.stAttr.bEnable) && \
               (pstTaskAttr->u32Time_sec  < s_stPWRCTRLMgr.astTaskContext[CVI_PWRCTRL_TASK_SCREENDORMANT].stCfg.stAttr.u32Time_sec )) {
                enAutoTaskType = CVI_PWRCTRL_TASK_SCREENDORMANT;
            }
            break;
        default:
            CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
    ...
    for (s32Index = CVI_PWRCTRL_TASK_SCREENDORMANT;s32Index < CVI_PWRCTRL_TASK_BUIT;s32Index++) {
        if (s32Index == enType) {
            s32Ret = CVI_TIMEDTASK_SetAttr(s_stPWRCTRLMgr.astTaskContext[enType].TimedTaskId,pstTaskAttr);
            if (0 != s32Ret) {
                CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
            ...
            if (CVI_PWRCTRL_TASK_BUIT != enAutoTaskType) {
                s32Ret = CVI_TIMEDTASK_SetAttr(s_stPWRCTRLMgr.astTaskContext[enAutoTaskType].TimedTaskId,pstTaskAttr);
                if (0 != s32Ret) {
                    CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
                ...
                s32Ret = CVI_PWRCTRL_ETASKTIMEAUTO;
            }
            break;
        }
    }
...
int32_t CVI_POWERCTRL_EventPreProc(const CVI_PWRCTRL_EVENT_ATTR_S* pstEventAttr,bool* pbEventContinueHandle)
{
    ...
    CVI_APPCOMM_CHECK_POINTER(pstEventAttr,CVI_PWRCTRL_EINVAL);
    CVI_APPCOMM_CHECK_POINTER(pbEventContinueHandle,CVI_PWRCTRL_EINVAL);
    CVI_MUTEX_LOCK(s_stPWRCTRLMgr.Mutex);
    if (true != s_stPWRCTRLMgr.bInitState) {
        CVI_MUTEX_UNLOCK(s_stPWRCTRLMgr.Mutex);
        CVI_LOGE("power control not init\n");
    ...
    {
        case CVI_PWRCTRL_EVENT_TYPE_WAKEUP:
        ...
        case CVI_PWRCTRL_EVENT_TYPE_CONTROL:
            switch(pstEventAttr->unCfg.stCtrlCfg.enType)
            {

                case CVI_PWRCTRL_EVENT_CONTROL_PAUSE:
                    ...
                    break;
                case CVI_PWRCTRL_EVENT_CONTROL_RESUME:
            ...
            break;
        case CVI_PWRCTRL_EVENT_TYPE_COMMON:
