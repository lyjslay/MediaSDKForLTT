...
static bool cvi_checkname_iswav(const char *infilename)
{
    ...
    do {
        ...
        case ID_FMT:
            ...
            *sample_rate = chunk_fmt.sample_rate;
            printf("chunk_fmt.num_channels = %d\n", chunk_fmt.num_channels);
...
int32_t CVI_VOICEPLAY_Push(const CVI_VOICEPLAY_VOICE_S* pstVoice, int32_t u32Timeout_ms)
{
    int32_t s32Ret = -1;
    int32_t u32Timewait_ms = 0;
    CVI_APPCOMM_CHECK_POINTER(pstVoice, CVI_VOLMNG_EINVAL);
    // CVI_MUTEX_LOCK(s_stVOICEPLAYCtx.mutex);

    s32Ret = CVI_QUEUE_GetLen(s_stVOICEPLAYCtx.queueHdl);
    if (s32Ret == 0) {
        /*if queue is empty ,push voice into queue*/
        s32Ret = CVI_QUEUE_Push((CVI_QUEUE_HANDLE_T)s_stVOICEPLAYCtx.queueHdl, pstVoice);
        sem_post(&s_stVOICEPLAYCtx.volSem);
    } else if (s32Ret > 0) {
        /*if queue is not empty ,drop droppable voice*/
        if (pstVoice->bDroppable == true) {
            CVI_LOGE("voice queue is busy, drop this voice.s32Ret=%d\n", s32Ret);
        } else {
            /*wait queue is not full*/
            while ((u32Timewait_ms < u32Timeout_ms) && (VOICE_QUEUE_MAXLEN == CVI_QUEUE_GetLen(s_stVOICEPLAYCtx.queueHdl))) {
                printf("voice queue is full, Waiting ......\n");
    ...
    // CVI_MUTEX_UNLOCK(s_stVOICEPLAYCtx.mutex);

    return s32Ret;
}

static CVI_CHAR* VOICEPLAY_GetPath(uint32_t u32Index)
{
...
static int32_t CVI_VOICE_PLAY_File(const char* pathname, CVI_VOICEPLAY_CFG_S* pvParam)
{
    CVI_APPCOMM_CHECK_POINTER(pathname, CVI_VOLMNG_EINTER);
    CVI_APPCOMM_CHECK_POINTER(pvParam, CVI_VOLMNG_EINTER);
    ...
    CVI_VOICEPLAY_CFG_S* pstCfg = (CVI_VOICEPLAY_CFG_S*)pvParam;
    CVI_MAPI_AO_CTX_T *et = (CVI_MAPI_AO_CTX_T *)pstCfg->stAoutOpt.hAudDevHdl;

    /*1.deal with the wav file*/
    if (cvi_checkname_iswav(pathname)) {
    ...
    CVI_MAPI_AO_Start((void *)pstCfg->stAoutOpt.hAudDevHdl, 1);
    // CVI_MAPI_AO_SetVolume((void *)pstCfg->stAoutOpt.hAudDevHdl, VOLUME_DEFAULT);

    /*3.open the amplifier*/
    CVI_MAPI_AO_SetAmplifier((void *)pstCfg->stAoutOpt.hAudDevHdl, true);
    ...
    while (fread(pBuffer,1,s32FrameBytes,pfile)) {
        ...
        s32Ret = CVI_MAPI_AO_SendFrame((void *)pstCfg->stAoutOpt.hAudDevHdl, 1, &stFrame, 1000);
        if(s32Ret) {
            CVI_LOGE("CVI_MAPI_AO_SendFrame failed!\n");
            continue;
        }
        s32CountDelay++;
    ...
    free(pBuffer);
    CVI_MAPI_AO_Stop((void *)pstCfg->stAoutOpt.hAudDevHdl, 1);

    // if (s_stVOICEPLAYCtx.bAmplifier == true){
    //     CVI_MAPI_AO_SetAmplifier((void *)pstCfg->stAoutOpt.hAudDevHdl, false);
    // }

    return 0;
}

static CVI_VOID* VOICEPLAY_TaskQueue_Proc(CVI_VOID* pvParam)
{
    int32_t s32Ret = 0;
    prctl(PR_SET_NAME, __FUNCTION__, 0, 0, 0);
    CVI_QUEUE_HANDLE_T queue = s_stVOICEPLAYCtx.queueHdl;
    CVI_VOICEPLAY_CFG_S* p_vol_play = (CVI_VOICEPLAY_CFG_S*)pvParam;

    while (s_stVOICEPLAYCtx.bRunFlag) {
        while((0 != sem_wait(&s_stVOICEPLAYCtx.volSem)) && (errno == EINTR));
        /*if not msg ,the thread will block*/
        if (0 == CVI_QUEUE_GetLen(queue)) {
            // cvi_usleep(VOICE_TIME_INTERVAL_MS * 1000);
            continue;
        } else {
            CVI_VOICEPLAY_VOICE_S stVoice;
            s32Ret = CVI_QUEUE_Pop(queue, &stVoice);
            if (0 != s32Ret) {
            ...
            for (uint32_t i = 0; i < stVoice.u32VoiceCnt; i++) {
                CVI_VOICE_PLAY_File(VOICEPLAY_GetPath(stVoice.au32VoiceIdx[i]), p_vol_play);
            }
            CVI_MUTEX_UNLOCK(s_stVOICEPLAYCtx.mutex);
        }
    }
...
{
    int32_t s32Ret = 0;
    CVI_APPCOMM_CHECK_POINTER(pstCfg, CVI_VOLMNG_EINVAL);
    CVI_APPCOMM_CHECK_EXPR(pstCfg->u32MaxVoiceCnt > 0, CVI_VOLMNG_EINVAL);
    CVI_MUTEX_LOCK(s_stVOICEPLAYCtx.mutex);

    /*return volpaly queue handler*/
    s_stVOICEPLAYCtx.queueHdl = CVI_QUEUE_Create(sizeof(CVI_VOICEPLAY_VOICE_S), VOICE_QUEUE_MAXLEN);
    if (s_stVOICEPLAYCtx.queueHdl == 0) {
        CVI_MUTEX_UNLOCK(s_stVOICEPLAYCtx.mutex);
        CVI_LOGE("CVI_QUEUE_Create failed\n");
        return CVI_EINTER;
    }

    s_stVOICEPLAYCtx.bRunFlag = true;

    memcpy(&s_stVOICEPLAYCtx.stCfg, pstCfg, sizeof(CVI_VOICEPLAY_CFG_S));
    ...
    s_stVOICEPLAYCtx.stCfg.pstVoiceTab = (CVI_VOICEPLAY_VOICETABLE_S*)malloc(pstCfg->u32MaxVoiceCnt * sizeof(CVI_VOICEPLAY_VOICETABLE_S));
...
int32_t CVI_VOICEPLAY_DeInit(void)
{
    ...
    CVI_QUEUE_Destroy(s_stVOICEPLAYCtx.queueHdl);
...
int32_t CVI_VOICEPLAY_SetAmplifier(bool en)
{
    ...
    if (en == true) {
        CVI_MAPI_AO_SetAmplifier(s_stVOICEPLAYCtx.stCfg.stAoutOpt.hAudDevHdl, false);
    } else {
        CVI_MAPI_AO_SetAmplifier(s_stVOICEPLAYCtx.stCfg.stAoutOpt.hAudDevHdl, true);
    }
...
int32_t CVI_VOICEPLAY_SetVolume(int32_t volume)
{
    ...
    s32Ret = CVI_MAPI_AO_SetVolume(s_stVOICEPLAYCtx.stCfg.stAoutOpt.hAudDevHdl, volume);
    if (s32Ret != 0) {
        CVI_LOGE("CVI_MAPI_AO_SetVolume failed\n");
        CVI_MUTEX_UNLOCK(s_stVOICEPLAYCtx.mutex);
        return CVI_EINTER;
    }
...
int32_t CVI_VOICEPLAY_GetVolume(int32_t *volume)
{
    ...
    s32Ret = CVI_MAPI_AO_GetVolume(s_stVOICEPLAYCtx.stCfg.stAoutOpt.hAudDevHdl, volume);
    if (s32Ret != 0) {
        CVI_LOGE("CVI_MAPI_AO_SetVolume failed\n");
        CVI_MUTEX_UNLOCK(s_stVOICEPLAYCtx.mutex);
        return CVI_EINTER;
