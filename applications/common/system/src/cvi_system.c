...
{
    CVI_APPCOMM_CHECK_POINTER(pstDateTime, CVI_EINVAL);
    CVI_APPCOMM_CHECK_EXPR(1970 < pstDateTime->s32year, CVI_EINVAL);
    CVI_APPCOMM_CHECK_EXPR(CVI_APPCOMM_CHECK_RANGE(pstDateTime->s32mon, 1, 12), CVI_EINVAL);
    CVI_APPCOMM_CHECK_EXPR(CVI_APPCOMM_CHECK_RANGE(pstDateTime->s32mday, 1, 31), CVI_EINVAL);
    CVI_APPCOMM_CHECK_EXPR(CVI_APPCOMM_CHECK_RANGE(pstDateTime->s32hour, 0, 23), CVI_EINVAL);
    CVI_APPCOMM_CHECK_EXPR(CVI_APPCOMM_CHECK_RANGE(pstDateTime->s32min, 0, 59), CVI_EINVAL);
    CVI_APPCOMM_CHECK_EXPR(CVI_APPCOMM_CHECK_RANGE(pstDateTime->s32sec, 0, 59), CVI_EINVAL);
    /*1.set systime*/
    time_t t = 0L;
    struct tm stTime;
...
{
    int32_t s32Ret = 0;
    CVI_SYSTEM_TM_S stDateTime = {0};

    s32Ret = CVI_SYSTEM_GetRTCDateTime(&stDateTime);
    if (s32Ret != 0) {
        CVI_LOGE("CVI_SYSTEM_GetRTCDateTime get failed!\n");
        return -1;
    }
    /*set systime*/
    time_t t = 0L;
    ...
    if (stDateTime.s32year < CVI_DATETIME_DEFAULT_YEAR) {
        stDateTime.s32year = CVI_DATETIME_DEFAULT_YEAR;
        stDateTime.s32mon = CVI_DATETIME_DEFAULT_MONTH;
        stDateTime.s32mday = CVI_DATETIME_DEFAULT_DAY;
        stDateTime.s32hour = CVI_DATETIME_DEFAULT_HOUR;
        stDateTime.s32min = CVI_DATETIME_DEFAULT_MINUTE;
        stDateTime.s32sec = CVI_DATETIME_DEFAULT_SECOND;
        s32Ret = CVI_SYSTEM_SetDateTime(&stDateTime);
        if (s32Ret != 0) {
            CVI_LOGE("CVI_SYSTEM_GetRTCDateTime get failed!\n");
            return -1;
        }
...
{
    CVI_APPCOMM_CHECK_POINTER(penStartupSrc, CVI_EINVAL);
    ...
    if ((u8PowerSource>>19 & 0x01) == 0) {
        *penStartupSrc = CVI_SYSTEM_STARTUP_SRC_STARTUP;
    } else {
        if ((u8PowerSource>>17 & 0x01) == 1) {
            CVI_LOGD("%s: gsensorvalue === %d", __func__, (u8PowerSource>>21 & 0x01));
            *penStartupSrc = CVI_SYSTEM_STARTUP_SRC_GSENSORWAKEUP;
        } else if((u8PowerSource>>21 & 0x01) == 1) {
            *penStartupSrc = CVI_SYSTEM_STARTUP_SRC_USBWAKEUP;
        } else {
            *penStartupSrc = CVI_SYSTEM_STARTUP_SRC_USBWAKEUP;
        }
    }
...
int32_t CVI_SYSTEM_BootSound(CVI_MAPI_AO_HANDLE_T AoHdl)
{
    ...
    while (u32FrameCnt < (u32DataLen / s32FrameBytes)) {
        ...
        s32Ret = CVI_MAPI_AO_SendFrame(AoHdl, et->AoChn, &stFrame, 1000);
        if(s32Ret) {
    ...
    CVI_MAPI_AO_SetAmplifier(AoHdl, false);

    CVI_MAPI_AO_Stop(AoHdl, et->AoChn);
    return 0;
}
