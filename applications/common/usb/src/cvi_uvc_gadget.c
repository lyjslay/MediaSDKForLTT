...
typedef struct tagUVC_DEVICE_CTX_S {
    ...
    char uvc_devname[CVI_APPCOMM_MAX_PATH_LEN];
...
int32_t UVC_GADGET_Init(const CVI_UVC_DEVICE_CAP_S *pstDevCaps, u_int32_t u32MaxFrameSize) {
    CVI_APPCOMM_CHECK_POINTER(pstDevCaps, -1);
    ...
    s_stUVCDevCtx.uvc_fd = CVI_APPCOMM_FD_INVALID_VAL;

    return 0;
}

int32_t UVC_GADGET_DeviceOpen(const char *pDevPath) {
    if (s_stUVCDevCtx.uvc_fd == CVI_APPCOMM_FD_INVALID_VAL) {
