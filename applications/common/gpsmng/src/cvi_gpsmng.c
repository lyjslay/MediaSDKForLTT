...
typedef struct tagGPSMNG_CONTEXT
{
    ...
    CVI_GPSMNG_CALLBACK fnGpsCB[CVI_GPSMNG_CALLBACK_MAX_NUM];
} GPSMNG_CONTEXT;
...
static CVI_GPSMNG_MSG_PACKET g_gpsMngPacket;
...
static int32_t GPSMNG_MsgProc(char* msgStr)
{
    ...
    {
        snprintf(g_gpsRawdataBuff.gpsRawData.ggaStr, CVI_GPSMNG_MESSAGE_MAX_LEN, "%s", msgStr);
        g_gpsRawdataBuff.msgCount |= 1 << 0;
    }
    else if (NULL != strstr(msgStr, "GPGLL") || NULL != strstr(msgStr, "GNGLL"))
    {
        snprintf(g_gpsRawdataBuff.gpsRawData.gllStr, CVI_GPSMNG_MESSAGE_MAX_LEN, "%s", msgStr);
        g_gpsRawdataBuff.msgCount |= 1 << 1;
    }
    else if (NULL != strstr(msgStr, "GPGSA") || NULL != strstr(msgStr, "GNGSA"))
    {
        snprintf(g_gpsRawdataBuff.gpsRawData.gsaStr, CVI_GPSMNG_MESSAGE_MAX_LEN, "%s", msgStr);
        g_gpsRawdataBuff.msgCount |= 1 << 2;
    }
    else if (NULL != strstr(msgStr, "GPRMC") || NULL != strstr(msgStr, "GNRMC"))
    {
        snprintf(g_gpsRawdataBuff.gpsRawData.rmcStr, CVI_GPSMNG_MESSAGE_MAX_LEN, "%s", msgStr);
        g_gpsRawdataBuff.msgCount |= 1 << 3;
    }
    else if (NULL != strstr(msgStr, "GPVTG") || NULL != strstr(msgStr, "GNVTG"))
    {
        snprintf(g_gpsRawdataBuff.gpsRawData.vtgStr, CVI_GPSMNG_MESSAGE_MAX_LEN, "%s", msgStr);
    ...
    else if (NULL != strstr(msgStr, "GPGSV") || NULL != strstr(msgStr, "GNGSV") || NULL != strstr(msgStr, "GLGSV"))
    {
        ...
        if (curMsgNum <= CVI_GPSMNG_GSV_MAX_MSG_NUM)
        {
            g_gpsRawdataBuff.gpsGPGSVMsgTotalNum = (totalMsgNum > CVI_GPSMNG_GSV_MAX_MSG_NUM) ? CVI_GPSMNG_GSV_MAX_MSG_NUM : totalMsgNum;
            g_gpsRawdataBuff.gpsGPGSVMsgCurNum = curMsgNum;
            snprintf(g_gpsRawdataBuff.gpsRawData.gsvStr[curMsgNum - 1], CVI_GPSMNG_MESSAGE_MAX_LEN, "%s", msgStr);
...
static int32_t GPS_DataProc(void)
{
    ...
    for (i = 0; i < CVI_GPSMNG_CALLBACK_MAX_NUM; i++)
    {
...
void* MNG_GPS_MsgReadThread(void* para)
{
    ...
    char buffer[CVI_GPSMNG_MESSAGE_MAX_LEN];
    CVI_GPSDATA_S gpsData = {0};
    ...
    while (g_gpsMngCtx.gpsReadFlg) {
        ...
        ret = CVI_HAL_GPS_GetRawData(&gpsData, GPSMNG_GET_DATA_TIMEOUT_MS);
        if(ret == -11) {
            continue;
            ...
            if (reset && (recv != '\n' && recv != '\r')) {
                ...
                if (recvNum >= (CVI_GPSMNG_MESSAGE_MAX_LEN - 1)) {
                    CVI_LOGE("should expand buff size,discard data[%c]\n", recv);
                    buffer[CVI_GPSMNG_MESSAGE_MAX_LEN - 1] = '\n';
                    continue;
                }
            ...
            if (recv == '$') {
                reset = true;
                recvNum = 0;
                memset(buffer, 0 , CVI_GPSMNG_MESSAGE_MAX_LEN);
                /* not to cut '$' symbol*/
                buffer[recvNum] = recv;
                recvNum++;
            }
...
int32_t CVI_GPSMNG_Start(void)
{
    ...
    {
        CVI_LOGE("gps mng has already start!\n");
        return CVI_GPSMNG_EALREADYSTART;
    }
    ...
    if (ret != 0)
    {
        ...
        return CVI_GPSMNG_EREGISTEREVENT;
    }

    CVI_LOGI("CVI_GPSMNG_Start END !\n");
    g_gpsMngCtx.gpsMngState = GPSMNG_STATE_STARTED;
    return ret;
}

int32_t CVI_GPSMNG_Register(CVI_GPSMNG_CALLBACK* pfnGpsCB)
{
    int32_t recordIdx = -1;
    int32_t i;
    ...
    return CVI_GPSMNG_EREGISTER;
}

int32_t CVI_GPSMNG_UnRegister(CVI_GPSMNG_CALLBACK* pfnGpsCB)
{
    int32_t i;
...
int32_t CVI_GPSMNG_DeInit(void)
{
    ...
    ret = CVI_HAL_GPS_Deinit();

    if (ret != 0)
    {
        CVI_LOGE("hal gps init failed!\n");
        return CVI_GPSMNG_EDEINIT;
    }
...
int32_t CVI_GPSMNG_GetData(CVI_GPSMNG_MSG_PACKET* msgPacket)
{
    ...
    if (g_gpsMngCtx.gpsMngState != GPSMNG_STATE_STARTED)
    {
        ...
        return CVI_GPSMNG_EGETDATA;
    }
    memcpy(msgPacket, &g_gpsMngPacket, sizeof(CVI_GPSMNG_MSG_PACKET));
