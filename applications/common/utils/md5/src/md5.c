...
void CVI_MD5_Update(CVI_MD5_CTX_S *context, unsigned char *input, uint32_t inputlen)
{
	...
	if(inputlen >= partlen) {
		...
		CVI_MD5_Transform(context->state, context->buffer);

		for(i = partlen; i+64 <= inputlen; i+=64)
			CVI_MD5_Transform(context->state, &input[i]);
...
void CVI_MD5_Final(CVI_MD5_CTX_S *context, unsigned char digest[16])
{
	...
	CVI_MD5_Encode(bits, context->count, 8);
	CVI_MD5_Update(context, PADDING, padlen);
	CVI_MD5_Update(context, bits, 8);
	CVI_MD5_Encode(digest, context->state, 16);
}
