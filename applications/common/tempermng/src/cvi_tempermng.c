...
int32_t cpu_tpu_contrl(int32_t cpu_div, int32_t tpu_div)
{
    ...
    if (vir_addr != NULL)
    {
		*(CVI_U32 *)vir_addr |= (0x1<<0);
        *(CVI_U32 *)vir_addr |= (0x1<<3);
    ...
    if (vir_addr != NULL)
    {
        *(CVI_U32 *)vir_addr &= ~(0x1F<<16);
		*(CVI_U32 *)vir_addr |= (cpu_div<<16);
		CVI_SYS_Munmap(vir_addr, 0x4);
	}
...
int32_t CVI_Detect_Temperature_Thread(void)
{
    int32_t s32Ret = 0;
    pthread_t taskid;
