#include <stdio.h>
#include <time.h>
#include <string.h>
#include <pthread.h>
#include <stdlib.h>
#include <sys/prctl.h>
#include <unistd.h>

#include "cvi_hal_watchdog.h"
#include "cvi_watchdogmng.h"
#include "cvi_sysutils.h"

#ifdef __cplusplus
#if __cplusplus
extern "C" {
#endif
#endif /* End of #ifdef __cplusplus */


typedef struct tagWATCHDOGMNG_CONTEXT_S
{
    CVI_BOOL bWDTMNGInitState;
    CVI_BOOL bWdtCheckRun;
    pthread_t WdtCheckId;
    CVI_S32 s32Time;
} WATCHDOGMNG_CONTEXT_S;
static WATCHDOGMNG_CONTEXT_S s_stWdtCtx;


static CVI_VOID*  WATCHDOG_CheckThread(CVI_VOID* pData)
{
    CVI_LOGD("thread WATCHDOG Check enter\n");
    CVI_CHAR szThreadName[CVI_APPCOMM_MAX_PATH_LEN];
    snprintf(szThreadName, CVI_APPCOMM_MAX_PATH_LEN, "%s", "WDT_CHECK");
    prctl(PR_SET_NAME, szThreadName, 0, 0, 0); /**< Set Task Name */

    while (s_stWdtCtx.bWdtCheckRun)
    {
        CVI_HAL_WATCHDOG_Feed();
        cvi_usleep((s_stWdtCtx.s32Time - 1) * 1000 * 1000);
    }

    CVI_LOGD("thread WATCHDOG_CheckThread exit\n");
    return NULL;
}


CVI_S32 CVI_WATCHDOGMNG_Init(CVI_S32 s32Time_s)
{
    CVI_S32 s32Ret = CVI_SUCCESS;
    if (s_stWdtCtx.bWDTMNGInitState == CVI_TRUE)
    {
        CVI_LOGE("wdtmng has already been started\n");
        return CVI_FAILURE;
    }

    if (s32Time_s < 2 || s32Time_s > 1000)
    {
        CVI_LOGE("Interval time should not be less then two and bigger then 100. %d\n", s32Time_s);
        return CVI_FAILURE;
    }

    s32Ret = CVI_HAL_WATCHDOG_Init(s32Time_s);
    if(CVI_SUCCESS != s32Ret)
    {
        CVI_LOGE("CVI_HAL_WATCHDOG_Init: failed, errno(%d)\n", s32Ret);
        return CVI_FAILURE;
    }

    s_stWdtCtx.bWdtCheckRun = CVI_TRUE;
    s_stWdtCtx.s32Time = s32Time_s;
    s32Ret = pthread_create(&(s_stWdtCtx.WdtCheckId), NULL, WATCHDOG_CheckThread, NULL);
    if (s32Ret != CVI_SUCCESS)
    {
        CVI_LOGE("Create WATCHDOG_CheckThread Thread Fail!\n");
        return CVI_FAILURE;
    }

    s_stWdtCtx.bWDTMNGInitState = CVI_TRUE;
    return CVI_SUCCESS;
}



CVI_S32 CVI_WATCHDOGMNG_DeInit(CVI_VOID)
{
    CVI_S32 s32Ret = CVI_SUCCESS;

    if (s_stWdtCtx.bWDTMNGInitState  == CVI_FALSE)
    {
        CVI_LOGE("watchdogmng no init\n");
        return CVI_FAILURE;
    }

     /** Destory Key Check Task */
    s_stWdtCtx.bWdtCheckRun = CVI_FALSE;
    s32Ret = pthread_join(s_stWdtCtx.WdtCheckId, NULL);
    if (s32Ret != CVI_SUCCESS)
    {
        CVI_LOGE("Join WATCHDOG_CheckThread  Fail!\n");
        return CVI_FAILURE;
    }

    CVI_HAL_WATCHDOG_Deinit();
    s_stWdtCtx.bWDTMNGInitState = CVI_FALSE;
    return CVI_SUCCESS;
}


#ifdef __cplusplus
#if __cplusplus
}
#endif
#endif /* End of #ifdef __cplusplus */

