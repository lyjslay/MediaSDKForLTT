...
typedef struct tagGSENSORMNG_CONTEXT_S
{
    ...
    bool bThreadRun;
    CVI_GSENSORMNG_CFG_S stCfg;
} GSENSORMNG_CONTEXT_S;

static GSENSORMNG_CONTEXT_S s_stGSENSORMNGContext ={.bInitState = false,.Mutex= PTHREAD_MUTEX_INITIALIZER};

static int32_t GSENSORMNG_InParmValidChck(const CVI_GSENSORMNG_CFG_S* stCfg)
{
    /** parm check */
    CVI_APPCOMM_CHECK_POINTER(stCfg, -1);
    CVI_APPCOMM_CHECK_POINTER((stCfg->enSensitity >= CVI_HAL_GSENSOR_SENSITITY_OFF &&
        stCfg->enSensitity < CVI_HAL_GSENSOR_SENSITITY_BUIT),CVI_GSENSORMNG_EINVAL);

    return 0;
}

static int32_t GSENSORMNG_InternalParmInit(const CVI_GSENSORMNG_CFG_S* pstCfg)
{
    CVI_APPCOMM_CHECK_POINTER(pstCfg, -1);
    s_stGSENSORMNGContext.bInitState = false;
    s_stGSENSORMNGContext.bThreadRun = false;
    memcpy(&s_stGSENSORMNGContext.stCfg,pstCfg,sizeof(CVI_GSENSORMNG_CFG_S));
    return 0;
}

static void GSENSORMNG_GsensorHalParmwitch(const CVI_GSENSORMNG_CFG_S* pstMngCfg,CVI_HAL_GSENSOR_CFG_S* psthalCfg)
{
...
static void GSENSORMNG_CollisionEventPublish(void)
{
    ...
    stEvent.topic = CVI_EVENT_GSENSORMNG_COLLISION;
...
{
    CVI_HAL_GSENSOR_VALUE_S stCurValue;
    int32_t ret;

    if(0 == CVI_HAL_GSENSOR_GetCurValue(&stCurValue)) {
...
int32_t CVI_GSENSORMNG_Init(const CVI_GSENSORMNG_CFG_S* pstCfg)
{
    ...
    if (GSENSORMNG_InParmValidChck(pstCfg) != 0)
    {
        ...
        return CVI_GSENSORMNG_EINVAL;
    }
    g_GSensorSensitivity = pstCfg->enSensitity;
    g_GSensorLevel = pstCfg->enSensitity;
    CVI_MUTEX_LOCK(s_stGSENSORMNGContext.Mutex);

    if (true  == s_stGSENSORMNGContext.bInitState)
    {
        CVI_LOGE("gsensormng has already been started\n");
        CVI_MUTEX_UNLOCK(s_stGSENSORMNGContext.Mutex);
        return CVI_GSENSORMNG_EINITIALIZED;
    }

    extern CVI_HAL_GSENSOR_OBJ_S gsensorObj;
    s32Ret = CVI_HAL_GSENSOR_Register(&gsensorObj);
    if (s32Ret < 0) {
        CVI_LOGE("sensor obj register failed\n");
        return CVI_GSENSORMNG_EINTER;
    }

    CVI_HAL_GSENSOR_CFG_S stGsensorCfg;
    GSENSORMNG_GsensorHalParmwitch(pstCfg,&stGsensorCfg);
    s32Ret = CVI_HAL_GSENSOR_Init(&stGsensorCfg);

    if (0 != s32Ret)
    {
        CVI_LOGE("CVI_HAL_GSENSOR_Init Failed\n");
        CVI_MUTEX_UNLOCK(s_stGSENSORMNGContext.Mutex);
        return CVI_GSENSORMNG_EINTER;
    }
    ...
    if (0 != s32Ret)
    {
        ...
        CVI_HAL_GSENSOR_DeInit();
        CVI_MUTEX_UNLOCK(s_stGSENSORMNGContext.Mutex);
        return CVI_GSENSORMNG_ETHREAD;
    }
...
{
    int32_t s32Ret;
    CVI_APPCOMM_CHECK_POINTER((enSensitity >= CVI_HAL_GSENSOR_SENSITITY_OFF &&
        enSensitity < CVI_HAL_GSENSOR_SENSITITY_BUIT),CVI_GSENSORMNG_EINVAL);
    ...
    if (false  == s_stGSENSORMNGContext.bInitState)
    {
        ...
        CVI_MUTEX_UNLOCK(s_stGSENSORMNGContext.Mutex);
        return CVI_GSENSORMNG_ENOINIT;
    }

    s32Ret = CVI_HAL_GSENSOR_SetSensitity((CVI_HAL_GSENSOR_SENSITITY_E)enSensitity);
    if(0 != s32Ret)
    {
        CVI_LOGE("gsensor set Sensitity error\n");
        s32Ret = CVI_GSENSORMNG_EINTER;
    }
    CVI_MUTEX_UNLOCK(s_stGSENSORMNGContext.Mutex);
    return s32Ret;
}
...
{
    CVI_APPCOMM_CHECK_POINTER(pstAttr, CVI_GSENSORMNG_EINVAL);
...
{
    CVI_HAL_GSENSOR_ATTR_S stGsensorAttr;
    int32_t s32Ret;
    CVI_APPCOMM_CHECK_POINTER(pstAttr, CVI_GSENSORMNG_EINVAL);
    ...
    s32Ret = CVI_HAL_GSENSOR_SetAttr(&stGsensorAttr);
    if(0 != s32Ret)
    {
...
int32_t CVI_GSENSORMNG_OpenInterrupt(int32_t IntNum)
{
    ...
    s32Ret = CVI_HAL_GSENSOR_OpenInterrupt(IntNum);
    if(0 != s32Ret) {
...
int32_t CVI_GSENSORMNG_DeInit(void)
{
    ...
    /** Close hal gsensor */
    s32Ret = CVI_HAL_GSENSOR_DeInit();

    if (0 != s32Ret)
    {
        CVI_LOGE("CVI_HAL_GSENSOR_DeInit Fail!\n");
        CVI_MUTEX_UNLOCK(s_stGSENSORMNGContext.Mutex);
        return CVI_GSENSORMNG_EINTER;
    }
    s_stGSENSORMNGContext.bInitState = false;
...
int32_t CVI_GSENSORMNG_SetSensitity(CVI_GSENSORMNG_SENSITITY_E enSensitity)
{
    return 0;
}
...
{
    int32_t s32Ret = 0;
    s32Ret = CVI_EVENTHUB_RegisterTopic(CVI_EVENT_GSENSORMNG_COLLISION);
    if(0 != s32Ret)
    {
        CVI_LOGE("Register collision event fail\n");
        return CVI_GSENSORMNG_EREGISTEREVENT;
    }
    return 0;
}
