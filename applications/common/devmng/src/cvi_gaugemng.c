...
typedef struct tagGAUGEMNG_INFO_S
{
    ...
    bool bCharge;
    bool bLastCharge;
    CVI_GAUGEMNG_CFG_S stCfg;
} GAUGEMNG_INFO_S;

static GAUGEMNG_INFO_S s_stGAUGEMNGInfo;

static int32_t _CVI_GAUGEMNG_GetAverage(int32_t *battery_array, int32_t num)
{
...
static int32_t GAUGEMNG_InParmValidChck(const CVI_GAUGEMNG_CFG_S* stCfg)
{
    /** parm check */
    if (NULL == stCfg) {
    ...
    if (stCfg->s32LowLevel > 100 || stCfg->s32UltraLowLevel <= 0 \
    || stCfg->s32UltraLowLevel > stCfg->s32LowLevel) {
...
static int32_t GAUGEMNG_InternalParmInit(const CVI_GAUGEMNG_CFG_S* pstCfg)
{
    if (NULL == pstCfg) {
        CVI_LOGE("GAUGEMNG_InParmValidChck fail\n");
        return -1;
...
static void GAUGEMNG_UltraLowLevelCheck(GAUGEMNG_INFO_S* pstGaugemngInfo)
{
    ...
    if (0 == pstGaugemngInfo->bUltraLowLevelState) {
        if (pstGaugemngInfo->s32Level <= pstGaugemngInfo->stCfg.s32UltraLowLevel) {
            ...
            stEvent.topic = CVI_EVENT_GAUGEMNG_LEVEL_ULTRALOW;
...
static void GAUGEMNG_LowLevelCheck(GAUGEMNG_INFO_S* pstGaugemngInfo)
{
    ...
    if (0 == pstGaugemngInfo->bLowLevelState) {
        if (pstGaugemngInfo->s32Level <= pstGaugemngInfo->stCfg.s32LowLevel && pstGaugemngInfo->bCharge == 0) {
            memset(&stEvent, '\0', sizeof(stEvent));
            stEvent.topic = CVI_EVENT_GAUGEMNG_LEVEL_LOW;
            stEvent.arg1 = pstGaugemngInfo->s32Level;
            CVI_EVENTHUB_Publish(&stEvent);
        ...
        if (pstGaugemngInfo->s32Level >= (pstGaugemngInfo->stCfg.s32LowLevel + GAUGEMNG_LOWLEVEL_RESET)) {
            memset(&stEvent, '\0', sizeof(stEvent));
            stEvent.topic = CVI_EVENT_GAUGEMNG_LEVEL_NORMAL;
            stEvent.arg1 = pstGaugemngInfo->s32Level;
            CVI_EVENTHUB_Publish(&stEvent);
...
{
    int32_t ret = 0;
    ret =  CVI_GPIO_Get_Value(CVI_GPIOE_06 , value);
    if (ret) {
        CVI_LOGI("CVI_GPIO_Get_Value error\n");
        return -1;
    }
    return 0;
}
...
static void GAUGEMNG_ChargeStateChangeCheck(GAUGEMNG_INFO_S* pstGaugemngInfo)
{
    ...
    if (pstGaugemngInfo->bCharge != pstGaugemngInfo->bLastCharge) {
        memset(&stEvent, '\0', sizeof(stEvent));
        stEvent.topic = CVI_EVENT_GAUGEMNG_CHARGESTATE_CHANGE;
...
int32_t CVI_GAUGENBG_IdexChangeCheck(GAUGEMNG_INFO_S* pstGaugemngInfo)
{
    ...
    if (pstGaugemngInfo->bCharge == 1) {
        ...
        if (s32Level > 87 && s32Level <= 100) {
            index = 0;
        } else if(s32Level > 82 && s32Level <= 87) {
            index = 1;
        } else if(s32Level > 80 && s32Level <= 82) {
            index = 2;
        } else if(s32Level > 77 && s32Level <= 80) {
            index = 3;
        } else if(s32Level > 74 && s32Level <= 77) {
            index = 4;
        }
    }
...
static void GAUGEMNG_LevelChangeCheck(GAUGEMNG_INFO_S* pstGaugemngInfo)
{
    ...
    index = CVI_GAUGENBG_IdexChangeCheck(pstGaugemngInfo);

    if (index != pstGaugemngInfo->last_index) {
        pstGaugemngInfo->last_index = index;
        memset(&stEvent, '\0', sizeof(stEvent));
        stEvent.topic = CVI_EVENT_GAUGEMNG_LEVEL_CHANGE;
        stEvent.arg1 = index;
        CVI_EVENTHUB_Publish(&stEvent);
    }
    return;
...
static int32_t GAUGEMNG_ChargeStateCheck(GAUGEMNG_INFO_S* pstGaugemngInfo)
{
    ...
    //CVI_LOGE("charge = %d\n", charge);
    if (CVI_USB_STATE_INSERT == charge) {
        bCharge = 1;
    } else if (CVI_USB_STATE_OUT == charge) {
        bCharge = 0;
        //cvi_system("poweroff -f");
    }
...
int32_t CVI_GAUGEMNG_GetPercentage(void)
{
    ...
    for(index = 0; index < count; index++) {
        ret = CVI_HAL_ADC_GetValue(ADC_CHANNEL_VBAT);
        if(-1 == ret) {
    ...
    vbat = _CVI_GAUGEMNG_GetAverage(vbat_sample, count);
...
static int32_t GAUGEMNG_LevelCheck(GAUGEMNG_INFO_S* pstGaugemngInfo)
{
    ...
    s32Level = CVI_GAUGEMNG_GetPercentage();
...
{
    int32_t s32Ret = 0;
    s32Ret=CVI_EVENTHUB_RegisterTopic(CVI_EVENT_GAUGEMNG_LEVEL_CHANGE);
    if (0 != s32Ret) {
        CVI_LOGE("Register battery level change event fail\n");
        return -1;
    }
    s32Ret=CVI_EVENTHUB_RegisterTopic(CVI_EVENT_GAUGEMNG_LEVEL_LOW);
    if (0 != s32Ret) {
        CVI_LOGE("Register battery level low event fail\n");
        return -1;
    }
    s32Ret=CVI_EVENTHUB_RegisterTopic(CVI_EVENT_GAUGEMNG_LEVEL_ULTRALOW);
    if (0 != s32Ret) {
        CVI_LOGE("Register battery level ultra low event fail\n");
        return -1;
    }
    s32Ret=CVI_EVENTHUB_RegisterTopic(CVI_EVENT_GAUGEMNG_LEVEL_NORMAL);
    if (0 != s32Ret) {
        CVI_LOGE("Register battery level recover normal event fail\n");
        return -1;
    }
    s32Ret=CVI_EVENTHUB_RegisterTopic(CVI_EVENT_GAUGEMNG_CHARGESTATE_CHANGE);
    if (0 != s32Ret) {
        CVI_LOGE("Register battery level recover normal event fail\n");
        return -1;
    }
...
int32_t CVI_GAUGEMNG_Init(const CVI_GAUGEMNG_CFG_S* pstCfg)
{
    ...
    CVI_MUTEX_LOCK(s_GAUGEMNGMutex);
...
int32_t CVI_GAUGEMNG_GetBatteryLevel(uint8_t* ps32Level)
{
    ...
    s32Ret = CVI_GAUGENBG_IdexChangeCheck(&s_stGAUGEMNGInfo);
    if (s32Ret < 0) {
...
int32_t CVI_GAUGEMNG_DeInit(void)
{
    ...
    /** Close hal gauge */
    s32Ret = CVI_HAL_ADC_Deinit();
    if (0 != s32Ret) {
        CVI_LOGE("HI_HAL_GAUGE_Deinit Fail!\n");
        CVI_MUTEX_UNLOCK(s_GAUGEMNGMutex);
        return -1;
    }
