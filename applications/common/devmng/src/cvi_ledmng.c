...
static void* CVI_LED_SET_Value()
{
    int32_t ret = 0;
    CVI_GPIO_VALUE_E value = CVI_GPIO_VALUE_L;
    prctl(PR_SET_NAME, "LED_SET", 0, 0, 0); /**< Set Task Name */
    while (true == g_valueflage) {
        ret = CVI_HAL_LED_SetValue(CVI_GPIOA_29, CVI_GPIO_VALUE_L);
        if (0 != ret) {
            CVI_LOGE("set blue gpio Low level error");
        }
        if (false == g_red) {
            ret = CVI_HAL_LED_SetValue(CVI_GPIOA_28, CVI_GPIO_VALUE_H);
            if (0 != ret) {
                CVI_LOGE("set red gpio high level error");
            }
        } else {
            ret = CVI_HAL_LED_SetValue(CVI_GPIOA_28, value);
            if (0 != ret) {
                CVI_LOGE("set red gpio error, value = %d", value);
            }
            if (g_twinkle) {
                if (CVI_GPIO_VALUE_H == value) {
                    value = CVI_GPIO_VALUE_L;
                } else {
                    value = CVI_GPIO_VALUE_H;
                }
            }
        }
        sleep(1);
    ...
    if (false == g_valueflage) {
        CVI_HAL_LED_SetValue(CVI_GPIOA_28, CVI_GPIO_VALUE_H);
        CVI_HAL_LED_SetValue(CVI_GPIOA_29, CVI_GPIO_VALUE_H);
    }
    return NULL;
}

int32_t CVI_LED_SET_ValueThread()
{
    int32_t ret = 0;
    ret = pthread_create(&s_LEDVALUETHREAD, NULL, CVI_LED_SET_Value, NULL);
    if (ret != 0) {
...
{
    int32_t ret = 0;
    ret = CVI_HAL_LED_Init();
    if (0 != ret) {
        CVI_LOGE("led gpio init error");
        return -1;
    }
    CVI_LED_SET_ValueThread();
